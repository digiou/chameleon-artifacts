...
FileMutex::FileMutex(const std::string filePath) : fileName(filePath) {
    ...
    x_ASSERT2_FMT(fd != -1, "Invalid file " << filePath << " " << strerror(errno));
}
...
void FileMutex::lock() {
    ...
    lock.l_start = 0;
    lock.l_len = 0;
    x_ASSERT(-1 != ::fcntl(fd, F_SETLKW, &lock), "Cannot acquire lock");
}
...
void FileMutex::unlock() {
    ...
    lock.l_start = 0;
    lock.l_len = 0;
    x_ASSERT(-1 != fcntl(fd, F_SETLK, &lock), "Cannot acquire lock");
}
