...
void jniErrorCheck(const std::source_location location) {
    ...
    if (exception) {
        ...
        x_THROW_RUNTIME_ERROR("An error occurred during a map java UDF execution in function "
...
void freeObject(jobject object) {
    ...
    switch (env->GetObjectRefType(object)) {
        case JNIInvalidRefType: x_THROW_RUNTIME_ERROR("Can't delete invalid ref!");
...
jobject createObjectType(T value, const std::string_view& className, const std::string_view& constructorSignature) {
    ...
    x_ASSERT(object != NULL, "the new global reference should not be null");
    return object;
}
...
T getObjectTypeValue(jobject object,
    ...
    x_ASSERT(getEnv()->GetObjectRefType(object) != JNIInvalidRefType, "object ref is invalid");
    auto env = getEnv();
    auto clazz = findClass(className);
    ...
    if constexpr (std::is_same<T, bool>::value) {
        ...
        x_THROW_RUNTIME_ERROR("Unsupported type: " + std::string(typeid(T).name()));
    }
    jniErrorCheck();
    return value;
}
...
std::string getStringObjectValue(jstring object) {
    x_ASSERT2_FMT(object != nullptr, "object should not be null");
...
jni::jobject deserializeInstance(const jni::JavaSerializedInstance& serializedInstance) {
    ...
    const auto clazz = env->FindClass("stream/IoTSPE/MapJavaUdfUtils");
    jniErrorCheck();
...
void loadClassesFromByteList(const jni::JavaUDFByteCodeList& byteCodeList) {
    for (auto& [className, byteCode] : byteCodeList) {
        ...
        x_DEBUG("Injecting Java class into JVM: {}", jniName);
