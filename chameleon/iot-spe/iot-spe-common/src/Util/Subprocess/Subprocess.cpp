...
Subprocess::Subprocess(std::string cmd, std::vector<std::string> argv) {
    ...
    switch (pid = ::fork()) {
        case -1: {
            x_FATAL_ERROR("Subprocess {} failed to start", cmd);
            return;
        }
        case 0: {
    ...
    x_DEBUG("Started process {} with pid: {}", cmd, pid);
...
Subprocess::~Subprocess() {
    x_INFO("Killing process->PID: {}", pid);
    ::kill(pid, SIGKILL);
    stopped = true;
    logThread.join();
}
...
bool Subprocess::kill() {
    x_DEBUG("Killing pid={}", pid);
    auto res = ::kill(pid, SIGKILL);
    x_ASSERT(res == 0, "process could not be killed");
    x_DEBUG("Process pid={} successfully killed", pid);
    return res;
}
