...
#ifndef x_COMMON_INCLUDE_UTIL_NONBLOCKINGMONOTONICSEQQUEUE_HPP_
#define x_COMMON_INCLUDE_UTIL_NONBLOCKINGMONOTONICSEQQUEUE_HPP_
  ...
  public:
    ...
    void emplace(uint64_t sequenceNumber, T value) {
        if (sequenceNumber <= currentSeq) {
            x_FATAL_ERROR("Invalid sequence number {} as it is <= {}", sequenceNumber, currentSeq);
...
#endif// x_COMMON_INCLUDE_UTIL_NONBLOCKINGMONOTONICSEQQUEUE_HPP_
