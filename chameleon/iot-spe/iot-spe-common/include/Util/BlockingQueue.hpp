...
#ifndef x_COMMON_INCLUDE_UTIL_BLOCKINGQUEUE_HPP_
#define x_COMMON_INCLUDE_UTIL_BLOCKINGQUEUE_HPP_
  ...
  public:
    ...
    inline void reset() {
        ...
        while (!bufferQueue.empty()) {
            x_TRACE("reset pop={}", bufferQueue.front())
            bufferQueue.pop();
        }
    ...
    inline void push(const T& elem) {
        {
            ...
            x_TRACE("BlockingQueue: pushing element {}", elem)
    ...
    inline const T pop() {
        {
            ...
            //      x_DEBUG("BlockingQueue: popping element {}", bufferQueue.front());
            bufferQueue.pop();
    ...
    inline const std::optional<T> popTimeout(uint64_t timeout_ms) {
        {
            ...
            T retVal = bufferQueue.front();
            //        x_DEBUG("BlockingQueue: popping element timeout {}", bufferQueue.front());
            bufferQueue.pop();
...
#endif// x_COMMON_INCLUDE_UTIL_BLOCKINGQUEUE_HPP_
