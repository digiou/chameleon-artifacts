...
#ifndef x_COMMON_INCLUDE_UTIL_TIMER_HPP_
#define x_COMMON_INCLUDE_UTIL_TIMER_HPP_
...
/**
 * @brief Util class to measure the time of x components and sub-components
 * using snapshots
 */
...
class Timer {
  public:
    ...
    void start() {
        if (running) {
            x_DEBUG("Timer: Trying to start an already running timer so will skip this operation");
        } else {
            running = true;
    ...
    void pause() {
        if (!running) {
            x_DEBUG("Timer: Trying to stop an already stopped timer so will skip this operation");
        } else {
            running = false;
    ...
    void snapshot(std::string snapshotName) {
        if (!running) {
            x_DEBUG("Timer: Trying to take a snapshot of an non-running timer so will skip this operation");
        } else {
            running = true;
            stop_p = ClockType::now();
            auto duration = std::chrono::duration_cast<TimeUnit>(stop_p - start_p);
    ...
    void merge(Timer timer) {
        if (running) {
            x_DEBUG("Timer: Trying to merge while timer is running so will skip this operation");
        } else {
    ...
    int64_t getRuntime() const {
        if (!running) {
            ...
            x_DEBUG("Timer: Trying get runtime while timer is running so will return zero");
            return 0;
        }
    };
    ...
    */
     ...
     * recursively goes through the (probably) xted snapshots and prints them
     */
...
#endif// x_COMMON_INCLUDE_UTIL_TIMER_HPP_
