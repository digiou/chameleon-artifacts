...
#ifndef x_COMMON_INCLUDE_UTIL_THREADBARRIER_HPP_
#define x_COMMON_INCLUDE_UTIL_THREADBARRIER_HPP_
...
class ThreadBarrier {
  public:
    ...
    explicit ThreadBarrier(uint32_t size) : size(size), count(0) {
        x_ASSERT2_FMT(size <= x::Runtime::xThread::MaxNumThreads, "Invalid thread count " << size);
    }
    ...
    ~ThreadBarrier() {
        ...
        x_ASSERT2_FMT(count >= size, "destroying not completed thread barrier count=" << count << " size=" << size);
        x_ASSERT2_FMT(size <= x::Runtime::xThread::MaxNumThreads, "Invalid thread count " << size);
    }
...
#endif// x_COMMON_INCLUDE_UTIL_THREADBARRIER_HPP_
