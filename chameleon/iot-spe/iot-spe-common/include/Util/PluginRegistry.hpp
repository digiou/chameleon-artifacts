...
#ifndef x_COMMON_INCLUDE_UTIL_PLUGINREGISTRY_HPP_
#define x_COMMON_INCLUDE_UTIL_PLUGINREGISTRY_HPP_
 ...
 * A plugin is a provider of a specific type T, which defix the plugin interface.
...
class NamedPluginRegistry {
  ...
  public:
    ...
    static std::unique_ptr<T>& getPlugin(std::string name) {
        ...
        if (found == items.end()) {
            x_THROW_RUNTIME_ERROR("No plugin with name " << name.c_str() << " found.");
        }
        return found->second;
    }
...
#endif// x_COMMON_INCLUDE_UTIL_PLUGINREGISTRY_HPP_
