...
void BaseUnitTest::onFatalException(std::shared_ptr<std::exception> exception, std::string callstack) {
    x_ERROR("onFatalException: exception=[{}] callstack=\n{}", exception->what(), callstack);
    failTest();
    FAIL();
}
        ...
        switch (future.wait_for(std::chrono::minutes(WAIT_TIME_SETUP))) {
            case std::future_status::ready: {
                try {
                    auto res = future.get();
                    if (!res) {
                        x_FATAL_ERROR("Got error in test [{}]", testName);
                        std::exit(-127);
                    }
                } catch (std::exception const& exception) {
                    x_FATAL_ERROR("Got exception in test [{}]: {}", testName, exception.what());
                    FAIL();
                    std::exit(-1);
                }
                break;
            ...
            case std::future_status::deferred: {
                x_ERROR("Cannot terminate test [{}] within deadline", testName);
                FAIL();
                std::exit(-127);
                break;
            }
