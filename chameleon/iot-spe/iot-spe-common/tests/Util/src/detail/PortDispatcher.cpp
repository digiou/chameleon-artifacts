...
static inline auto getMutexPath() {
    ...
    auto fileName = detail::trim(getUsername()) + ".x.lock"s;
...
static inline auto getPortPoolName() {
    ...
    return "/"s + username + ".x.port.pool"s;
}
...
BorrowedPortPtr PortDispatcher::getNextPort() {
    while (true) {
        ...
        x_ASSERT2_FMT(data[nextIndex].checksum == CHECKSUM, "invalid checksum");
    ...
    x_ASSERT(false, "Cannot find free port");
    return nullptr;
}
...
void PortDispatcher::recyclePort(uint32_t portIndex) {
    ...
    if (data[portIndex].free.compare_exchange_strong(expected, true)) {
        ...
        x_ASSERT2_FMT(false, "Port " << data[portIndex].port << " recycled twice");
    }
}

void PortDispatcher::tearDown() {
    x_ASSERT2_FMT(numOfBorrowedPorts == 0,
