...
#ifndef x_x_COMMON_TESTS_UTIL_INCLUDE_DETAIL_SHMFIXEDVECTOR_HPP_
#define x_x_COMMON_TESTS_UTIL_INCLUDE_DETAIL_SHMFIXEDVECTOR_HPP_
...
class SharedMemoryFixedVector {
  ...
  public:
    ...
    void open() {
        ...
        x_ASSERT2_FMT(shmemFd != -1, "cannot create shared area: " << strerror(errno));
        if (created) {
            auto ret = ftruncate(shmemFd, mmapSize) == 0;
            if (!ret) {
                std::cerr << "cannot ftruncate shared area: " << strerror(errno) << std::endl;
            }
            x_ASSERT(ret, "cannot create shared area");
        } else {
            std::string filePath = std::filesystem::temp_directory_path() / "x.tests.begin";
            while (!std::filesystem::exists(filePath)) {
                x_DEBUG("File {} does not exists", filePath);
        ...
        x_ASSERT(mem != MAP_FAILED, "cannot create shared area");
        ...
        if (created) {
            ...
            auto filePath = std::filesystem::temp_directory_path() / "x.tests.begin";
            std::ofstream file;
            file.open(filePath);
            x_ASSERT2_FMT(file.good(), "cannot open file " << filePath);
            file.close();
        } else {
    ...
    ~SharedMemoryFixedVector() {
        ...
        x_ASSERT(0 == munmap(mem, mmapSize), "Cannot munmap");
        x_ASSERT(0 == close(shmemFd), "Cannot close");
        if (doCleanup) {
            if (0 == shm_unlink(name.c_str())) {
                // no process has the shared memory segment open
                std::filesystem::remove_all(std::filesystem::temp_directory_path() / "x.tests.begin");
            }
        }
    }
...
#endif//x_x_COMMON_TESTS_UTIL_INCLUDE_DETAIL_SHMFIXEDVECTOR_HPP_
