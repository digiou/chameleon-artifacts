...
void MlHeuristicStrategy::performOperatorRedundancyElimination(QueryId queryId,
    ...
    x_DEBUG("MlHeuristicStrategy: Updated Global Execution Plan:\n{}", globalExecutionPlan->getAsString());
}
...
void MlHeuristicStrategy::performOperatorPlacement(QueryId queryId,
    ...
    x_DEBUG("MlHeuristicStrategy: Get the all source operators for performing the placement.");
    for (auto& pinnedUpStreamOperator : pinnedUpStreamOperators) {
        x_DEBUG("MlHeuristicStrategy: Get the topology node for source operator {} placement.",
        ...
        if (pinnedUpStreamOperator->getOperatorState() == OperatorState::PLACED) {
            ...
            if (candidateTopologyNode->getAvailableResources() == 0
                ...
                x_ERROR("MlHeuristicStrategy: Unable to find resources on the physical node for placement of source operator");
    ...
    x_DEBUG("MlHeuristicStrategy: Finished placing query operators into the global execution plan");
}
...
void MlHeuristicStrategy::identifyPinningLocation(QueryId queryId,
    ...
    if (!operatorToExecutionNodeMap.contains(logicalOperator->getId())) {
        ...
        x_DEBUG("MlHeuristicStrategy: Place operatorNode with Id:{}.", logicalOperator->getId());
        if ((logicalOperator->hasMultipleChildrenOrParents() && !logicalOperator->instanceOf<SourceLogicalOperatorNode>())
            ...
            x_TRACE("MlHeuristicStrategy: Received an NAry operator for placement.");
            //Check if all children operators already placed
            x_TRACE("MlHeuristicStrategy: Get the topology nodes where child operators are placed.");
            ...
            if (childTopologyNodes.empty()) {
                x_WARNING("MlHeuristicStrategy: No topology node isOperatorAPinnedDownStreamOperator where child operators are "
                            "placed.");
                return;
            }

            x_TRACE("MlHeuristicStrategy: Find a node reachable from all topology nodes where child operators are placed.");
            ...
            if (!candidateTopologyNode) {
                x_ERROR("MlHeuristicStrategy: Unable to find a common ancestor topology node to place the binary operator, "
                          "operatorId: {}",
            ...
            if (logicalOperator->instanceOf<SinkLogicalOperatorNode>()) {
                x_TRACE("MlHeuristicStrategy: Received Sink operator for placement.");
                auto nodeId = std::any_cast<uint64_t>(logicalOperator->getProperty(PINNED_NODE_ID));
                ...
                if (candidateTopologyNode->getAvailableResources() == 0) {
                    x_ERROR("MlHeuristicStrategy: Topology node where sink operator is to be placed has no capacity.");
                    throw Exceptions::RuntimeException(
        ...
        bool tfNotInstalled = logicalOperator->instanceOf<InferModel::InferModelLogicalOperatorNode>()
        ...
        if (logicalOperator->instanceOf<InferModel::InferModelLogicalOperatorNode>()) {
        ...
        if (candidateTopologyNode->getAvailableResources() == 0) {
            ...
            x_DEBUG("MlHeuristicStrategy: Find the next x node in the path where operator can be placed");
            while (!candidateTopologyNode->getParents().empty()) {
                ...
                if (candidateTopologyNode->getAvailableResources() > 0) {
                    x_DEBUG("MlHeuristicStrategy: Found x node for placing the operators with id : {}",
        ...
        if (!candidateTopologyNode || candidateTopologyNode->getAvailableResources() == 0) {
            x_ERROR("MlHeuristicStrategy: No node available for further placement of operators");
            throw Exceptions::RuntimeException("MlHeuristicStrategy: No node available for further placement of operators");
        }

        x_TRACE("MlHeuristicStrategy: Pinn operator to the candidate topology node.");
    ...
    if (isOperatorAPinnedDownStreamOperator != pinnedDownStreamOperators.end()) {
        x_DEBUG("MlHeuristicStrategy: Found pinned downstream operator. Skipping placement of further operators.");
        return;
    }

    x_TRACE("MlHeuristicStrategy: Place further upstream operators.");
