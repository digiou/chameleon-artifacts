...
BasePlacementStrategyPtr IFCOPStrategy::create(x::GlobalExecutionPlanPtr globalExecutionPlan,
...
IFCOPStrategy::IFCOPStrategy(x::GlobalExecutionPlanPtr globalExecutionPlan,
                             x::TopologyPtr topology,
                             x::Optimizer::TypeInferencePhasePtr typeInferencePhase)
    : BasePlacementStrategy(std::move(globalExecutionPlan), std::move(topology), std::move(typeInferencePhase)) {}

bool IFCOPStrategy::updateGlobalExecutionPlan(x::QueryPlanPtr queryPlan) {
    ...
    for (uint32_t i = 1; i < maxIter; i++) {
        ...
        x_TRACE("IFCOP: currentCost: {}", currentCost);
    }
...
PlacementMatrix IFCOPStrategy::getPlacementCandidate(x::QueryPlanPtr queryPlan) {
    ...
    for (auto topoItr = topologyIterator.begin(); topoItr != x::DepthFirstNodeIterator::end(); ++topoItr) {
...
double IFCOPStrategy::getCost(const PlacementMatrix& placementCandidate, x::QueryPlanPtr queryPlan, double costRatio) {
...
double IFCOPStrategy::getLocalCost(const std::vector<bool>& nodePlacement, x::QueryPlanPtr queryPlan) {
    QueryPlanIterator queryPlanIterator = QueryPlanIterator(queryPlan);
...
void IFCOPStrategy::initiateTopologyNodeIdToIndexMap() {
    ...
    uint32_t topoIdx = 0;
    for (auto topoItr = topologyIterator.begin(); topoItr != x::DepthFirstNodeIterator::end(); ++topoItr) {
        x_DEBUG("IFCOP::DEBUG:: topoid= {}", (*topoItr)->as<TopologyNode>()->getId());
        topologyNodeIdToIndexMap.insert({(*topoItr)->as<TopologyNode>()->getId(), topoIdx});
        topoIdx++;
    }
}

void IFCOPStrategy::assignRemainingOperator(x::QueryPlanPtr queryPlan,
                                            uint32_t topoIdx,
    ...
    QueryPlanIterator queryPlanIterator = QueryPlanIterator(queryPlan);
    for (auto qPlanIter = queryPlanIterator.begin(); qPlanIter != x::QueryPlanIterator::end(); ++qPlanIter) {
