...
bool ILPStrategy::updateGlobalExecutionPlan(QueryId queryId,
    ...
    x_INFO("ILPStrategy: Performing placement of the input query plan with id {}", queryId);
    ...
    for (const auto& pinnedUpStreamOperator : pinnedUpStreamOperators) {
        ...
        while (!operatorPath.back()->getParents().empty()) {
            ...
            if (isPinnedDownStreamOperator != pinnedDownStreamOperators.end()) {
                x_DEBUG("ILPStrategy: Found pinned downstream operator. Skipping further downstream operators.");
                break;
            }
            ...
            if (downstreamOperators.empty()) {
                x_ERROR("ILPStrategy: Unable to find pinned downstream operator.");
                return false;
            }
            ...
            for (auto& downstreamOperator : downstreamOperators) {
                ...
                if (unplacedDownStreamOperatorCount > 1) {
                    x_ERROR("ILPStrategy: Current implementation can not place plan with multiple downstream operators.");
                    return false;
                }
        ...
        while (!topologyPath.back()->getParents().empty()) {
            ...
            if (topologyPath[0]->getParents().size() > 1) {
                x_ERROR("ILPStrategy: Current implementation can not place operators on topology with multiple paths.");
                return false;
            }
    ...
    for (auto const& [operatorID, position] : operatorPositionMap) {
        ...
        for (auto downStreamOperator : logicalOperatorNode->getParents()) {
            ...
            if (operatorMap.find(downStreamOperatorId) != operatorMap.end()) {
                ...
                x_DEBUG("distance: {} {}", operatorID, distance.to_string());
                double output;
    ...
    x_DEBUG("cost_net: {}", cost_net.to_string());
    ...
    if (z3::sat != opt.check()) {
        x_ERROR("ILPStrategy: Solver failed.");
        return false;
    }
    ...
    x_DEBUG("ILPStrategy:model: {}", z3Model.to_string());
    x_INFO("Solver found solution with cost: {}", z3Model.eval(cost_net).get_decimal_string(4));
    ...
    for (auto const& [operatorNode, topologyNode] : operatorToTopologyNodeMap) {
        x_INFO("Operator {} is executed on Topology Node {}", operatorNode->toString(), topologyNode->toString());
    }
...
void ILPStrategy::addConstraints(z3::optimize& opt,
    ...
    for (uint64_t i = 0; i < operatorNodePath.size(); i++) {
        ...
        if (i > 1) {// if i == 0 noting to erase, if i == 1 we only have one "set" of values (which we still need)
            ...
            x_DEBUG("Deleting old placement constraints");
        }
...
bool ILPStrategy::pinOperators(z3::model& z3Model, std::map<std::string, z3::expr>& placementVariables) {
    for (const auto& placementMapping : placementVariables) {
        ...
        if (z3Model.eval(placementMapping.second).get_numeral_int() == 1) {
            x_DEBUG("Pinning operator with ID {}", operatorId);
