...
bool HashSignatureBasedCompleteQueryMergerRule::apply(GlobalQueryPlanPtr globalQueryPlan) {
    ...
    if (queryPlansToAdd.empty()) {
        x_WARNING("HashSignatureBasedCompleteQueryMergerRule: Found no new query metadata in the global query plan."
                    " Skipping the Signature Based Equal Query Merger Rule.");
        return true;
    }

    x_DEBUG("HashSignatureBasedCompleteQueryMergerRule: Iterating over all Shared Query MetaData in the Global Query Plan");
    ...
    for (auto& targetQueryPlan : queryPlansToAdd) {
        ...
        for (auto& hostSharedQueryPlan : hostSharedQueryPlans) {
            ...
            if (foundMatch) {
                x_TRACE("HashSignatureBasedCompleteQueryMergerRule: Merge target Shared metadata into address metadata");
        ...
        if (!merged) {
            x_DEBUG("HashSignatureBasedCompleteQueryMergerRule: computing a new Shared Query Plan");
