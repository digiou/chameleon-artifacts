...
bool Z3SignatureBasedPartialQueryMergerBottomUpRule::apply(GlobalQueryPlanPtr globalQueryPlan) {
    x_INFO("Z3SignatureBasedPartialQueryMergerBottomUpRule: Applying Signature Based Equal Query Merger Rule to the Global "
             "Query Plan");
    std::vector<QueryPlanPtr> queryPlansToAdd = globalQueryPlan->getQueryPlansToAdd();
    if (queryPlansToAdd.empty()) {
        x_WARNING("Z3SignatureBasedPartialQueryMergerBottomUpRule: Found only a single query metadata in the global query plan."
    ...
    for (const auto& targetQueryPlan : queryPlansToAdd) {
        ...
        for (auto& hostSharedQueryPlan : hostSharedQueryPlans) {
            ...
            if (!matchedTargetToHostOperatorMap.empty()) {
                x_TRACE("Z3SignatureBasedPartialQueryMergerBottomUpRule: Merge target Shared metadata into address metadata");
        ...
        if (!matched) {
            x_DEBUG("Z3SignatureBasedPartialQueryMergerBottomUpRule: computing a new Shared Query Plan");
...
Z3SignatureBasedPartialQueryMergerBottomUpRule::areQueryPlansEqual(const QueryPlanPtr& targetQueryPlan,
    ...
    x_DEBUG("Z3SignatureBasedPartialQueryMergerBottomUpRule: check if the target and address query plans are syntactically "
              "equal or not");
...
Z3SignatureBasedPartialQueryMergerBottomUpRule::areOperatorEqual(const LogicalOperatorNodePtr& targetOperator,
    ...
    if (targetOperator->instanceOf<SinkLogicalOperatorNode>() && hostOperator->instanceOf<SinkLogicalOperatorNode>()) {
        x_TRACE("Z3SignatureBasedPartialQueryMergerBottomUpRule: Both target and host operators are of sink type.");
        return {};
    }
    ...
    if (signatureEqualityUtil->checkEquality(targetOperator->getZ3Signature(), hostOperator->getZ3Signature())) {
        x_TRACE("Z3SignatureBasedPartialQueryMergerBottomUpRule: Check if parents of target and address operators are equal.");
    ...
    x_WARNING("Z3SignatureBasedPartialQueryMergerBottomUpRule: Target and host operators are not matched.");
    return {};
}
