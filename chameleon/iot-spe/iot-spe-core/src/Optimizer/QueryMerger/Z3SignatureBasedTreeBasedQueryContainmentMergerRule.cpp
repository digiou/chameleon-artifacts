...
bool Z3SignatureBasedTreeBasedQueryContainmentMergerRule::apply(GlobalQueryPlanPtr globalQueryPlan) {
    ...
    x_INFO("Applying Z3SignatureBasedPartialQueryContainmentMergerRule to the Global Query Plan");
    std::vector<QueryPlanPtr> queryPlansToAdd = globalQueryPlan->getQueryPlansToAdd();
    if (queryPlansToAdd.empty()) {
        x_WARNING("Found only a single query metadata in the global query plan."
                    " Skipping the Signature Based Equal Query Merger Rule.");
        return true;
    }

    x_DEBUG("Iterating over all Shared Query MetaData in the Global Query Plan");
    ...
    for (const auto& targetQueryPlan : queryPlansToAdd) {
        ...
        for (auto& hostSharedQueryPlan : hostSharedQueryPlans) {
            ...
            if (!matchedTargetToHostOperatorMap.empty()) {
                x_TRACE("Z3SignatureBasedPartialQueryMergerRule: Merge target Shared metadata into address metadata");
                ...
                x_TRACE("Size of matchedTargetToHostOperatorMap: {}", std::to_string(matchedTargetToHostOperatorMap.size()));
        ...
        if (!matched) {
            x_DEBUG("Z3SignatureBasedPartialQueryMergerRule: computing a new Shared Query Plan");
...
void Z3SignatureBasedTreeBasedQueryContainmentMergerRule::addContainmentOperatorChain(
    ...
    x_TRACE("ContainerQueryPlan: {}; ContainerOperator: {}; DownstreamOperator: {}; ContainedOperator: {}; "
    ...
    for (const auto& parent : parents) {
        ...
        x_TRACE("Parent: {}", parent->toString());
    ...
    x_TRACE("Children upstreamContainedOperator size: {}; Children upstreamContainedOperator: {}",
              upstreamContainedOperator->getChildren().size(),
              upstreamContainedOperator->getChildren()[0]->toString());
    if (!addedNewParent) {
        x_WARNING("Z3SignatureBasedPartialQueryMergerRule: Failed to add new parent");
    }
}
}// namespace x::Optimizer
