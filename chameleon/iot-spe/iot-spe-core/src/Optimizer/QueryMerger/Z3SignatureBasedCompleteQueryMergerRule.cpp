...
bool Z3SignatureBasedCompleteQueryMergerRule::apply(GlobalQueryPlanPtr globalQueryPlan) {
    ...
    if (queryPlansToAdd.empty()) {
        x_WARNING("Z3SignatureBasedCompleteQueryMergerRule: Found no new query plan to add in the global query plan."
                    " Skipping the Signature Based Equal Query Merger Rule.");
        return true;
    }

    x_DEBUG("Z3SignatureBasedCompleteQueryMergerRule: Iterating over all Shared Query MetaData in the Global Query Plan");
    ...
    for (const auto& targetQueryPlan : queryPlansToAdd) {
        ...
        if (!matched) {
            x_DEBUG("Z3SignatureBasedCompleteQueryMergerRule: computing a new Shared Query Plan");
