...
bool Z3SignatureBasedPartialQueryMergerRule::apply(GlobalQueryPlanPtr globalQueryPlan) {
    ...
    x_INFO("Z3SignatureBasedPartialQueryMergerRule: Applying Signature Based Equal Query Merger Rule to the Global Query Plan");
    std::vector<QueryPlanPtr> queryPlansToAdd = globalQueryPlan->getQueryPlansToAdd();
    if (queryPlansToAdd.empty()) {
        x_WARNING("Z3SignatureBasedPartialQueryMergerRule: Found only a single query metadata in the global query plan."
                    " Skipping the Signature Based Equal Query Merger Rule.");
        return true;
    }

    x_DEBUG("Z3SignatureBasedPartialQueryMergerRule: Iterating over all Shared Query MetaData in the Global Query Plan");
    ...
    for (const auto& targetQueryPlan : queryPlansToAdd) {
        ...
        for (auto& hostSharedQueryPlan : hostSharedQueryPlans) {
            x_INFO("HostSharedQueryPlan: {}", hostSharedQueryPlan->getQueryPlan()->toString());
            x_INFO("targetQueryPlan: {}", targetQueryPlan->toString());
                ...
                while (!targetOperators.empty()) {
                    ...
                    for (const auto& hostRootOperator : hostQueryPlan->getRootOperators()) {
                        ...
                        while (!hostOperators.empty()) {
                            ...
                            x_INFO("Check eq for target: {}, and host: {}",
