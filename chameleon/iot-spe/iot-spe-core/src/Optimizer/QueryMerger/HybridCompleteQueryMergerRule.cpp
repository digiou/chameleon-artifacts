...
bool HybridCompleteQueryMergerRule::apply(GlobalQueryPlanPtr globalQueryPlan) {
    x_INFO("HybridCompleteQueryMergerRule: Applying Signature Based Equal Query Merger Rule to the Global Query Plan");
    std::vector<QueryPlanPtr> queryPlansToAdd = globalQueryPlan->getQueryPlansToAdd();
    if (queryPlansToAdd.empty()) {
        x_WARNING("HybridCompleteQueryMergerRule: Found no new query plan to add in the global query plan."
                    " Skipping the Signature Based Equal Query Merger Rule.");
        return true;
    }

    x_DEBUG("HybridCompleteQueryMergerRule: Iterating over all Shared Query MetaData in the Global Query Plan");
    ...
    for (const auto& targetQueryPlan : queryPlansToAdd) {
        ...
        for (auto& hostSharedQueryPlan : hostSharedQueryPlans) {
            ...
            if (foundMatch) {
                x_TRACE("HybridCompleteQueryMergerRule: Merge target Shared metadata into address metadata");
        ...
        if (!matched) {
            x_DEBUG("HybridCompleteQueryMergerRule: computing a new Shared Query Plan");
