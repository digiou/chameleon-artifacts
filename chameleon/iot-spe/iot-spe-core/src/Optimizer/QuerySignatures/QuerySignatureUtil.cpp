...
QuerySignaturePtr QuerySignatureUtil::createQuerySignatureForOperator(const z3::ContextPtr& context,
    ...
    try {
        x_DEBUG("QuerySignatureUtil: Creating query signature for operator {}", operatorNode->toString());
        auto children = operatorNode->getChildren();
        if (operatorNode->isUnaryOperator()) {
            if (operatorNode->instanceOf<SourceLogicalOperatorNode>() && !children.empty()) {
                x_THROW_RUNTIME_ERROR("QuerySignatureUtil: Source can't have children : " + operatorNode->toString());
            } else if (operatorNode->instanceOf<SinkLogicalOperatorNode>() && children.empty()) {
                x_THROW_RUNTIME_ERROR("QuerySignatureUtil: Source can't have empty children set : " + operatorNode->toString());
            } else if (!(operatorNode->instanceOf<SourceLogicalOperatorNode>()
                         || operatorNode->instanceOf<SinkLogicalOperatorNode>())
                       && children.size() != 1) {
                x_THROW_RUNTIME_ERROR("QuerySignatureUtil: Unary operator can have only one children : "
                                        + operatorNode->toString() + " found : " + std::to_string(children.size()));
            }
        } else if (operatorNode->isBinaryOperator() && children.size() != 2) {
            x_THROW_RUNTIME_ERROR("QuerySignatureUtil: Binary operator can't have empty or only one children : "
                                    + operatorNode->toString());
        }

        if (operatorNode->instanceOf<SourceLogicalOperatorNode>()) {
            x_TRACE("QuerySignatureUtil: Computing Signature for Source operator");
        ...
        if (operatorNode->instanceOf<SinkLogicalOperatorNode>()) {
            x_TRACE("QuerySignatureUtil: Computing Signature for Sink operator");
            x_ASSERT(!children.empty(), "Sink operator should have at least one child.");
            return children[0]->as<LogicalOperatorNode>()->getZ3Signature();
        } else if (operatorNode->instanceOf<FilterLogicalOperatorNode>()) {
            x_TRACE("QuerySignatureUtil: Computing Signature for filter operator");
            ...
            x_TRACE("QuerySignatureUtil: Computing Signature for Merge operator");
            ...
            x_TRACE("QuerySignatureUtil: Computing Signature for Map operator");
            ...
            x_TRACE("QuerySignatureUtil: Computing Signature for window operator");
            ...
            x_TRACE("QuerySignatureUtil: Computing Signature for Project operator");
            ...
            x_TRACE("QuerySignatureUtil: Computing Signature for watermark operator");
            ...
            x_TRACE("QuerySignatureUtil: Computing Signature for join operator");
...
#ifdef TFDEF
            x_TRACE("QuerySignatureUtil: Computing Signature for infer model operator");
...
#else
            x_THROW_RUNTIME_ERROR("Trying to use InferModelLogicalOperatorNode but TFDEF not defined!");
#endif// TFDEF
        }
...
QuerySignaturePtr QuerySignatureUtil::createQuerySignatureForProject(const ProjectionLogicalOperatorNodePtr& projectOperator) {
    ...
    x_ASSERT(children.size() == 1, "Project operator should only have one and non null children.");
...
QuerySignaturePtr QuerySignatureUtil::createQuerySignatureForInferModel(
    ...
    const x::InferModel::InferModelLogicalOperatorNodePtr& inferModelOperator) {
    //Fetch query signature of the child operator
    std::vector<NodePtr> children = inferModelOperator->getChildren();
    x_ASSERT(children.size() == 1, "InferModel operator should only have one non null children.");
...
QuerySignaturePtr QuerySignatureUtil::createQuerySignatureForMap(const z3::ContextPtr& context,
    ...
    x_ASSERT(children.size() == 1, "Map operator should only have one non null children.");
    auto child = children[0];
    auto childQuerySignature = child->as<LogicalOperatorNode>()->getZ3Signature();
    ...
    for (auto& schemaFieldToExprMap : schemaFieldToExprMaps) {
        ...
        for (auto& [operandExprName, operandExpr] : rhsOperandFieldMap) {
            ...
            if (found == schemaFieldToExprMap.end()) {
                x_THROW_RUNTIME_ERROR("QuerySignatureUtil: " + operandExprName + " doesn't exists");
            }
...
QuerySignaturePtr QuerySignatureUtil::createQuerySignatureForFilter(const z3::ContextPtr& context,
    ...
    x_TRACE("QuerySignatureUtil: Replace Z3 Expression for the filed with corresponding column values from "
              "children signatures");
    //Fetch the signature of only children and get the column values
    auto schemaFieldToExprMaps = childQuerySignature->getSchemaFieldToExprMaps();
...
QuerySignatureUtil::createQuerySignatureForWatermark(const z3::ContextPtr& context,
    ...
    if (watermarkDescriptor->instanceOf<Windowing::EventTimeWatermarkStrategyDescriptor>()) {
        ...
        //Compute equal condition for allowed latexs
        auto allowedLatexsVar = context->int_const("allowedLatexs");
        auto allowedLatexs = eventTimeWatermarkStrategy->getAllowedLatexs().getTime();
        auto allowedLatexsVal = context->int_val(allowedLatexs);
        auto allowedLatexsExpr = to_expr(*context, Z3_mk_eq(*context, allowedLatexsVar, allowedLatexsVal));
        ...
        auto eventTimeFieldNameAndSource =
            x::Util::splitWithStringDelimiter<std::string>(eventTimeFieldName, "$")[0] + "." + "eventTimeField";
        ...
        Z3_ast andConditions[] = {allowedLatexsExpr, eventTimeFieldExpr};
        ...
        x_THROW_RUNTIME_ERROR("QuerySignatureUtil: Unrecognized watermark descriptor found.");
    }
...
QuerySignaturePtr QuerySignatureUtil::createQuerySignatureForUnion(const z3::ContextPtr& context,
    ...
    x_DEBUG("QuerySignatureUtil: Computing Signature from children signatures");
    ...
    x_TRACE("LeftSignature Conditions: {}", leftSignature->getConditions()->to_string());
    x_TRACE("RightSignature Conditions: {}", rightSignature->getConditions()->to_string());
    ...
    x_TRACE("LeftSignature source name qualifier: {}", unionOperator->getLeftInputSchema()->getSourceNameQualifier());
...
QuerySignaturePtr QuerySignatureUtil::createQuerySignatureForJoin(const z3::ContextPtr& context,
    ...
    if (children.size() != 2) {
        x_THROW_RUNTIME_ERROR("Join operator can have only 2 children. Found " + std::to_string(children.size()));
    }
    ...
    //        windowTimeKeyVal = context->string_val(timeCharacteristic->getField()->getName());
    //    } else {
    //        x_THROW_RUNTIME_ERROR("QuerySignatureUtil: Unknown window Time Characteristic");
    //    }
    ...
    if (windowType->getTimeBasedSubWindowType() == Windowing::TimeBasedWindowType::TUMBLINGWINDOW) {
        ...
        x_ERROR("QuerySignatureUtil: Cant serialize window Time Type");
    }
...
QuerySignaturePtr QuerySignatureUtil::createQuerySignatureForWindow(const z3::ContextPtr& context,
    ...
    x_ASSERT(children.size() == 1 && children[0], "Window operator should only have one non null child.");
    auto child = children[0];
    auto childQuerySignature = child->as<LogicalOperatorNode>()->getZ3Signature();

    x_DEBUG("QuerySignatureUtil: compute signature for window operator");
    ...
    x_TRACE("Create Window Signature");
    uint64_t length = 0;
    uint64_t slide = 0;
    //Compute the expression for window time key
    if (windowDefinition->getWindowType()->isTimeBasedWindowType()) {
        ...
        if (timeCharacteristic->getType() == Windowing::TimeCharacteristic::Type::EventTime) {
            ...
            x_ERROR("QuerySignatureUtil: Cant serialize window Time Characteristic");
        }
        auto windowTimeKeyVar = context->constant(context->str_symbol("time-key"), context->string_sort());
        auto windowTimeKeyExpression = to_expr(*context, Z3_mk_eq(*context, windowTimeKeyVar, windowTimeKeyVal));
        ...
        x_TRACE("Time based window signature created.");
    ...
    for (auto windowAggregation : windowDefinition->getWindowAggregation()) {
        x_TRACE("Current window aggregation: {}", windowAggregation->toString());
        switch (windowAggregation->getType()) {
            ...
            default: {
                x_FATAL_ERROR("QuerySignatureUtil: could not cast aggregation type");
            }
        }
    ...
    for (auto& schemaFieldToExprMap : schemaFieldToExprMaps) {
        std::map<std::string, z3::ExprPtr> updatedSchemaMap;
        x_TRACE("Output Schema: {}", outputSchema->toString());
        for (auto& outputField : outputSchema->fields) {
            x_TRACE("Current outputField: {}", outputField->toString());
            x_TRACE("Current outputField: {}", outputField->getName());
            ...
            if (originalAttributeName.find("start") != std::string ::npos
                ...
                x_TRACE("UpdatedFieldExpr: {}", updatedFieldExpr->to_string());
