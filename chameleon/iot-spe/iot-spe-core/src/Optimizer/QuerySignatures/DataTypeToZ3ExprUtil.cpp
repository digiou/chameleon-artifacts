...
DataTypeToZ3ExprUtil::createForField(const std::string& fieldName, const DataTypePtr& dataType, const z3::ContextPtr& context) {
    x_DEBUG("DataTypeToZ3ExprUtil: creating z3 expression for field");
    z3::ExprPtr expr;
    if (dataType->isInteger()) {
        ...
        x_THROW_RUNTIME_ERROR("Creating Z3 expression is not possible for " + dataType->toString());
    }
...
Z3ExprAndFieldMapPtr DataTypeToZ3ExprUtil::createForDataValue(const ValueTypePtr& valueType, const z3::ContextPtr& context) {
    x_DEBUG("DataTypeToZ3ExprUtil: creating z3 expression for value type");
    if (valueType->dataType->isArray()) {
        x_THROW_RUNTIME_ERROR("Can't support creating Z3 expression for data value of array type.");
        return nullptr;
    }
    ...
    if (valueTypeType->isUndefined()) {
        x_THROW_RUNTIME_ERROR("Can't support creating Z3 expression for data value of undefined type.");
        ...
        x_THROW_RUNTIME_ERROR("Creating Z3 expression is not possible for " + valueTypeType->toString());
    }
