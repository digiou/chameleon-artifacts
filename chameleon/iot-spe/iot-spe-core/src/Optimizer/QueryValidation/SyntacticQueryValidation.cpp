...
x::QueryPlanPtr SyntacticQueryValidation::validate(const std::string& inputQuery) {
    x::QueryPlanPtr queryPlan = nullptr;
    try {
        ...
        if (inputQuery.starts_with("PATTERN")) {
            x_DEBUG("SyntacticQueryValidation: parse pattern query from declarative PSL.");
            queryPlan = queryParsingService->createPatternFromCodeString(inputQuery);
        } else {
            x_DEBUG("SyntacticQueryValidation: parse C++ query from query string.");
