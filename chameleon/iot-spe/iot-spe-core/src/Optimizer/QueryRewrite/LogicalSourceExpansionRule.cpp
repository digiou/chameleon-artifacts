...
QueryPlanPtr LogicalSourceExpansionRule::apply(QueryPlanPtr queryPlan) {
    x_INFO("LogicalSourceExpansionRule: Plan before\n{}", queryPlan->toString());
    ...
    for (const auto& sourceOperator : sourceOperators) {
        ...
        x_TRACE("LogicalSourceExpansionRule: Get the number of physical source locations in the topology.");
        ...
        x_TRACE("LogicalSourceExpansionRule: Found {} physical source locations in the topology.", sourceCatalogEntries.size());
        ...
        x_TRACE("LogicalSourceExpansionRule: Create {} duplicated logical sub-graph and add to original graph",
        ...
        for (const auto& sourceCatalogEntry : sourceCatalogEntries) {
            x_TRACE("LogicalSourceExpansionRule: Create duplicated logical sub-graph");
    ...
    x_DEBUG("LogicalSourceExpansionRule: Plan after\n{}", queryPlan->toString());
    return queryPlan;
}
...
void LogicalSourceExpansionRule::removeConnectedBlockingOperators(const NodePtr& operatorNode) {
    ...
    x_TRACE("LogicalSourceExpansionRule: For each parent look if their ancestor has a n-ary operator or a sink operator.");
