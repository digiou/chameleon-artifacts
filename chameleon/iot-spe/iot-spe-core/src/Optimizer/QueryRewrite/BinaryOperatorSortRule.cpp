...
BinaryOperatorSortRule::BinaryOperatorSortRule() { x_DEBUG("BinaryOperatorSortRule()"); };

QueryPlanPtr BinaryOperatorSortRule::apply(QueryPlanPtr queryPlanPtr) {
    x_INFO("Apply BinaryOperatorSortRule ");
...
void BinaryOperatorSortRule::sortChildren(const BinaryOperatorNodePtr& binaryOperator) {
    ...
    x_ASSERT(children.size() == 2, "Binary operator should have only 2 children");
