  ...
  public:
    ...
    QueryCompilationResultPtr compileQuery(QueryCompilation::QueryCompilationRequestPtr request) override {
        ...
        try {
            ...
            for (auto& pipe : pipelinedQueryPlan->getPipelix()) {
                if (pipe->getQueryPlan()->getRootOperators()[0]->instanceOf<QueryCompilation::ExecutableOperator>()) {
                    ...
                    x_DEBUG("executable pipeline id {}", pipe->getPipelineId());
                    auto stage = executableOperator->getExecutablePipelixtage();
                    auto cStage = std::dynamic_pointer_cast<Runtime::Execution::CompiledExecutablePipelixtage>(stage);
