...
GlobalQueryPlanPtr GlobalQueryPlanUpdatePhase::execute(const std::vector<xRequestPtr>& xRequests) {
    //FIXME: Proper error handling #1585
    try {
        //TODO: Parallelize this loop #1738
        for (const auto& xRequest : xRequests) {
            auto requestType = xRequest->getRequestType();
            switch (requestType) {
                case RequestType::StopQuery: {
                    processStopQueryRequest(xRequest->as<StopQueryRequest>());
                    break;
                }
                case RequestType::FailQuery: {
                    processFailQueryRequest(xRequest->as<FailQueryRequest>());
                    break;
                }
                case RequestType::AddQuery: {
                    processAddQueryRequest(xRequest->as<AddQueryRequest>());
                    break;
                }
                case RequestType::RemoveTopologyLink: {
                    processRemoveTopologyLinkRequest(xRequest->as<x::Experimental::RemoveTopologyLinkRequest>());
                    break;
                }
                case RequestType::RemoveTopologyNode: {
                    processRemoveTopologyNodeRequest(xRequest->as<x::Experimental::RemoveTopologyNodeRequest>());
                    break;
                }
                default:
                    x_ERROR("QueryProcessingService: Received unhandled request type  {}", xRequest->toString());
                    x_WARNING("QueryProcessingService: Skipping to process next request.");
            }
        }
        x_DEBUG("GlobalQueryPlanUpdatePhase: Successfully updated global query plan");
        return globalQueryPlan;
    } catch (std::exception& ex) {
        x_ERROR("GlobalQueryPlanUpdatePhase: Exception occurred while updating global query plan with:  {}", ex.what());
...
void GlobalQueryPlanUpdatePhase::processStopQueryRequest(const StopQueryRequestPtr& stopQueryRequest) {
    ...
    x_INFO("QueryProcessingService: Request received for stopping the query {}", queryId);
...
void GlobalQueryPlanUpdatePhase::processFailQueryRequest(const FailQueryRequestPtr& failQueryRequest) {
    ...
    x_INFO("QueryProcessingService: Request received for stopping the query {}", sharedQueryId);
...
void GlobalQueryPlanUpdatePhase::processAddQueryRequest(const AddQueryRequestPtr& addQueryRequest) {
    ...
    x_INFO("QueryProcessingService: Request received for optimizing and deploying of the query {}", queryId);
    ...
    x_DEBUG("QueryProcessingService: Performing Query type inference phase for query:  {}", queryId);
    queryPlan = typeInferencePhase->execute(queryPlan);

    //4. Set memory layout of each logical operator
    x_DEBUG("QueryProcessingService: Performing query choose memory layout phase:  {}", queryId);
    queryPlan = setMemoryLayoutPhase->execute(queryPlan);

    //5. Perform query re-write
    x_DEBUG("QueryProcessingService: Performing Query rewrite phase for query:  {}", queryId);
    ...
    x_DEBUG("QueryProcessingService: Applying Query Merger Rules as Query Merging is enabled.");
    queryMergerPhase->execute(globalQueryPlan);
}

void GlobalQueryPlanUpdatePhase::processRemoveTopologyLinkRequest(
    const x::Experimental::RemoveTopologyLinkRequestPtr& removeTopologyLinkRequest) {
    ...
    if (!upstreamExecutionNode || !downstreamExecutionNode) {
        x_INFO("RemoveTopologyLinkRequest: {} has no effect on the running queries", removeTopologyLinkRequest->toString());
        return;
    }
    ...
    if (impactedSharedQueryIds.empty()) {
        x_INFO("Found no shared query plan that was using the removed link");
        return;
    }
...
void GlobalQueryPlanUpdatePhase::processRemoveTopologyNodeRequest(
    const x::Experimental::RemoveTopologyNodeRequestPtr& removeTopologyNodeRequest) {
    ...
    if (!removedExecutionNode) {
        x_INFO("RemoveTopologyNodeRequest: {} has no effect on the running queries as there are no queries "
                 "placed on the node.",
    ...
    if (impactedSharedQueryIds.empty()) {
        x_INFO("RemoveTopologyNodeRequest: {} has no effect on the running queries as there are no queries placed "
                 "on the node.",
                 removeTopologyNodeRequest->toString());
        return;
    }
