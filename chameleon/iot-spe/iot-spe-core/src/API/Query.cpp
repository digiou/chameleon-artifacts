...
x::Experimental::BatchJoinOperatorBuilder::Join Query::batchJoinWith(const Query& subQueryRhs) {
    return x::Experimental::BatchJoinOperatorBuilder::Join(subQueryRhs, *this);
}
...
And::And(const Query& subQueryRhs, Query& originalQuery)
    ...
    x_DEBUG("Query: add map operator to andWith to add virtual key to originalQuery");
    ...
    x_DEBUG("Query: add name cepLeftKey {}", cepLeftKey);
    x_DEBUG("Query: add name cepRightKey {}", cepRightKey);
...
Seq::Seq(const Query& subQueryRhs, Query& originalQuery)
    : subQueryRhs(const_cast<Query&>(subQueryRhs)), originalQuery(originalQuery) {
    x_DEBUG("Query: add map operator to seqWith to add virtual key to originalQuery");
    //here, we add artificial key attributes to the sources in order to reuse the join-logic later
    std::string cepLeftKey = keyAssignment("cep_leftKey");
    std::string cepRightKey = keyAssignment("cep_rightKey");
    //next: map the attributes with value 1 to the left and right source
...
Query& Seq::window(const Windowing::WindowTypePtr& windowType) const {
    x_DEBUG("Sequence enters window function");
    ...
    x_DEBUG("ExpressionItem for Left Source {}", sourceNameLeft);
    x_DEBUG("ExpressionItem for Right Source {}", sourceNameRight);
...
Query Query::from(const std::string& sourceName) {
    x_DEBUG("Query: create new Query with source {}", sourceName);
...
Query& Query::joinWith(const Query& subQueryRhs,
    ...
    x_DEBUG("Query: add JoinType (INNER_JOIN) to Join Operator");
...
Query& Query::batchJoinWith(const Query& subQueryRhs, ExpressionItem onProbeKey, ExpressionItem onBuildKey) {
    x_DEBUG("Query: add Batch Join Operator to Query");
...
Query& Query::andWith(const Query& subQueryRhs,
                      ...
                      ExpressionItem onRightKey,
                      const Windowing::WindowTypePtr& windowType) {
    x_DEBUG("Query: add JoinType (CARTESIAN_PRODUCT) to AND Operator");
...
Query& Query::seqWith(const Query& subQueryRhs,
                      ...
                      ExpressionItem onRightKey,
                      const Windowing::WindowTypePtr& windowType) {
    x_DEBUG("Query: add JoinType (CARTESIAN_PRODUCT) to SEQ Operator");
    Join::LogicalJoinDefinition::JoinType joinType = Join::LogicalJoinDefinition::JoinType::CARTESIAN_PRODUCT;
    this->queryPlan =
        QueryPlanBuilder::addJoin(this->queryPlan, subQueryRhs.getQueryPlan(), onLeftKey, onRightKey, windowType, joinType);
    return *this;
...
Query& Query::orWith(const Query& subQueryRhs) {
    x_DEBUG("Query: finally we translate the OR into a union OP ");
...
Query& Query::mapUDF(const Catalogs::UDF::UDFDescriptorPtr& descriptor) {
    x_DEBUG("Query: add map java udf operator to query");
...
Query& Query::flatMapUDF(const Catalogs::UDF::UDFDescriptorPtr& descriptor) {
    x_DEBUG("Query: add flat map java udf operator to query");
...
Query& Query::map(const FieldAssignmentExpressionNodePtr& mapExpression) {
    x_DEBUG("Query: add map operator to query");
...
Query& Query::inferModel(const std::string model,
    ...
    x_DEBUG("Query: add map inferModel to query");
    ...
    x_DEBUG("Query::inferModel: Current Operator: {}", op->toString());
    queryPlan->appendOperatorAsNewRoot(op);
    return *this;
}

Query& Query::sink(const SinkDescriptorPtr sinkDescriptor) {
    x_DEBUG("Query: add sink operator to query");
...
Query& Query::assignWatermark(const Windowing::WatermarkStrategyDescriptorPtr& watermarkStrategyDescriptor) {
    x_DEBUG("Query: add assignWatermark operator to query");
