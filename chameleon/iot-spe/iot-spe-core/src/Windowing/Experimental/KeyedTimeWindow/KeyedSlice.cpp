...
KeyedSlice::KeyedSlice(std::shared_ptr<x::Experimental::HashMapFactory> hashMapFactory, uint64_t start, uint64_t end)
    : start(start), end(end), state(hashMapFactory->create()) {}

KeyedSlice::KeyedSlice(std::shared_ptr<x::Experimental::HashMapFactory> hashMapFactory) : KeyedSlice(hashMapFactory, 0, 0) {}
