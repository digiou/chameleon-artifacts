...
BatchJoinOperatorHandler::BatchJoinOperatorHandler(LogicalBatchJoinDefinitionPtr batchJoinDefinition, SchemaPtr resultSchema)
    ...
    x_DEBUG("BatchJoinOperatorHandler(LogicalBatchJoinDefinitionPtr batchJoinDefinition, SchemaPtr resultSchema)");
}
...
void BatchJoinOperatorHandler::start(Runtime::Execution::PipelineExecutionContextPtr context,
                                     ...
                                     uint32_t) {
    x_ASSERT(this->buildPipelineID != 0, "BatchJoinOperatorHandler: The Build Pipeline is not registered at start().");
    x_ASSERT(this->probePipelineID != 0, "BatchJoinOperatorHandler: The Probe Pipeline is not registered at start().");

    x_DEBUG("BatchJoinOperatorHandler::start: Analyzing pipeline. # predecessors of pipeline: {}",
              context->getPredecessors().size());
    if (context->getPipelineID() == this->buildPipelineID) {
        x_DEBUG("BatchJoinOperatorHandler: Identified build pipeline by ID.");
        x_ASSERT(context->getPredecessors().size() >= 1,
                   "BatchJoinOperatorHandler: The Build Pipeline should have at least one predecessor.");

        if (foundAndStartedBuildSide) {
            x_THROW_RUNTIME_ERROR("BatchJoinOperatorHandler::start() was called a second time on a build pipeline.");
            return;
        }

        // send start source event to all build side sources.
        x::Runtime::StartSourceEvent event;
        ...
        for (auto buildPredecessor : this->buildPredecessors) {
            if (const auto* sourcePredecessor = std::get_if<std::weak_ptr<x::DataSource>>(&(buildPredecessor))) {
                x_DEBUG("BatchJoinOperatorHandler: Found Build Source in predecessors of Build pipeline. "
                          "Starting it now.");
                sourcePredecessor->lock()->onEvent(event);
            } else if (const auto* pipelinePredecessor =
                           std::get_if<std::weak_ptr<x::Runtime::Execution::ExecutablePipeline>>(&(buildPredecessor))) {
                x_DEBUG("BatchJoinOperatorHandler: Found Pipeline in predecessors of Build pipeline. "
    ...
    if (context->getPipelineID() == this->probePipelineID) {
        x_DEBUG("BatchJoinOperatorHandler: Identified probe pipeline by ID.");
        x_ASSERT(
        ...
        x_ASSERT(!foundProbeSide, "BatchJoinOperatorHandler::start() was called a second time on a probe pipeline");

        // one of the predecessors will be the Batch Join Build Pipeline
        // we want to get a pointer to the other predecessor
        bool encounteredBuildPipeline = false;// (within the probe pipelix predecessors)
        for (auto probePredecessor : context->getPredecessors()) {
            if (const auto* sourcePredecessor = std::get_if<std::weak_ptr<x::DataSource>>(&probePredecessor)) {
                x_DEBUG("BatchJoinOperatorHandler: Found Probe Source in predecessors of Probe pipeline.");
                this->probePredecessors.push_back(probePredecessor);
            } else if (const auto* pipelinePredecessor =
                           std::get_if<std::weak_ptr<x::Runtime::Execution::ExecutablePipeline>>(&probePredecessor)) {
                ...
                x_DEBUG("BatchJoinOperatorHandler: Found Pipeline in predecessors of Probe pipeline.");
                this->probePredecessors.push_back(probePredecessor);
            }
        }
        // one of the predecessors should have been the build side
        x_ASSERT(encounteredBuildPipeline,
                   "BatchJoinOperatorHandler: One Predecessors of Probe Pipeline should have ID of Build pipeline.");
        return;
    }

    x_FATAL_ERROR("BatchJoinOperatorHandler::start() called with pipeline that didn't have one or two successors!");
    return;
}
...
void BatchJoinOperatorHandler::startProbeSide(Runtime::WorkerContext& context) {
    x_DEBUG("BatchJoinOperatorHandler: Starting Probe Predecessors.");
    x::Runtime::StartSourceEvent event;

    for (auto probePredecessor : this->probePredecessors) {
        if (const auto* sourcePredecessor = std::get_if<std::weak_ptr<x::DataSource>>(&probePredecessor)) {
            x_DEBUG("BatchJoinOperatorHandler: Starting Probe Source predecessor.");
            sourcePredecessor->lock()->onEvent(event, context);
        } else if (const auto* pipelinePredecessor =
                       std::get_if<std::weak_ptr<x::Runtime::Execution::ExecutablePipeline>>(&probePredecessor)) {
            x_DEBUG("BatchJoinOperatorHandler: Send Start Source event to Pipeline (a Probe Side Predecessor).");
