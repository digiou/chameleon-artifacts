...
LogicalBatchJoinDefinition::LogicalBatchJoinDefinition(FieldAccessExpressionNodePtr keyTypeBuild,
    ...
    x_ASSERT(this->keyTypeBuild, "Invalid left join key type");
    x_ASSERT(this->keyTypeProbe, "Invalid right join key type");

    x_ASSERT(this->numberOfInputEdgesBuild > 0, "Invalid number of left edges");
    x_ASSERT(this->numberOfInputEdgesProbe > 0, "Invalid number of right edges");
}
