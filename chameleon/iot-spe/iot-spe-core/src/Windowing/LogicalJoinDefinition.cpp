...
LogicalJoinDefinition::LogicalJoinDefinition(FieldAccessExpressionNodePtr leftJoinKeyType,
    ...
    x_ASSERT(this->leftJoinKeyType, "Invalid left join key type");
    x_ASSERT(this->rightJoinKeyType, "Invalid right join key type");

    x_ASSERT(this->windowType, "Invalid window type");
    x_ASSERT(this->triggerPolicy, "Invalid trigger policy");
    x_ASSERT(this->triggerAction, "Invalid trigger action");
    x_ASSERT(this->numberOfInputEdgesLeft > 0, "Invalid number of left edges");
    x_ASSERT(this->numberOfInputEdgesRight > 0, "Invalid number of right edges");
    x_ASSERT((this->joinType == JoinType::INNER_JOIN || this->joinType == JoinType::CARTESIAN_PRODUCT), "Invalid Join Type");
}
