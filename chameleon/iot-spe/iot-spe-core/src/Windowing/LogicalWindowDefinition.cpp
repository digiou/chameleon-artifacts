...
LogicalWindowDefinition::LogicalWindowDefinition(const std::vector<FieldAccessExpressionNodePtr> keys,
    ...
    : windowAggregation(std::move(windowAggregation)), triggerPolicy(std::move(triggerPolicy)),
      ...
      distributionType(std::move(distChar)), allowedLatexs(allowedLatexs) {
    x_TRACE("LogicalWindowDefinition: create new window definition");
}
...
LogicalWindowDefinitionPtr LogicalWindowDefinition::create(std::vector<WindowAggregationPtr> windowAggregations,
                                                           ...
                                                           uint64_t allowedLatexs) {
    return create({}, windowAggregations, windowType, distChar, triggerPolicy, triggerAction, allowedLatexs);
}
...
LogicalWindowDefinitionPtr LogicalWindowDefinition::copy() {
    return create(onKey, windowAggregation, windowType, distributionType, triggerPolicy, triggerAction, allowedLatexs);
}
...
uint64_t LogicalWindowDefinition::getAllowedLatexs() const { return allowedLatexs; }
