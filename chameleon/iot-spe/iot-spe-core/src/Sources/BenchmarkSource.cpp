...
BenchmarkSource::BenchmarkSource(SchemaPtr schema,
    ...
    x_ASSERT(this->memoryArea && this->memoryAreaSize > 0, "invalid memory area");
    ...
    x_DEBUG("BenchmarkSource() numberOfBuffersToProcess={} memoryAreaSize={}", numberOfBuffersToProcess, memoryAreaSize);
}
...
void BenchmarkSource::runningRoutine() {
    try {
        ...
        x_INFO("Going to produce {}", numberOfTuplesToProduce);
...
#else
                    x_THROW_RUNTIME_ERROR("COPY_BUFFER_SIMD_RTE source mode is not supported.");
#endif
                    break;
                }
...
x::SourceType BenchmarkSource::getType() const { return SourceType::MEMORY_SOURCE; }
}// namespace x