...
LambdaSource::LambdaSource(
    ...
    x_DEBUG("Create LambdaSource with id={} func is {}", operatorId, (this->generationFunction ? "callable" : "not callable"));
...
std::optional<Runtime::TupleBuffer> LambdaSource::receiveData() {
    x_TRACE("LambdaSource::receiveData called on operatorId= {}", operatorId);
    using namespace std::chrono_literals;

    auto buffer = allocateBuffer();
    x_ASSERT2_FMT(numberOfTuplesToProduce * schema->getSchemaSizeInBytes() <= buffer.getBuffer().getBufferSize(),
    ...
    x_TRACE("LambdaSource: Current buffer content {}", buffer.toString(schema));
    x_TRACE("LambdaSource: ReceiveData filled buffer with tuples={}, outOrgID={}",
