...
JSONParser::JSONParser(uint64_t numberOfSchemaFields,
                       ...
                       std::vector<x::PhysicalTypePtr> physicalTypes)
...
bool JSONParser::writeInputTupleToTupleBuffer(const std::string& jsonTuple,
    ...
    x_TRACE("JSONParser::writeInputTupleToTupleBuffer: Current TupleCount:  {}", tupleCount);
    ...
    for (uint64_t fieldIndex = 0; fieldIndex < numberOfSchemaFields; fieldIndex++) {
        ...
        try {
            ...
            // 2. to have a single place for xBasicPhysicalType conversion (could change this)
            x_TRACE("JSONParser::writeInputTupleToTupleBuffer: Current Field:  {}", schemaKeys[fieldIndex]);
            ...
            x_ERROR("JSONParser::writeInputTupleToTupleBuffer: Error when parsing jsonTuple: {}", jsonException.what());
            return false;
        }
        ...
        jsonValue = x::Util::trim(jsonValue, '"');
        jsonValue = x::Util::trim(jsonValue, '\'');
