...
MQTTSource::MQTTSource(SchemaPtr schema,
    ...
    x_TRACE("MQTTSource::MQTTSource: Init MQTTSource to {} with client id: {}.", serverAddress, clientId);
}

MQTTSource::~MQTTSource() {
    x_TRACE("MQTTSource::~MQTTSource()");
    bool success = disconnect();
    if (success) {
        x_DEBUG("MQTTSource::~MQTTSource: Destroy MQTT Source");
    } else {
        x_ERROR("MQTTSource::~MQTTSource: Destroy MQTT Source failed cause it could not be disconnected");
        assert(0);
    }
    x_DEBUG("MQTTSource::~MQTTSource: Destroy MQTT Source");
}
...
std::optional<Runtime::TupleBuffer> MQTTSource::receiveData() {
    x_TRACE("MQTTSource: receiveData ");
    auto buffer = allocateBuffer();
    if (connect()) {
        if (!fillBuffer(buffer)) {
            x_ERROR("MQTTSource::receiveData: Failed to fill the TupleBuffer.");
            return std::nullopt;
        }
    } else {
        x_ERROR("MQTTSource::receiveData: Not connected!");
        return std::nullopt;
    }
...
bool MQTTSource::fillBuffer(Runtime::MemoryLayouts::DynamicTupleBuffer& tupleBuffer) {
    ...
    x_TRACE("MQTTSource::fillBuffer: Fill buffer with #tuples= {}  of size= {}", tuplesThisPass, tupleSize);
    ...
    while (tupleCount < tuplesThisPass && !flushIntervalPassed) {
        ...
        try {
            x_TRACE("Waiting for messages on topic: '{}'", topic);
            ...
            if (connected) {
                ...
                if (message) {// Check if message was received correctly (not nullptr)
                    x_TRACE("Client consume message: '{}'", message->get_payload_str());
                    ...
                    if (!inputParser->writeInputTupleToTupleBuffer(receivedMessageString,
                                                                   ...
                                                                   schema,
                                                                   localBufferManager)) {
                        x_ERROR("MQTTSource::getBuffer: Failed to write input tuple to TupleBuffer.");
                        return false;
                    }
                    x_TRACE("MQTTSource::fillBuffer: Tuples processed for current buffer: {} / {}", tupleCount, tuplesThisPass);
                    tupleCount++;
                } else if (!client->is_connected()) {// message is a nullptr. Check if still connected to broker.
                    x_WARNING("MQTTSource::fillBuffer: Not connected anymore!");
                    connected = false;
                }
            } else if (client->is_connected()) {// We lost connection (connection=false), check if we are connected again.
                x_DEBUG("MQTTSource::fillBuffer: Reconnected, subscribing again!");
            ...
            try {
                ...
                x_ERROR("MQTTSource::fillBuffer: {}", error.what());
                return false;
            } catch (std::exception& error) {
                x_ERROR("MQTTSource::fillBuffer: General Error: {}", error.what());
                return false;
            }
        }
        ...
        if ((bufferFlushIntervalMs > 0 && tupleCount > 0
            ...
            x_TRACE("MQTTSource::fillBuffer: Reached TupleBuffer flush interval. Finishing writing to current TupleBuffer.");
...
bool MQTTSource::connect() {
    if (!connected) {
        x_DEBUG("MQTTSource was !connect now connect: connected");
        ...
        try {
            ...
            x_DEBUG("MQTTSource::connect Connecting to the MQTT server...");
            ...
            x_WARNING("MQTTSource::connect: {}", error.to_string());
            connected = false;
            return connected;
        }

        if (connected) {
            x_DEBUG("MQTTSource::connect: Connection established with topic: {}", topic);
            x_DEBUG("MQTTSource::connect: connected");
        } else {
            x_DEBUG("MQTTSource::connect: NOT connected");
        }
    }
    return connected;
}
...
bool MQTTSource::disconnect() {
    x_DEBUG("MQTTSource::disconnect connected={}", connected);
    if (connected) {
        // If we're here, the client was almost certainly disconnected.
        // But we check, just to make sure.
        if (client->is_connected()) {
            x_DEBUG("MQTTSource: Shutting down and disconnecting from the MQTT server.");
            ...
            x_DEBUG("MQTTSource::disconnect: disconnected.");
        } else {
            x_DEBUG("MQTTSource::disconnect: Client was already disconnected");
        }
        connected = client->is_connected();
    }
    if (!connected) {
        ...
        x_DEBUG("MQTTSource::disconnect: NOT disconnected");
        return connected;
    }
    return !connected;
}
