...
GeoLocation::GeoLocation(double latitude, double longitude) {
    ...
    if (!(std::isnan(latitude) && std::isnan(longitude)) && !checkValidityOfCoordinates(latitude, longitude)) {
        x_ERROR("Trying to create a location with invalid coordinates");
        throw x::Spatial::Exception::CoordinatesOutOfRangeException();
    }
    this->latitude = latitude;
    this->longitude = longitude;
}

GeoLocation::GeoLocation(const x::Spatial::Protobuf::GeoLocation& geoLocation)
...
GeoLocation GeoLocation::fromString(const std::string& coordinates) {
    ...
    if (separator != ',') {
        x_ERROR("input string is not of format \"<latitude>, <longitude>\". Node will be created as non field node");
        throw x::Spatial::Exception::CoordinatesOutOfRangeException();
    }
    double lng;
    stringStream >> lng;
