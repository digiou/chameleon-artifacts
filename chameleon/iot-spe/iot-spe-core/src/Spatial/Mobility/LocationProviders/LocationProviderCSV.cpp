...
void LocationProviderCSV::loadMovementSimulationDataFromCsv() {
    ...
    x_DEBUG("Started csv location source at {}", startTime)
    ...
    while (std::getline(inputStream, csvLine)) {
        ...
        try {
            values = x::Util::splitWithStringDelimiter<std::string>(csvLine, delimiter);
        } catch (std::exception& e) {
            std::string errorString =
                std::string("An error occurred while splitting delimiter of waypoint CSV. ERROR: ") + strerror(errno);
            x_ERROR("LocationProviderCSV:  {}", errorString);
    ...
    x_DEBUG("read {} waypoints from csv", waypoints.size());
    x_DEBUG("first timestamp is {}, last timestamp is {}",
...
DataTypes::Experimental::Waypoint LocationProviderCSV::getCurrentWaypoint() {
    ...
    x_TRACE("Location: {}; Time:{}", currentWayPoint.getLocation().toString(), currentWayPoint.getTimestamp().value());
