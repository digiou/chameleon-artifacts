...
x::Spatial::Mobility::Experimental::ReconnectSchedulePredictor::ReconnectSchedulePredictor(
    ...
    if (configuration->defaultCoverageRadius.getValue() > configuration->nodeIndexUpdateThreshold.getValue()) {
        x_FATAL_ERROR("Default Coverage Radius: {} is bigger than the node index update threshold: {}. this would lead to "
    ...
    if (configuration->nodeIndexUpdateThreshold.getValue() > nodeInfoDownloadRadius) {
        x_FATAL_ERROR("Node info download radius: {} is smaller than the node index update threshold: {}. this would lead to "
...
#else
    (void) configuration;
    x_FATAL_ERROR("cannot construct trajectory predictor without s2 library");
    exit(EXIT_FAILURE);
#endif
}

x::Spatial::Mobility::Experimental::ReconnectSchedulePredictorPtr
x::Spatial::Mobility::Experimental::ReconnectSchedulePredictor::create(
    [[maybe_unused]] const x::Configurations::Spatial::Mobility::Experimental::WorkerMobilityConfigurationPtr& configuration) {
#ifdef S2DEF
    return std::make_shared<x::Spatial::Mobility::Experimental::ReconnectSchedulePredictor>(configuration);
#else
    return nullptr;
#endif
}
...
#ifdef S2DEF
std::optional<x::Spatial::Mobility::Experimental::ReconnectSchedule>
x::Spatial::Mobility::Experimental::ReconnectSchedulePredictor::getReconnectSchedule(
    ...
    if (!locationBuffer.empty() && currentOwnLocation.getLocation() == locationBuffer.back().getLocation()) {
        x_DEBUG("Location has not changed, do not recalculate schedule");
        return std::nullopt;
    }
    ...
    if (locationBuffer.size() < locationBufferSize) {
        if (stepsSinceLastLocationSave == locationBufferSaveRate) {
            ...
            stepsSinceLastLocationSave = 0;
            x_DEBUG("Location buffer is not filled yet, do not recalculate schedule");
        } else {
    ...
    if (isIndexUpdated || isPathUpdated || isSpeedChanged) {
        x_INFO("reconnect prediction data has changed");
        //todo #2815: instead of updating right away, look at if the new trajectory stabilizes itself after a turn
        scheduleReconnects(x::Spatial::Util::S2Utilities::geoLocationToS2Point(parentLocation), fieldNodeIndex);
...
bool x::Spatial::Mobility::Experimental::ReconnectSchedulePredictor::updateAverageMovementSpeed() {
#ifdef S2DEF
    ...
    if (abs(meanDegreesPerNanosec - bufferAverageMovementSpeed) > bufferAverageMovementSpeed * speedDifferenceThresholdFactor) {
        ...
        x_TRACE("average movement speed was updated to {}", bufferAverageMovementSpeed);
        x_TRACE("threshhold is {}", bufferAverageMovementSpeed * speedDifferenceThresholdFactor);
        return true;
    }
    return false;
#else
    x_WARNING("s2 library is needed to update average movement speed");
    return false;
#endif
}

bool x::Spatial::Mobility::Experimental::ReconnectSchedulePredictor::updatePredictedPath(
    ...
    x_WARNING("s2 library is needed to update predicted path");
    return false;
#endif
}
...
x::Spatial::Mobility::Experimental::ReconnectSchedulePredictor::findPathCoverage(const S2Polyline& path,
...
#ifdef S2DEF
bool x::Spatial::Mobility::Experimental::ReconnectSchedulePredictor::scheduleReconnects(
    ...
    while (currentUncoveredRemainingPathDistance > S1Angle(defaultCoverageRadiusAngle)) {
        ...
        if (nextReconnectLocationOnPath.operator!=(reconnectLocationOnPath)) {
            ...
            reconnectPoints.emplace_back(
                x::Spatial::Mobility::Experimental::ReconnectPoint{*currLoc, reconnectParentId, estimatedReconnectTime});
            x_DEBUG("scheduled reconnect to worker with id{}", reconnectParentId);
            reconnectLocationOnPath = nextReconnectLocationOnPath;
            estimatedReconnectTime = nextEstimatedReconnectTime;
        } else {
            x_DEBUG("no nodes available to cover rest of path");
            break;
        }
    }
    return true;
