...
std::optional<Runtime::TupleBuffer> TupleView::receiveData() {
    ...
    if (consumePosition < vector.size()) {
        x_INFO("TupleView::receiveData: success. current consumePosition: {}", consumePosition);
        return vector[consumePosition++];
    } else {
        x_INFO("TupleView::receiveData: failed. current consumePosition: {}", consumePosition);
        return std::nullopt;
    }
};
...
bool TupleView::writeData(Runtime::TupleBuffer buffer) {
    std::unique_lock<std::mutex> lock(mutex);
    if (!buffer) {
        x_ERROR("TupleView::writeData: input buffer invalid");
        return false;
    }
    vector.push_back(buffer);
    x_INFO("TupleView::writeData: success. new vector size: {}", vector.size());
    return true;
};
