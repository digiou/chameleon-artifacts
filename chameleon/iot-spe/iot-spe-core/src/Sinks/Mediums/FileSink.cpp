...
FileSink::FileSink(SinkFormatPtr format,
    ...
    if (!append) {
        if (std::filesystem::exists(filePath.c_str())) {
            ...
            x_ASSERT2_FMT(success, "cannot remove file " << filePath.c_str());
        }
    }
    x_DEBUG("FileSink: open file= {}", filePath);
    ...
    if (sinkFormat->getSinkFormat() != FormatTypes::ARROW_IPC_FORMAT) {
        ...
        x_ASSERT(outputFile.is_open(), "file is not open");
        x_ASSERT(outputFile.good(), "file not good");
    }
    ...
    if (sinkFormat->getSinkFormat() == FormatTypes::ARROW_IPC_FORMAT) {
        ...
        if (!(filePath.find(".arrows"))) {
            x_WARNING("FileSink: An arrow ipc file without '.arrows' extension created as a file sink.");
        }
    }
#endif
}
...
FileSink::~FileSink() {
    x_DEBUG("~FileSink: close file={}", filePath);
    outputFile.close();
}
...
bool FileSink::writeDataToFile(Runtime::TupleBuffer& inputBuffer) {
    ...
    x_DEBUG("FileSink: getSchema medium {} format {} mode {}", toString(), sinkFormat->toString(), this->getAppendAsString());

    if (!inputBuffer) {
        x_ERROR("FileSink::writeDataToFile input buffer invalid");
        return false;
    }

    if (!schemaWritten && sinkFormat->getSinkFormat() != FormatTypes::x_FORMAT) {
        auto schemaStr = sinkFormat->getFormattedSchema();
        outputFile.write(schemaStr.c_str(), (int64_t) schemaStr.length());
        schemaWritten = true;
    } else if (sinkFormat->getSinkFormat() == FormatTypes::x_FORMAT) {
        x_DEBUG("FileSink::getData: writing schema skipped, not supported for x_FORMAT");
    } else {
        x_DEBUG("FileSink::getData: schema already written");
    }

    auto fBuffer = sinkFormat->getFormattedBuffer(inputBuffer);
    x_DEBUG("FileSink::getData: writing to file {} following content {}", filePath, fBuffer);
...
bool FileSink::writeDataToArrowFile(Runtime::TupleBuffer& inputBuffer) {
    ...
    // preliminary checks
    x_TRACE("FileSink: getSchema medium {} format {} and mode {}",
              toString(),
              sinkFormat->toString(),
              this->getAppendAsString());

    if (!inputBuffer) {
        x_ERROR("FileSink::writeDataToArrowFile input buffer invalid");
        return false;
    }
