# ANTLR for x Declarative Languages
...
(java -jar antlr-4.9.2-complete.jar -package x::Parsers -Dlanguage=Cpp IoTDB/x-core/src/Parsers/IoTSPESQL/gen/IoTSPESQL.g4)
(java -jar antlr-4.9.2-complete.jar -package x::Parsers -Dlanguage=Cpp IoTDB/x-core/src/Parsers/IoTSPEPSL/gen/xCEP.g4)

(The plugin currently does not work as vcpkg only provides version 4.9.3, plugin is on 4.10)

The current state does not require the generation of an ANTLR visitor of x, so please remove it from ANTLR configurations for generation.
...
from #include "IoTSPESQLListener.h"
to : #include <Parsers/IoTSPESQL/gen/IoTSPESQLListener.h>
