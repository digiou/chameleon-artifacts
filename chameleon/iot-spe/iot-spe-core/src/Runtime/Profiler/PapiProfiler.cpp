...
static double measureRdtscFreq() {
    ...
    x_ASSERT(sum == 13580802877818827968ull, "Error in RDTSC freq measurement");
    ...
    x_ASSERT(freq_ghz >= 0.5 && freq_ghz <= 5.0, "Invalid RDTSC frequency");
...
class PapiInitializer {
  public:
     ...
     */
    PapiInitializer() {
        x_ASSERT2_FMT(PAPI_library_init(PAPI_VER_CURRENT) == PAPI_VER_CURRENT,
                        "Failed to load PAPI " << PAPI_VERSION << "!=" << PAPI_VER_CURRENT);
        x_ASSERT2_FMT(PAPI_multiplex_init() == PAPI_OK, "Failed to init multiplexing PAPI");
        x_ASSERT2_FMT(PAPI_thread_init(static_cast<unsigned long (*)()>(pthread_self)) == PAPI_OK,
...
PapiCpuProfiler::PapiCpuProfiler(Presets preset, std::ofstream&& csvWriter, uint32_t threadId, uint32_t coreId)
    ...
    x_ASSERT2_FMT(err == PAPI_OK, "Cannot register thread on PAPI worker err=" << err);
    err = PAPI_create_eventset(&eventSet);
    x_ASSERT2_FMT(err == PAPI_OK, "Cannot create PAPI event err=" << err);
    switch (preset) {
        case Presets::BranchPresets: {
            ...
            x_ASSERT2_FMT(err == PAPI_OK, "Cannot register events on PAPI reason: " << err);
            break;
        }
        ...
        case Presets::MemoryBound: {
            ...
            x_ASSERT2_FMT(err == PAPI_OK, "Cannot assign PAPI event set to component 0 worker={} with err {}" << err);
            err = PAPI_set_multiplex(eventSet);
            x_ASSERT2_FMT(err == PAPI_OK, "Cannot promote PAPI event set to multiplex worker={} with err {}" << err);
        ...
        case Presets::CacheBandwidth: {
            // LONGEST_LAT_CACHE:MISS OFFCORE_REQUESTS:ALL_REQUESTS L2_LIx_IN:ALL L1D:REPLACEMENT
            x_ASSERT2_FMT(PAPI_add_named_event(eventSet, "LONGEST_LAT_CACHE:MISS") == PAPI_OK,
                            "Cannot register events on PAPI worker={}");
            x_ASSERT2_FMT(PAPI_add_named_event(eventSet, "OFFCORE_REQUESTS:ALL_REQUESTS") == PAPI_OK,
                            "Cannot register events on PAPI worker={}");
            x_ASSERT2_FMT(PAPI_add_named_event(eventSet, "L2_LIx_IN:ALL") == PAPI_OK,
                            "Cannot register events on PAPI worker={}");
            x_ASSERT2_FMT(PAPI_add_named_event(eventSet, "L1D:REPLACEMENT") == PAPI_OK,
                            "Cannot register events on PAPI worker={}");
            currSamples.resize(4, 0);
            this->csvWriter
                << "core_id,numRecords,worker_id,ts,LONGEST_LAT_CACHE:MISS,OFFCORE_REQUESTS:ALL_REQUESTS,L2_LIx_IN:ALL,"
        ...
        default: {
            x_ASSERT2_FMT(false, "Not supported");
            break;
        }
    }
...
PapiCpuProfiler::~PapiCpuProfiler() {
    ...
    x_ASSERT2_FMT(err == PAPI_OK, "Cannot remove PAPI event set with err= " << err);
    err = PAPI_destroy_eventset(&eventSet);
    x_ASSERT2_FMT(err == PAPI_OK, "Cannot destroy PAPI event set with err=" << err);
    x_ASSERT2_FMT(PAPI_unregister_thread() == PAPI_OK, "Cannot unregister thread on PAPI worker=" << threadId);
}

uint64_t PapiCpuProfiler::startSampling() {
    x_ASSERT2_FMT(PAPI_start(eventSet) == PAPI_OK, "Cannot start PAPI sampling on worker=" << threadId);
    isStarted = true;
...
uint64_t PapiCpuProfiler::stopSampling(std::size_t numRecords) {
    ...
    x_ASSERT2_FMT(PAPI_stop(eventSet, currSamples.data()) == PAPI_OK, "Cannot stop PAPI sampling on worker=" << threadId);
