...
AsyncTaskExecutor::AsyncTaskExecutor(const HardwareManagerPtr& hardwareManager, uint32_t numOfThreads)
    ...
    x_ASSERT(this->hardwareManager, "Invalid hardware manager");
...
bool AsyncTaskExecutor::destroy() {
    ...
    if (running.compare_exchange_strong(expected, false)) {
        try {
            ...
            x_ASSERT(result, "Cannot shutdown AsyncTaskExecutor");
            {
                std::unique_lock lock(workMutex);
            ...
            x_ASSERT(false, "Cannot shutdown AsyncTaskExecutor");
        }
    }
    return false;
}
