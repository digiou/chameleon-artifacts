...
DynamicQueryManager::DynamicQueryManager(std::shared_ptr<AbstractQueryStatusListener> queryStatusListener,
    ...
    x_DEBUG("QueryManger: use dynamic mode with numThreads= {}", numThreads);
}
...
MultiQueueQueryManager::MultiQueueQueryManager(std::shared_ptr<AbstractQueryStatusListener> queryStatusListener,
    ...
    x_DEBUG("QueryManger: use static mode for numberOfQueues={} numThreads={} numberOfThreadsPerQueue=",
              numberOfQueues,
              numThreads,
              numberOfThreadsPerQueue);
    if (numberOfQueues * numberOfThreadsPerQueue != numThreads) {
        x_THROW_RUNTIME_ERROR("number of queues and threads have to match");
    }
...
AbstractQueryManager::~AbstractQueryManager() x_NOEXCEPT(false) { destroy(); }

bool DynamicQueryManager::startThreadPool(uint64_t numberOfBuffersPerWorker) {
    x_DEBUG("startThreadPool: setup thread pool for nodeEngineId= {}  with numThreads= {}", nodeEngineId, numThreads);
    ...
    x_ASSERT2_FMT(false, "Cannot start query manager workers");
    return false;
}
...
bool MultiQueueQueryManager::startThreadPool(uint64_t numberOfBuffersPerWorker) {
    x_DEBUG("startThreadPool: setup thread pool for nodeId= {}  with numThreads= {}", nodeEngineId, numThreads);
    //Note: the shared_from_this prevents from starting this in the ctor because it expects one shared ptr from this
    auto expected = QueryManagerStatus::Created;
    if (queryManagerStatus.compare_exchange_strong(expected, QueryManagerStatus::Running)) {
#ifdef ENABLE_PAPI_PROFILER
...
void AbstractQueryManager::destroy() {
    ...
    x_ASSERT2_FMT(successful, "Cannot stop running queryIdAndCatalogEntryMapping upon query manager destruction");
    ...
    if (queryManagerStatus.compare_exchange_strong(expected, QueryManagerStatus::Destroyed)) {
        ...
        x_DEBUG("AbstractQueryManager::resetQueryManager finished");
    }
}
...
void AbstractQueryManager::reconfigure(ReconfigurationMessage& task, WorkerContext& context) {
    ...
    switch (task.getType()) {
        ...
        default: {
            x_THROW_RUNTIME_ERROR("AbstractQueryManager: task type not supported");
        }
    }
}
    ...
    switch (task.getType()) {
        case ReconfigurationType::Destroy: {
            ...
            if (status == Execution::ExecutableQueryPlanStatus::Invalid) {
                x_WARNING("Query {} was already removed or never deployed", qepId);
                return;
            }
            x_ASSERT(status == Execution::ExecutableQueryPlanStatus::Stopped
            ...
            x_DEBUG("AbstractQueryManager: removed running QEP  {}", qepId);
            break;
        }
        default: {
            x_THROW_RUNTIME_ERROR("AbstractQueryManager: task type not supported");
