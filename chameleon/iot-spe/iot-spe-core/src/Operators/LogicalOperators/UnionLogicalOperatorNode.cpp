...
bool UnionLogicalOperatorNode::inferSchema(Optimizer::TypeInferencePhaseContext& typeInferencePhaseContext) {
    ...
    if (!leftInputSchema->hasEqualTypes(rightInputSchema)) {
        x_ERROR("Found Schema mismatch for left and right schema types. Left schema {} and Right schema {} ",
    ...
    if (leftInputSchema->getLayoutType() != rightInputSchema->getLayoutType()) {
        x_ERROR("Left and right should have same memory layout");
...
void UnionLogicalOperatorNode::inferStringSignature() {
    ...
    x_TRACE("UnionLogicalOperatorNode: Inferring String signature for {}", operatorNode->toString());
    x_ASSERT(!children.empty() && children.size() == 2, "UnionLogicalOperatorNode: Union should have 2 children.");
