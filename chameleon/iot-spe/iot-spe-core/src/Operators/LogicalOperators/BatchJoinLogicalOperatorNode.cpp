...
bool BatchJoinLogicalOperatorNode::inferSchema(Optimizer::TypeInferencePhaseContext& typeInferencePhaseContext) {
    ...
    if (rightInputSchema->equals(leftInputSchema, false)) {
        x_ERROR("BatchJoinLogicalOperatorNode: Found both left and right input schema to be same.");
        throw TypeInferenceException("BatchJoinLogicalOperatorNode: Found both left and right input schema to be same.");
    }

    x_DEBUG("Binary infer left schema={} right schema={}", leftInputSchema->toString(), rightInputSchema->toString());
    x_ASSERT(leftInputSchema->getSchemaSizeInBytes() != 0, "left schema is emtpy");
    x_ASSERT(rightInputSchema->getSchemaSizeInBytes() != 0, "right schema is emtpy");
    ...
    x_DEBUG("Output schema for join={}", outputSchema->toString());
...
void BatchJoinLogicalOperatorNode::inferStringSignature() {
    ...
    x_TRACE("BatchJoinLogicalOperatorNode: Inferring String signature for {}", operatorNode->toString());
    x_ASSERT(!children.empty() && children.size() == 2, "BatchJoinLogicalOperatorNode: Join should have 2 children.");
