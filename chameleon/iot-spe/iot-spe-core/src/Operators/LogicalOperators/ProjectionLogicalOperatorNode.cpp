...
std::vector<ExpressionNodePtr> ProjectionLogicalOperatorNode::getExpressions() const { return expressions; }
...
bool ProjectionLogicalOperatorNode::inferSchema(Optimizer::TypeInferencePhaseContext& typeInferencePhaseContext) {
    ...
    x_DEBUG("proj input={}  outputSchema={} this proj={}", inputSchema->toString(), outputSchema->toString(), toString());
    outputSchema->clear();
    for (auto& expression : expressions) {
        ...
        if (expression->instanceOf<FieldRenameExpressionNode>()) {
            ...
            x_ERROR("ProjectionLogicalOperatorNode: Expression has to be an FieldAccessExpression or a FieldRenameExpression "
                      "but it was a {}",
                      expression->toString());
...
void ProjectionLogicalOperatorNode::inferStringSignature() {
    ...
    x_TRACE("ProjectionLogicalOperatorNode: Inferring String signature for {}", operatorNode->toString());
    x_ASSERT(!children.empty(), "ProjectionLogicalOperatorNode: Project should have children.");
