...
bool JoinLogicalOperatorNode::inferSchema(Optimizer::TypeInferencePhaseContext& typeInferencePhaseContext) {
    ...
    x_ASSERT_THROW_EXCEPTION(foundLeftKey,
    ...
    x_ASSERT_THROW_EXCEPTION(foundRightKey,
                               TypeInferenceException,
    ...
    x_ASSERT_THROW_EXCEPTION(!!leftInputSchema,
                               TypeInferenceException,
                               "JoinLogicalOperatorNode: Left input schema is not initialized for left join key "
                                   + leftJoinKeyName);
    x_ASSERT_THROW_EXCEPTION(!!rightInputSchema,
                               TypeInferenceException,
    ...
    x_ASSERT_THROW_EXCEPTION(leftInputSchema->getSchemaSizeInBytes() > 0,
                               TypeInferenceException,
                               "JoinLogicalOperatorNode: left schema is emtpy");
    x_ASSERT_THROW_EXCEPTION(rightInputSchema->getSchemaSizeInBytes() > 0,
                               TypeInferenceException,
                               "JoinLogicalOperatorNode: right schema is emtpy");
    x_ASSERT_THROW_EXCEPTION(!rightInputSchema->equals(leftInputSchema, false),
                               TypeInferenceException,
    ...
    x_DEBUG("Outputschema for join={}", outputSchema->toString());
...
void JoinLogicalOperatorNode::inferStringSignature() {
    ...
    x_TRACE("JoinLogicalOperatorNode: Inferring String signature for {}", operatorNode->toString());
    x_ASSERT(!children.empty() && children.size() == 2, "JoinLogicalOperatorNode: Join should have 2 children.");
