...
bool LogicalBinaryOperatorNode::inferSchema(Optimizer::TypeInferencePhaseContext& typeInferencePhaseContext) {
    ...
    if (children.size() < 2) {
        x_ERROR("BinaryOperatorNode: this operator should have at least two child operators");
    ...
    for (const auto& child : children) {
        if (!child->as<LogicalOperatorNode>()->inferSchema(typeInferencePhaseContext)) {
            x_ERROR("BinaryOperatorNode: failed inferring the schema of the child operator");
