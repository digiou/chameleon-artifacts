...
FieldAssignmentExpressionNodePtr MapLogicalOperatorNode::getMapExpression() const { return mapExpression; }
...
bool MapLogicalOperatorNode::inferSchema(Optimizer::TypeInferencePhaseContext& typeInferencePhaseContext) {
    ...
    if (outputSchema->hasFieldName(fieldName)) {
        ...
        x_TRACE("MAP Logical Operator: the field {} is already in the schema, so we updated its type.", fieldName);
        ...
        x_TRACE("MAP Logical Operator: the field {} is not part of the schema, so we added it.", fieldName);
...
void MapLogicalOperatorNode::inferStringSignature() {
    x_TRACE("MapLogicalOperatorNode: Inferring String signature for {}", toString());
    x_ASSERT(children.size() == 1, "MapLogicalOperatorNode: Map should have 1 child.");
