...
bool FilterLogicalOperatorNode::inferSchema(Optimizer::TypeInferencePhaseContext& typeInferencePhaseContext) {
    ...
    if (!predicate->isPredicate()) {
        x_THROW_RUNTIME_ERROR("FilterLogicalOperator: the filter expression is not a valid predicate");
    }
    return true;
}
...
void FilterLogicalOperatorNode::inferStringSignature() {
    ...
    x_TRACE("FilterLogicalOperatorNode: Inferring String signature for {}", operatorNode->toString());
    x_ASSERT(!children.empty(), "FilterLogicalOperatorNode: Filter should have children");
...
std::vector<std::string> FilterLogicalOperatorNode::getFieldNamesUsedByFilterPredicate() {
    x_TRACE("FilterLogicalOperatorNode: Find all field names used in filter operator");
