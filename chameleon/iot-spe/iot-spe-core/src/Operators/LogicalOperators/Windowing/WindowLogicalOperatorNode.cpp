...
bool WindowLogicalOperatorNode::inferSchema(Optimizer::TypeInferencePhaseContext& typeInferencePhaseContext) {
    ...
    x_DEBUG("WindowLogicalOperatorNode: TypeInferencePhase: infer types for window operator with input schema {}",
    ...
    if (windowDefinition->getWindowType()->isTimeBasedWindowType()) {
        ...
        x_THROW_RUNTIME_ERROR("Unsupported window type" << windowDefinition->getWindowType()->toString());
    }
    ...
    x_TRACE("Outputschema for window={}", outputSchema->toString());
...
void WindowLogicalOperatorNode::inferStringSignature() {
    ...
    x_TRACE("Inferring String signature for {}", operatorNode->toString());
