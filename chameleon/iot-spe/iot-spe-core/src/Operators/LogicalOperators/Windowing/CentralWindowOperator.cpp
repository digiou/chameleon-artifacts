...
bool CentralWindowOperator::inferSchema(Optimizer::TypeInferencePhaseContext& typeInferencePhaseContext) {
    ...
    x_DEBUG("SliceCreationOperator: TypeInferencePhase: infer types for window operator with input schema {}",
    ...
    for (auto& agg : windowAggregation) {
        x_INFO("Add the following field to the output schema of the NonKeyedWindow {}",
    ...
    x_INFO("The final output schema is {}", outputSchema->toString())

    return true;
}

void CentralWindowOperator::inferStringSignature() { x_NOT_IMPLEMENTED(); }
}// namespace x
