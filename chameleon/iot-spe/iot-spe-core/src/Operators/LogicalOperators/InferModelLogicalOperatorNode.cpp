...
InferModelLogicalOperatorNode::InferModelLogicalOperatorNode(std::string model,
    ...
    x_DEBUG("InferModelLogicalOperatorNode: reading from model {}", this->model);
}
...
bool InferModelLogicalOperatorNode::inferSchema(Optimizer::TypeInferencePhaseContext& typeInferencePhaseContext) {
    ...
    for (auto outputField : outputFields) {
        ...
        if (outputSchema->hasFieldName(fieldName)) {
            ...
            x_TRACE("Infer Model Logical Operator: the field {} is already in the schema, so we updated its type.", fieldName);
            ...
            x_TRACE("Infer Model Logical Operator: the field {} is not part of the schema, so we added it.", fieldName);
...
void InferModelLogicalOperatorNode::inferStringSignature() {
    ...
    x_TRACE("InferModelOperatorNode: Inferring String signature for {}", operatorNode->toString());
    x_ASSERT(!children.empty(), "InferModelLogicalOperatorNode: InferModel should have children (?)");
