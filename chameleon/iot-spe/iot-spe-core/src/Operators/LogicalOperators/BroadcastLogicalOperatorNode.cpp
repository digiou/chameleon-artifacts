...
bool BroadcastLogicalOperatorNode::inferSchema(Optimizer::TypeInferencePhaseContext& typeInferencePhaseContext) {
    ...
    if (children.empty()) {
        x_THROW_RUNTIME_ERROR("ExchangeOperatorNode: this node should have at least one child operator");
    }
    ...
    for (const auto& child : children) {
        if (!child->as<OperatorNode>()->getOutputSchema()->equals(childSchema)) {
            x_ERROR("ExchangeOperatorNode: infer schema failed. The schema has to be the same across all child operators.");
            return false;
        }
    }
...
void BroadcastLogicalOperatorNode::inferStringSignature() { x_NOT_IMPLEMENTED(); }
