...
std::vector<AbstractRequestPtr> StopQueryRequest::executeRequestLogic(const StorageHandlerPtr& storageHandler) {
    x_TRACE("Start Stop Request logic.");
    std::vector<AbstractRequestPtr> failureRequests = {};
    try {
        x_TRACE("Acquire Resources.");
        ...
        x_TRACE("Locks acquired. Create Phases");
        ...
        x_TRACE("Phases created. Stop request initialized.");
...
void StopQueryRequest::postExecution([[maybe_unused]] const StorageHandlerPtr& storageHandler) { x_TRACE("Release locks."); }
...
std::vector<AbstractRequestPtr> StopQueryRequest::rollBack(RequestExecutionException& ex,
    ...
    try {
        ...
        if (ex.instanceOf<Exceptions::QueryPlacementException>()) {
            ...
            x_ERROR("RuntimeException: {}", ex.what());
        } else {
            ...
            //RPC call errors:
            //1. asynchronous call to worker to stop shared query plan failed --> currently invokes x_THROW_RUNTIME_ERROR;
            //2. asynchronous call to worker to unregister shared query plan failed --> currently invokes x_THROW_RUNTIME_ERROR:
            x_ERROR("Unknown exception: {}", ex.what());
        }
    } catch (RequestExecutionException& e) {
        if (retry()) {
            handleError(e, storageHandler);
        } else {
            x_ERROR("StopQueryRequest: Final failure to rollback. No retries left. Error: {}", e.what());
        }
    }
    return failRequest;
}
