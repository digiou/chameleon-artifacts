...
std::vector<AbstractRequestPtr> AddQueryRequest::rollBack([[maybe_unused]] RequestExecutionException& exception,
    ...
    try {
        ...
        if (exception.instanceOf<Exceptions::QueryNotFoundException>()) {
            ...
            x_ERROR("Unknown exception: {}", exception.what());
        }
    } catch (RequestExecutionException& exception) {
        if (retry()) {
            handleError(exception, storageHandler);
        } else {
            x_ERROR("StopQueryRequest: Final failure to rollback. No retries left. Error: {}", exception.what());
        }
    }
    return {};
}
...
std::vector<AbstractRequestPtr> AddQueryRequest::executeRequestLogic(const StorageHandlerPtr& storageHandler) {
    try {
        x_DEBUG("Acquiring required resources.");
        ...
        x_DEBUG("Initializing various optimization phases.");
        ...
        if (!queryString.empty() && !queryPlan) {
            ...
            x_ERROR("Please supply either query string or query plan while creating this request.");
            x_NOT_IMPLEMENTED();
        }
        ...
        x_DEBUG("Performing Query type inference phase for query:  {}", queryId);
        queryPlan = typeInferencePhase->execute(queryPlan);

        //4. Set memory layout of each logical operator
        x_DEBUG("Performing query choose memory layout phase:  {}", queryId);
        queryPlan = memoryLayoutSelectionPhase->execute(queryPlan);

        //5. Perform query re-write
        x_DEBUG("Performing Query rewrite phase for query:  {}", queryId);
        ...
        x_DEBUG("Applying Query Merger Rules as Query Merging is enabled.");
        ...
        if (SharedQueryPlanStatus::Updated == sharedQueryPlan->getStatus()) {
            ...
            x_DEBUG("Shared Query Plan is non empty and an older version is already "
                      "running.");
        ...
        x_DEBUG("Performing Operator placement for shared query plan");
        ...
        x_ERROR("Exception occurred while processing AddQueryRequest with error {}", exception.what());
        handleError(exception, storageHandler);
    }
    return {};
}
