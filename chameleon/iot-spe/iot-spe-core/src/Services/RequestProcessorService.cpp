...
RequestProcessorService::RequestProcessorService(const GlobalExecutionPlanPtr& globalExecutionPlan,
    ...
    x_DEBUG("QueryRequestProcessorService()");
...
void RequestProcessorService::start() {
    try {
        while (isQueryProcessorRunning()) {
            x_DEBUG("QueryRequestProcessorService: Waiting for new query request trigger");
            auto requests = queryRequestQueue->getNextBatch();
            x_TRACE("QueryProcessingService: Found {} query requests to process", requests.size());
            if (requests.empty()) {
                continue;
            }
            ...
            try {
                ...
                x_INFO("QueryProcessingService: Calling GlobalQueryPlanUpdatePhase");
                ...
                for (const auto& sharedQueryPlan : sharedQueryPlanToDeploy) {
                    ...
                    if (!queryReconfiguration) {
                        ...
                        x_DEBUG("QueryProcessingService: Updating Query Plan with global query id : {}", sharedQueryId);

                        //3.2. If the shared query plan is newly created
                        if (SharedQueryPlanStatus::Created == sharedQueryPlan->getStatus()) {

                            x_DEBUG("QueryProcessingService: Shared Query Plan is new.");

                            //3.2.1. Perform placement of new shared query plan
                            auto queryPlan = sharedQueryPlan->getQueryPlan();
                            x_DEBUG("QueryProcessingService: Performing Operator placement for shared query plan");
                            ...
                            x_DEBUG("QueryProcessingService: Shared Query Plan is non empty and an older version is already "
                                      "running.");
                            ...
                            x_DEBUG("QueryProcessingService: Shared Query Plan is empty and an older version is already "
                                      "running.");
                ...
                x_ERROR("QueryRequestProcessingService: QueryPlacementException: {}", ex.what());
                ...
                x_ERROR("QueryRequestProcessingService: QueryDeploymentException: {}", ex.what());
                auto sharedQueryId = ex.getQueryId();
                queryUndeploymentPhase->execute(sharedQueryId, SharedQueryPlanStatus::Failed);
                auto sharedQueryPlan = globalQueryPlan->getSharedQueryPlan(sharedQueryId);
                for (auto queryId : sharedQueryPlan->getQueryIds()) {
                ...
                x_ERROR("QueryRequestProcessingService: TypeInferenceException: {}", ex.what());
                auto queryId = ex.getQueryId();
                queryCatalogService->updateQueryStatus(queryId, QueryState::FAILED, ex.what());
            } catch (Exceptions::InvalidQueryStateException& ex) {
                x_ERROR("QueryRequestProcessingService: InvalidQueryStateException: {}", ex.what());
            } catch (Exceptions::QueryNotFoundException& ex) {
                x_ERROR("QueryRequestProcessingService: QueryNotFoundException: {}", ex.what());
            } catch (Exceptions::QueryUndeploymentException& ex) {
                x_ERROR("QueryRequestProcessingService: QueryUndeploymentException: {}", ex.what());
            } catch (InvalidQueryException& ex) {
                x_ERROR("QueryRequestProcessingService InvalidQueryException: {}", ex.what());
            } catch (std::exception& ex) {
                x_FATAL_ERROR("QueryProcessingService: Received unexpected exception while scheduling the "
                                "queryIdAndCatalogEntryMapping: {}",
                                ex.what());
                shutDown();
            }
        }
        x_WARNING("QueryProcessingService: Terminated");
    } catch (std::exception& ex) {
        x_FATAL_ERROR("QueryProcessingService: Received unexpected exception while scheduling the queryId : {}", ex.what());
        shutDown();
    }
    shutDown();
}
...
void RequestProcessorService::shutDown() {
    std::unique_lock<std::mutex> lock(queryProcessorStatusLock);
    x_INFO("Request Processor Service is shutting down! No further requests can be processed!");
