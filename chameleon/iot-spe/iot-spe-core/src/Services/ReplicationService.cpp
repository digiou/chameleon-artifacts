...
ReplicationService::ReplicationService(xCoordinatorPtr coordinatorPtr) : coordinatorPtr(std::move(coordinatorPtr)) {}
...
std::vector<SourceLogicalOperatorNodePtr> ReplicationService::getLogicalSources(uint64_t queryId) const {
    ...
    x_ERROR("ReplicationService: no shared query plan found for the queryId {}", queryId);
    return {};
}
...
bool ReplicationService::notifyEpochTermination(uint64_t epochBarrier, uint64_t queryId) const {
    ...
    x_DEBUG("ReplicationService: send timestamp {} to sources with queryId {}", epochBarrier, queryId);
        ...
        for (auto& sourceOperator : sources) {
            ...
            if (!sourceLocations.empty()) {
                ...
                for (auto& sourceLocation : sourceLocations) {
                    ...
                    x_ASSERT(success, false);
                    x_DEBUG("ReplicationService: success= {}", success);
                }
                return success;
            } else {
                x_ERROR("ReplicationService: no physical sources found for the queryId {}", queryId);
            }
        }
    } else {
        x_ERROR("ReplicationService: no sources found for the queryId {}", queryId);
    }
    return false;
}
