...
bool QueryCatalogService::checkAndMarkForSoftStop(SharedQueryId sharedQueryId, QuerySubPlanId subPlanId, OperatorId operatorId) {
    ...
    x_INFO("checkAndMarkForSoftStop sharedQueryId={} subQueryId={} source={}", sharedQueryId, subPlanId, operatorId);
    ...
    for (auto& queryCatalogEntry : queryCatalogEntries) {
        ...
        if (currentQueryState == QueryState::MARKED_FOR_HARD_STOP || currentQueryState == QueryState::FAILED
            ...
            x_WARNING("QueryCatalogService: Soft stop can not be initiated as query in {} status.",
    ...
    x_INFO("QueryCatalogService: Shared query id {} is marked as soft stopped", sharedQueryId);
    return true;
}
...
bool QueryCatalogService::checkAndMarkForHardStop(QueryId queryId) {
    ...
    x_INFO("QueryCatalogService: Handle hard stop request.");

    //Check if query exists
    if (!queryCatalog->queryExists(queryId)) {
        x_ERROR("QueryCatalogService: Query Catalog does not contains the input queryId {}", std::to_string(queryId));
    ...
    if (currentStatus == QueryState::MARKED_FOR_SOFT_STOP || currentStatus == QueryState::MARKED_FOR_HARD_STOP
        ...
        x_ERROR("QueryCatalog: Found query status already as {}. Ignoring stop query request.",
                  queryCatalogEntry->getQueryStatusAsString());
        return false;
    }
    x_DEBUG("QueryCatalog: Changing query status to Mark query for stop.");
...
void QueryCatalogService::checkAndMarkForFailure(SharedQueryId sharedQueryId, QuerySubPlanId querySubPlanId) {
    ...
    x_INFO("checkAndMarkForFailure sharedQueryId={} subQueryId={}", sharedQueryId, querySubPlanId);
    //Fetch query catalog entries
    auto queryCatalogEntries = queryCatalog->getQueryCatalogEntriesForSharedQueryId(sharedQueryId);

    if (queryCatalogEntries.empty()) {
        x_FATAL_ERROR("Unable to find the shared query plan with id {}", sharedQueryId);
    ...
    for (auto& queryCatalogEntry : queryCatalogEntries) {
        ...
        if (currentQueryStatus == QueryState::MARKED_FOR_FAILURE || currentQueryStatus == QueryState::FAILED
            ...
            x_WARNING("QueryCatalogService: Query can not be marked for failure as query in {} status.",
                        queryCatalogEntry->getQueryStatusAsString());
    ...
    x_INFO("QueryCatalogService: Shared query id {} is marked as failed", sharedQueryId);
}
...
std::map<uint64_t, std::string> QueryCatalogService::getAllQueriesInStatus(std::string queryStatus) {
    ...
    if (status.has_value()) {
        ...
        x_ERROR("No valid query status to parse");
...
bool QueryCatalogService::handleSoftStop(SharedQueryId sharedQueryId, QuerySubPlanId querySubPlanId, QueryState subQueryStatus) {
    std::unique_lock lock(serviceMutex);
    x_DEBUG("QueryCatalogService: Updating the status of sub query to ({}) for sub query plan with id {} for shared query "
              "plan with id {}",
    ...
    for (auto& queryCatalogEntry : queryCatalogEntries) {
        ...
        if (currentQueryStatus != QueryState::MARKED_FOR_SOFT_STOP) {
            x_WARNING("Found query in {} but received {} for the sub query with id {} for query id {}",
                        queryCatalogEntry->getQueryStatusAsString(),
                        std::string(magic_enum::enum_name(subQueryStatus)),
                        querySubPlanId,
                        queryId);
        ...
        if (currentStatus == QueryState::SOFT_STOP_COMPLETED && subQueryStatus == QueryState::SOFT_STOP_COMPLETED) {
            x_WARNING("Received multiple soft stop completed for sub query with id {} for query {}", querySubPlanId, queryId);
            x_WARNING("Skipping remaining operation");
            continue;
        } else if (currentStatus == QueryState::SOFT_STOP_COMPLETED && subQueryStatus == QueryState::SOFT_STOP_TRIGGERED) {
            x_ERROR("Received soft stop triggered for sub query with id {} for query {} but sub query is already marked as "
                      "soft stop completed.",
                      querySubPlanId,
                      sharedQueryId);
            x_WARNING("Skipping remaining operation");
            ...
            x_ERROR("Received multiple soft stop triggered for sub query with id {} for query {}",
                      querySubPlanId,
                      sharedQueryId);
            x_WARNING("Skipping remaining operation");
            continue;
        ...
        if (subQueryStatus == QueryState::SOFT_STOP_COMPLETED) {
            for (auto& querySubPlanMetaData : queryCatalogEntry->getAllSubQueryPlanMetaData()) {
                x_DEBUG("Updating query subplan status for query id= {} subplan= {} is {}",
                          queryId,
            ...
            if (stopQuery) {
                ...
                x_INFO("Query with id {} is now stopped", queryCatalogEntry->getQueryId());
            }
        }
    }
    return true;
