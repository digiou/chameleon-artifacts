...
void SharedQueryPlan::addQuery(QueryId queryId, const std::vector<Optimizer::MatchedOperatorPairPtr>& matchedOperatorPairs) {
    ...
    x_DEBUG("SharedQueryPlan: Add the matched operators of query with id {} to the shared query plan.", queryId);
    ...
    for (const auto& matchedOperatorPair : matchedOperatorPairs) {
        ...
        switch (matchedOperatorPair->containmentRelationship) {
            ...
            case Optimizer::ContainmentRelationship::NO_CONTAINMENT:
                x_WARNING("NO_CONTAINMENT relationship was passed to SharedQueryPlan::addQuery(). This should not happen. "
    ...
    for (const auto& targetSinkOperator : sinkOperators) {
        x_TRACE("New sink operators: {}", targetSinkOperator->toString());
    ...
    x_TRACE("Size of queryIdToSinkOperatorMap: {}", queryIdToSinkOperatorMap.size());
...
bool SharedQueryPlan::removeQuery(QueryId queryId) {
    x_DEBUG("SharedQueryPlan: Remove the Query Id {} and associated Global Query Nodes with sink operators.", queryId);
    if (queryIdToSinkOperatorMap.find(queryId) == queryIdToSinkOperatorMap.end()) {
        x_ERROR("SharedQueryPlan: query id {} is not present in metadata information.", queryId);
        return false;
    }

    x_TRACE("SharedQueryPlan: Remove the Global Query Nodes with sink operators for query  {}", queryId);
    ...
    for (const auto& sinkOperator : sinkOperatorsToRemove) {
        ...
        if (upstreamOperators.empty()) {
            x_ERROR("SharedQueryPlan: unable to remove Root operator from the shared query plan {}", sharedQueryId);
            return false;
        }
...
bool SharedQueryPlan::isEmpty() {
    x_TRACE("SharedQueryPlan: Check if Global Query Metadata is empty. Found :  {}", queryIdToSinkOperatorMap.empty());
    return queryIdToSinkOperatorMap.empty();
}

std::vector<LogicalOperatorNodePtr> SharedQueryPlan::getSinkOperators() {
    x_TRACE("SharedQueryPlan: Get all Global Query Nodes with sink operators for the current Metadata");
...
void SharedQueryPlan::clear() {
    x_DEBUG("SharedQueryPlan: clearing all metadata information.");
