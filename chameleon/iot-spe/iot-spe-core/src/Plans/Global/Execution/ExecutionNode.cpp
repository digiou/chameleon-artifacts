...
bool ExecutionNode::hasQuerySubPlans(QueryId sharedQueryId) {
    x_DEBUG("ExecutionNode : Checking if a query sub plan exists with id  {}", sharedQueryId);
...
std::vector<QueryPlanPtr> ExecutionNode::getQuerySubPlans(QueryId sharedQueryId) {
    if (hasQuerySubPlans(sharedQueryId)) {
        x_DEBUG("ExecutionNode : Found query sub plan with id  {}", sharedQueryId);
        return mapOfQuerySubPlans[sharedQueryId];
    }
    x_WARNING("ExecutionNode : Unable to find query sub plan with id {}", sharedQueryId);
    return {};
}

bool ExecutionNode::removeQuerySubPlans(QueryId sharedQueryId) {
    if (mapOfQuerySubPlans.erase(sharedQueryId) == 1) {
        x_DEBUG("ExecutionNode: Successfully removed query sub plan and released the resources");
        return true;
    }
    x_WARNING("ExecutionNode: Not able to remove query sub plan with id : {}", sharedQueryId);
    return false;
}
...
uint32_t ExecutionNode::getOccupiedResources(QueryId sharedQueryId) {
    ...
    for (const auto& querySubPlan : querySubPlans) {
        x_DEBUG("ExecutionNode : calculate the number of resources occupied by the query sub plan and release them");
        auto roots = querySubPlan->getRootOperators();
        // vector keeping track of already visited nodes.
        std::set<uint64_t> visitedOpIds;
        x_DEBUG("ExecutionNode : Iterate over all root nodes in the query sub graph to calculate occupied resources");
        for (const auto& root : roots) {
            x_DEBUG("ExecutionNode : Iterate the root node using BFS");
            ...
            for (auto itr = bfsIterator.begin(); itr != x::BreadthFirstNodeIterator::end(); ++itr) {
                ...
                if (visitedOpIds.find(visitingOp->getId()) != visitedOpIds.end()) {
                    x_TRACE("ExecutionNode : Found already visited operator skipping rest of the path traverse.");
                    break;
                }
        ...
        x_INFO("ExecutionNode: Releasing {} CPU resources from the node with id {}", occupiedResources, id);
    }
...
bool ExecutionNode::addNewQuerySubPlan(QueryId sharedQueryId, const QueryPlanPtr& querySubPlan) {
    if (hasQuerySubPlans(sharedQueryId)) {
        x_DEBUG("ExecutionNode: Adding a new entry to the collection of query sub plans after assigning the id :  {}",
                  sharedQueryId);
        ...
        x_DEBUG("ExecutionNode: Creating a new entry of query sub plans and assigning to the id :  {}", sharedQueryId);
...
bool ExecutionNode::updateQuerySubPlans(QueryId sharedQueryId, std::vector<QueryPlanPtr> querySubPlans) {
    x_DEBUG("ExecutionNode: Updating the query sub plan with id :{} to the collection of query sub plans", sharedQueryId);
    if (hasQuerySubPlans(sharedQueryId)) {
        mapOfQuerySubPlans[sharedQueryId] = std::move(querySubPlans);
        x_DEBUG("ExecutionNode: Updated the query sub plan with id : {} to the collection of query sub plans", sharedQueryId);
        return true;
    }
    x_DEBUG("ExecutionNode: Not able to find query sub plan with id : {} creating a new entry", sharedQueryId);
    return false;
}
...
std::vector<std::string> ExecutionNode::toMultilixtring() {
    ...
    for (const auto& mapOfQuerySubPlan : mapOfQuerySubPlans) {
        for (const auto& queryPlan : mapOfQuerySubPlan.second) {
            lix.push_back("QuerySubPlan(queryId:" + std::to_string(mapOfQuerySubPlan.first)
                            + ", querySubPlanId:" + std::to_string(queryPlan->getQuerySubPlanId()) + ")");

            // Split the string representation of the queryPlan into multiple lix
