...
std::vector<SourceLogicalOperatorNodePtr> QueryPlan::getSourceOperators() {
    x_DEBUG("QueryPlan: Get all source operators by traversing all the root nodes.");
    ...
    for (const auto& rootOperator : rootOperators) {
        ...
        x_DEBUG("QueryPlan: insert all source operators to the collection");
        sourceOperatorsSet.insert(sourceOptrs.begin(), sourceOptrs.end());
    }
    x_DEBUG("QueryPlan: Found {} source operators.", sourceOperatorsSet.size());
...
std::vector<SinkLogicalOperatorNodePtr> QueryPlan::getSinkOperators() {
    x_DEBUG("QueryPlan: Get all sink operators by traversing all the root nodes.");
    ...
    x_DEBUG("QueryPlan: Found {} sink operators.", sinkOperators.size());
    return sinkOperators;
}

void QueryPlan::appendOperatorAsNewRoot(const OperatorNodePtr& operatorNode) {
    x_DEBUG("QueryPlan: Appending operator {} as new root of the plan.", operatorNode->toString());
    for (const auto& rootOperator : rootOperators) {
        if (!rootOperator->addParent(operatorNode)) {
            x_THROW_RUNTIME_ERROR("QueryPlan: Unable to add operator " + operatorNode->toString() + " as parent to "
                                    + rootOperator->toString());
        }
    }
    x_DEBUG("QueryPlan: Clearing current root operators.");
    rootOperators.clear();
    x_DEBUG("QueryPlan: Pushing input operator node as new root.");
    rootOperators.push_back(operatorNode);
}

void QueryPlan::prependOperatorAsLeafNode(const OperatorNodePtr& operatorNode) {
    x_DEBUG("QueryPlan: Prepending operator {} as new leaf of the plan.", operatorNode->toString());
    auto leafOperators = getLeafOperators();
    if (leafOperators.empty()) {
        x_DEBUG("QueryPlan: Found empty query plan. Adding operator as root.");
        rootOperators.push_back(operatorNode);
    } else {
        x_DEBUG("QueryPlan: Adding operator as child to all the leaf nodes.");
...
std::vector<OperatorNodePtr> QueryPlan::getLeafOperators() {
    ...
    x_DEBUG("QueryPlan: Get all leaf nodes in the query plan.");
    std::vector<OperatorNodePtr> leafOperators;
    // Maintain a list of visited nodes as there are multiple root nodes
    std::set<uint64_t> visitedOpIds;
    x_DEBUG("QueryPlan: Iterate over all root nodes to find the operator.");
    for (const auto& rootOperator : rootOperators) {
        ...
        for (auto itr = bfsIterator.begin(); itr != x::BreadthFirstNodeIterator::end(); ++itr) {
            ...
            x_DEBUG("QueryPlan: Inserting operator in collection of already visited node.");
            visitedOpIds.insert(visitingOp->getId());
            if (visitingOp->getChildren().empty()) {
                x_DEBUG("QueryPlan: Found leaf node. Adding to the collection of leaf nodes.");
...
bool QueryPlan::hasOperatorWithId(uint64_t operatorId) {
    x_DEBUG("QueryPlan: Checking if the operator exists in the query plan or not");
    for (const auto& rootOperator : rootOperators) {
        if (rootOperator->getId() == operatorId) {
            x_DEBUG("QueryPlan: Found operator {} in the query plan", queryId);
            return true;
        }
    ...
    x_DEBUG("QueryPlan: Unable to find operator with matching Id");
    return false;
}

OperatorNodePtr QueryPlan::getOperatorWithId(uint64_t operatorId) {
    x_DEBUG("QueryPlan: Checking if the operator with id {} exists in the query plan or not", operatorId);
    for (auto rootOperator : rootOperators) {
        if (rootOperator->getId() == operatorId) {
            x_DEBUG("QueryPlan: Found operator {} in the query plan", operatorId);
            return rootOperator;
        }
        for (const auto& child : rootOperator->getChildren()) {
            x_TRACE("QueryPlan: Searching for {} in the children", operatorId);
...
void QueryPlan::removeAsRootOperator(OperatorNodePtr root) {
    x_DEBUG("QueryPlan: removing operator {} as root operator.", root->toString());
...
QueryPlanPtr QueryPlan::copy() {
    x_INFO("QueryPlan: make copy of this query plan");
    ...
    while (!operatorsToProcess.empty()) {
        ...
        if (operatorIdToOperatorMap[operatorId]) {
            x_TRACE("QueryPlan: Operator was processed previously");
            operatorNode = operatorIdToOperatorMap[operatorId];
        } else {
            x_TRACE("QueryPlan: Adding the operator into map");
        ...
        for (const auto& parentNode : operatorNode->getParents()) {
            ...
            if (operatorIdToOperatorMap[parentOperatorId]) {
                x_TRACE("QueryPlan: Found the parent operator. Adding as parent to the current operator.");
                ...
                x_ERROR("QueryPlan: unable to find the parent operator. This should not have occurred!");
                return nullptr;
            }
        }

        x_TRACE("QueryPlan: add the child global query nodes for further processing.");
    ...
    for (const auto& rootOperator : rootOperators) {
        x_TRACE("QueryPlan: Finding the operator with same id in the map.");
