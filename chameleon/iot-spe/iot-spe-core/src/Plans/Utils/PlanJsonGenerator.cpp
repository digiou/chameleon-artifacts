...
std::string PlanJsonGenerator::getOperatorType(const OperatorNodePtr& operatorNode) {
    x_INFO("Util: getting the type of the operator");
    ...
    x_DEBUG("UtilityFunctions: operatorType =  {}", operatorType);
    return operatorType;
}
...
void PlanJsonGenerator::getChildren(OperatorNodePtr const& root,
    ...
    if (children.empty()) {
        x_DEBUG("UtilityFunctions::getChildren : children is empty()");
        return;
    }

    x_DEBUG("UtilityFunctions::getChildren : children size =  {}", children.size());
    for (const NodePtr& child : children) {
        ...
        if (childOPeratorType == "WINDOW AGGREGATION") {
            ...
            x_DEBUG("{}", childLogicalOperatorNode->as<WindowLogicalOperatorNode>()->toString());
        } else {
...
nlohmann::json PlanJsonGenerator::getExecutionPlanAsJson(const GlobalExecutionPlanPtr& globalExecutionPlan, QueryId queryId) {
    x_INFO("UtilityFunctions: getting execution plan as JSON");
...
nlohmann::json PlanJsonGenerator::getQueryPlanAsJson(const QueryPlanPtr& queryPlan) {
    ...
    if (!root) {
        x_DEBUG("UtilityFunctions::getQueryPlanAsJson : root operator is empty");
        nlohmann::json node;
        node["id"] = "NONE";
        node["name"] = "NONE";
        nodes.push_back(node);
    } else {
        x_DEBUG("UtilityFunctions::getQueryPlanAsJson : root operator is not empty");
