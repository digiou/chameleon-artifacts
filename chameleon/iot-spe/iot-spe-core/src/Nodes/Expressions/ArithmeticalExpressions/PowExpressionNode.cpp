...
void PowExpressionNode::inferStamp(const Optimizer::TypeInferencePhaseContext& typeInferencePhaseContext, SchemaPtr schema) {
    ...
    if (stamp->isInteger()) {
        ...
        x_TRACE("PowExpressionNode: Updated stamp from Integer (assigned in ArithmeticalBinaryExpressionNode) to Int64.");
    } else if (stamp->isFloat()) {
        stamp = DataTypeFactory::createDouble();
        x_TRACE("PowExpressionNode: Update Float stamp (assigned in ArithmeticalBinaryExpressionNode) to Double: {}",
                  toString());
    }
}
