...
void SqrtExpressionNode::inferStamp(const Optimizer::TypeInferencePhaseContext& typeInferencePhaseContext, SchemaPtr schema) {
    ...
    if ((stamp->isInteger() && DataType::as<Integer>(stamp)->upperBound <= 0)
        ...
        x_ERROR("Log10ExpressionNode: Non-positive DataType is passed into Log10 expression. Arithmetic errors would occur at "
                  "run-time.");
    }
    ...
    x_TRACE("SqrtExpressionNode: converted stamp to float and increased the lower bound of stamp to 0: {}", toString());
}
