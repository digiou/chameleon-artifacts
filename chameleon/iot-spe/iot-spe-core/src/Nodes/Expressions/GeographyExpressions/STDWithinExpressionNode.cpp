...
std::string STDWithinExpressionNode::toString() const {
    if (children.size() != 2) {
        x_FATAL_ERROR("A STDWithinExpressionNode should always access two children, but it had: {}", children.size());
...
ExpressionNodePtr STDWithinExpressionNode::getPoint() const {
    if (children.size() != 2) {
        x_FATAL_ERROR("An STDWithinExpressionNode should always have two children, but it has: {}", children.size());
...
void STDWithinExpressionNode::inferStamp(const Optimizer::TypeInferencePhaseContext& typeInferencePhaseContext,
    ...
    x_TRACE("ST_DWithinExpressionNode: The following stamp was assigned: {}", toString());
}
