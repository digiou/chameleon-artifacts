...
std::string STKnnExpressionNode::toString() const {
    if (children.size() != 3) {
        x_FATAL_ERROR("A STKnnExpressionNode should always access two children, but it had: {}", children.size());
...
ExpressionNodePtr STKnnExpressionNode::getPoint() const {
    if (children.size() != 3) {
        x_FATAL_ERROR("An ST_Knn Expression should always have three children, but it has: {}", children.size());
...
void STKnnExpressionNode::inferStamp(const Optimizer::TypeInferencePhaseContext& typeInferencePhaseContext, SchemaPtr schema) {
    ...
    x_TRACE("ST_KnnExpressionNode: The following stamp was assigned: {}", toString());
}
