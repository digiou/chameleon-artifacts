...
std::string GeographyFieldsAccessExpressionNode::toString() const {
    if (children.size() != 2) {
        x_FATAL_ERROR("A geography access expression should always access two fields, but it had: {}", children.size());
...
void GeographyFieldsAccessExpressionNode::inferStamp(const Optimizer::TypeInferencePhaseContext& typeInferencePhaseContext,
    ...
    x_TRACE("GeographyFieldsAccessExpressionNode: The following stamp was assigned: {}", toString());
}
