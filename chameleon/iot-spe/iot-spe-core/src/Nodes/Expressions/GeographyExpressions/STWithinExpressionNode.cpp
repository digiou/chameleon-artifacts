...
std::string STWithinExpressionNode::toString() const {
    if (children.size() != 2) {
        x_FATAL_ERROR("An STWithin Expression should always have two children, but it has: {}", children.size());
...
void STWithinExpressionNode::inferStamp(const Optimizer::TypeInferencePhaseContext& typeInferencePhaseContext, SchemaPtr schema) {
    ...
    x_TRACE("STWithinExpressionNode: The following stamp was assigned: {}", toString());
}
