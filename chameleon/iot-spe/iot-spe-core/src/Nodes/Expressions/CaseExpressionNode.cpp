...
void CaseExpressionNode::inferStamp(const Optimizer::TypeInferencePhaseContext& typeInferencePhaseContext, SchemaPtr schema) {
    ...
    x_TRACE("CaseExpressionNode: we assigned the following stamp: {}", stamp->toString());
}
...
std::vector<ExpressionNodePtr> CaseExpressionNode::getWhenChildren() const {
    if (children.size() < 2) {
        x_FATAL_ERROR("A case expression always should have at least two children, but it had: {}", children.size());
    }
