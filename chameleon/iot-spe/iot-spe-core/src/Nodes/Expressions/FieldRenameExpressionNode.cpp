...
void FieldRenameExpressionNode::inferStamp(const Optimizer::TypeInferencePhaseContext& typeInferencePhaseContext,
    ...
    if (newFieldName.find(Schema::ATTRIBUTE_NAME_SEPARATOR) == std::string::npos) {
        if (!fieldAttribute) {
            x_ERROR("FieldRenameExpressionNode: Original field with name {} does not exists in the schema {}",
                      fieldName,
                      schema->toString());
    ...
    if (fieldName == newFieldName) {
        x_WARNING("FieldRenameExpressionNode: Both existing and new fields are same: existing: {} new field name: {}",
                    fieldName,
                    newFieldName);
    } else {
        auto newFieldAttribute = schema->hasFieldName(newFieldName);
        if (newFieldAttribute) {
            x_ERROR("FieldRenameExpressionNode: The new field name {} already exists in the input schema {}. "
