...
Node::Node() {
// The CMAKE_x_TRACE_NODE_CREATION flag enables the stack trace collection for every node creation.
// This can be useful for the debugging of optimizations, but could lead to a substantial performance impact.
#ifdef x_TRACE_NODE_CREATION
...
bool Node::addChildWithEqual(const NodePtr& newNode) {
    if (newNode.get() == this) {
        x_DEBUG("Node: Adding node to its self so skip add child with equal operation.");
        return false;
    }
...
bool Node::addChild(const NodePtr newNode) {
    if (newNode.get() == this) {
        x_ERROR("Node: Adding node to its self so will skip add child operation.");
        return false;
    }
    // checks if current new node is not part of children
    if (vectorContainsTheNode(children, newNode)) {
        x_ERROR("Node: the node is already part of its children so skip add chld operation.");
        return false;
    }
    // add the node to the children
    children.push_back(newNode);
...
bool Node::removeChild(NodePtr const& node) {
    ...
    if (!node) {
        x_ERROR("Node: Can't remove null node");
        return false;
    }
    ...
    x_DEBUG("Node: node was not found and could not be removed from children.");
    return false;
}

bool Node::addParent(const NodePtr newNode) {
    if (newNode.get() == this) {
        x_WARNING("Node: Adding node to its self so will skip add parent operation.");
        return false;
    }

    // checks if current new node is not part of parents
    if (vectorContainsTheNode(parents, newNode)) {
        x_WARNING("Node: the node is already part of its parents so ignore add parent operation.");
        return false;
    }
...
bool Node::insertBetweenThisAndParentNodes(NodePtr const& newNode) {
    ...
    if (newNode.get() == this) {
        x_WARNING("Node:  Adding node to its self so will skip insertBetweenThisAndParentNodes operation.");
        return false;
    }

    if (vectorContainsTheNode(parents, newNode)) {
        x_WARNING("Node: the node is already part of its parents so ignore insertBetweenThisAndParentNodes operation.");
        return false;
    }

    //replace this with the new node in all its parent
    x_DEBUG("Node: Create temporary copy of this nodes parents.");
    ...
    for (auto& parent : copyOfParents) {
        for (uint64_t i = 0; i < parent->children.size(); i++) {
            if (parent->children[i] == shared_from_this()) {
                ...
                x_DEBUG("Node: Add copy of this nodes parent as parent to the input node.");
                if (!newNode->addParent(parent)) {
                    x_ERROR("Node: Unable to add parent of this node as parent to input node.");
                    return false;
                }
            }
        }
    ...
    x_INFO("Node: Remove all parents of this node.");
    removeAllParent();

    if (!addParent(newNode)) {
        x_ERROR("Node: Unable to add input node as parent to this node.");
        return false;
    }
    return true;
}
...
bool Node::insertBetweenThisAndChildNodes(const NodePtr& newNode) {
    ...
    x_INFO("Node: Create temporary copy of this nodes parents.");
    std::vector<NodePtr> copyOfChildren = children;

    x_INFO("Node: Remove all childs of this node.");
    removeChildren();
    ...
    x_INFO("Node: Add copy of this nodes parent as parent to the input node.");
    for (const NodePtr& child : copyOfChildren) {
        if (!newNode->addChild(child)) {
            x_ERROR("Node: Unable to add child of this node as child to input node.");
            return false;
        }
    }
...
void Node::removeAllParent() {
    x_INFO("Node: Removing all parents for current node");
    auto nodeItr = parents.begin();
    while (nodeItr != parents.end()) {
        if (!this->removeParent(*nodeItr)) {
            nodeItr++;
        }
        x_INFO("Node: Removed node as parent of this node");
    }
}

void Node::removeChildren() {
    x_INFO("Node: Removing all children for current node");
    auto nodeItr = children.begin();
    while (nodeItr != children.end()) {
        if (!this->removeChild(*nodeItr)) {
            nodeItr++;
        }
        x_INFO("Node: Removed node as child of this node");
    }
}
...
bool Node::removeParent(NodePtr const& node) {
    ...
    x_DEBUG("Node: node was not found and could not be removed from parents.");
    return false;
}
...
bool Node::replace(const NodePtr& newNode, const NodePtr& oldNode) {
    ...
    if (!newNode || !oldNode) {
        x_ERROR("Node: Can't replace null node");
        return false;
    }
    ...
    if (oldNode->isIdentical(newNode)) {
        x_WARNING("Node: the new node was the same so will skip replace operation.");
        return true;
    }
    ...
    if (!oldNode->equal(newNode)) {
        ...
        if (find(children, newNode) || find(parents, newNode)) {
            x_DEBUG("Node: the new node is already part of the children or predessessors of the current node.");
            return false;
        }
    }
    ...
    x_ERROR("Node: could not remove child from  old node: {}", oldNode->toString());

    success = removeParent(oldNode);
    x_DEBUG("Node: remove parent old node: {}", oldNode->toString());
    if (success) {
        parents.push_back(newNode);
    ...
    x_ERROR("Node: could not remove parent from  old node: {}", oldNode->toString());
...
bool Node::swapLeftAndRightBranch() {
    if (children.size() != 2) {
        x_WARNING("Operation failed: method is supposed to swap left and right branch, but the node had ",
                    children.size(),
...
bool Node::removeAndJoinParentAndChildren() {
    try {
        x_DEBUG("Node: Joining parents with children");
        ...
        for (auto& parent : parentCopy) {
            for (auto& child : childCopy) {
                ...
                x_DEBUG("Node: Add child of this node as child of this node's parent");
                parent->addChild(child);

                x_DEBUG("Node: remove this node as parent of the child");
                child->removeParent(shared_from_this());
            }
            parent->removeChild(shared_from_this());
            x_DEBUG("Node: remove this node as child of this node's parents");
        }
        return true;
    } catch (...) {
        x_ERROR("Node: Error ocurred while joining this node's children and parents");
        return false;
    }
}
...
bool Node::containAsParent(NodePtr node) {
    x_DEBUG("Node: Checking if the input node is contained in the parent list");
...
bool Node::containAsGrandParent(NodePtr node) {
    ...
    for (auto& parent : parents) {
        x_TRACE("Node: Get this node, all its parents, and Ancestors");
        std::vector<NodePtr> parentAndAncestors = parent->getAndFlattenAllAncestors();
        x_TRACE("Node: Add them to the result");
...
bool Node::containAsChild(NodePtr node) {
    x_DEBUG("Node: Checking if the input node is contained in the children list");
...
std::vector<NodePtr> Node::getAllRootNodes() {
    x_DEBUG("Node: Get all root nodes for this node");
    std::vector<NodePtr> rootNodes;

    if (getParents().empty()) {
        x_DEBUG("Node: Inserting this node to the collection");
        rootNodes.push_back(shared_from_this());
    }

    for (auto& parent : parents) {
        if (parent->getParents().empty()) {
            x_DEBUG("Node: Inserting root node to the collection");
            rootNodes.push_back(parent);
        } else {
            x_DEBUG("Node: Iterating over all parents to find more root nodes");
            for (const auto& parentOfParent : parent->getParents()) {
                std::vector<NodePtr> parentNodes = parentOfParent->getAllRootNodes();
                x_DEBUG("Node: inserting parent nodes into the collection of parent nodes");
                rootNodes.insert(rootNodes.end(), parentNodes.begin(), parentNodes.end());
            }
        }
    }
    x_DEBUG("Node: Found {} leaf nodes", rootNodes.size());
    return rootNodes;
}

std::vector<NodePtr> Node::getAllLeafNodes() {
    x_DEBUG("Node: Get all leaf nodes for this node");
    std::vector<NodePtr> leafNodes;

    if (children.empty()) {
        x_DEBUG("Node: found no children. Returning itself as leaf.");
        leafNodes.push_back(shared_from_this());
    }

    for (auto& child : children) {
        if (child->getChildren().empty()) {
            x_DEBUG("Node: Inserting leaf node to the collection");
            leafNodes.push_back(child);
        } else {
            x_DEBUG("Node: Iterating over all children to find more leaf nodes");
            for (const auto& childOfChild : child->getChildren()) {
                std::vector<NodePtr> childrenLeafNodes = childOfChild->getAllLeafNodes();
                x_DEBUG("Node: inserting leaf nodes into the collection of leaf nodes");
                leafNodes.insert(leafNodes.end(), childrenLeafNodes.begin(), childrenLeafNodes.end());
            }
        }
    }
    x_DEBUG("Node: Found {} leaf nodes", leafNodes.size());
    return leafNodes;
}

bool Node::vectorContainsTheNode(std::vector<NodePtr> const& nodes, x::NodePtr const& nodeToFind) {
...
std::vector<NodePtr> Node::split(const NodePtr& splitNode) {
    ...
    if (!node) {
        x_DEBUG("Node: operator is not in graph so dont split.");
...
std::vector<std::string> Node::toMultilixtring() {
    std::vector<std::string> lix;
    lix.push_back(toString());
    return lix;
}
