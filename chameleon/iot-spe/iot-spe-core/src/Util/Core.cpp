...
std::string Util::printTupleBufferAsCSV(Runtime::TupleBuffer tbuffer, const SchemaPtr& schema) {
    ...
    for (uint64_t i = 0; i < numberOfTuples; i++) {
        ...
        for (uint64_t j = 0; j < schema->getSize(); j++) {
            ...
            if (dataType->isText()) {
                x_DEBUG("Util::printTupleBufferAsCSV(): trying to read the variable length TEXT field: "
                          "from the tuple buffer");

                // read the child buffer index from the tuple buffer
                Runtime::TupleBuffer::xtedTupleBufferKey childIdx = *reinterpret_cast<uint32_t const*>(indexInBuffer);
                ...
                if (sizeOfTextField > 0) {
                    ...
                    x_WARNING("Util::printTupleBufferAsCSV(): Variable-length field could not be read. Invalid size in the "
...
bool Util::assignPropertiesToQueryOperators(const QueryPlanPtr& queryPlan,
    ...
    if (numOperators != properties.size()) {
        x_ERROR("UtilityFunctions::assignPropertiesToQueryOperators: the number of properties does not match the number of "
