...
void RestServerInterruptHandler::handleUserInterrupt(int signal) {
    x_DEBUG("handleUserInterrupt");
    if (signal == SIGTERM) {
        x_DEBUG("SIGINT trapped ...");
        condition.notify_all();
    }
}

void RestServerInterruptHandler::waitForUserInterrupt() {
    x_DEBUG("Wait for interrupt of RestServer");
    std::unique_lock<std::mutex> lock{mutex};
    x_TRACE("Wait for interrupt: got lock");
    condition.wait(lock);
    x_DEBUG("User has signaled to interrupt program");
    lock.unlock();
}
