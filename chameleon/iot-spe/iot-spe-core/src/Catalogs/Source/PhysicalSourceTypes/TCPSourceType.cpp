...
TCPSourceType::TCPSourceType()
    : PhysicalSourceType(SourceType::TCP_SOURCE),
      ...
      inputFormat(Configurations::ConfigurationOption<Configurations::InputFormat>::create(
          ...
          "Input format defix how the data will arrive in x. Current Option: CSV (comma separated list with separator "
          "between lix/tuples), JSON.")),
      ...
      tupleSeparator(Configurations::ConfigurationOption<char>::create(
          ...
          '\n',
          "Tuple separator defix how the incoming TCP messages can be distinguished into tuples.")),
...
TCPSourceType::TCPSourceType(std::map<std::string, std::string> sourceConfigMap) : TCPSourceType() {
    x_INFO("TCPSourceType: Init default TCP source config object with values from command line args.");
    ...
    if (sourceConfigMap.find(Configurations::SOCKET_HOST_CONFIG) != sourceConfigMap.end()) {
        ...
        x_THROW_RUNTIME_ERROR("TCPSourceType:: no socket host defined! Please define a host.");
    }
    if (sourceConfigMap.find(Configurations::SOCKET_PORT_CONFIG) != sourceConfigMap.end()) {
        ...
        x_THROW_RUNTIME_ERROR("TCPSourceType:: no socket port defined! Please define a port.");
    }
    ...
    switch (decideMessageSize->getValue()) {
        case Configurations::TCPDecideMessageSize::TUPLE_SEPARATOR:
            if (sourceConfigMap.find(Configurations::TUPLE_SEPARATOR_CONFIG) != sourceConfigMap.end()) {
                ...
                x_THROW_RUNTIME_ERROR("TCPSourceType: You want to use a tuple separator to obtain the size of a message, "
        ...
        case Configurations::TCPDecideMessageSize::USER_SPECIFIED_BUFFER_SIZE:
            if (sourceConfigMap.find(Configurations::SOCKET_BUFFER_SIZE_CONFIG) != sourceConfigMap.end()) {
                ...
                x_THROW_RUNTIME_ERROR("TCPSourceType: You want to use a pre-specified size as message size, "
        ...
        case Configurations::TCPDecideMessageSize::BUFFER_SIZE_FROM_SOCKET:
            if (sourceConfigMap.find(Configurations::BYTES_USED_FOR_SOCKET_BUFFER_SIZE_TRANSFER_CONFIG)
                ...
                x_THROW_RUNTIME_ERROR("TCPSourceType: You want to use a fixed number of bytes send between messages to "
...
TCPSourceType::TCPSourceType(Yaml::Node yamlConfig) : TCPSourceType() {
    x_INFO("TCPSourceType: Init default TCP source config object with values from YAML file.");
...
void TCPSourceType::setSocketDomainViaString(std::string domainValue) {
    if (strcasecmp(domainValue.c_str(), "AF_INET") == 0) {
        ...
        x_ERROR("TCPSourceType::setSocketDomainViaString: Value unknown.");
    }
}
