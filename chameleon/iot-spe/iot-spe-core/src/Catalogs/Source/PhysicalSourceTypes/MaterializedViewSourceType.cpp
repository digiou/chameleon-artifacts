...
MaterializedViewSourceType::MaterializedViewSourceType(std::map<std::string, std::string> sourceConfigMap)
    ...
    x_INFO("MaterializedViewSourceType: Init source config object with new values.");
    if (sourceConfigMap.find(MATERIALIZED_VIEW_ID_CONFIG) != sourceConfigMap.end()) {
        ...
        x_THROW_RUNTIME_ERROR("MaterializedViewSourceType:: no id defined! Please define an id.");
    }
}
...
MaterializedViewSourceType::MaterializedViewSourceType()
    ...
    x_INFO("MaterializedViewSourceType: Init source config object with default values.");
}
