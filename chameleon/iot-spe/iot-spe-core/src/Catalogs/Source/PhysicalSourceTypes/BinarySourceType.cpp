...
BinarySourceType::BinarySourceType(Yaml::Node yamlConfig) : BinarySourceType() {
    x_INFO("CSVSourceType: Init default CSV source config object with values from YAML.");
    if (!yamlConfig[Configurations::FILE_PATH_CONFIG].As<std::string>().empty()
        ...
        x_THROW_RUNTIME_ERROR("BinarySourceType:: no filePath defined! Please define a filePath using "
...
BinarySourceType::BinarySourceType(std::map<std::string, std::string> sourceConfigMap) : BinarySourceType() {
    x_INFO("CSVSourceType: Init default CSV source config object with values from command line.");
...
BinarySourceType::BinarySourceType()
    ...
    x_INFO("BinarySourceTypeConfig: Init source config object with default params.");
}
