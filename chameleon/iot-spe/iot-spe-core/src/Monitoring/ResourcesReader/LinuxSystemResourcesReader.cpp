...
RuntimeMetrics LinuxSystemResourcesReader::readRuntimexMetrics() {
    ...
    try {
        x_TRACE("LinuxSystemResourcesReader: Reading memory.usage_in_bytes for metrics");
        ...
        if (access(metricLocations[0].c_str(), F_OK) != -1) {
            ...
            x_ERROR("LinuxSystemResourcesReader: File for memory.usage_in_bytes not available");
            output.memoryUsageInBytes = 0;
        }
    } catch (const Exceptions::RuntimeException& e) {
        x_ERROR("LinuxSystemResourcesReader: Error reading memory metrics {}", e.what());
    }

    try {
        x_TRACE("LinuxSystemResourcesReader: Reading cpuacct.stat for metrics");
        ...
        if (access(metricLocations[1].c_str(), F_OK) != -1) {
            ...
            x_ERROR("LinuxSystemResourcesReader: File for cpuacct.stat not available");
            output.cpuLoadInJiffies = 0;
        }
    } catch (const Exceptions::RuntimeException& e) {
        x_ERROR("LinuxSystemResourcesReader: Error reading cpu metrics {}", e.what());
    }

    try {
        x_TRACE("LinuxSystemResourcesReader: Reading blkio.throttle.io_service_bytes for metrics");
        ...
        if (access(metricLocations[2].c_str(), F_OK) != -1) {
            ...
            x_ERROR("LinuxSystemResourcesReader: File for blkio.throttle.io_service_bytes not available");
            output.blkioBytesRead = 0;
            output.blkioBytesWritten = 0;
        }

    } catch (const Exceptions::RuntimeException& e) {
        x_ERROR("LinuxSystemResourcesReader: Error reading disk metrics {}", e.what());
    }
...
RegistrationMetrics LinuxSystemResourcesReader::readRegistrationMetrics() {
    ...
    try {
        ...
        x_ERROR("LinuxSystemResourcesReader: Error reading static memory metrics {}", e.what());
    }
    ...
    try {
        ...
        if (access(metricLocations[1].c_str(), F_OK) != -1) {
            ...
            x_WARNING("LinuxSystemResourcesReader: File for cpu.cfs_period_us not available");
        ...
        if (access(metricLocations[2].c_str(), F_OK) != -1) {
            ...
            x_WARNING("LinuxSystemResourcesReader: File for cpu.cfs_quota_us not available");
            output.cpuQuotaUS = 0;
        }
    } catch (const Exceptions::RuntimeException& e) {
        x_ERROR("LinuxSystemResourcesReader: Error reading static cpu metrics {}", e.what());
    }
    return output;
}
...
CpuMetricsWrapper LinuxSystemResourcesReader::readCpuStats() {
    ...
    try {
        x_TRACE("LinuxSystemResourcesReader: Reading CPU stats for number of CPUs {}", numCpuMetrics);
        ...
        while (std::getline(fileStat, line)) {
            ...
            if (!line.compare(0, 3, "cpu")) {
                ...
                // check columns
                if (tokens.size() != 11) {
                    x_THROW_RUNTIME_ERROR("LinuxSystemResourcesReader: /proc/stat incorrect");
                }
        ...
        x_ERROR("LinuxSystemResourcesReader: Error calling readCpuStats() {}", e.what());
    }
...
NetworkMetricsWrapper LinuxSystemResourcesReader::readNetworkStats() {
    ...
    try {
        x_TRACE("LinuxSystemResourcesReader: Reading network stats.");
        ...
        x_ERROR("LinuxSystemResourcesReader: Error reading network stats {}", e.what());
    }
    return output;
}
...
MemoryMetrics LinuxSystemResourcesReader::readMemoryStats() {
    ...
    try {
        x_TRACE("LinuxSystemResourcesReader: Reading memory stats.");

        auto* sinfo = (struct sysinfo*) malloc(sizeof(struct sysinfo));

        int ret = sysinfo(sinfo);
        if (ret == EFAULT) {
            x_THROW_RUNTIME_ERROR("LinuxSystemResourcesReader: Error reading memory stats");
        }
        ...
        x_ERROR("LinuxSystemResourcesReader: Error reading memory stats {}", e.what());
    }
    return output;
}
...
DiskMetrics LinuxSystemResourcesReader::readDiskStats() {
    ...
    try {
        x_TRACE("LinuxSystemResourcesReader: Reading disk stats.");
        auto* svfs = (struct statvfs*) malloc(sizeof(struct statvfs));

        int ret = statvfs("/", svfs);
        if (ret == EFAULT) {
            x_THROW_RUNTIME_ERROR("LinuxSystemResourcesReader: Error reading disk stats");
        }
        ...
        x_ERROR("LinuxSystemResourcesReader: Error reading disk stats {}", e.what());
    }
    return output;
}
