...
NetworkCollector::NetworkCollector()
    ...
    x_INFO("NetworkCollector: Init NetworkCollector with schema {}", schema->toString());
}
...
bool NetworkCollector::fillBuffer(Runtime::TupleBuffer& tupleBuffer) {
    try {
        ...
        x_TRACE("NetworkCollector: Written metrics for {}: {}", getNodeId(), asJson(measuredVal));
    } catch (const std::exception& ex) {
        x_ERROR("NetworkCollector: Error while collecting metrics {}", ex.what());
        return false;
    }
    return true;
}
