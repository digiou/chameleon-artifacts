...
CpuCollector::CpuCollector()
    ...
    x_INFO("CpuCollector: Init CpuCollector with schema {}", schema->toString());
}
...
bool CpuCollector::fillBuffer(Runtime::TupleBuffer& tupleBuffer) {
    try {
        ...
        x_TRACE("CpuCollector: Written metrics for {}: {}", getNodeId(), asJson(measuredVal));
    } catch (const std::exception& ex) {
        x_ERROR("CpuCollector: Error while collecting metrics {}", ex.what());
        return false;
    }
    return true;
}
