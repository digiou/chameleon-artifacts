...
bool MetricUtils::validateFieldsInSchema(SchemaPtr metricSchema, SchemaPtr bufferSchema, uint64_t i) {
    ...
    auto hasName = x::Util::endsWith(bufferSchema->fields[i]->getName(), metricSchema->get(0)->getName());
    auto hasLastField = x::Util::endsWith(bufferSchema->fields[i + metricSchema->getSize() - 1]->getName(),
...
MetricCollectorPtr MetricUtils::createCollectorFromCollectorType(MetricCollectorType type) {
    switch (type) {
        ...
        default: x_FATAL_ERROR("MetricUtils: Not supported collector type {}", std::string(magic_enum::enum_name(type)));
    }
    return nullptr;
}
...
MetricPtr MetricUtils::createMetricFromCollectorType(MetricCollectorType type) {
    switch (type) {
        ...
        default: {
            x_FATAL_ERROR("MetricUtils: Collector type not supported {}", std::string(magic_enum::enum_name(type)));
        }
    }
    return nullptr;
}
...
MetricCollectorType MetricUtils::createCollectorTypeFromMetricType(MetricType type) {
    switch (type) {
        ...
        default: {
            x_ERROR("MetricUtils: Metric type not supported {}", std::string(magic_enum::enum_name(type)));
