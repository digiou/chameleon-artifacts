...
AllEntriesMetricStore::AllEntriesMetricStore() { x_DEBUG("AllEntriesMetricStore: Init NewestMetricStore"); }
...
void AllEntriesMetricStore::addMetrics(uint64_t nodeId, MetricPtr metric) {
    ...
    if (storedMetrics.contains(nodeId)) {
        x_TRACE("AllEntriesMetricStore: Found stored metrics for node with ID {}", nodeId);
        nodeMetrics = storedMetrics[nodeId];
        // check if the metric type exists
        if (!nodeMetrics->contains(metric->getMetricType())) {
            x_TRACE("AllEntriesMetricStore: Creating metrics {} of {}",
                      nodeId,
        ...
        x_TRACE("AllEntriesMetricStore: Creating node {} of {}",
                  nodeId,
                  std::string(magic_enum::enum_name(metric->getMetricType())));
    ...
    x_TRACE("AllEntriesMetricStore: Adding metrics for {} with type {}: {}",
              nodeId,
              std::string(magic_enum::enum_name(metric->getMetricType())),
