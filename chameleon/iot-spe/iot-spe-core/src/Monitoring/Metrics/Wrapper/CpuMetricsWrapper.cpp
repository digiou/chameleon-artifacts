...
CpuMetricsWrapper::CpuMetricsWrapper(std::vector<CpuMetrics>&& arr) : CpuMetricsWrapper() {
    if (!arr.empty()) {
        ...
        x_THROW_RUNTIME_ERROR("CpuMetricsWrapper: Object cannot be allocated with less than 0 cores.");
    }
    x_TRACE("CpuMetricsWrapper: Allocating memory for {} metrics.", std::to_string(arr.size()));
}
...
void CpuMetricsWrapper::writeToBuffer(Runtime::TupleBuffer& buf, uint64_t tupleIndex) const {
    ...
    if (schema->getSchemaSizeInBytes() > buf.getBufferSize()) {
        x_ERROR("CpuMetricsWrapper: At least one tuple of CpuMetrics has to fit into buffer");
    } else if (totalSize > buf.getBufferSize()) {
        x_WARNING("CpuMetricsWrapper: Content does not fit in TupleBuffer totalSize: {} > getBufferSize:{}",
                    totalSize,
                    buf.getBufferSize());
    }
...
void CpuMetricsWrapper::readFromBuffer(Runtime::TupleBuffer& buf, uint64_t tupleIndex) {
    ...
    x_TRACE("CpuMetricsWrapper: Parsing buffer with number of tuples {}", buf.getNumberOfTuples());
...
bool CpuMetricsWrapper::operator==(const CpuMetricsWrapper& rhs) const {
    if (cpuMetrics.size() != rhs.size()) {
        x_ERROR("CpuMetricsWrapper: Sizes are not equal {}!={}", cpuMetrics.size(), rhs.size());
        return false;
    }

    for (unsigned int i = 0; i < cpuMetrics.size(); i++) {
        if (cpuMetrics[i] != rhs.cpuMetrics[i]) {
            x_ERROR("CpuMetricsWrapper: Cpu core {} are not equal.", i);
            return false;
        }
    }
