...
NetworkMetricsWrapper::NetworkMetricsWrapper(std::vector<NetworkMetrics>&& arr) : NetworkMetricsWrapper() {
    if (!arr.empty()) {
        ...
        x_THROW_RUNTIME_ERROR("NetworkMetricsWrapper: Object cannot be allocated with less than 0 cores.");
    }
    x_TRACE("NetworkMetricsWrapper: Allocating memory for {} metrics.", std::to_string(arr.size()));
}
...
void NetworkMetricsWrapper::writeToBuffer(Runtime::TupleBuffer& buf, uint64_t tupleIndex) const {
    ...
    if (schema->getSchemaSizeInBytes() > buf.getBufferSize()) {
        x_ERROR("NetworkMetricsWrapper: At least one tuple of NetworkMetrics has to fit into buffer");
    } else if (totalSize > buf.getBufferSize()) {
        x_WARNING("NetworkMetricsWrapper: Content does not fit in TupleBuffer totalSize: {} > getBufferSize: {}",
                    totalSize,
                    buf.getBufferSize());
    }
...
void NetworkMetricsWrapper::readFromBuffer(Runtime::TupleBuffer& buf, uint64_t tupleIndex) {
    ...
    x_TRACE("NetworkMetricsWrapper: Parsing buffer with number of tuples {}", buf.getNumberOfTuples());

    for (unsigned int n = 0; n < buf.getNumberOfTuples(); n++) {
        //for each core parse the according CpuMetrics
        NetworkMetrics metrics{};
        x::Monitoring::readFromBuffer(metrics, buf, tupleIndex + n);
...
NetworkMetrics NetworkMetricsWrapper::getNetworkValue(uint64_t interfaceNo) const {
    if (interfaceNo >= size()) {
        x_THROW_RUNTIME_ERROR("NetworkMetricsWrapper: ArrayType index out of bound " + std::to_string(interfaceNo)
                                + ">=" + std::to_string(size()));
    }
