...
void writeToBuffer(const uint64_t& metrics, Runtime::TupleBuffer&, uint64_t) {
    x_THROW_RUNTIME_ERROR("Metric: Serialization for uint64_t not possible for metric " << metrics);
}

void writeToBuffer(const std::string& metrics, Runtime::TupleBuffer&, uint64_t) {
    x_THROW_RUNTIME_ERROR("Metric: Serialization for std::string not possible for metric " << metrics);
}
...
void readFromBuffer(uint64_t&, Runtime::TupleBuffer&, uint64_t) {
    x_THROW_RUNTIME_ERROR("Metric: Deserialization for uint64_t not possible");
}
