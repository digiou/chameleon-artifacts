...
MonitoringAgent::MonitoringAgent(MonitoringPlanPtr monitoringPlan, MonitoringCatalogPtr catalog, bool enabled)
    ...
    x_DEBUG("MonitoringAgent: Init with monitoring plan {} and enabled={}", monitoringPlan->toString(), enabled);
}
...
const std::vector<MetricPtr> MonitoringAgent::getMetricsFromPlan() const {
    ...
    if (enabled) {
        x_DEBUG("MonitoringAgent: Monitoring enabled, reading metrics for getMetricsFromPlan().");
        ...
        x_WARNING("MonitoringAgent: Monitoring disabled, getMetricsFromPlan() returns empty vector.");
    }
    return output;
}
...
nlohmann::json MonitoringAgent::getMetricsAsJson() {
    ...
    if (enabled) {
        for (auto type : monitoringPlan->getMetricTypes()) {
            x_INFO("MonitoringAgent: Collecting metrics of type {}", std::string(magic_enum::enum_name(type)));
            auto collector = catalog->getMetricCollector(type);
            collector->setNodeId(nodeId);
    ...
    x_INFO("MonitoringAgent: Metrics collected {}", metricsJson.dump());
...
RegistrationMetrics MonitoringAgent::getRegistrationMetrics() {
    ...
    x_WARNING("MonitoringAgent: Metrics disabled. Return empty metric object for registration.");
...
bool MonitoringAgent::addMonitoringStreams(const Configurations::WorkerConfigurationPtr workerConfig) {
    if (enabled) {
        for (auto metricType : monitoringPlan->getMetricTypes()) {
            ...
            x_INFO("MonitoringAgent: Adding physical source to config {} _ph with wait time {}",
                     metricTypeString,
    ...
    x_WARNING("MonitoringAgent: Monitoring is disabled, registering of physical monitoring streams not possible.");
    return false;
}
