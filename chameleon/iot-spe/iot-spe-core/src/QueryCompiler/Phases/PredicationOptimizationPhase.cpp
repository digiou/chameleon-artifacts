...
PipelineQueryPlanPtr PredicationOptimizationPhase::apply(PipelineQueryPlanPtr pipelinedQueryPlan) {
    if (filterProcessingStrategy == QueryCompilerOptions::FilterProcessingStrategy::BRANCHED) {
        x_DEBUG("PredicationOptimizationPhase: No optimization requested or applied.");
...
OperatorPipelinePtr PredicationOptimizationPhase::apply(OperatorPipelinePtr operatorPipeline) {
    ...
    x_DEBUG("PredicationOptimizationPhase: Scanning pipeline for optimization potential.");
    ...
    for (const auto& node : nodes) {
        if (node->instanceOf<GeneratableOperators::GeneratableFilterOperator>()) {
            ...
            x_DEBUG("PredicationOptimizationPhase: Replaced filter operator with equivalent predicated filter operator.");
        }
    }
