...
DataSourcePtr SourceSharingDataSourceProvider::lower(OperatorId operatorId,
    ...
    x_ASSERT(sourceDescriptor->getLogicalSourceName() != "", "The source name is not allowed to be null");
    ...
    if (sourceDescriptorToDataSourceMap.contains(searchEntry)) {
        x_DEBUG("using already existing source for source sharing for logical name {}",
        ...
        x_DEBUG("Create first source for source sharing for logical name  {}", sourceDescriptor->getLogicalSourceName());
