...
#include <Execution/Operators/Streaming/Join/xtedLoopJoin/JoinPhases/NLJBuild.hpp>
#include <Execution/Operators/Streaming/Join/xtedLoopJoin/JoinPhases/NLJProbe.hpp>
...
LowerPhysicalToNautilusOperators::lower(Runtime::Execution::PhysicalOperatorPipeline& pipeline,
                                        ...
                                        size_t bufferSize,
                                        std::vector<Runtime::Execution::OperatorHandlerPtr>& operatorHandlers) {
    x_INFO("Lower node:{} to NautilusOperator.", operatorNode->toString());
    if (operatorNode->instanceOf<PhysicalOperators::PhysicalScanOperator>()) {
        ...
        x_DEBUG("Added streamJoinOpHandler to operatorHandlers!");
        ...
        auto joinSinkNautilus = std::make_shared<Runtime::Execution::Operators::StreamHashJoinProbe>(
            ...
            sinkOperator->x::BinaryOperatorNode::getOutputSchema(),
    ...
    } else if (operatorNode->instanceOf<PhysicalOperators::PhysicalxtedLoopJoinProbeOperator>()) {
        const auto sinkOperator = operatorNode->as<PhysicalOperators::PhysicalxtedLoopJoinProbeOperator>();
    ...
    } else if (operatorNode->instanceOf<PhysicalOperators::PhysicalxtedLoopJoinBuildOperator>()) {
        auto buildOperator = operatorNode->as<PhysicalOperators::PhysicalxtedLoopJoinBuildOperator>();

        x_DEBUG("Added PhysicalxtedLoopJoinBuildOperator to operatorHandlers!");
        operatorHandlers.push_back(buildOperator->getOperatorHandler());
        auto handlerIndex = operatorHandlers.size() - 1;
...
LowerPhysicalToNautilusOperators::lowerTimeFunction(const Windowing::TimeBasedWindowTypePtr& timeWindow) {
    ...
    x_THROW_RUNTIME_ERROR("Timefunction could not be created for the following window definition: " << timeWindow->toString());
}
...
LowerPhysicalToNautilusOperators::lowerKeyedThreadLocalPreAggregationOperator(
    ...
    x_ASSERT(!keys.empty(), "A keyed window should have keys");
...
LowerPhysicalToNautilusOperators::lowerScan(Runtime::Execution::PhysicalOperatorPipeline&,
    ...
    x_ASSERT(schema->getLayoutType() == Schema::MemoryLayoutType::ROW_LAYOUT, "Currently only row layout is supported");
...
LowerPhysicalToNautilusOperators::lowerThresholdWindow(Runtime::Execution::PhysicalOperatorPipeline&,
    ...
    x_INFO("lowerThresholdWindow {} and handlerid {}", operatorPtr->toString(), handlerIndex);
    ...
    x_INFO("lowerThresholdWindow Predicate {}", thresholdWindowType->getPredicate()->toString());
...
LowerPhysicalToNautilusOperators::lowerAggregations(const std::vector<Windowing::WindowAggregationPtr>& aggs) {
    x_INFO("Lower Window Aggregations to Nautilus Operator");
    ...
    std::transform(aggs.cbegin(),
                   ...
                   [&](const Windowing::WindowAggregationDescriptorPtr& agg)
                       ...
                       if (auto fieldAccessExpression = agg->as()->as_if<FieldAccessExpressionNode>()) {
                           ...
                           x_THROW_RUNTIME_ERROR("Currently complex expression in as fields are not supported");
                       }
                       switch (agg->getType()) {
...
LowerPhysicalToNautilusOperators::getAggregationValueForThresholdWindow(
    ...
    // TODO 3468: Check if we can make this ugly xted switch case better
    switch (aggregationType) {
        case Windowing::WindowAggregationDescriptor::Type::Avg:
            switch (basicType->nativeType) {
                ...
                default: x_THROW_RUNTIME_ERROR("Unsupported data type");
            }
        case Windowing::WindowAggregationDescriptor::Type::Count:
            switch (basicType->nativeType) {
                case BasicPhysicalType::NativeType::INT_8:
        ...
        default: x_THROW_RUNTIME_ERROR("Unsupported aggregation type");
    }
}
