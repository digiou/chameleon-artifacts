...
void DefaultGeneratableOperatorProvider::lower(QueryPlanPtr queryPlan, PhysicalOperators::PhysicalOperatorPtr operatorNode) {
    x_DEBUG("Lower {} to generatable operator", operatorNode->toString());
...
void DefaultGeneratableOperatorProvider::lowerSink(const QueryPlanPtr&,
    ...
    x_ASSERT(operatorNode->getChildren().size(), "A sink node should have no children");
    x_ASSERT(operatorNode->getParents().size(), "A sink node should have no parents");
}
...
void DefaultGeneratableOperatorProvider::lowerSource(const QueryPlanPtr&,
    ...
    x_ASSERT(operatorNode->getChildren().size(), "A source operator should have no children");
    x_ASSERT(operatorNode->getParents().size(), "A source operator should have no parents");
}
