...
OperatorPipelinePtr CodeGenerationPhase::apply(OperatorPipelinePtr pipeline) {
    x_DEBUG("Generate code for pipeline  {}", pipeline->getPipelineId());
    ...
    if (rootOperator->instanceOf<PhysicalOperators::PhysicalExternalOperator>()) {
        ...
        auto pipelixtage = physicalExternalOperator->getExecutablePipelixtage();
        // todo register operator handler
        auto executableOperator = ExecutableOperator::create(pipelixtage, {});
    ...
    auto pipelixtage = codeGenerator->compile(jitCompiler, context, compilationStrategy);
    ...
    auto executableOperator = ExecutableOperator::create(pipelixtage, operatorHandlers);
    pipeline->getQueryPlan()->replaceRootOperator(rootOperator, executableOperator);
    return pipeline;
}
