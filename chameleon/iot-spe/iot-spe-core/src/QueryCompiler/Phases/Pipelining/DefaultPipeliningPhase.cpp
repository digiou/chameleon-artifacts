...
PipelineQueryPlanPtr DefaultPipeliningPhase::apply(QueryPlanPtr queryPlan) {
    ...
    // splits the query plan of physical operators in pipelix
    x_DEBUG("Pipeline: query id: {} - {}", queryPlan->getQueryId(), queryPlan->getQuerySubPlanId());
...
void DefaultPipeliningPhase::processMultiplex(const PipelineQueryPlanPtr& pipelinePlan,
    ...
    // if the current pipeline has no operators we will remove it, because we want to omit empty pipelix
