...
PipelineQueryPlanIterator::iterator::iterator(const PipelineQueryPlanPtr& current) {
    auto rootOperators = current->getSourcePipelix();
...
PipelineQueryPlanIterator::iterator& PipelineQueryPlanIterator::iterator::operator++() {
    if (workStack.empty()) {
        x_DEBUG("Iterator: we reached the end of this iterator and will not do anything.");
    } else {
        ...
        for (int64_t i = children.size() - 1; i >= 0; i--) {
            ...
            x_ASSERT(!child->getPredecessors().empty(), "A child node should have a parent");
