...
void RecordHandler::registerAttribute(const std::string& name, ExpressionStatementPtr variableAccessStatement) {
    if (hasAttribute(name)) {
        x_DEBUG("RecordHandler: replace attribute with name {}", name);
    } else {
        x_DEBUG("RecordHandler: place new attribute with name {}", name);
    }
...
ExpressionStatementPtr RecordHandler::getAttribute(const std::string& name) {
    if (!hasAttribute(name)) {
        x_ASSERT2_FMT(hasAttribute(name), "RecordHandler: Attribute name: " << name << " is not registered.");
    }
