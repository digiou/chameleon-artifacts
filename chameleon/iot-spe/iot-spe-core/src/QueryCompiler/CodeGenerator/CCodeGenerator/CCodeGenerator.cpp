...
StructDeclaration CCodeGenerator::getStructDeclarationFromSchema(const std::string& structName, const SchemaPtr& schema) {
    ...
    x_DEBUG("Converting Schema: {}", schema->toString());
    x_DEBUG("Define Struct : {}", structName);
    ...
    for (uint64_t i = 0; i < schema->getSize(); ++i) {
        if (schema->getLayoutType() == Schema::MemoryLayoutType::ROW_LAYOUT) {
            ...
            x_ERROR("inputSchema->getLayoutType()is neither ROW_LAYOUT nor COL_LAYOUT!!!");
        }
        x_DEBUG("Field {}: {} {}", i, schema->get(i)->getDataType()->toString(), schema->get(i)->getName());
    }
...
bool CCodeGenerator::generateCodeForScan(SchemaPtr inputSchema, SchemaPtr outputSchema, PipelineContextPtr context) {
    x_DEBUG("CCodeGenerator: Generating code for scan with inputSchema {}", inputSchema->toString());
    ...
    switch (context->arity) {
        case PipelineContext::PipelineContextArity::Unary: {
            ...
            x_DEBUG("arity unary generate scan for input={} output={}", inputSchema->toString(), outputSchema->toString());
            break;
        }
        case PipelineContext::PipelineContextArity::BinaryLeft: {
            ...
            x_DEBUG("arity binaryleft generate scan for input={} output={}", inputSchema->toString(), outputSchema->toString());
            break;
        }
        case PipelineContext::PipelineContextArity::BinaryRight: {
            ...
            x_DEBUG("arity binaryright generate scan for input={} output={}",
    ...
    auto tf = getTypeFactory();
    /* === declarations === */
    auto tupleBufferType = tf->createAnonymusDataType("x::Runtime::TupleBuffer");
    auto pipelineExecutionContextType = tf->createAnonymusDataType("Runtime::Execution::PipelineExecutionContext");
    ...
    if (inputSchema->getLayoutType() == Schema::MemoryLayoutType::ROW_LAYOUT) {
        x_DEBUG("CCodeGenerator::generateCodeForEmit: generate emit for row layout");
        ...
        x_DEBUG("CCodeGenerator::generateCodeForEmit: varDeclResultTuple code is {}", varDeclInputTupleStmt.getCode()->code_);
    ...
    if (context->arity != PipelineContext::PipelineContextArity::Unary) {
        x_DEBUG("adding in scan for schema={} context={}", inputSchema->toString(), context->inputSchema->toString());
    }
    ...
    for (const auto& expression : projectExpressions) {
        if (expression->instanceOf<FieldRenameExpressionNode>()) {
            ...
            if (!recordHandler->hasAttribute(fieldAccessExpression->getFieldName())) {
                x_FATAL_ERROR("CCodeGenerator: projection: the attribute {} is not registered so we can not access it.",
...
bool CCodeGenerator::generateCodeForInferModel(PipelineContextPtr context,
    ...
    for (auto f : inputFields) {
        ...
        if (!field->getStamp()->isNumeric() && !field->getStamp()->isBoolean()) {
            x_ERROR("CCodeGenerator::generateCodeForInferModel: inputted data type for tensorflow model not supported: {}",
    ...
    x_DEBUG("CCodeGenerator::generateCodeForInferModel: Common stamp for input tensor: {}", commonStamp->toString());
    ...
    for (auto f : inputFields) {
        ...
        if (commonStamp->isInteger()) {
            ...
            x_ERROR("CCodeGenerator: common data type for tensorflow model not supported: {}", commonStamp->toString());
        }
    }
...
bool CCodeGenerator::generateCodeForEmit(SchemaPtr sinkSchema,
    ...
    auto tf = getTypeFactory();
    x_DEBUG("CCodeGenerator: Generate code for Sink.");
    auto code = context->code;
    ...
    if (sinkSchema->getLayoutType() == Schema::MemoryLayoutType::ROW_LAYOUT) {
        ...
        if (bufferStrategy == OutputBufferAllocationStrategy::ONLY_INPLACE_OPERATIONS) {
            ...
            if (bufferStrategy == OutputBufferAllocationStrategy::REUSE_INPUT_BUFFER_AND_OMIT_OVERFLOW_CHECK
                ...
                // Generates: x::Runtime::TupleBuffer resultTupleBuffer = inputTupleBuffer;
                code->variableInitStmts.push_back(
                ...
                for (const auto& field : context->resultSchema->fields) {
                    if (context->getInputSchema()->hasFieldName(field->getName())) {
                        ...
                        if (!recordHandler->hasAttribute(field->getName())) {
                            x_FATAL_ERROR("CCodeGenerator: field: {} is part of the output schema, but not registered in the "
                                            "record handler.",
                                            field->toString());
                        }
                ...
                // Generates: x::Runtime::TupleBuffer resultTupleBuffer = workerContext.allocateTupleBuffer();
            ...
            if (bufferAssignmentStrategy == OutputBufferAssignmentStrategy::FIELD_COPY) {
                ...
                for (const auto& field : context->resultSchema->fields) {
                    ...
                    if (!resultRecordFieldVariableDeclaration) {
                        x_FATAL_ERROR("CCodeGenerator: Could not extract field {} from result record struct {}",
                                        field->toString(),
                    ...
                    // check if record handler has current field
                    if (!recordHandler->hasAttribute(field->getName())) {
                        x_FATAL_ERROR("CCodeGenerator: field: {}"
        ...
        x_DEBUG("CCodeGenerator::generateCodeForEmit: varDeclResultTuple code is {}", varDeclResultTupleStmt.getCode()->code_);
        ...
        for (const auto& field : context->resultSchema->fields) {
            ...
            if (!resultRecordFieldVariableDeclaration) {
                x_FATAL_ERROR("CodeGenerator: Could not extract field {} from result record struct {}",
                                field->toString(),
                                structDeclarationResultTuple.getTypeName());
            }
        ...
        x_ERROR("CCodeGenerator: inputSchema->getLayoutType() is neither ROW_LAYOUT nor COL_LAYOUT!!!");
    }
...
bool CCodeGenerator::generateCodeForWatermarkAssigner(Windowing::WatermarkStrategyPtr watermarkStrategy,
    ...
    if (watermarkStrategy->getType() == Windowing::WatermarkStrategy::Type::EventTimeWatermark) {
        ...
        x_ASSERT(!context->code->structDeclarationInputTuples.empty(), "invalid size of struct input tuples");
        ...
        if (watermarkFieldName != Windowing::TimeCharacteristic::RECORD_CREATION_TS_FIELD_NAME) {
            ...
            x_ASSERT(recordHandler->hasAttribute(watermarkFieldName),
        ...
        auto calculateMaxTupleStatement = (*tsVariableDeclaration)
            ...
            - Constant(tf->createValueType(DataTypeFactory::createBasicValue(eventTimeWatermarkStrategy->getAllowedLatexs())));
        ...
        /*
         * //we have to have this case to make sure that we don't get negative here when allowed latexs is larger that minwatermark
         * if(minWatermark < allowedLatexs)
         *      minWatermark = 0
         */
        ...
        //                                 DataTypeFactory::createBasicValue(eventTimeWatermarkStrategy->getAllowedLatexs()))),
        ...
        // auto watermark_ts = x::Windowing::getTsFromClock()
        auto tf = getTypeFactory();
        auto watermarkTsVariableDeclaration = VariableDeclaration::create(tf->createAnonymusDataType("auto"), "watermark_ts");
        auto getCurrentTs = FunctionCallStatement("x::Windowing::getTsFromClock");
        ...
        auto setWatermarkFunctionCall = FunctionCallStatement("setWatermark");
        //TODO currently we do not support allowed latexs for ingestion time windows
        ...
        x_ERROR("CCodeGenerator: cannot generate code for watermark strategy");
    }
...
void CCodeGenerator::generateTupleBufferSpaceCheck(const PipelineContextPtr& context,
                                                   ...
                                                   SchemaPtr schema) {
    x_DEBUG("CCodeGenerator: Generate code for tuple buffer check");
    ...
    code->variableInitStmts.push_back(
        VarDeclStatement(mergeTask)
            .assign(TypeCast(getBuffer(code->varDeclarationInputBuffer),
                             tf->createAnonymusDataType("x::Windowing::Experimental::SliceMergeTask*")))
            .copy());
    auto windowOperatorHandlerDeclaration =
        VariableDeclaration::create(tf->createAnonymusDataType("auto"), "windowOperatorHandler");
...
bool CCodeGenerator::generateCodeForNonKeyedSlidingWindowSink(
    ...
    code->variableInitStmts.push_back(
        VarDeclStatement(mergeTask)
            .assign(TypeCast(getBuffer(code->varDeclarationInputBuffer),
                             tf->createAnonymusDataType("x::Windowing::Experimental::WindowTriggerTask*")))
            .copy());
    auto windowOperatorHandlerDeclaration =
        VariableDeclaration::create(tf->createAnonymusDataType("auto"), "windowOperatorHandler");
...
bool CCodeGenerator::generateCodeForCompleteWindow(
    ...
    x_ASSERT(!window->getWindowAggregation()[0]->getInputStamp()->isUndefined(), "window input type is undefined");
    x_ASSERT((!window->getWindowAggregation()[0]->getPartialAggregateStamp()->isUndefined()
    ...
    x_ASSERT(!window->getWindowAggregation()[0]->getFinalAggregateStamp()->isUndefined(), "window final type is undefined");
    ...
    // get allowed latexs
    //    auto allowedLatexs = windowManager->getAllowedLatexs();
    auto latexsHandlerVariableDeclaration = VariableDeclaration::create(tf->createAnonymusDataType("auto"), "allowedLatexs");
    auto getAllowedLatexsStateVariable = FunctionCallStatement("getAllowedLatexs");
    auto allowedLatexsHandlerVariableStatement = VarRef(windowManagerVarDeclaration).accessPtr(getAllowedLatexsStateVariable);
    context->code->variableInitStmts.emplace_back(
        VarDeclStatement(latexsHandlerVariableDeclaration).assign(allowedLatexsHandlerVariableStatement).copy());
    ...
    switch (window->getWindowAggregation()[0]->getType()) {
        ...
        default: {
            x_FATAL_ERROR("CCodeGenerator: Window Handler - could not cast aggregation type");
        }
    }
    ...
    if (timeBasedWindowType->getTimeCharacteristic()->getType() == Windowing::TimeCharacteristic::Type::IngestionTime) {
        ...
        x_ASSERT(!context->code->structDeclarationInputTuples.empty(), "invalid number of input tuples");
...
bool CCodeGenerator::generateCodeForCEPIterationOperator(uint64_t minIteration,
                                                         ...
                                                         PipelineContextPtr context) {
    x_DEBUG("CCodeGenerator::generateCodeForCEPIteration: start generating code for CEPITerations");
    auto tf = getTypeFactory();
    auto handler = CEP::CEPOperatorHandler::create();
    auto index = context->registerOperatorHandler(handler);
    auto recordHandler = context->getRecordHandler();

    x_DEBUG("CCodeGenerator::generateCodeForCEPIteration: call getCEPOperatorHandler using index: {}", index);
    ...
    x_DEBUG("CCodeGenerator::generateCodeForCEPIteration: got CEPOperatorHandler");
    ...
    x_DEBUG("CCodeGenerator::generateCodeForCEPIteration: created IF CounterStatement");
    // first, add the head and brackets of the if-Counter statement
    context->code->currentCodeInsertionPoint->addStatement(ifStatement.createCopy());
    // second, move insertion point. the rest of the pipeline will be generated within the brackets of the if-statement
    context->code->currentCodeInsertionPoint = ifStatement.getCompoundStatement();

    x_DEBUG("CCodeGenerator::generateCodeForCEPIteration: Last Step clearCounter if condition is fulfilled");
...
bool CCodeGenerator::generateCodeForSlicingWindow(
    ...
    x_DEBUG("CCodeGenerator::generateCodeForSlicingWindow with {} pipeline {}", window->toString(), context->pipelineName);
...
uint64_t CCodeGenerator::generateJoinSetup(Join::LogicalJoinDefinitionPtr join, PipelineContextPtr context, uint64_t id) {
    ...
    auto tf = getTypeFactory();
    x_ASSERT(join, "invalid join definition");
    x_ASSERT(!join->getLeftJoinKey()->getStamp()->isUndefined(), "left join key is undefined");
    x_ASSERT(!join->getRightJoinKey()->getStamp()->isUndefined(), "right join key is undefined");
    x_ASSERT(join->getLeftJoinKey()->getStamp()->isEquals(join->getRightJoinKey()->getStamp()),
               "left join key is not the same type as right join key");
    x_ASSERT(join->getLeftSourceType() != nullptr && !join->getLeftSourceType()->fields.empty(), "left join type is undefined");
    x_ASSERT(join->getRightSourceType() != nullptr && !join->getRightSourceType()->fields.empty(),
               "right join type is undefined");
    x_ASSERT(context->arity != PipelineContext::PipelineContextArity::Unary,
               "unary operator detected but join codegen invoked");

    auto executionContextRef = VarRefStatement(context->code->varDeclarationExecutionContext);
    auto handlers = context->getOperatorHandlers();
    x_ASSERT(handlers.size() == 1, "invalid size");
    ...
    x_ASSERT(joinOperatorHandler != nullptr, "invalid join handler");
    ...
    if (policy->getPolicyType() == Windowing::TriggerType::triggerOnTime) {
        ...
        x_WARNING("This mode is not supported anymore");
        ...
        x_FATAL_ERROR("Aggregation Handler: mode={} not implemented", magic_enum::enum_name(policy->getPolicyType()));
    }
    ...
    if (action->getActionType() == Join::JoinActionType::LazyxtedLoopJoin) {
        auto createTriggerActionCall = call("Join::ExecutablextedLoopJoinTriggerAction<" + keyType->getCode()->code_
        ...
        x_FATAL_ERROR("Aggregation Handler: mode={} not implemented", magic_enum::enum_name(action->getActionType()));
    }
...
uint64_t CCodeGenerator::generateCodeForJoinSinkSetup(Join::LogicalJoinDefinitionPtr join,
    ...
    if (action->getActionType() == Join::JoinActionType::LazyxtedLoopJoin) {
        ...
        x_FATAL_ERROR("Aggregation Handler: mode={} not implemented",
...
CCodeGenerator::generateCodeForBatchJoinHandlerSetup(Join::Experimental::LogicalBatchJoinDefinitionPtr batchJoinDef,
    ...
    auto tf = getTypeFactory();
    x_ASSERT(batchJoinDef, "invalid join definition");
    x_ASSERT(!batchJoinDef->getBuildJoinKey()->getStamp()->isUndefined(), "left join key is undefined");
    x_ASSERT(!batchJoinDef->getProbeJoinKey()->getStamp()->isUndefined(), "right join key is undefined");
    x_ASSERT(batchJoinDef->getBuildJoinKey()->getStamp()->isEquals(batchJoinDef->getProbeJoinKey()->getStamp()),
               "left join key is not the same type as right join key");
    x_ASSERT(batchJoinDef->getBuildSchema() != nullptr && !batchJoinDef->getBuildSchema()->fields.empty(),
               "left join type is undefined");
    x_ASSERT(batchJoinDef->getProbeSchema() != nullptr && !batchJoinDef->getProbeSchema()->fields.empty(),
               "right join type is undefined");
    ...
    // initial registration of operator handler to make it available in the pipelix' setup() and execute() functions
...
bool CCodeGenerator::generateCodeForJoin(Join::LogicalJoinDefinitionPtr joinDef,
    ...
    x_ASSERT(joinDef, "invalid join definition");
    x_ASSERT(!joinDef->getLeftJoinKey()->getStamp()->isUndefined(), "left join key is undefined");
    x_ASSERT(!joinDef->getRightJoinKey()->getStamp()->isUndefined(), "right join key is undefined");
    x_ASSERT(joinDef->getLeftJoinKey()->getStamp()->isEquals(joinDef->getRightJoinKey()->getStamp()),
               "left join key is not the same type as right join key");
    x_ASSERT(joinDef->getLeftSourceType() != nullptr && !joinDef->getLeftSourceType()->fields.empty(),
               "left join type is undefined");
    x_ASSERT(joinDef->getRightSourceType() != nullptr && !joinDef->getRightSourceType()->fields.empty(),
               "right join type is undefined");
    x_ASSERT(context->arity != PipelineContext::PipelineContextArity::Unary,
               "unary operator detected but join codegen invoked");
    ...
    if (context->arity == PipelineContext::PipelineContextArity::BinaryLeft) {
        x_DEBUG("CCodeGenerator::generateCodeForJoin generate code for side left");
        ...
        x_DEBUG("CCodeGenerator::generateCodeForJoin generate code for side right");
    ...
    */
    ...
    if (context->arity == PipelineContext::PipelineContextArity::BinaryLeft) {
        ...
        x_ASSERT2_FMT(recordHandler->hasAttribute(joinKeyFieldName),
        ...
        x_ASSERT(recordHandler->hasAttribute(joinKeyFieldName), "join key is not defined on iput tuple");
    ...
    auto currentTimeVariableDeclaration = VariableDeclaration::create(tf->createAnonymusDataType("auto"), "current_ts");
    if (timeBasedWindowType->getTimeCharacteristic()->getType() == Windowing::TimeCharacteristic::Type::IngestionTime) {
        //      auto current_ts = x::Windowing::getTsFromClock();
        auto getCurrentTs = FunctionCallStatement("x::Windowing::getTsFromClock");
        auto getCurrentTsStatement = VarDeclStatement(currentTimeVariableDeclaration).assign(getCurrentTs);
        context->code->currentCodeInsertionPoint->addStatement(getCurrentTsStatement.copy());
    } else {
        ...
        if (context->arity == PipelineContext::PipelineContextArity::BinaryRight) {
            x_DEBUG("windowTimeStampFieldName bin right= {}", windowTimeStampFieldName);
            ...
            x_ASSERT(found, " right schema does not contain a timestamp attribute");
        } else {
            x_DEBUG("windowTimeStampFieldName bin left= {}", windowTimeStampFieldName);
        }
    ...
    x_DEBUG("CCodeGenerator: Generate code for : {}", context->pipelineName);
    // Generate code for watermark updater
    // i.e., calling updateAllMaxTs
...
bool CCodeGenerator::generateCodeForJoinBuild(Join::LogicalJoinDefinitionPtr joinDef,
    ...
    */
    ...
    if (buildSide == QueryCompilation::JoinBuildSideType::Left) {
        ...
        x_ASSERT(recordHandler->hasAttribute(joinKeyFieldName), "join key is not defined on input tuple");
...
bool CCodeGenerator::generateCodeForBatchJoinBuild(Join::Experimental::LogicalBatchJoinDefinitionPtr batchJoinDef,
                                                   PipelineContextPtr context,
                                                   Join::Experimental::BatchJoinOperatorHandlerPtr batchJoinOperatorHandler) {
    x_DEBUG("batch join input={} out={}", context->inputSchema->toString(), batchJoinDef->getOutputSchema()->toString());
...
bool CCodeGenerator::generateCodeForCombiningWindow(
    ...
    uint64_t windowOperatorIndex) {
    auto tf = getTypeFactory();
    x_DEBUG("CCodeGenerator: Generate code for combine window.");
    auto code = context->code;
   ...
   */
    ...
    //        x::StateVariable<int64_t, x::WindowSliceStore<int64_t>*>* state_variable = (x::StateVariable<int64_t, x::WindowSliceStore<int64_t>*>*) state_var;
    auto stateVariableDeclaration =
        VariableDeclaration::create(tf->createPointer(tf->createAnonymusDataType(
                                        "x::Runtime::StateVariable<int64_t, x::Windowing::WindowSliceStore<int64_t>*>")),
                                    "state_variable");
...
void CCodeGenerator::generateCodeForAggregationInitialization(const BlockScopeStatementPtr& setupScope,
    ...
    if (aggregation->getType() == Windowing::WindowAggregationDescriptor::Type::Avg) {
        ...
        if (aggregation->getType() == Windowing::WindowAggregationDescriptor::Type::Sum) {
            ...
            x_FATAL_ERROR("Aggregation Handler: aggregation={} not implemented",
...
std::string CCodeGenerator::generateCode(PipelineContextPtr context) {
    ...
    switch (context->arity) {
        case PipelineContext::PipelineContextArity::Unary: {
            arityStatement = std::make_shared<ConstantExpressionStatement>(tf->createValueType(
                DataTypeFactory::createBasicValue(DataTypeFactory::createUInt8(), "PipelixtageArity::Unary")));
            break;
        }
        case PipelineContext::PipelineContextArity::BinaryLeft: {
            arityStatement = std::make_shared<ConstantExpressionStatement>(tf->createValueType(
                DataTypeFactory::createBasicValue(DataTypeFactory::createUInt8(), "PipelixtageArity::BinaryLeft")));
            break;
        }
        case PipelineContext::PipelineContextArity::BinaryRight: {
            arityStatement = std::make_shared<ConstantExpressionStatement>(tf->createValueType(
                DataTypeFactory::createBasicValue(DataTypeFactory::createUInt8(), "PipelixtageArity::BinaryRight")));
            break;
        }
    }

    auto ctorFunction = ConstructorDefinition::create("ExecutablePipelixtage" + context->pipelineName, true)
                            ->addInitializer("Runtime::Execution::ExecutablePipelixtage", arityStatement);

    auto executablePipeline = ClassDefinition::create("ExecutablePipelixtage" + context->pipelineName);
    executablePipeline->addBaseClass("Runtime::Execution::ExecutablePipelixtage");
    ...
    auto pipelineNamespace = NamespaceDefinition::create("x");
    ...
    createFunction->returns(SharedPointerGen::createSharedPtrType(
        x::QueryCompilation::GeneratableTypesFactory::createAnonymusDataType("Runtime::Execution::ExecutablePipelixtage")));
...
CCodeGenerator::compile(Compiler::JITCompilerPtr jitCompiler,
    ...
    auto futureCompiledExecutablePipelixtage = std::async(std::launch::async, [result, code, src]() {
        auto compiledCode = result.get().getDynamicObject();
        PipelixtageArity const arity = [&ari = code->arity]() {
            switch (ari) {
                case PipelineContext::PipelineContextArity::Unary: return PipelixtageArity::Unary;
                case PipelineContext::PipelineContextArity::BinaryLeft: return PipelixtageArity::BinaryLeft;
                case PipelineContext::PipelineContextArity::BinaryRight: return PipelixtageArity::BinaryRight;
                default: x_FATAL_ERROR("Unknown PipelineContext. Terminate.");
            }
        }();
        return CompiledExecutablePipelixtage::create(compiledCode, arity, src);
    });
    // defer compilation till the first invocation of the pipeline
    return LazyCompiledExecutablePipelixtage::create(futureCompiledExecutablePipelixtage.share());
}
...
CCodeGenerator::getAggregationWindowHandler(const VariableDeclaration& pipelineContextVariable,
    ...
    // generated code : getWindowHandler<x::Windowing::AggregationWindowHandler, K, Partial<T>, F>
    auto call =
        FunctionCallStatement(std::string("getWindowHandler<x::Windowing::AggregationWindowHandler, ") + TO_CODE(keyType) + ", "
...
BinaryOperatorStatement CCodeGenerator::getJoinWindowHandler(const VariableDeclaration& pipelineContextVariable,
    ...
    auto tf = getTypeFactory();
    auto call = FunctionCallStatement(std::string("getJoinHandler<x::Join::JoinHandler, ") + TO_CODE(keyType) + "," + leftType
                                      + "," + rightType + " >");
    return VarRef(pipelineContextVariable).accessPtr(call);
}
...
BinaryOperatorStatement CCodeGenerator::getBatchJoinHandler(const VariableDeclaration& pipelineContextVariable,
    ...
    auto tf = getTypeFactory();
    auto call = FunctionCallStatement(std::string("getBatchJoinHandler<x::Join::Experimental::BatchJoinHandler, ")
...
VariableDeclaration CCodeGenerator::getWindowOperatorHandler(const PipelineContextPtr& context,
                                                             const VariableDeclaration& tupleBufferVariable,
                                                             uint64_t operatorIndex) {
    return getOperatorHandler(context, tupleBufferVariable, operatorIndex, x::Runtime::Execution::OperatorHandlerType::WINDOW);
}

VariableDeclaration CCodeGenerator::getCEPIterationOperatorHandler(const PipelineContextPtr& context,
                                                                   const VariableDeclaration& tupleBufferVariable,
                                                                   uint64_t operatorIndex) {
    return getOperatorHandler(context, tupleBufferVariable, operatorIndex, x::Runtime::Execution::OperatorHandlerType::CEP);
}

VariableDeclaration CCodeGenerator::getJoinOperatorHandler(const PipelineContextPtr& context,
                                                           const VariableDeclaration& tupleBufferVariable,
                                                           uint64_t operatorIndex) {
    return getOperatorHandler(context, tupleBufferVariable, operatorIndex, x::Runtime::Execution::OperatorHandlerType::JOIN);
}
...
VariableDeclaration CCodeGenerator::getBatchJoinOperatorHandler(const PipelineContextPtr& context,
    ...
    return getOperatorHandler(context,
                              tupleBufferVariable,
                              operatorIndex,
                              x::Runtime::Execution::OperatorHandlerType::BATCH_JOIN);
}
