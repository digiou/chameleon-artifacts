...
GeneratableDataTypePtr StructDeclaration::getType() const {
    return x::QueryCompilation::GeneratableTypesFactory::createUserDefinedType(*this);
}
...
uint32_t StructDeclaration::getTypeSizeInBytes() {
    x_ERROR("Called unimplemented function!");
    return 0;
}
...
VariableDeclaration StructDeclaration::getVariableDeclaration(const std::string& field_name) const {
    ...
    if (!decl) {
        x_ERROR("Error during Code Generation: Field '{}' does not exist in struct '{}'", field_name, getTypeName());
        x_THROW_RUNTIME_ERROR("Error during Code Generation");
    }
