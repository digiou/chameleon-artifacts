...
ExpressionStatementPtr PredicateItem::generateCode(GeneratedCodePtr&, RecordHandlerPtr recordHandler) const {
    if (attribute) {
        ...
        x_FATAL_ERROR("UserAPIExpression: Could not Retrieve Attribute from record handler!");

    } else if (value) {
        // todo remove if compiler refactored
        return ConstantExpressionStatement(x::QueryCompilation::GeneratableTypesFactory::createValueType(value)).copy();
    }
    return nullptr;
}
...
ExpressionStatementPtr WhenPredicate::generateCode(GeneratedCodePtr& code, RecordHandlerPtr recordHandler) const {
    x_INFO("LegacyWhenPredicate: This predicate generates code, but it should never generate code on its own. The code will "
...
ExpressionStatementPtr CasePredicate::generateCode(GeneratedCodePtr& code, RecordHandlerPtr recordHandler) const {
    try {
        ...
        x_DEBUG("CasePredicate: Generating code for expression. {} It looks like: {}",
                  this->toString(),
                  curBaseExpr->getCode()->code_);
        return curBaseExpr;
    } catch (const std::bad_cast& e) {
        x_FATAL_ERROR("CasePredicate: Could not cast when expressions to generate code for them.");
    }
...
bool CasePredicate::equals(const LegacyExpression& _rhs) const {
    try {
        ...
        if (whenExprs.size() == rhs.getWhenExprs().size() && defautlExpr->equals(*rhs.getDefaultExpr().get())) {
            x_DEBUG("CasePredicate: Equals in this legacy predicate only checks for same number of when cases and whether or "
...
std::string PredicateItem::toString() const {
    switch (mutation) {
        ...
        case PredicateItemMutation::VALUE: {
            return x::QueryCompilation::GeneratableTypesFactory::createValueType(value)->getCodeExpression()->code_;
        }
    }
    return "";
}
...
PredicatePtr createPredicate(const LegacyExpression& expression) {
    ...
    if (!value) {
        x_ERROR("UserAPIExpression is not a predicate");
    }
    return value;
}
