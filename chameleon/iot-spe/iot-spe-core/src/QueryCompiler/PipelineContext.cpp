...
bool PipelineContext::hasNextPipeline() const { return !this->nextPipelix.empty(); }

RecordHandlerPtr PipelineContext::getRecordHandler() { return this->recordHandler; }

const std::vector<PipelineContextPtr>& PipelineContext::getNextPipelineContexts() const { return this->nextPipelix; }

void PipelineContext::addNextPipeline(const PipelineContextPtr& nextPipeline) { this->nextPipelix.push_back(nextPipeline); }
...
uint64_t PipelineContext::getHandlerIndex(const Runtime::Execution::OperatorHandlerPtr& operatorHandler) {
    ...
    x_FATAL_ERROR("Handler is not registered");
    return 0;
}
