...
x::Spatial::DataTypes::Experimental::GeoLocation
GeoLocationFactory::createFromString(std::string, std::map<std::string, std::string>& commandLineParams) {
    ...
    return x::Spatial::DataTypes::Experimental::GeoLocation::fromString(std::move(coordStr));
}

x::Spatial::DataTypes::Experimental::GeoLocation GeoLocationFactory::createFromYaml(Yaml::Node& yamlConfig) {
    auto configString = yamlConfig.As<std::string>();
    if (!configString.empty() && configString != "\n") {
        return x::Spatial::DataTypes::Experimental::GeoLocation::fromString(configString);
    }
    return {};
}
}//namespace x::Configurations::Spatial::Index::Experimental
