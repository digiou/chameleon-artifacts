...
LogicalSourcePtr LogicalSourceFactory::createFromString(std::string, std::map<std::string, std::string>& commandLineParams) {
    std::string logicalSourceName, fieldNodeName, fieldNodeType, fieldNodexType, fieldNodeLength;
    ...
    if (logicalSourceName.empty()) {
        x_WARNING("No logical source name is supplied for creating the logical source. Please supply "
    ...
    if (yamlConfig[LOGICAL_SOURCE_SCHEMA_FIELDS_CONFIG].IsSequence()) {
        ...
        for (uint64_t index = 0; index < sequenceSize; ++index) {
            ...
            if (fieldNodeName.empty() || fieldNodeName == "\n") {
                x_THROW_RUNTIME_ERROR("Found Invalid Logical Source Configuration. Please define Schema Field Name.");
            }
            ...
            if (fieldNodeType.empty() || fieldNodeType == "\n") {
                x_THROW_RUNTIME_ERROR("Found Invalid Logical Source Configuration. Please define Schema Field Type.");
            }
        ...
        x_THROW_RUNTIME_ERROR("Found Invalid Logical Source Configuration. Please define Logical Source Schema Fields.");
    }
...
DataTypePtr LogicalSourceFactory::stringToFieldType(std::string fieldNodeType, std::string fieldNodeLength) {
    if (fieldNodeType == "CHAR") {
        if (fieldNodeLength.empty() || fieldNodeLength == "\n" || fieldNodeLength == "0") {
            x_THROW_RUNTIME_ERROR("Found Invalid Logical Source Configuration. Please define Schema Field Length properly.");
        }
    ...
    x_THROW_RUNTIME_ERROR("Found Invalid Logical Source Configuration. " << fieldNodeType
