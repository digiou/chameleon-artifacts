...
DataSinkPtr ConvertLogicalToPhysicalSink::createDataSink(OperatorId operatorId,
    ...
    x_DEBUG("Convert sink  {}", operatorId);
    x_ASSERT(nodeEngine, "Invalid node engine");
    x_ASSERT(querySubPlan, "Invalid query sub-plan");
    if (sinkDescriptor->instanceOf<PrintSinkDescriptor>()) {
        x_DEBUG("ConvertLogicalToPhysicalSink: Creating print sink {}", schema->toString());
    ...
    if (sinkDescriptor->instanceOf<NullOutputSinkDescriptor>()) {
        ...
        x_DEBUG("ConvertLogicalToPhysicalSink: Creating nulloutput sink {}", schema->toString());
        ...
        x_INFO("ConvertLogicalToPhysicalSink: Creating ZMQ sink");
        ...
        x_INFO("ConvertLogicalToPhysicalSink: Creating Monitoring sink");
        ...
        x_INFO("ConvertLogicalToPhysicalSink: Creating Kafka sink");
        ...
        if (kafkaSinkDescriptor->getSinkFormatAsString() == "CSV_FORMAT") {
            ...
            x_THROW_RUNTIME_ERROR("Sinkformat " << kafkaSinkDescriptor->getSinkFormatAsString()
        ...
        x_INFO("ConvertLogicalToPhysicalSink: Creating OPC sink");
        ...
        x_INFO("ConvertLogicalToPhysicalSink: Creating MQTT sink");
        ...
        x_INFO("ConvertLogicalToPhysicalSink: Creating file sink for format={}", fileSinkDescriptor->getSinkFormatAsString());
        if (fileSinkDescriptor->getSinkFormatAsString() == "CSV_FORMAT") {
        ...
        } else if (fileSinkDescriptor->getSinkFormatAsString() == "x_FORMAT") {
            return createBinaryxFileSink(schema,
                                           querySubPlan->getQueryId(),
                                           querySubPlan->getQuerySubPlanId(),
                                           nodeEngine,
                                           numOfProducers,
            ...
            x_ERROR("createDataSink: unsupported format");
            throw std::invalid_argument("Unknown File format");
        }
    } else if (sinkDescriptor->instanceOf<Network::NetworkSinkDescriptor>()) {
        x_INFO("ConvertLogicalToPhysicalSink: Creating network sink");
        ...
        return createNetworkSink(schema,
                                 ...
                                 networkSinkDescriptor->getxPartition(),
                                 nodeEngine,
                                 numOfProducers,
    ...
    } else if (sinkDescriptor->instanceOf<x::Experimental::MaterializedView::MaterializedViewSinkDescriptor>()) {
        x_INFO("ConvertLogicalToPhysicalSink: Creating materialized view sink");
        auto materializedViewSinkDescriptor =
            sinkDescriptor->as<x::Experimental::MaterializedView::MaterializedViewSinkDescriptor>();
        return x::Experimental::MaterializedView::createMaterializedViewSink(
            schema,
            nodeEngine,
            numOfProducers,
            querySubPlan->getQueryId(),
        ...
        x_ERROR("ConvertLogicalToPhysicalSink: Unknown Sink Descriptor Type");
