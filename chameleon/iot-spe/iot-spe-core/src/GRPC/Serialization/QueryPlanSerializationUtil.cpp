...
void QueryPlanSerializationUtil::serializeQueryPlan(const QueryPlanPtr& queryPlan,
    ...
    x_DEBUG("QueryPlanSerializationUtil: serializing query plan {}", queryPlan->toString());
    std::vector<OperatorNodePtr> rootOperators = queryPlan->getRootOperators();
    x_DEBUG("QueryPlanSerializationUtil: serializing the operator chain for each root operator independently");
    ...
    for (auto itr = bfsIterator.begin(); itr != QueryPlanIterator::end(); ++itr) {
        ...
        x_TRACE("QueryPlan: Inserting operator in collection of already visited node.");
    ...
    if (!isClientOriginated) {
        ...
        x_TRACE("QueryPlanSerializationUtil: serializing the Query sub plan id and query id");
...
QueryPlanPtr QueryPlanSerializationUtil::deserializeQueryPlan(SerializableQueryPlan* serializedQueryPlan) {
    x_TRACE("QueryPlanSerializationUtil: Deserializing query plan {}", serializedQueryPlan->DebugString());
