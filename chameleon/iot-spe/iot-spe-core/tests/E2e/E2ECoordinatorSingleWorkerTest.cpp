...
class E2ECoordinatorSingleWorkerTest : public Testing::BaseIntegrationTest {
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("E2ECoordinatorSingleWorkerTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup E2e test class.");
    }
};
...
TEST_F(E2ECoordinatorSingleWorkerTest, testExecutingValidUserQueryWithFileOutput) {
    ...
    x_INFO("content={}", content);
    x_INFO("expContent={}", expectedContent);
...
TEST_F(E2ECoordinatorSingleWorkerTest, testExecutingValidUserQueryWithFileOutputWithFilter) {
    ...
    x_INFO("query string submit={}", ss.str());
    nlohmann::json json_return = TestUtils::startQueryViaRest(ss.str(), std::to_string(*restPort));
    ...
    x_INFO("read file={}", outputFilePath);
...
TEST_F(E2ECoordinatorSingleWorkerTest, testExecutingValidUserQueryWithFileOutputcompanyUseCase) {
    x_INFO("start coordinator");
    std::string testFile = "company-results.csv";
    ...
    schema << "{\"logicalSourceName\" : \"company\",\"schema\" "
    ...
    schema << endl;
    x_INFO("schema submit={}", schema.str());
    ...
    auto worker = TestUtils::startWorker({TestUtils::rpcPort(0),
                                          ...
                                          TestUtils::csvSourceFilePath(std::string(TEST_DATA_DIRECTORY) + "company.csv"),
                                          TestUtils::physicalSourceName("test_stream"),
                                          TestUtils::logicalSourceName("company"),
...
TEST_F(E2ECoordinatorSingleWorkerTest, testKillWorkerWithoutQuery) {
    ...
    EXPECT_TRUE(TestUtils::waitForWorkers(*restPort, timeout, 0));
    x_DEBUG("start crd with pid={}", coordinator.getPid());
    ...
    ASSERT_TRUE(TestUtils::waitForWorkers(*restPort, timeout, 1));
    x_DEBUG("start worker with pid={}", worker.getPid());
    sleep(5);
    worker.kill();
...
TEST_F(E2ECoordinatorSingleWorkerTest, DISABLED_testKillCoordinatorWithoutQuery) {
    ...
    inFile.open("xWorkerStarter.log");
...
TEST_F(E2ECoordinatorSingleWorkerTest, testExecutingValidUserQueryWithThresholdWindowFileOutputcompanyUseCase) {
    x_INFO("start coordinator");
    std::string testFile = getTestResourceFolder() / "company-results.csv";
    x_INFO("testFile = {}", testFile);
    remove(testFile.c_str());
    ...
    std::stringstream schema;
    schema
        << "{\"logicalSourceName\" : \"company\",\"schema\" "
    ...
    auto worker = TestUtils::startWorker(
        {TestUtils::rpcPort(0),
         ...
         TestUtils::csvSourceFilePath(std::string(TEST_DATA_DIRECTORY)
                                      + "company_thresholdtest.csv"),//I created a new file to open and close a threshold window
         TestUtils::physicalSourceName("test_stream"),
         TestUtils::logicalSourceName("company"),
         TestUtils::numberOfBuffersToProduce(1),
    ...
    string expectedContent =
        "company$ABS_Lean_Angle:Float(32 bits),company$ABS_Front_Wheel_Speed:Float(64 bits),company$count:INTEGER(64 bits)\n"
...
TEST_F(E2ECoordinatorSingleWorkerTest, DISABLED_testExecutingThresholdWindowcompanyByKey) {
    x_INFO("start coordinator");
    std::string testFile = getTestResourceFolder() / "company-results.csv";
    x_INFO("testFile = {}", testFile);
    remove(testFile.c_str());
    ...
    string expectedContent = "company$count:INTEGER(64 bits),company$ABS_Lean_Angle:Float(64 bits),company$ABS_Pitch_Info:Float(64 "
                             "bits),company$ABS_Front_Wheel_Speed:Float(64 bits)\n"
