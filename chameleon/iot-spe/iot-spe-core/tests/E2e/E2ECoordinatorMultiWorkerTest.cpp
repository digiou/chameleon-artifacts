...
class E2ECoordinatorMultiWorkerTest : public Testing::BaseIntegrationTest {
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("E2ECoordinatorWorkerTest.log", x::LogLevel::LOG_DEBUG);
...
/**
 * @brief Testing x with a config using a hierarchical topology.
 */
...
TEST_F(E2ECoordinatorMultiWorkerTest, testExecutingValidQueryWithFileOutputTwoWorkerDifferentSource) {
    ...
    while (std::getline(ifs, line)) {
        x_INFO("print line from content{}", line);
        std::vector<string> content = Util::splitWithStringDelimiter<std::string>(line, ",");
        if (content.at(0) == "R2000073") {
            x_INFO("First content={}", content.at(2));
            x_INFO("First: expContent= 102.629631");
            if (content.at(2) == "102.629631") {
                resultWrk1 = true;
            }
        } else {
            x_INFO("Second: content={}", content.at(2));
            x_INFO("Second: expContent= 108.166664");
...
TEST_F(E2ECoordinatorMultiWorkerTest, DISABLED_testWindowingWithTwoWorkerWithTwoAggregationFunctions) {
    ...
    x_DEBUG("Starting the coordinator.");
    ...
    x_DEBUG("Configure a schema that consists of a timestamp, a grouping key and a value.");
    auto schema = "{\n"
                  ...
                  "BasicType::UINT64));\"\n"
                  "}";
    x_DEBUG("Schema: {}", schema);
    ASSERT_TRUE(TestUtils::addLogicalSource(schema, std::to_string(*restPort)));

    x_DEBUG("Create an input CSV file for the worker.");
    ...
    x_DEBUG("Execute an aggregation query and write output to a file.");
    ...
    x_DEBUG("Read in output file: {}", outputPathAsString.str());
