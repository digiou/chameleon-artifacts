...
#ifndef x_TESTS_UTIL_TEST_SINK_HPP_
#define x_TESTS_UTIL_TEST_SINK_HPP_
...
class CollectTestSink : public SinkMedium {
  public:
    ...
    CollectTestSink(const SchemaPtr& schema, const Runtime::NodeEnginePtr& nodeEngine, uint32_t numOfProducers = 1)
        : SinkMedium(std::make_shared<xFormat>(schema, nodeEngine->getBufferManager(0)), nodeEngine, numOfProducers, 0, 0) {
        auto bufferManager = nodeEngine->getBufferManager(0);
        x_ASSERT(schema->getLayoutType() == Schema::MemoryLayoutType::ROW_LAYOUT, "Currently only support for row layouts");
    };
    ...
    bool writeData(Runtime::TupleBuffer& inputBuffer, Runtime::WorkerContext&) override {
        ...
        x_TRACE("TestSink: emit buffer with {} tuples", inputBuffer.getNumberOfTuples());
        ...
        x_DEBUG("CollectTestSink saw now {} tuples!", results.size())
        cv.notify_all();
        return true;
    }
    ...
    void waitTillCompletedOrTimeout(size_t numberOfRecords, uint64_t timeoutInMilliseconds) {
        ...
        auto waitForExpectedNumberOfRecords = [&] {
            ...
            if (this->results.size() < numberOfRecords) {
                x_DEBUG("Already saw {} records and expects a total of {}.", this->results.size(), numberOfRecords);
            } else if (this->results.size() == numberOfRecords) {
                x_DEBUG("Saw exactly as many records ({}) as expected ({}).", this->results.size(), numberOfRecords);
                isFinished = true;
            } else if (this->results.size() > numberOfRecords) {
                x_ERROR("Number of result tuples {} and expected number of tuples {} do not match.",
                          this->results.size(),
                          numberOfRecords);
                EXPECT_TRUE(false);
                isFinished = true;
...
#endif// x_TESTS_UTIL_TEST_SINK_HPP_
