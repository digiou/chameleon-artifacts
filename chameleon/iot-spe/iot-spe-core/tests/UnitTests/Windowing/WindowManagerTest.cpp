...
class WindowManagerTest : public Testing::BaseIntegrationTest {
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("WindowManagerTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup WindowMangerTest test class.");
    }
...
class MockedExecutablePipelixtage : public Runtime::Execution::ExecutablePipelixtage {
  public:
    static Runtime::Execution::ExecutablePipelixtagePtr create() { return std::make_shared<MockedExecutablePipelixtage>(); }
...
TEST_F(WindowManagerTest, testWindowTriggerCompleteWindowWithAvg) {
    ...
    aggregates[sliceIndex].addToCount();
    x_DEBUG("{}", aggregates[sliceIndex].getSum());
    x_DEBUG("{}", aggregates[sliceIndex].getCount());
...
TEST_F(WindowManagerTest, testWindowTriggerCompleteWindowWithCharArrayKey) {
    ...
    auto windowHandler = createWindowHandler<x::ExecutableTypes::Array<char, 32>,
                                             uint64_t,
                                             uint64_t,
                                             uint64_t,
    ...
    x::ExecutableTypes::Array<char, 32> keyOne = {'K', 'e', 'y', ' ', 'O', 'n', 'e'};
    ...
    auto windowAction = std::dynamic_pointer_cast<
        Windowing::
            ExecutableCompleteAggregationTriggerAction<x::ExecutableTypes::Array<char, 32>, uint64_t, uint64_t, uint64_t>>(
        windowHandler->getWindowAction());
