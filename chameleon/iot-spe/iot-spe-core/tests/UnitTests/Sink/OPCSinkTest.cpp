...
class OPCSinkTest : public Testing::BaseUnitTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("OPCSinkTest.log", x::LogLevel::LOG_DEBUG);
        x_DEBUG("OPCSINKTEST::SetUpTestCase()");
    }

    void SetUp() {
        x_DEBUG("OPCSINKTEST::SetUp() OPCSinkTest cases set up.");
    ...
    void TearDown() {
        nodeEngine->stop();
        nodeEngine.reset();
        x_DEBUG("OPCSINKTEST::TearDown() Tear down OPCSourceTest");
    }

    /* Will be called after all tests in this class are finished. */
    static void TearDownTestCase() { x_DEBUG("OPCSINKTEST::TearDownTestCases() Tear down OPCSourceTest test class."); }
...
TEST_F(OPCSinkTest, OPCSourceValue) {
    ...
    Runtime::WorkerContext wctx(Runtime::xThread::getId());
    ...
    x_DEBUG("OPCSINKTEST::TEST_F(OPCSinkTest, OPCSinkValue) buffer before write: {}", dynamicTupleBuffer);

    opcSink->writeData(write_buffer, wctx);
    x_DEBUG("OPCSINKTEST::TEST_F(OPCSinkTest, OPCSinkValue) data was written");
    ...
    x_DEBUG("OPCSINKTEST::TEST_F(OPCSinkTest, OPCSinkValue) Received value is: {}", *(uint32_t*) tuple_buffer->getBuffer());
    value = *tuple;
    uint64_t expected = 45;
    x_DEBUG("OPCSINKTEST::TEST_F(OPCSinkTest, OPCSinkValue) expected value is: {}. Received value is: {}", expected, value);
