...
class KafkaSinkTest : public Testing::BaseIntegrationTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("KafkaSinkTest.log", x::LogLevel::LOG_DEBUG);
        x_DEBUG("KafkaSinkTest::SetUpTestCase()");
    }

    void SetUp() override {
        Testing::BaseIntegrationTest::SetUp();
        dataPort = Testing::BaseIntegrationTest::getAvailablePort();
        x_DEBUG("Setup KafkaSinkTest test case.");
    ...
    void TearDown() override {
        ...
        x_DEBUG("KafkaSinkTest::TearDown() Tear down KafkaSinkTest");
        Testing::BaseIntegrationTest::TearDown();
    }

    /* Will be called after all tests in this class are finished. */
    static void TearDownTestCase() { x_DEBUG("KafkaSinkTest::TearDownTestCases() Tear down KafkaSinkTest test class."); }

    static x::Runtime::TupleBuffer createSimpleBuffer(uint64_t bufferSize,
...
TEST_F(KafkaSinkTest, KafkaSourcePrint) {
    ...
    x_DEBUG("kafka string={}", kafkaSink->toString());
}
