...
class SinkTest : public Testing::BaseIntegrationTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("SinkTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup SinkTest class.");
    }
...
TEST_F(SinkTest, testCSVFileSink) {
    ...
    Runtime::WorkerContext wctx(Runtime::xThread::getId(), nodeEngine->getBufferManager(), 64);
    ...
    x_TRACE("Buffer Content= {}", bufferContentBeforeWrite);
    ...
    x_TRACE("Buffer Content= {}", bufferContentAfterWrite);
    ...
    x::Util::findAndReplaceAll(contentWOHeader, "\n", ",");
...
TEST_F(SinkTest, testMonitoringSink) {
    ...
    x_INFO("MetricStoreTest: Stored metrics{}", Monitoring::MetricUtils::toJson(storedMetrics));
    ...
    x_INFO("MetricStoreTest: Stored metrics{}", Monitoring::MetricUtils::toJson(storedMetricsCpu));
