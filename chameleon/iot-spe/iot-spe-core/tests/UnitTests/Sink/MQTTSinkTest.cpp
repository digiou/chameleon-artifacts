...
class MQTTTSinkTest : public Testing::BaseIntegrationTest {
  public:
    ...
    const std::string CLIENT_ID = "x-mqtt-test-client";
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("MQTTTSinkTest.log", x::LogLevel::LOG_DEBUG);
        x_DEBUG("Setup MQTTTSinkTest test class.");
    }
    ...
    void SetUp() override {
        ...
        x_DEBUG("Setup MQTTTSinkTest test case.");
    ...
    void TearDown() override {
        ...
        x_DEBUG("Setup MQTT test case.");
        Testing::BaseIntegrationTest::TearDown();
    }

    static x::Runtime::TupleBuffer createTupleBuffer(uint64_t bufferSize,
    ...
    bool createMQTTSinkConnectToBrokerWriteData(uint64_t numTuples,
                                                ...
                                                bool printBuffer) {
        // Create MQTT Sink
        Runtime::WorkerContext workerContext(Runtime::xThread::getId(), nodeEngine->getBufferManager(), 64);
        ...
        if (printBuffer) {
            ...
            x_DEBUG("bufferContent before write={}", dynamicTupleBufferAsString.str());
        }
        bool connectSuccessful = mqttSink->connect();
        if (connectSuccessful) {
            return mqttSink->writeData(inputBuffer, workerContext);
        }
        x_ERROR("MQTTSinkTest:createMQTTSinkConnectToBrokerWriteData: Could not connect to MQTT broker");
        return false;
    }

    /* Will be called after all tests in this class are finished. */
    static void TearDownTestCase() { x_DEBUG("Tear down MQTT test class."); }
...
TEST_F(MQTTTSinkTest, DISABLED_testMQTTbrokerDeathToClientStopAsynchronous) {
    bool bufferDataSuccessfullyWrittenToBroker =
        createMQTTSinkConnectToBrokerWriteData(3600,
                                               ...
                                               true,
                                               false);
    x_INFO("testMQTTbrokerDeathToClientStopAsynchronous result: {}", bufferDataSuccessfullyWrittenToBroker);
}
