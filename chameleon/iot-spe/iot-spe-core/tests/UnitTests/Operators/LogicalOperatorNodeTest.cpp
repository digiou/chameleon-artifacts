...
class LogicalOperatorNodeTest : public Testing::BaseUnitTest {
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("LogicalOperatorNodeTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup LogicalOperatorNodeTest test class.");
    }
 ...
 * replaced: sourceOp -> filterOp1 -> filterOp2
 */
TEST_F(LogicalOperatorNodeTest, replaceWithNoxuccessor) {
    children = filterOp3->getChildren();
    EXPECT_EQ(children.size(), 0U);
