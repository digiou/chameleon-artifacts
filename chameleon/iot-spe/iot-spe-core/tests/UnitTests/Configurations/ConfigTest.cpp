...
class ConfigTest : public Testing::BaseIntegrationTest {
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("Config.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup Configuration test class.");
    }
    ...
    static void TearDownTestCase() { x_INFO("Tear down Configuration test class."); }
};
...
TEST_F(ConfigTest, testWorkerYAMLFileFixedLocationNode) {
    ...
    EXPECT_EQ(workerConfigPtr->locationCoordinates.getValue(), x::Spatial::DataTypes::Experimental::GeoLocation(23.88, -3.4));
    EXPECT_EQ(workerConfigPtr->nodeSpatialType.getValue(), x::Spatial::Experimental::SpatialType::FIXED_LOCATION);
}
