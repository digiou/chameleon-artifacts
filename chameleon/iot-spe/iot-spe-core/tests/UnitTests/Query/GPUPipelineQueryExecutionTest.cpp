...
class GPUQueryExecutionTest : public Testing::BaseUnitTest {
  public:
    static void SetUpTestCase() { x::Logger::setupLogging("GPUQueryExecutionTest.log", x::LogLevel::LOG_DEBUG); }
    ...
    void TearDown() override {
        x_DEBUG("QueryExecutionTest: Tear down GPUQueryExecutionTest test case.");
...
void cleanUpPlan(Runtime::Execution::ExecutableQueryPlanPtr plan) {
    ...
    std::for_each(plan->getPipelix().begin(), plan->getPipelix().end(), [plan](auto pipeline) {
...
class SimpleGPUPipelixtage : public Runtime::Execution::ExecutablePipelixtage {
...
class MultifieldGPUPipelixtage : public Runtime::Execution::ExecutablePipelixtage {
    class InputRecord {
      public:
  ...
  public:
    uint32_t setup(Runtime::Execution::PipelineExecutionContext& pipelineExecutionContext) override {
        ...
        const char* const MultifieldKernel_cu = "MultifieldKernel_cu.cu\n"
                                                "#include \"x-core/tests/UnitTests/Query/GPUInputRecord.cuh\"\n"
        ...
        const char* const header = "x-core/tests/UnitTests/Query/GPUInputRecord.cuh\n"
                                   "#ifndef x_GPUINPUTRECORD_CUH\n"
                                   "#define x_GPUINPUTRECORD_CUH\n"
                                   "\n"
                                   "#include <cstdint>"
                                   "\n"
                                   "class InputRecord {\n"
...
class ColumnLayoutGPUPipelixtage : public Runtime::Execution::ExecutablePipelixtage {
    class InputRecord {
      public:
        [[maybe_unused]] int64_t id;
        [[maybe_unused]] int64_t one;
  ...
  public:
    ...
    ExecutionResult execute(Runtime::TupleBuffer& buffer,
        ...
        auto columnLayout = x::Runtime::MemoryLayouts::ColumnLayout::create(testSchemaColumnLayout, buffer.getBufferSize());
...
TEST_F(GPUQueryExecutionTest, GPUOperatorSimpleQuery) {
    ...
    auto customPipelixtage = std::make_shared<SimpleGPUPipelixtage>();
    auto externalOperator =
        x::QueryCompilation::PhysicalOperators::PhysicalExternalOperator::create(SchemaPtr(), SchemaPtr(), customPipelixtage);
    ...
    if (auto buffer = nodeEngine->getBufferManager()->getBufferBlocking(); !!buffer) {
        ...
        ASSERT_EQ(plan->getPipelix()[1]->execute(buffer, workerContext), ExecutionResult::Ok);
...
TEST_F(GPUQueryExecutionTest, GPUOperatorWithMultipleFields) {
    ...
    auto customPipelixtage = std::make_shared<MultifieldGPUPipelixtage>();
    auto externalOperator =
        x::QueryCompilation::PhysicalOperators::PhysicalExternalOperator::create(SchemaPtr(), SchemaPtr(), customPipelixtage);
...
TEST_F(GPUQueryExecutionTest, GPUOperatorOnColumnLayout) {
    ...
    auto customPipelixtage = std::make_shared<ColumnLayoutGPUPipelixtage>();
    auto externalOperator =
        x::QueryCompilation::PhysicalOperators::PhysicalExternalOperator::create(SchemaPtr(), SchemaPtr(), customPipelixtage);
    ...
    if (auto buffer = nodeEngine->getBufferManager()->getBufferBlocking(); !!buffer) {
        ...
        for (uint32_t recordIndex = 0u; recordIndex < 5u; ++recordIndex) {
            // id
            EXPECT_EQ(resultRecordValueFields[recordIndex], (recordIndex % 2) + 42);
            x_DEBUG("expected: {} actual: {}", (recordIndex % 2) + 42, resultRecordValueFields[recordIndex]);
        }
    } else {
        FAIL();
    }
...
class WindowedAggregationGPUPipelixtage : public Runtime::Execution::ExecutablePipelixtage {
  public:
    ...
    uint32_t setup(Runtime::Execution::PipelineExecutionContext& pipelineExecutionContext) override {
        const char* const header = R"(x-core/tests/UnitTests/Query/GPUInputRecord.cuh
            #ifndef x_GPUINPUTRECORD_CUH
            #define x_GPUINPUTRECORD_CUH
...
TEST_F(GPUQueryExecutionTest, GPUOperatorWindowedAggregation) {
    ...
    auto numOutputTuples = WindowedAggregationGPUPipelixtage::NUMBER_OF_OUTPUT_TUPLES;
    ...
    auto customPipelixtage = std::make_shared<WindowedAggregationGPUPipelixtage>();
    auto externalOperator =
        x::QueryCompilation::PhysicalOperators::PhysicalExternalOperator::create(SchemaPtr(), SchemaPtr(), customPipelixtage);
    ...
    fillBufferToWindowSchema(buffer, memoryLayout, WindowedAggregationGPUPipelixtage::NUMBER_OF_INPUT_TUPLES);
    ...
    ASSERT_EQ(plan->getPipelix()[0]->execute(buffer, workerContext), ExecutionResult::Ok);
