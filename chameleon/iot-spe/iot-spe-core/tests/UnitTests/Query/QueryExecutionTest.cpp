...
class QueryExecutionTest : public Testing::BaseUnitTest {
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("QueryExecutionTest.log", x::LogLevel::LOG_DEBUG);
        x_DEBUG("QueryExecutionTest: Setup QueryCatalogServiceTest test class.");
    }
    ...
    void TearDown() override {
        x_DEBUG("QueryExecutionTest: Tear down QueryExecutionTest test case.");
        ASSERT_TRUE(nodeEngine->stop());
        Testing::BaseUnitTest::TearDown();
    }

    /* Will be called after all tests in this class are finished. */
    static void TearDownTestCase() { x_DEBUG("QueryExecutionTest: Tear down QueryExecutionTest test class."); }
  ...
  public:
    explicit NonRunnableDataSource(const SchemaPtr& schema,
        ...
        : DefaultSource(schema,
                        ...
                        successors) {
        wasGracefullyStopped = x::Runtime::QueryTerminationType::HardStop;
    }
    ...
    bool stop(Runtime::QueryTerminationType termination) override {
        ...
        return x::DefaultSource::stop(termination);
    }
 ...
 * Ts = #Iteration
 */
class WindowSource : public x::DefaultSource {
  public:
    uint64_t runCnt = 0;
    int64_t timestamp;
    bool varyWatermark;
    ...
    std::optional<TupleBuffer> receiveData() override {
        ...
        x_DEBUG("QueryExecutionTest: source buffer={}", bufferAsString.str());
...
void fillBufferWithData(TupleBuffer& buf,
    ...
    for (uint64_t fieldIndex = 0; fieldIndex < fieldCount; ++fieldIndex) {
        ...
        x_ASSERT(expectedType->isEquals(field->getDataType()),
...
TEST_F(QueryExecutionTest, filterQuery) {
    ...
    auto outputBufferOptimizationLevels = {
        x::QueryCompilation::QueryCompilerOptions::OutputBufferOptimizationLevel::ALL,
        x::QueryCompilation::QueryCompilerOptions::OutputBufferOptimizationLevel::NO,
        x::QueryCompilation::QueryCompilerOptions::OutputBufferOptimizationLevel::ONLY_INPLACE_OPERATIONS_NO_FALLBACK,
        x::QueryCompilation::QueryCompilerOptions::OutputBufferOptimizationLevel::
            REUSE_INPUT_BUFFER_AND_OMIT_OVERFLOW_CHECK_NO_FALLBACK,
        x::QueryCompilation::QueryCompilerOptions::OutputBufferOptimizationLevel::REUSE_INPUT_BUFFER_NO_FALLBACK,
        x::QueryCompilation::QueryCompilerOptions::OutputBufferOptimizationLevel::OMIT_OVERFLOW_CHECK_NO_FALLBACK};

    auto filterProcessingStrategies = {x::QueryCompilation::QueryCompilerOptions::FilterProcessingStrategy::BRANCHED,
                                       x::QueryCompilation::QueryCompilerOptions::FilterProcessingStrategy::PREDICATION};
    for (auto outputBufferOptimizationLevel :
        ...
        for (auto filterProcessingStrategy : filterProcessingStrategies) {// try Predication on/off: bool
            ...
            EXPECT_EQ(plan->getPipelix().size(), 1u);
            ...
            if (auto inputBuffer = nodeEngine->getBufferManager()->getBufferBlocking(); !!inputBuffer) {
                ...
                ASSERT_EQ(plan->getPipelix()[0]->execute(inputBuffer, workerContext), ExecutionResult::Ok);
...
TEST_F(QueryExecutionTest, projectionQuery) {
    ...
    if (auto buffer = nodeEngine->getBufferManager()->getBufferBlocking(); !!buffer) {
        ...
        plan->getPipelix()[0]->execute(buffer, workerContext);
...
TEST_F(QueryExecutionTest, streamingJoinQuery) {
    ...
    ASSERT_EQ(plan->getStatus(), Runtime::Execution::ExecutableQueryPlanStatus::Created);
    EXPECT_EQ(plan->getPipelix().size(), 3U);
};

// @Reviewers: This is the Pipeline code generated for the build and probe pipelix, for direct review:
...
//
//    using namespace x;
//    class ExecutablePipelixtage : public x::Runtime::Execution::ExecutablePipelixtage {
//
//        ExecutionResult execute(x::Runtime::TupleBuffer &inputTupleBuffer,
...
//            auto batchJoinHandler = batchJoinOperatorHandler->getBatchJoinHandler<x::Join::Experimental::BatchJoinHandler, int64_t, InputTuple>();
...
//
//            auto batchJoinOperatorHandler = pipelineExecutionContext.getOperatorHandler<Join::Experimental::BatchJoinOperatorHandler>(0);
//            auto batchJoinHandler = batchJoinOperatorHandler->getBatchJoinHandler<x::Join::Experimental::BatchJoinHandler, int64_t, InputTupleBuild>();
//            auto hashTable = batchJoinHandler->getHashTable();
//            x::Runtime::TupleBuffer resultTupleBuffer = workerContext.allocateTupleBuffer();
...
TEST_F(QueryExecutionTest, batchJoinQuery) {
    ...
    // The plan should have two pipeline
    ASSERT_EQ(plan->getStatus(), Runtime::Execution::ExecutableQueryPlanStatus::Created);
    EXPECT_EQ(plan->getPipelix().size(), 2U);
    ...
    plan->getPipelix()[1]->execute(bufferBuildSide, workerContext);
    plan->getPipelix()[0]->execute(bufferProbeSide, workerContext);
    plan->getPipelix()[0]->execute(bufferProbeSide2, workerContext);
...
TEST_F(QueryExecutionTest, arithmeticOperatorsQuery) {
    ...
    auto query = TestQuery::from(testSourceDescriptor)
                     ...
                     .map(Attribute("result_batch_test") =// batch test of many arithmetic operators, should always be 1/TRUE:
                          ...
                          Attribute("result_ceil") == LN(EXP(Attribute("result_ceil")))
    ...
    if (auto buffer = nodeEngine->getBufferManager()->getBufferBlocking(); !!buffer) {
        ...
        std::stringstream bufferAsString;
        bufferAsString << buffer;
        x_DEBUG("QueryExecutionTest: buffer={}", bufferAsString.str());
...
TEST_F(QueryExecutionTest, watermarkAssignerTest) {
    uint64_t millisecondOfallowedLatexs = 2U; /*milliseconds of allowedLatexs*/
    ...
    EXPECT_EQ(resultBuffer.getWatermark(), 14 - millisecondOfallowedLatexs);
...
TEST_F(QueryExecutionTest, SlidingWindowQueryWindowSourcesize10slide5) {
    ...
    // wait till all buffers have been produced
    ASSERT_EQ(testSink->completed.get_future().get(), 1UL);
    x_INFO("QueryExecutionTest: The test sink contains {} result buffers.", testSink->getNumberOfResultBuffers());

    // get result buffer
    if (auto resultBuffer = testSink->get(0); !!resultBuffer) {

        x_INFO("QueryExecutionTest: The result buffer contains {} tuples.", resultBuffer.getNumberOfTuples());
        ...
        x_INFO("QueryExecutionTest: buffer={}", dynamicTupleBufferAsString.str());
        std::string expectedContent = "+----------------------------------------------------+\n"
...
TEST_F(QueryExecutionTest, SlidingWindowQueryWindowSourceSize15Slide5) {
...

    {
        ...
        x_INFO("QueryExecutionTest: The result buffer contains {} tuples.", resultBuffer2.getNumberOfTuples());
        std::stringstream dynamicTupleBufferString;
        dynamicTupleBufferString << dynamicTupleBufferActual2;
        x_INFO("QueryExecutionTest: buffer={}", dynamicTupleBufferString.str());
...
TEST_F(QueryExecutionTest, DISABLED_mergeQuery) {
    ...
    // EXPECT_EQ(plan->getNumberOfPipelix(), 3);
