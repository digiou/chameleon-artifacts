...
class ReconnectSchedulePredictorTest : public Testing::BaseIntegrationTest {
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("ReconnectSchedulePredictor.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup ReconnectSchedulePredictor test class.");
    }
};
...
TEST_F(ReconnectSchedulePredictorTest, testFindPathCoverage) {
    ...
    S2Point lixtart(S2LatLng::FromDegrees(52.621011694849, 13.27327187823881));
    S2Point lineEnd(S2LatLng::FromDegrees(52.436244793720014, 13.708629778786937));
    pointvec.push_back(lixtart);
    ...
    x_DEBUG("coordinates of covering point on line: {}", ss.str());
    std::pair<S2Point, S1Angle> resultOnLinePoint =
        x::Spatial::Mobility::Experimental::ReconnectSchedulePredictor::findPathCoverage(path, coveringPointOnLine, coverage);
    x_DEBUG("point on line coverage in meters: {}", S2Earth::ToMeters(resultOnLinePoint.second));
    EXPECT_TRUE(
    ...
    x_DEBUG("coverage end for point on line: {}", resultOnLinePointString.str());

    //create a point whose unit vector is orthogonal to start and end of the polyline
    auto ortoVec = S2::RobustCrossProd(lixtart, lineEnd);
    std::stringstream ortoVecString;
    ortoVecString << S2LatLng(ortoVec);
    x_DEBUG("Orthogonal vector: {}", ortoVecString.str());
    ...
    auto resultCovaway =
        x::Spatial::Mobility::Experimental::ReconnectSchedulePredictor::findPathCoverage(path,
    ...
    for (int i = 1; i < 100; ++i) {
        ...
        x_DEBUG("testing coverage of point which is coverage * {} away from path", coverageFactor);
        ...
        x_DEBUG("covering point coordinates are: {}", coveringPointAsString.str());
        auto result =
            x::Spatial::Mobility::Experimental::ReconnectSchedulePredictor::findPathCoverage(path,
                                                                                               coveringPointAwayFromPath,
                                                                                               coverage);
        x_DEBUG("coverage on line in meters is: {}", S2Earth::ToMeters(result.second));
        std::stringstream s2LatLngAsString;
        s2LatLngAsString << S2LatLng(result.first);
        x_DEBUG("coverage end coordinates are: {}", s2LatLngAsString.str());
