...
class LocationProviderCSVTest : public Testing::BaseUnitTest {
  ...
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("GeoSourceCSV.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup LocationProviderCSV test class.");
    }
    ...
    static void TearDownTestCase() {
        x_INFO("Tear down LocationProviderCSV test class.");
        auto csvPath = std::string(TEST_DATA_DIRECTORY) + "testLocations.csv";
...
TEST_F(LocationProviderCSVTest, testInvalidCsv) {
    ...
    remove(csvPath.c_str());
    auto locationProvider = std::make_shared<x::Spatial::Mobility::Experimental::LocationProviderCSV>(csvPath);
    ASSERT_THROW(auto waypoint = locationProvider->getCurrentWaypoint(), x::Spatial::Exception::LocationProviderException);
    ...
    locationProvider = std::make_shared<x::Spatial::Mobility::Experimental::LocationProviderCSV>(csvPath);
    ASSERT_THROW(auto waypoint = locationProvider->getCurrentWaypoint(), x::Spatial::Exception::LocationProviderException);
...
TEST_F(LocationProviderCSVTest, testCsvMovementWithSimulatedLocationInFuture) {
    ...
    auto locationProvider =
        std::make_shared<x::Spatial::Mobility::Experimental::LocationProviderCSV>(csvPath, simulatedStartTime);
    ...
    while (currentTimeStamp <= endTime) {
        ...
        if (currentWayPoint.getTimestamp().value() <= initialExpectedWaypoint.getTimestamp().value()) {
            x_DEBUG("comparing initial location");
