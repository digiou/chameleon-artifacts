...
class NetworkStackTest : public Testing::BaseIntegrationTest {
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("NetworkStackTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("SetUpTestCase NetworkStackTest");
    }
...
class DummyExchangeProtocolListener : public ExchangeProtocolListener {
  public:
    ...
    void onEvent(xPartition, Runtime::BaseEvent&) override {}
...
TEST_F(NetworkStackTest, startCloseChannel) {
    try {
        ...
        auto xPartition = xPartition(0, 0, 0, 0);

        struct DataEmitterImpl : public DataEmitter {
            void emitWork(TupleBuffer&) override {}
        };

        std::thread t([&netManager, &completed, &xPartition] {
            // register the incoming channel
            auto cnt = netManager->registerSubpartitionConsumer(xPartition,
                                                                netManager->getServerLocation(),
                                                                std::make_shared<DataEmitterImpl>());
            x_INFO("NetworkStackTest: SubpartitionConsumer registered with cnt {}", cnt);
        ...
        auto senderChannel =
            netManager->registerSubpartitionProducer(nodeLocation, xPartition, buffMgr, std::chrono::seconds(1), 3);
    ...
    try {
        ...
        std::thread t([&netManagerReceiver, &netManagerSender, &completed, &connectionsReady] {
            ...
            for (auto i = 0; i < maxExpectedConnections; ++i) {
                auto xPartition = xPartition(i, 0, 0, 0);
                ASSERT_TRUE(netManagerReceiver->registerSubpartitionConsumer(xPartition,
            ...
            for (auto i = 0; i < maxExpectedConnections; ++i) {
                auto xPartition = xPartition(i, 0, 0, 0);
                (netManagerReceiver->unregisterSubpartitionConsumer(xPartition));
            }
        });
        ...
        for (auto i = 0; i < maxExpectedConnections; ++i) {
            ...
            channels.emplace_back(
                netManagerSender->registerSubpartitionProducer(nodeLocation, xPartition, buffMgr, std::chrono::seconds(1), 3));
            if (!channels.back()) {
                break;
            }
        }
        ...
        for (auto&& senderChannel : channels) {
            auto xPartition = xPartition(i++, 0, 0, 0);
            senderChannel->close(Runtime::QueryTerminationType::Graceful);
            netManagerSender->unregisterSubpartitionProducer(xPartition);
        }
    ...
    try {
        ...
        class ExchangeListener : public ExchangeProtocolListener {
          ...
          public:
            ...
            void onDataBuffer(xPartition id, TupleBuffer& buf) override {
        ...
        std::thread t([&netManager, &xPartition, &completedProm, &completed] {
            // register the incoming channel
            sleep(3);// intended stalling to simulate latency
            netManager->registerSubpartitionConsumer(xPartition,
                                                     netManager->getServerLocation(),
                                                     std::make_shared<DataEmitterImpl>());
            ...
            if (future.wait_for(std::chrono::seconds(5)) == std::future_status::ready) {
                ...
                x_ERROR("NetworkStackTest: Receiving thread timed out!");
            }
            netManager->unregisterSubpartitionConsumer(xPartition);
        });
        ...
        if (senderChannel == nullptr) {
            x_INFO("NetworkStackTest: Error in registering DataChannel!");
    ...
    try {
        class ExchangeListener : public ExchangeProtocolListener {
          ...
          public:
            ...
            void onDataBuffer(xPartition id, TupleBuffer& buffer) override {
        ...
        std::thread t([&netManager, &xPartition, &completedProm, totalNumBuffer, nodeLocation] {
            // register the incoming channel
            ...
            x_DEBUG("Sent {} bytes :: throughput {}", bytes, throughput);
            netManager->unregisterSubpartitionConsumer(xPartition);
        });
        ...
        if (senderChannel == nullptr) {
            x_DEBUG("NetworkStackTest: Error in registering DataChannel!");
            completedProm.set_value(false);
        } else {
        ...
        if (senderChannel == nullptr) {
            ...
            for (uint64_t i = 0; i < totalNumBuffer; ++i) {
                ...
                for (uint64_t j = 0; j < bufferSize / sizeof(Record); ++j) {
                    ...
                    ASSERT_TRUE(!!optBuf);
                    memcpy((*optBuf).getBuffer<char>(), "IoTSPE", 7);
...
TEST_F(NetworkStackTest, testHandleUnregisteredBuffer) {
    ...
    try {
        ...
        class ExchangeListener : public ExchangeProtocolListener {
          ...
          public:
            ...
            void onServerError(Messages::ErrorMessage errorMsg) override {
                ...
                x_INFO("NetworkStackTest: Server error called!");
            ...
            void onChannelError(Messages::ErrorMessage errorMsg) override {
                ...
                x_INFO("NetworkStackTest: Channel error called!");
                ASSERT_EQ(errorMsg.getErrorType(), Messages::ErrorType::PartitionNotRegisteredError);
            }
        ...
        netManager->unregisterSubpartitionProducer(xPartition(1, 22, 333, 4445));
    } catch (...) {
        ASSERT_EQ(true, false);
    }
    ASSERT_EQ(true, true);
...
TEST_F(NetworkStackTest, testMassiveMultiSending) {
    ...
    // create a couple of xPartitions
    auto sendingThreads = std::vector<std::thread>();
    auto xPartitions = std::vector<xPartition>();
    ...
    for (uint64_t i = 0ull; i < numSendingThreads; ++i) {
        xPartitions.emplace_back(i, i + 10, i + 20, i + 30);
    ...
    try {
        class ExchangeListenerImpl : public ExchangeProtocolListener {
          ...
          public:
            ...
            void onEndOfStream(Messages::EndOfStreamMessage p) override {
                completedPromises[p.getChannelId().getxPartition().getQueryId()].set_value(true);
            }
            void onEvent(xPartition, Runtime::BaseEvent&) override {}
        };
        ...
        std::thread receivingThread([&netManager, &xPartitions, &completedPromises, this] {
            // register the incoming channel
            NodeLocation nodeLocation{0, "127.0.0.1", *freeDataPort};
            for (xPartition p : xPartitions) {
                //add random latency
                sleep(rand() % 3);
        ...
        for (auto i = 0ULL; i < numSendingThreads; ++i) {
            std::thread sendingThread([&, i] {
                // register the incoming channel
                xPartition xPartition(i, i + 10, i + 20, i + 30);
                auto senderChannel =
                    netManager->registerSubpartitionProducer(nodeLocation, xPartition, buffMgr, std::chrono::seconds(2), 10);
