...
class QueryPlanBuilderTest : public Testing::BaseUnitTest {
  ...
  public:
    ...
    void SetUp() override {
        x::Logger::setupLogging("QueryPlanBuilderTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup QueryPlanTest test case.");
    }

    /* Will be called before a test is executed. */
    void TearDown() override { x_INFO("Setup QueryPlanBuilderTest test case."); }

    /* Will be called after all tests in this class are finished. */
    static void TearDownTestCase() { x_INFO("Tear down QueryPlanBuilderTest test class."); }
};
...
TEST_F(QueryPlanBuilderTest, testHasOperator) {
    ...
    auto filterExpression = ExpressionNodePtr(
        LessExpressionNode::create(x::Attribute("a").getExpressionNode(), x::Attribute("b").getExpressionNode()));
    ...
    // test addSink
    auto sinkDescriptorPtr = x::PrintSinkDescriptor::create();
