...
class GlobalQueryPlanTest : public Testing::BaseUnitTest {
  ...
  public:
    static void SetUpTestCase() {
        Logger::setupLogging("GlobalQueryPlanTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup GlobalQueryPlanTest test case.");
    }
};
...
TEST_F(GlobalQueryPlanTest, testNewGlobalQueryPlan) {
    try {
        x_DEBUG("GlobalQueryPlanTest: create an empty global query plan");
...
TEST_F(GlobalQueryPlanTest, testNewGlobalQueryPlanAndAddingQueryPlanWithEmptyId) {
    ...
    x_DEBUG("GlobalQueryPlanTest: creating an empty global query plan");
    GlobalQueryPlanPtr globalQueryPlan = GlobalQueryPlan::create();
    x_DEBUG("GlobalQueryPlanTest: Adding a query plan without id to the global query plan");
...
TEST_F(GlobalQueryPlanTest, testNewGlobalQueryPlanAndGetAllNewGlobalQueryNodesWithATypeOfLogicalOperator) {
    ...
    x_DEBUG("GlobalQueryPlanTest: creating an empty global query plan");
    GlobalQueryPlanPtr globalQueryPlan = GlobalQueryPlan::create();
    x_DEBUG("GlobalQueryPlanTest: Adding a query plan without to the global query plan");
    ...
    //Assert
    x_DEBUG("GlobalQueryPlanTest: A global query node containing operator of type SinkLogicalOperatorNode should be returned");
...
TEST_F(GlobalQueryPlanTest, testNewGlobalQueryPlanByAddingSameQueryPlanTwice) {
    ...
    x_DEBUG("GlobalQueryPlanTest: creating an empty global query plan");
    GlobalQueryPlanPtr globalQueryPlan = GlobalQueryPlan::create();
    x_DEBUG("GlobalQueryPlanTest: Adding same query plan twice to the global query plan");
...
TEST_F(GlobalQueryPlanTest, testNewGlobalQueryPlanAndAddMultipleQueries) {
    ...
    x_DEBUG("GlobalQueryPlanTest: creating an empty global query plan");
    GlobalQueryPlanPtr globalQueryPlan = GlobalQueryPlan::create();
    x_DEBUG("GlobalQueryPlanTest: Adding a query plan to the global query plan");
    ...
    //Assert
    x_DEBUG("GlobalQueryPlanTest: should return 1 global query node with logical sink operator");
    auto listOfGQMsToDeploy = globalQueryPlan->getSharedQueryPlansToDeploy();
    EXPECT_TRUE(listOfGQMsToDeploy.size() == 1);

    x_DEBUG("GlobalQueryPlanTest: Adding another query plan to the global query plan");
    ...
    //Assert
    x_DEBUG("GlobalQueryPlanTest: should return 2 global query node with logical sink");
...
TEST_F(GlobalQueryPlanTest, testNewGlobalQueryPlanAndAddAndRemoveQuery) {
    ...
    //Assert
    x_DEBUG("GlobalQueryPlanTest: should 1 global query node with logical sink");
    auto listOfSQPsToDeploy = globalQueryPlan->getSharedQueryPlansToDeploy();
    EXPECT_TRUE(listOfSQPsToDeploy.size() == 1);

    x_DEBUG("GlobalQueryPlanTest: Removing the query plan for the query with id Q1");
    globalQueryPlan->removeQuery(1, x::RequestType::StopQuery);

    //Assert
    x_DEBUG("GlobalQueryPlanTest: Should return empty global query nodes");
...
TEST_F(GlobalQueryPlanTest, testUpdateMetaDataInformationForGlobalQueryPlanWithMixedQueriesAfterApplyingMergeRules) {
    ...
    //Assert To check if Global Query Plan is created properly
    x_DEBUG("GlobalQueryPlanTest: should return 3 global query metadata");
