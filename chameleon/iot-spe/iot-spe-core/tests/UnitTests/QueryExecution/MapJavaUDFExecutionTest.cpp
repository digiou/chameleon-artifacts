...
class MapJavaUDFQueryExecutionTest : public Testing::BaseUnitTest {
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("MapJavaUDFQueryExecutionTest.log", x::LogLevel::LOG_DEBUG);
        x_DEBUG("QueryExecutionTest: Setup MapJavaUDFQueryExecutionTest test class.");
    }
    ...
    void TearDown() override {
        ...
        x_DEBUG("QueryExecutionTest: Tear down MapJavaUDFQueryExecutionTest test case.");
        ASSERT_TRUE(executionEngine->stop());
    }

    /* Will be called after all tests in this class are finished. */
    static void TearDownTestCase() { x_DEBUG("MapJavaUDFQueryExecutionTest: Tear down QueryExecutionTest test class."); }
...
std::vector<char> loadClassFileIntoBuffer(const std::string& path, const std::string& className) {
    ...
    if (!file.is_open()) {
        x_ERROR("Could not open file: {} {}.class", path, className);
        return {};
    }
    ...
    if (!file.read(buffer.data(), fileSize)) {
        x_ERROR("Could not read file: {} {}.class", path, className);
        return {};
    }
    return buffer;
}
...
TEST_F(MapJavaUDFQueryExecutionTest, MapJavaUdf) {
    ...
    std::vector<std::string> classNames = {"stream/IoTSPE/MapFunction", "IntegerMapFunction"};
    auto methodName = "map";
