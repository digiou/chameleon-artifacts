...
class Z3ValidationTest : public Testing::BaseUnitTest {
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("Z3ValidationTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup Z3ValidationTest test class.");
    }
};
...
TEST_F(Z3ValidationTest, deMorganDualityValidation) {
    ...
    x_INFO("Expression: {}", expressionAsString.str());
    ...
    solverString << s;
    x_INFO("Content of the solver: {}", solverString.str());
    x_INFO("Statement folding inside z3 solver: {}", s.to_smt2());
