...
class OperatorCodeGenerationTest : public Testing::BaseIntegrationTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("OperatorOperatorCodeGenerationTest.log", x::LogLevel::LOG_DEBUG);
        x_DEBUG("Setup OperatorOperatorCodeGenerationTest test class.");
    }

    /* Will be called before a test is executed. */
    void SetUp() override {
        x_DEBUG("Setup OperatorOperatorCodeGenerationTest test case.");
    ...
    void TearDown() override {
        x_DEBUG("Tear down OperatorOperatorCodeGenerationTest test case.");
        dataPort.reset();
        Testing::BaseIntegrationTest::TearDown();
    }

    /* Will be called after all tests in this class are finished. */
    static void TearDownTestCase() { x_DEBUG("Tear down OperatorOperatorCodeGenerationTest test class."); }
...
TEST_F(OperatorCodeGenerationTest, codeGenerationCopy) {
    ...
    codeGenerator->generateCodeForEmit(Schema::create()->addField("campaign_id", DataTypeFactory::createUInt64()),
                                       QueryCompilation::OutputBufferAllocationStrategy::NO_OPTIMIZATION,
                                       QueryCompilation::OutputBufferAssignmentStrategy::FIELD_COPY,
                                       context);
    ...
    if (auto buffer = source->receiveData().value(); !!buffer) {
        /* execute Stage */
        x_INFO("Processing {} tuples: ", buffer.getNumberOfTuples());
        stage->execute(buffer, *queryContext, wctx);
        if (auto resultBuffer = queryContext->buffers[0]; !!resultBuffer) {
            /* check for correctxs, input source produces uint64_t tuples and stores a 1 in each tuple */
...
TEST_F(OperatorCodeGenerationTest, codeGenerationFilterPredicate) {
    ...
    x_INFO("Processing {} tuples: ", inputBuffer.getNumberOfTuples());
    ...
    /* check for correctxs, input source produces tuples consisting of two uint32_t values, 5 values will match the predicate */
    x_INFO("Number of generated output tuples: {}", resultBuffer.getNumberOfTuples());
...
TEST_F(OperatorCodeGenerationTest, codeGenerationScanOperator) {
    ...
    /* compile code to pipeline stage */
    auto stage1 =
        codeGenerator->compile(jitCompiler, context1, QueryCompilation::QueryCompilerOptions::CompilationStrategy::DEBUG);
}
...
TEST_F(OperatorCodeGenerationTest, codeGenerationWindowAssigner) {
    ...
    auto stage2 =
        codeGenerator->compile(jitCompiler, context2, QueryCompilation::QueryCompilerOptions::CompilationStrategy::DEBUG);
}
...
TEST_F(OperatorCodeGenerationTest, codeGenerationDistributedCombiner) {
    ...
    x_DEBUG("buffer={}", dynamicTupleBufferAsString.str());
    ...
    for (auto& [key, val] : stateVar->rangeAll()) {
        ...
        valueAsString << val;
        x_DEBUG("Key: {} Value: {}", key, valueAsString.str());
        for (auto& slice : val->getSliceMetadata()) {
            x_DEBUG("start={} end={}", slice.getStartTs(), slice.getEndTs());
...
TEST_F(OperatorCodeGenerationTest, codeGenerationStringComparePredicateTest) {
    ...
    /* generate code for writing result tuples to output buffer */
    codeGenerator->generateCodeForEmit(inputSchema,
                                       QueryCompilation::OutputBufferAllocationStrategy::NO_OPTIMIZATION,
                                       QueryCompilation::OutputBufferAssignmentStrategy::FIELD_COPY,
                                       context);
    ...
    if (auto inputBuffer = *optVal; !!inputBuffer) {
        ...
        /* check for correctxs, input source produces tuples consisting of two uint32_t values, 3 values will match the predicate */
...
TEST_F(OperatorCodeGenerationTest, codeGenerationInferModelTest) {
    ...
    auto valF = std::make_shared<ExpressionItem>(Attribute("valueFloat", x::BasicType::FLOAT32));
    ...
    /* generate code for writing result tuples to output buffer */
    codeGenerator->generateCodeForEmit(outputSchema,
                                       QueryCompilation::OutputBufferAllocationStrategy::NO_OPTIMIZATION,
                                       QueryCompilation::OutputBufferAssignmentStrategy::FIELD_COPY,
                                       context);
...
TEST_F(OperatorCodeGenerationTest, codeGenerationMapPredicateTestColLayout) {
    ...
    /* prepare objects for test */
    auto source = createTestSourceCodeGenPredicate(nodeEngine->getBufferManager(),
...
TEST_F(OperatorCodeGenerationTest, DISABLED_codeGenerations) {
    ...
    /* compile code to pipeline stage */
    auto stage1 =
        codeGenerator->compile(jitCompiler, pipelineContext1, QueryCompilation::QueryCompilerOptions::CompilationStrategy::DEBUG);
    ...
    auto stage3 =
        codeGenerator->compile(jitCompiler, context3, QueryCompilation::QueryCompilerOptions::CompilationStrategy::DEBUG);
    ...
    std::stringstream dynamicTupleBufferAsString;
    dynamicTupleBufferAsString << dynamicTupleBuffer;
    x_DEBUG("buffer content={}", dynamicTupleBufferAsString.str());
...
/**
 * @brief This test checks the correctxs of the current CEPiteration OP
 */
TEST_F(OperatorCodeGenerationTest, codeGenerationCEPIterationOPinitialTest) {
    ...
    /* check for correctxs, as minIteration = 50, we expect 2 outputs from the 131 tuples in the inputBuffer*/
    ...
    std::stringstream dynamicTupleBufferAsString;
    dynamicTupleBufferAsString << dynamicTupleBuffer;
    x_DEBUG("{}", dynamicTupleBufferAsString.str());
}
}// namespace x
