...
class CodeGenerationTest : public Testing::BaseIntegrationTest {
  public:
    ...
    static void SetUpTestCase() { x::Logger::setupLogging("CodeGenerationTest.log", x::LogLevel::LOG_DEBUG); }
...
TEST_F(CodeGenerationTest, codeGenerationApiTest) {
    ...
    }
    {
        ...
        EXPECT_EQ(varDeclN.getCode(),
                  "x::ExecutableTypes::Array<char, 12> n = x::ExecutableTypes::Array {'a', 'b', 'c', static_cast<char>(0)}");
        ...
        EXPECT_EQ(varDeclO.getCode(), "x::ExecutableTypes::Array<uint8_t, 4> o = x::ExecutableTypes::Array {2, 3, 4}");
...

    {
        ...
        auto plusOperatorCode =
            BinaryOperatorStatement(VarRefStatement(varDeclI), BinaryOperatorType::PLUS_OP, VarRefStatement(varDeclJ))
                .getCode()
                ->code_;
        ...
        auto negate =
            ((~VarRefStatement(varDeclI) >= VarRefStatement(varDeclJ)
                  << ConstantExpressionStatement(x::QueryCompilation::GeneratableTypesFactory::createValueType(
        ...
        auto ifStatementReturn = IFStatement(
            BinaryOperatorStatement(VarRefStatement(varDeclI), BinaryOperatorType::GREATER_THAN_OP, VarRefStatement(varDeclJ))
                .createCopy(),
        ...
        auto ternStatement = TernaryOperatorStatement(
            ...
            *TernaryOperatorStatement(*BinaryOperatorStatement(VarRefStatement(varDeclI),
...

    {
        ...
        EXPECT_EQ(UnaryOperatorStatement(VarRefStatement(variableDeclaration), UnaryOperatorType::ADDRESS_OF_OP).getCode()->code_,
                  "&num_tuples");
        EXPECT_EQ(UnaryOperatorStatement(VarRefStatement(variableDeclaration), UnaryOperatorType::DEREFERENCE_POINTER_OP)
                      .getCode()
                      ->code_,
                  "*num_tuples");
        EXPECT_EQ(
            UnaryOperatorStatement(VarRefStatement(variableDeclaration), UnaryOperatorType::PREFIX_INCREMENT_OP).getCode()->code_,
            "++num_tuples");
        EXPECT_EQ(
            UnaryOperatorStatement(VarRefStatement(variableDeclaration), UnaryOperatorType::PREFIX_DECREMENT_OP).getCode()->code_,
            "--num_tuples");
        EXPECT_EQ(UnaryOperatorStatement(VarRefStatement(variableDeclaration), UnaryOperatorType::POSTFIX_INCREMENT_OP)
                      .getCode()
                      ->code_,
                  "num_tuples++");
        EXPECT_EQ(UnaryOperatorStatement(VarRefStatement(variableDeclaration), UnaryOperatorType::POSTFIX_DECREMENT_OP)
                      .getCode()
                      ->code_,
                  "num_tuples--");
        EXPECT_EQ(UnaryOperatorStatement(VarRefStatement(variableDeclaration), UnaryOperatorType::BITWISE_COMPLEMENT_OP)
                      .getCode()
                      ->code_,
                  "~num_tuples");
        EXPECT_EQ(
            UnaryOperatorStatement(VarRefStatement(variableDeclaration), UnaryOperatorType::LOGICAL_NOT_OP).getCode()->code_,
            "!num_tuples");
        EXPECT_EQ(
            UnaryOperatorStatement(VarRefStatement(variableDeclaration), UnaryOperatorType::SIZE_OF_TYPE_OP).getCode()->code_,
            "sizeof(num_tuples)");
        EXPECT_EQ(UnaryOperatorStatement(VarRefStatement(variableDeclaration), UnaryOperatorType::ABSOLUTE_VALUE_OF_OP)
                      .getCode()
                      ->code_,
                  "abs(num_tuples)");
    }

    {
        ...
        ForLoopStatement loopStmt(
            varDeclQ.copy(),
            BinaryOperatorStatement(VarRefStatement(varDeclQ), BinaryOperatorType::LESS_THAN_OP, VarRefStatement(varDeclNumTuple))
                .copy(),
...

    {
        ...
        auto val =
            x::QueryCompilation::GeneratableTypesFactory::createPointer(tf.createDataType(DataTypeFactory::createInt32()));
        assert(val != nullptr);
   ...
   */
        ...
        auto varDeclTupleBuffer = VariableDeclaration::create(
            x::QueryCompilation::GeneratableTypesFactory::createUserDefinedType(tupleBufferStructDecl),
            "buffer");
        EXPECT_EQ(varDeclTupleBuffer.getCode(), "TupleBuffer buffer");

        auto varDeclTupleBufferPointer = VariableDeclaration::create(
            x::QueryCompilation::GeneratableTypesFactory::createPointer(
                x::QueryCompilation::GeneratableTypesFactory::createUserDefinedType(tupleBufferStructDecl)),
            "buffer");
        EXPECT_EQ(varDeclTupleBufferPointer.getCode(), "TupleBuffer* buffer");

        auto pointerDataType = x::QueryCompilation::GeneratableTypesFactory::createPointer(
            x::QueryCompilation::GeneratableTypesFactory::createUserDefinedType(tupleBufferStructDecl));
...
TEST_F(CodeGenerationTest, codeGenRunningSum) {
    ...
    auto tupleBufferType = x::QueryCompilation::GeneratableTypesFactory::createAnonymusDataType("Runtime::TupleBuffer");
    auto pipelineExecutionContextType =
        x::QueryCompilation::GeneratableTypesFactory::createAnonymusDataType("Runtime::Execution::PipelineExecutionContext");
    auto workerContextType = x::QueryCompilation::GeneratableTypesFactory::createAnonymusDataType("Runtime::WorkerContext");
    ...
    auto varDeclTupleBuffers =
        VariableDeclaration::create(x::QueryCompilation::GeneratableTypesFactory::createReference(tupleBufferType),
                                    "input_buffer");
    auto varDeclPipelineExecutionContext =
        VariableDeclaration::create(x::QueryCompilation::GeneratableTypesFactory::createReference(pipelineExecutionContextType),
                                    "pipelineExecutionContext");
    auto varDeclWorkerContext =
        VariableDeclaration::create(x::QueryCompilation::GeneratableTypesFactory::createReference(workerContextType),
                                    "workerContext");
    /* Tuple *tuples; */
    auto varDeclTuple =
        VariableDeclaration::create(x::QueryCompilation::GeneratableTypesFactory::createPointer(
                                        x::QueryCompilation::GeneratableTypesFactory::createUserDefinedType(structDeclTuple)),
                                    "tuples");

    auto varDeclResultTuple = VariableDeclaration::create(
        x::QueryCompilation::GeneratableTypesFactory::createPointer(
            x::QueryCompilation::GeneratableTypesFactory::createUserDefinedType(structDeclResultTuple)),
        "resultTuples");
    ...
    auto varDeclReturn =
        VariableDeclaration::create(x::QueryCompilation::GeneratableTypesFactory::createAnonymusDataType("ExecutionResult"),
                                    "ret",
    ...
    auto mainFunction =
        FunctionDefinition::create("execute")
            ...
            ->returns(x::QueryCompilation::GeneratableTypesFactory::createAnonymusDataType("ExecutionResult"))
            ...
            ->addStatement(
                ...
                VarRef(varDeclResultTuple)[Constant(x::QueryCompilation::GeneratableTypesFactory::createValueType(
                                               DataTypeFactory::createBasicValue(DataTypeFactory::createInt32(), "0")))]
    ...
    auto executablePipeline = ClassDefinition::create("ExecutablePipelixtage1");
    ...
    auto stage = CompiledExecutablePipelixtage::create(compiledCode, PipelixtageArity::Unary);
    ...
    x_INFO("{}", dynamicTupleBufferAsString.str());

    /* check result for correctxs */
