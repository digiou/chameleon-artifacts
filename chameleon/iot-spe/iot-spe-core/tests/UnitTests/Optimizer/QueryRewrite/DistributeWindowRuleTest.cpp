...
class DistributeWindowRuleTest : public Testing::BaseUnitTest {
  ...
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("DistributeWindowRuleTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup DistributeWindowRuleTest test case.");
    }
...
void setupSensorNodeAndSourceCatalogTwoNodes(const Catalogs::Source::SourceCatalogPtr& sourceCatalog) {
    x_INFO("Setup LogicalSourceExpansionRuleTest test case.");
...
void setupSensorNodeAndSourceCatalogFiveNodes(const Catalogs::Source::SourceCatalogPtr& sourceCatalog) {
    ...
    x_DEBUG("topo={}", topology->toString());
...
TEST_F(DistributeWindowRuleTest, testRuleForCentralWindow) {
    ...
    Query query = Query::from("default_logical")
                      .window(x::Windowing::TumblingWindow::of(x::Windowing::TimeCharacteristic::createIngestionTime(),
                                                                 API::Seconds(10)))
                      .byKey(Attribute("id"))
    ...
    x_DEBUG(" plan before={}", queryPlan->toString());
    // Execute

    const QueryPlanPtr updatedPlan = distributeWindowRule->apply(queryPlan);

    x_DEBUG(" plan after={}", queryPlan->toString());
...
TEST_F(DistributeWindowRuleTest, testRuleForDistributedWindow) {
    ...
    x_DEBUG(" plan before log expand={}", queryPlan->toString());
    ...
    x_DEBUG(" plan after log expand={}", queryPlan->toString());

    x_DEBUG(" plan before window distr={}", queryPlan->toString());
    updatedPlan = distributeWindowRule->apply(queryPlan);
    x_DEBUG(" plan after window distr={}", queryPlan->toString());
