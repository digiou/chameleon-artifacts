...
class ILPPlacementTest : public Testing::BaseUnitTest {
  ...
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("ILPPlacementTest.log", x::LogLevel::LOG_DEBUG);
        x_DEBUG("Setup ILPPlacementTest test class.");
    }

    /* Will be called before a test is executed. */
    void SetUp() override {
        Testing::BaseUnitTest::SetUp();
        x_DEBUG("Setup ILPPlacementTest test case.");
    ...
    while (true) {
        if (sat == opt.check()) {
            ...
            if (m.eval(P21).get_numeral_int() == 1) {
                x_DEBUG("Operator on Node 1");
            } else if (m.eval(P22).get_numeral_int() == 1) {
                x_DEBUG("Operator on Node 2");
            } else if (m.eval(P23).get_numeral_int() == 1) {
                x_DEBUG("Operator on Node 3");
            }
            x_DEBUG("-------------------------------");
            break;
        } else {
...
TEST_F(ILPPlacementTest, testPlacingUpdatedSharedQueryPlanWithILPStrategy) {
    ...
    x_INFO("{}", sharedQueryPlansToDeploy[0]->getQueryPlan()->toString());
    queryPlacementPhase->execute(sharedQueryPlansToDeploy[0]);
