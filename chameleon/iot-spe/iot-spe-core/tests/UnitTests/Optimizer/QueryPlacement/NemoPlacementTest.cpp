...
class NemoPlacementTest : public Testing::BaseUnitTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("NemoPlacementTest.log", x::LogLevel::LOG_DEBUG);
        x_DEBUG("Setup NemoPlacementTest test class.");
    }

    /* Will be called before a test is executed. */
    void SetUp() override {
        Testing::BaseUnitTest::SetUp();
        x_DEBUG("Setup NemoPlacementTest test case.");
    ...
    void setupTopologyAndSourceCatalog(uint64_t layers, uint64_t nodesPerNode, uint64_t leafNodesPerNode) {
        ...
        x_DEBUG("NemoPlacementTest: topology: {}", topology->toString());
...
TEST_F(NemoPlacementTest, testNemoPlacementFlatTopologyNoMerge) {
    ...
    x_DEBUG("NemoPlacementTest: topology: {}\n", topology->toString());
    x_DEBUG("NemoPlacementTest: query plan {}\n", globalExecutionPlan->getAsString());
    x_DEBUG("NemoPlacementTest: shared plan {}\n", sharedQueryPlan->getQueryPlan()->toString());
    ...
    for (const auto& executionNode : executionNodes) {
        ...
        if (executionNode->getId() == 1u) {
            ...
            verifySourceOperators<x::Network::NetworkSourceDescriptor>(querySubPlans, 1, 10);
        } else {
...
TEST_F(NemoPlacementTest, testNemoPlacementThreeLevelsTopology) {
    ...
    for (const auto& executionNode : executionNodes) {
        ...
        if (executionNode->getId() == 1u) {
            //coordinator
            x_DEBUG("NemoPlacementTest: Testing Coordinator");
            verifyChildrenOfType<SourceLogicalOperatorNode>(querySubPlans, 1);
            verifySourceOperators<x::Network::NetworkSourceDescriptor>(querySubPlans, 1, 10);
        } else if (executionNode->getId() >= 2u && executionNode->getId() <= 11u) {
            //intermediate nodes
            x_DEBUG("NemoPlacementTest: Testing 1st level of the tree");
            verifyChildrenOfType<CentralWindowOperator>(querySubPlans);
            verifySourceOperators<x::Network::NetworkSourceDescriptor>(querySubPlans, 1, 10);
        } else {
            //leaves
            x_DEBUG("NemoPlacementTest: Testing leaves of the tree");
            verifyChildrenOfType<WatermarkAssignerLogicalOperatorNode>(querySubPlans);
            verifySourceOperators<LogicalSourceDescriptor>(querySubPlans, 1, 1);
        }
    }
...
TEST_F(NemoPlacementTest, testNemoPlacementFourLevelsSparseTopology) {
    ...
    for (const auto& executionNode : executionNodes) {
        ...
        if (executionNode->getId() == 1u) {
            ...
            x_DEBUG("NemoPlacementTest: Testing Coordinator");
            verifyChildrenOfType<CentralWindowOperator>(querySubPlans);
            verifySourceOperators<x::Network::NetworkSourceDescriptor>(querySubPlans, 1, 4);
        } else if (executionNode->getId() >= 2u && executionNode->getId() <= 3u) {
            //intermediate nodes
            x_DEBUG("NemoPlacementTest: Testing 1st level of the tree");
            verifyChildrenOfType<SourceLogicalOperatorNode>(querySubPlans, 2);
            verifySourceOperators<x::Network::NetworkSourceDescriptor>(querySubPlans, 2, 1);
        } else if (executionNode->getId() >= 4u && executionNode->getId() <= 7u) {
            //intermediate nodes
            x_DEBUG("NemoPlacementTest: Testing 2nd level of the tree");
            verifyChildrenOfType<SourceLogicalOperatorNode>(querySubPlans);
            verifySourceOperators<x::Network::NetworkSourceDescriptor>(querySubPlans, 1, 1);
        } else {
            //leaves
            x_DEBUG("NemoPlacementTest: Testing leaves of the tree");
            verifyChildrenOfType<WatermarkAssignerLogicalOperatorNode>(querySubPlans);
...
TEST_F(NemoPlacementTest, testNemoPlacementFourLevelsDenseTopology) {
    ...
    for (const auto& executionNode : executionNodes) {
        ...
        if (executionNode->getId() == 1u) {
            ...
            x_DEBUG("NemoPlacementTest: Testing Coordinator");
            verifyChildrenOfType<SourceLogicalOperatorNode>(querySubPlans);
            verifySourceOperators<x::Network::NetworkSourceDescriptor>(querySubPlans, 1, 9);
        } else if (executionNode->getId() >= 2u && executionNode->getId() <= 4u) {
            //intermediate nodes
            x_DEBUG("NemoPlacementTest: Testing 1st level of the tree");
            verifyChildrenOfType<SourceLogicalOperatorNode>(querySubPlans, 3);
            verifySourceOperators<x::Network::NetworkSourceDescriptor>(querySubPlans, 3, 1);
        } else if (executionNode->getId() >= 5u && executionNode->getId() <= 13u) {
            //intermediate nodes
            x_DEBUG("NemoPlacementTest: Testing 2nd level of the tree");
            verifyChildrenOfType<CentralWindowOperator>(querySubPlans);
            verifySourceOperators<x::Network::NetworkSourceDescriptor>(querySubPlans, 1, 3);
        } else {
            //leaves
            x_DEBUG("NemoPlacementTest: Testing leaves of the tree");
            verifyChildrenOfType<WatermarkAssignerLogicalOperatorNode>(querySubPlans);
