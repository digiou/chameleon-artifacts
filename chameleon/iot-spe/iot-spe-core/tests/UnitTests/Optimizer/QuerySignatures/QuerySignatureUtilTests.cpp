...
class QuerySignatureUtilTests : public Testing::BaseUnitTest {
  ...
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("QuerySignatureUtilTests.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup QuerySignatureUtilTests test case.");
    }
...
TEST_F(QuerySignatureUtilTests, testSignatureComputationForWatermarkAssignerOperatorWithDifferentLatexs) {
    std::shared_ptr<z3::context> context = std::make_shared<z3::context>();
    //Define Sources
    auto sourceDescriptor1 = LogicalSourceDescriptor::create("Car");
    sourceDescriptor1->setSchema(schema);
