...
class GlobalQueryPlanUpdatePhaseTest : public Testing::BaseUnitTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("GlobalQueryPlanUpdatePhaseTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup GlobalQueryPlanUpdatePhaseTest test case.");
    }
...
TEST_F(GlobalQueryPlanUpdatePhaseTest, DISABLED_executeQueryMergerPhaseForSingleInvalidQueryPlan) {
    ...
    //Prepare
    x_INFO("GlobalQueryPlanUpdatePhaseTest: Create a new query without assigning it a query id.");
    auto q1 = Query::from("source1").sink(PrintSinkDescriptor::create());
    x_INFO("GlobalQueryPlanUpdatePhaseTest: Create the query merger phase.");
    ...
    std::vector<xRequestPtr> batchOfQueryRequests = {request};
    //Assert
...
TEST_F(GlobalQueryPlanUpdatePhaseTest, executeQueryMergerPhaseForSingleQueryPlan) {
    ...
    //Prepare
    x_INFO("GlobalQueryPlanUpdatePhaseTest: Create a new query and assign it an id.");
    ...
    //Assert
    x_INFO("GlobalQueryPlanUpdatePhaseTest: Should return 1 global query node with sink operator.");
...
TEST_F(GlobalQueryPlanUpdatePhaseTest, executeQueryMergerPhaseForDuplicateValidQueryPlan) {
    ...
    //Prepare
    x_INFO("GlobalQueryPlanUpdatePhaseTest: Create a new valid query.");
    const auto* queryString = R"(Query::from("source1").sink(PrintSinkDescriptor::create()))";
    auto q1 = Query::from("source1").sink(PrintSinkDescriptor::create());
    q1.getQueryPlan()->setQueryId(1);
    queryCatalog->createNewEntry(queryString, q1.getQueryPlan(), Optimizer::PlacementStrategy::TopDown);
    ...
    auto phase = Optimizer::GlobalQueryPlanUpdatePhase::create(topology,
                                                               ...
                                                               udfCatalog,
                                                               globalExecutionPlan);
    x_INFO("GlobalQueryPlanUpdatePhaseTest: Create the batch of query plan with duplicate query plans.");
    auto catalogEntry1 = queryCatalog->getQueryCatalogEntry(1);
    auto request = AddQueryRequest::create(catalogEntry1->getInputQueryPlan(), catalogEntry1->getQueryPlacementStrategy());
    std::vector<xRequestPtr> xRequests = {request, request};
    //Assert
    EXPECT_THROW(phase->execute(xRequests), GlobalQueryPlanUpdateException);
}
...
TEST_F(GlobalQueryPlanUpdatePhaseTest, executeQueryMergerPhaseForMultipleValidQueryPlan) {
    //Prepare
    x_INFO("GlobalQueryPlanUpdatePhaseTest: Create two valid queryIdAndCatalogEntryMapping.");
    ...
    std::vector<xRequestPtr> requests = {request1, request2};
...
TEST_F(GlobalQueryPlanUpdatePhaseTest, DISABLED_executeQueryMergerPhaseForAValidQueryPlanInInvalidState) {
    ...
    x_INFO("GlobalQueryPlanUpdatePhaseTest: Create the batch of query plan with duplicate query plans.");
    auto xRequest1 = AddQueryRequest::create(catalogEntry1->getInputQueryPlan(), catalogEntry1->getQueryPlacementStrategy());
    std::vector<xRequestPtr> batchOfQueryRequests = {xRequest1};
...
TEST_F(GlobalQueryPlanUpdatePhaseTest, executeQueryMergerPhaseForMultipleValidQueryRequestsWithAddAndRemoval) {
    ...
    auto xRequest1 = AddQueryRequest::create(catalogEntry1->getInputQueryPlan(), catalogEntry1->getQueryPlacementStrategy());
    auto xRequest2 = AddQueryRequest::create(catalogEntry2->getInputQueryPlan(), catalogEntry1->getQueryPlacementStrategy());
    auto xRequest3 = StopQueryRequest::create(catalogEntry2->getInputQueryPlan()->getQueryId());

    std::vector<xRequestPtr> batchOfQueryRequests = {xRequest1, xRequest2, xRequest3};
    auto resultPlan = phase->execute(batchOfQueryRequests);
...
TEST_F(GlobalQueryPlanUpdatePhaseTest, queryMergerPhaseForSingleQueryPlan) {
    ...
    std::vector<xRequestPtr> batchOfxRequests;
    auto allQueries = queryCatalog->getAllQueryCatalogEntries();
    for (auto& [key, value] : allQueries) {
        auto xRequest = AddQueryRequest::create(value->getInputQueryPlan(), value->getQueryPlacementStrategy());
        batchOfxRequests.emplace_back(xRequest);
    }
    ...
    auto resultPlan = phase->execute(batchOfxRequests);
    //Assert
    x_INFO("GlobalQueryPlanUpdatePhaseTest: Should return 1 global query node with sink operator.");
...
TEST_F(GlobalQueryPlanUpdatePhaseTest, executeQueryMergerPhaseForMultipleValidQueryRequestsWithRemovalById) {
    ...
    std::vector<xRequestPtr> batchOfQueryRequests = {xRequest1, xRequest2};
    auto resultPlan = phase->execute(batchOfQueryRequests);
    const auto& sharedQueryMetadataToDeploy = resultPlan->getSharedQueryPlansToDeploy();
    ...
    x_INFO("Id2 {}, id1 {}", sharedQueryPlanId, sharedQueryPlanId1);
    resultPlan = phase->execute({xRequest3});
...
TEST_F(GlobalQueryPlanUpdatePhaseTest, testLinkRemovalRequestForUnusedLink) {
    ...
    q1.getQueryPlan()->setQueryId(queryId);
    queryCatalog->createNewEntry(queryString, q1.getQueryPlan(), Optimizer::PlacementStrategy::TopDown);
    auto xAddQueryRequest = AddQueryRequest::create(q1.getQueryPlan(), Optimizer::PlacementStrategy::TopDown);
    ...
    globalQueryPlanUpdatePhase->execute({xAddQueryRequest});
...
TEST_F(GlobalQueryPlanUpdatePhaseTest, testLinkRemovalRequestForUsedLinkWithFilterQuery) {
    ...
    auto xAddQueryRequest = AddQueryRequest::create(q1.getQueryPlan(), Optimizer::PlacementStrategy::BottomUp);
