...
class OriginIdInferencePhaseTest : public Testing::BaseUnitTest {
  ...
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("OriginIdInferencePhaseTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup OriginIdInferencePhaseTest test case.");
    }
...
TEST_F(OriginIdInferencePhaseTest, testRuleForJoinAggregationAndUnionOperators) {
    ...
    if (aggregations[0]->getType() == x::Windowing::WindowAggregationDescriptor::Type::Sum) {
        ASSERT_EQ(windowOps[0]->getInputOriginIds().size(), 3);
    } else if (aggregations[0]->getType() == x::Windowing::WindowAggregationDescriptor::Type::Avg) {
