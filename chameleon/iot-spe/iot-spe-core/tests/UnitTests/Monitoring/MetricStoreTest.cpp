...
class MetricStoreTest : public Testing::BaseUnitTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("MetricStoreTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("ResourcesReaderTest: Setup MetricStoreTest test class.");
    }

    /* Will be called before a  test is executed. */
    void SetUp() override {
        Testing::BaseUnitTest::SetUp();
        x_DEBUG("MetricStoreTest: Setup MetricStoreTest test case.");
...
TEST_F(MetricStoreTest, testNewestEntryMetricStore) {
    ...
    x_INFO("MetricStoreTest: Stored metrics {}", Monitoring::MetricUtils::toJson(storedMetrics).dump());
