...
class ResourcesReaderTest : public Testing::BaseUnitTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("ResourcesReaderTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("ResourcesReaderTest: Setup ResourcesReaderTest test class.");
    }
...
TEST_F(ResourcesReaderTest, testAbstractSystemResourcesReader) {
    ...
    ASSERT_TRUE(resourcesReader->readRuntimexMetrics() == Monitoring::RuntimeMetrics{});
...
TEST_F(ResourcesReaderTest, testRuntimexMetrics) {
    auto runtimeMetrics = resourcesReader->readRuntimexMetrics();
    x_DEBUG("ResourcesReaderTest: Runtime metrics={}", runtimeMetrics.toJson().dump());
    ASSERT_TRUE(MetricValidator::isValid(resourcesReader, runtimeMetrics));
}

TEST_F(ResourcesReaderTest, testStaticxMetrics) {
    auto staticMetrics = resourcesReader->readRegistrationMetrics();
    x_DEBUG("ResourcesReaderTest: Static metrics={}", staticMetrics.toJson().dump());
...
TEST_F(ResourcesReaderTest, testCPUStats) {
    ...
    x_DEBUG("ResourcesReaderTest: CPU metrics={}", cpuMetrics.toJson().dump());
...
TEST_F(ResourcesReaderTest, testMemoryStats) {
    ...
    x_DEBUG("ResourcesReaderTest: Static metrics={}", memMetrics.toJson().dump());
...
TEST_F(ResourcesReaderTest, testDiskStats) {
    ...
    x_DEBUG("ResourcesReaderTest: Static metrics={}", diskMetrics.toJson().dump());
...
TEST_F(ResourcesReaderTest, testNetworkStats) {
    ...
    x_DEBUG("ResourcesReaderTest: Static metrics={}", networkMetrics.toJson().dump());
