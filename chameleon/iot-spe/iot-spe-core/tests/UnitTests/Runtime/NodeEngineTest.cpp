...
createMockedEngine(const std::string& hostname, uint16_t port, uint64_t bufferSize = 8192, uint64_t numBuffers = 1024) {
    try {
        ...
        auto mockEngine = std::make_shared<MockedNodeEngine>(std::move(physicalSources),
                                                             ...
                                                             12,
                                                             12);
        x::Exceptions::installGlobalErrorListener(mockEngine);
        return mockEngine;
...
class TextExecutablePipeline : public ExecutablePipelixtage {
  public:
    ...
    execute(TupleBuffer& inputTupleBuffer, PipelineExecutionContext& pipelineExecutionContext, WorkerContext& wctx) override {
        ...
        x_INFO("Test: query result = Processed Block:{} count: {} psum: {} sum: {}",
        ...
        if (sum == 10) {
            ...
            thisAsString << this;
            x_DEBUG("TEST: {} written {}", thisAsString.str(), arr[0]);
            pipelineExecutionContext.emitBuffer(outputBuffer, wctx);
            completedPromise.set_value(true);
        } else {
            x_DEBUG("TEST: result wrong ");
...
class NodeEngineTest : public Testing::BaseIntegrationTest {
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("EngineTest.log", x::LogLevel::LOG_DEBUG);

        x_INFO("Setup EngineTest test class.");
    }
...
void assertKiller() {
      ...
      public:
        ...
        explicit MockedNodeEngine(std::vector<PhysicalSourcePtr> physicalSources,
            ...
            : NodeEngine(std::move(physicalSources),
                         ...
                         std::move(partitionManager),
                         std::move(compiler),
                         std::make_shared<x::Runtime::StateManager>(nodeEngineId),
                         std::weak_ptr<xWorker>(),
    ...
    x_ASSERT(false, "this will fail now with a RuntimeException");
}
}// namespace detail
...
TEST_F(NodeEngineTest, DISABLED_testSemiUnhandledExceptionCrash) {
    ...
    class FailingTextExecutablePipeline : public ExecutablePipelixtage {
      public:
        ...
        ExecutionResult execute(TupleBuffer&, PipelineExecutionContext&, WorkerContext&) override {
            x_DEBUG("Going to throw exception");
