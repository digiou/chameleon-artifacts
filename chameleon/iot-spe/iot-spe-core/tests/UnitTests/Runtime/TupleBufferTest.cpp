...
class TupleBufferTest : public Testing::BaseUnitTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("TupleBufferTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("SetUpTestCase TupleBufferTest test case.");
    }
...
TEST_F(TupleBufferTest, testPrintingOfTupleBuffer) {
    ...
    x_DEBUG("RES={}", result);
    x_DEBUG("Reference size={} content=\n{}", reference.size(), reference);
    x_DEBUG("Result size={} content=\n{}", result.size(), result);
    x_DEBUG("----");
