...
class ColumnarMemoryLayoutTest : public Testing::BaseUnitTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("ColumnarMemoryLayoutTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup ColumnarMemoryLayoutTest test class.");
    }
...
TEST_F(ColumnarMemoryLayoutTest, columnLayoutLayoutFieldBoundaryCheck) {
    ...
    auto field1 = ColumnLayoutField<uint16_t, true>::create(1, columnLayout, tupleBuffer);
    auto field2 = ColumnLayoutField<uint32_t, true>::create(2, columnLayout, tupleBuffer);
    ASSERT_THROW((ColumnLayoutField<uint8_t, true>::create(3, columnLayout, tupleBuffer)), x::Exceptions::RuntimeException);
    ASSERT_THROW((ColumnLayoutField<uint16_t, true>::create(4, columnLayout, tupleBuffer)), x::Exceptions::RuntimeException);
    ASSERT_THROW((ColumnLayoutField<uint32_t, true>::create(5, columnLayout, tupleBuffer)), x::Exceptions::RuntimeException);
    ...
    ASSERT_THROW(field0[i], x::Exceptions::RuntimeException);
    ASSERT_THROW(field1[i], x::Exceptions::RuntimeException);
    ASSERT_THROW(field2[i], x::Exceptions::RuntimeException);
...
TEST_F(ColumnarMemoryLayoutTest, getFieldViaFieldNameColumnLayout) {
    ...
    ASSERT_THROW((ColumnLayoutField<uint32_t, true>::create("t4", columnLayout, tupleBuffer)), x::Exceptions::RuntimeException);
    ASSERT_THROW((ColumnLayoutField<uint32_t, true>::create("t5", columnLayout, tupleBuffer)), x::Exceptions::RuntimeException);
    ASSERT_THROW((ColumnLayoutField<uint32_t, true>::create("t6", columnLayout, tupleBuffer)), x::Exceptions::RuntimeException);
}
...
TEST_F(ColumnarMemoryLayoutTest, pushRecordTooManyRecordsColumnLayout) {
    ...
    for (; i < NUM_TUPLES + 1; i++) {
        std::tuple<uint8_t, uint16_t, uint32_t> writeRecord(rand(), rand(), rand());
        allTuples.emplace_back(writeRecord);
        ASSERT_THROW(dynamicBuffer->pushRecordToBuffer(writeRecord), x::Exceptions::RuntimeException);
    }
