...
class ZMQTest : public Testing::BaseIntegrationTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("ZMQTest.log", x::LogLevel::LOG_DEBUG);
        x_DEBUG("Setup ZMQTest test class.");
    }

    /* Will be called before a test is executed. */
    void SetUp() override {
        Testing::BaseIntegrationTest::SetUp();
        x_DEBUG("Setup ZMQTest test case.");
    ...
    static void TearDownTestCase() { x_DEBUG("Tear down ZMQTest test class."); }
...
TEST_F(ZMQTest, testZmqSourceReceiveData) {
    ...
    while (!receiving_finished) {
        ...
        if (auto const sentEnvelope = socket.send(message_tupleCnt, zmq::send_flags::sndmore).value_or(0);
            ...
            x_ERROR("ZMQ Test Error: Sending message metadata failed! {} {}", sentEnvelope, message_tupleCnt.size());
        }
        ...
        if (auto const sentPayload = socket.send(message_data, zmq::send_flags::none); sentPayload != test_data_size) {
            x_ERROR("ZMQ Test Error: Sending message payload failed!");
        }
    }
