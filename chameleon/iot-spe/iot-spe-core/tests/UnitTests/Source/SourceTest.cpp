...
// testing w/ mocked running routine (no need for actual routix)
...
class LambdaSourceProxy : public LambdaSource {
  public:
    LambdaSourceProxy(
        ...
        std::function<void(x::Runtime::TupleBuffer& buffer, uint64_t numberOfTuplesToProduce)>&& generationFunction,
        OperatorId operatorId,
        size_t numSourceLocalBuffers,
        GatheringMode gatheringMode,
        std::vector<Runtime::Execution::SuccessorExecutablePipeline> successors)
...
class MockedExecutablePipeline : public Runtime::Execution::ExecutablePipelixtage {
  public:
...
class SourceTest : public Testing::BaseIntegrationTest {
  public:
    ...
    static void TearDownTestCase() { x_INFO("Tear down SourceTest test class."); }

    static void SetUpTestCase() {
        x::Logger::setupLogging("SourceTest.log", x::LogLevel::LOG_NONE);
        x_INFO("Setup SourceTest test class.");
    }
...
TEST_F(SourceTest, testCpuPerfPerTuple) {
    ...
    x_INFO("Total time: {}μs", run_duration.count());
    x_INFO("Avg. time per tuple: {}μs", avg_run_duration.count());
...
TEST_F(SourceTest, testBinarySourceGetType) {
    BinarySourceProxy bDataSource(this->schema,
                                  ...
                                  this->numSourceLocalBuffersDefault,
                                  {std::make_shared<NullOutputSink>(this->nodeEngine, 1, 1, 1)});
    ASSERT_EQ(bDataSource.getType(), x::SourceType::BINARY_SOURCE);
}
...
TEST_F(SourceTest, testCSVSourceGetType) {
    ...
    CSVSourceProxy csvDataSource(this->schema,
                                 ...
                                 this->numSourceLocalBuffersDefault,
                                 {std::make_shared<NullOutputSink>(this->nodeEngine, 1, 1, 1)});
    ASSERT_EQ(csvDataSource.getType(), x::SourceType::CSV_SOURCE);
}
...
TEST_F(SourceTest, testIngestionRateFromQuery) {
    ...
    ASSERT_TRUE(x::TestUtils::waitForQueryToStart(queryId, queryCatalog));
    ...
    x_DEBUG("start={} stop={}", start, stop);

    x_INFO("SourceTest: Remove query");
