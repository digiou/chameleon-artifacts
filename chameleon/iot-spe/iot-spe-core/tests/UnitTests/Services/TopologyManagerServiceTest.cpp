...
class TopologyManagerServiceTest : public Testing::BaseIntegrationTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("TopologyManager.log", x::LogLevel::LOG_DEBUG);
        x_DEBUG("Setup x TopologyManagerService test class.");
    }

    /* Will be called before a test is executed. */
    void SetUp() override {
        Testing::BaseIntegrationTest::SetUp();
        x_DEBUG("Setup x TopologyManagerService test case.");
        x_DEBUG("FINISHED ADDING 5 Serialization to topology");
