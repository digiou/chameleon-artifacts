...
class LocationServiceTest : public Testing::BaseIntegrationTest {
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("LocationServiceTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Set up LocationServiceTest test class.")
    ...
    static void TearDownTestCase(){x_INFO("Tear down LocationServiceTest test class")}

    x::LocationServicePtr locationService;
    x::TopologyManagerServicePtr topologyManagerService;
};
...
TEST_F(LocationServiceTest, testRequestSingleNodeLocation) {
    ...
    topologyManagerService = std::make_shared<x::TopologyManagerService>(topology, locationIndex);
    ...
    //create mobile node
    properties[x::Worker::Configuration::SPATIAL_SUPPORT] = x::Spatial::Experimental::SpatialType::MOBILE_NODE;
...
TEST_F(LocationServiceTest, testRequestAllMobileNodeLocations) {
    ...
    //verify correctxs of the node information
    auto entry = nodes[0];
    ...
    //verify correctxs of the provided edge
