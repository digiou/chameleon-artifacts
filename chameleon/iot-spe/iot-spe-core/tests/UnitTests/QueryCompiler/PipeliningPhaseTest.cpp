...
class PipeliningPhaseTest : public Testing::BaseUnitTest {
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("PipeliningPhaseTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup PipeliningPhaseTest test class.");
    }
...
TEST_F(PipeliningPhaseTest, pipelineFilterQuery) {
    ...
    auto sourcePipelix = pipelinePlan->getSourcePipelix();
    ASSERT_EQ(sourcePipelix.size(), 1u);
    auto sourcePipeline = sourcePipelix[0];
...
TEST_F(PipeliningPhaseTest, pipelineMultiplexQuery) {
    ...
    auto sourcePipelix = pipelinePlan->getSourcePipelix();
    ASSERT_EQ(sourcePipelix.size(), 2U);
    auto sourcePipeline1 = sourcePipelix[0];

    ASSERT_INSTANCE_OF(sourcePipeline1->getQueryPlan()->getRootOperators()[0], PhysicalSourceOperator);
    auto sourcePipeline2 = sourcePipelix[1];
...
TEST_F(PipeliningPhaseTest, pipelineJoinQuery) {
    ...
    // check if both join pipelix have the same successor
...
TEST_F(PipeliningPhaseTest, pipelineJoinWithMultiplexQuery) {
    ...
    ASSERT_EQ(sourcePipelix.size(), 3U);
    auto sourcePipeline1 = sourcePipelix[0];
    auto sourcePipeline2 = sourcePipelix[1];
    auto sourcePipeline3 = sourcePipelix[2];
    ASSERT_INSTANCE_OF(sourcePipeline1->getQueryPlan()->getRootOperators()[0], PhysicalSourceOperator);
    ASSERT_INSTANCE_OF(sourcePipeline2->getQueryPlan()->getRootOperators()[0], PhysicalSourceOperator);
