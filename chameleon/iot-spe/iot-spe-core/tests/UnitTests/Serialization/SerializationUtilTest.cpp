...
class SerializationUtilTest : public Testing::BaseUnitTest {
  ...
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("SerializationUtilTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup SerializationUtilTest test class.");
    }
};
...
TEST_F(SerializationUtilTest, schemaSerializationTestColumnLayout) {
    ...
    schema->addField("f2", DataTypeFactory::createInt32());
    schema->addField("f3", DataTypeFactory::createArray(42, DataTypeFactory::createInt8()));
    schema->setLayoutType(x::Schema::MemoryLayoutType::COLUMNAR_LAYOUT);

    auto serializedSchema = SchemaSerializationUtil::serializeSchema(schema, new SerializableSchema());
    auto deserializedSchema = SchemaSerializationUtil::deserializeSchema(*serializedSchema);
    EXPECT_TRUE(deserializedSchema->equals(schema));
    EXPECT_EQ(deserializedSchema->getLayoutType(), x::Schema::MemoryLayoutType::COLUMNAR_LAYOUT);
}
...
TEST_F(SerializationUtilTest, sourceDescriptorSerialization) {
...

    {
        ...
        Network::xPartition xPartition{1, 22, 33, 44};
        auto source =
            Network::NetworkSourceDescriptor::create(schema, xPartition, nodeLocation, NSOURCE_RETRY_WAIT, NSOURCE_RETRIES);
        SerializableOperator_SourceDetails sourceDetails;
        OperatorSerializationUtil::serializeSourceDescriptor(*source, sourceDetails);
        auto deserializedSourceDescriptor = OperatorSerializationUtil::deserializeSourceDescriptor(sourceDetails);
        EXPECT_TRUE(source->equal(deserializedSourceDescriptor));
...
TEST_F(SerializationUtilTest, sinkDescriptorSerialization) {
    ...
    {
        auto sink = FileSinkDescriptor::create("test", "x_FORMAT", "OVERWRITE");
        SerializableOperator_SinkDetails sinkDescriptor;
        OperatorSerializationUtil::serializeSinkDescriptor(*sink, sinkDescriptor, 0);
        auto deserializedSourceDescriptor = OperatorSerializationUtil::deserializeSinkDescriptor(sinkDescriptor);
        EXPECT_TRUE(sink->equal(deserializedSourceDescriptor));
...

    {
        ...
        Network::xPartition xPartition{1, 22, 33, 44};
        auto sink = Network::NetworkSinkDescriptor::create(nodeLocation, xPartition, std::chrono::seconds(1), 5);
        SerializableOperator_SinkDetails sinkDescriptor;
        OperatorSerializationUtil::serializeSinkDescriptor(*sink, sinkDescriptor, 0);
        auto deserializedSourceDescriptor = OperatorSerializationUtil::deserializeSinkDescriptor(sinkDescriptor);
        EXPECT_TRUE(sink->equal(deserializedSourceDescriptor));
...
TEST_F(SerializationUtilTest, operatorSerialization) {
    ...
    {
        auto javaUDFDescriptor = x::Catalogs::UDF::JavaUDFDescriptorBuilder::createDefaultJavaUDFDescriptor();
...
#ifdef TFDEF
    {
        auto f1_in = std::make_shared<ExpressionItem>(Attribute("f1", x::BasicType::FLOAT32));
        auto f2_in = std::make_shared<ExpressionItem>(Attribute("f2", x::BasicType::FLOAT32));
        auto f1_out = std::make_shared<ExpressionItem>(Attribute("f1_out", x::BasicType::FLOAT32));
        auto f2_out = std::make_shared<ExpressionItem>(Attribute("f2_out", x::BasicType::FLOAT32));
        auto f3_out = std::make_shared<ExpressionItem>(Attribute("f3_out", x::BasicType::FLOAT32));
...

    {
        ...
        Join::LogicalJoinDefinitionPtr joinDef = Join::LogicalJoinDefinition::create(
            ...
            1,
            1,
            x::Join::LogicalJoinDefinition::JoinType::INNER_JOIN);
...
TEST_F(SerializationUtilTest, testSerializeDeserializeCilentOriginatedQueryPlan) {
    ...
    // Expect that the id of operators in the deserialized query plan are different to the original query plan, because the initial IDs are client-generated and x should provide its own IDs
