...
class TwoPhaseLockingStorageHandlerTest : public Testing::BaseUnitTest {
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("TwoPhaseLockingStorageHandlerTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup TwoPhaseLockingAccessHandle test class.")
    }
};
...
TEST_F(TwoPhaseLockingStorageHandlerTest, TestNoDeadLock) {
    ...
    for (uint64_t i = 1; i < numThreads; ++i) {
        threads.emplace_back([i, &lockHolder, &resourceVector, &reverseResourceVector, twoPLAccessHandle]() {
            while (true) {//todo: insert timeout
                try {
                    if (i % 2 == 0) {
                        ...
                        x_TRACE("Previous lock holder {}", lockHolder)
                        lockHolder = i;
                        x_TRACE("Locked using resource vector in thread {}", i)
                    } else {
                        ...
                        x_TRACE("Previous lock holder {}", lockHolder)
                        lockHolder = i;
                        x_TRACE("Locked using reverse resource vector in thread {}", i)
                    }
            ...
            twoPLAccessHandle->releaseResources(i);
            x_DEBUG("Thread {} released all resources", i);
        });
    }
