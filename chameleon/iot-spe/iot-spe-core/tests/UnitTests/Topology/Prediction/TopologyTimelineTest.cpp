...
class TopologyTimelineTest : public Testing::BaseIntegrationTest {
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("TopologyTimelineTest.log", x::LogLevel::LOG_DEBUG);
        x_DEBUG("Setup TopologyTimeline test class.");
    }
    ...
    static void testTopologyEquality(const TopologyPtr& original, const TopologyPtr& copy) {
        ...
        while (!originalQueue.empty()) {
            ...
            x_DEBUG("checking node {}", originalNode->getId());
            auto copiedNode = copy->findNodeWithId(originalNode->getId());
            x_DEBUG("copied node id {}", copiedNode->getId());
...
TEST_F(TopologyTimelineTest, testUpdatingMultiplePredictions) {
    ...
    x_DEBUG("adding new prediction that node 3 will reconnect to node 4 at time 7");
    ...
    x_DEBUG("updating prediction that node 3 will reconnect to node 4 at time 4 instead of time 7");
    ...
    x_DEBUG("adding new prediction that node 4 will reconnect to node 1 at time 6");
    ...
    x_DEBUG("adding prediction that a new node with id 5 will connect to the system at time 5");
    Experimental::TopologyPrediction::TopologyDelta delta5({{5, 1}}, {});
    versionTimeline.addTopologyDelta(5, delta5);

    x_DEBUG("node 4 changes prediction that it will connect to node 5 at time 7 instead of to 1 at time 6");
    ...
    x_DEBUG("schedule 2 to connect to 5 at 7, the same time when 4 connects to 5");
