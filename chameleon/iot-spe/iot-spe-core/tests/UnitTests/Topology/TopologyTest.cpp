...
class TopologyTest : public Testing::BaseUnitTest {
  public:
    ...
    static void SetUpTestCase() { x::Logger::setupLogging("xTopologyManager.log", x::LogLevel::LOG_DEBUG); }
};
/* - Nodes ----------------------------------------------------------------- */
/**
 * Create a new node. 
...
TEST_F(TopologyTest, removeAnExistingNode) {
    ...
    grpcPort++;
    dataPort++;
    auto childNode = TopologyNode::create(node2Id, node2Address, grpcPort, dataPort, resources, properties);
...
TEST_F(TopologyTest, printGraph) {
    ...
    for (uint32_t i = 7; i < 23; ++i) {
        sensors.push_back(TopologyNode::create(i, "localhost", grpcPort, dataPort, resource, properties));
        grpcPort = grpcPort + 2;
        dataPort = dataPort + 2;
    }
