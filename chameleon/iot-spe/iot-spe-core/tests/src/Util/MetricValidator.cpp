...
bool MetricValidator::isValid(Monitoring::AbstractSystemResourcesReaderPtr reader, Monitoring::RuntimeMetrics metrics) {
    ...
    if (reader->getReaderType() == SystemResourcesReaderType::AbstractReader) {
        x_WARNING("MetricValidator: AbstractReader used for RuntimeMetrics. Returning true");
        return true;
    }

    if (metrics.wallTimeNs <= 0) {
        x_ERROR("MetricValidator: Wrong wallTimeNs.");
        check = false;
    }
    if (!(metrics.blkioBytesWritten >= 0)) {
        x_ERROR("MetricValidator: Wrong blkioBytesWritten.");
        check = false;
    }
    if (!(metrics.blkioBytesRead >= 0)) {
        x_ERROR("MetricValidator: Wrong blkioBytesRead.");
        check = false;
    }
    if (!(metrics.memoryUsageInBytes >= 0)) {
        x_ERROR("MetricValidator: Wrong memoryUsageInBytes.");
        check = false;
    }
    if (!(metrics.cpuLoadInJiffies >= 0)) {
        x_ERROR("MetricValidator: Wrong cpuLoadInJiffies.");
        check = false;
    }
    if (!(metrics.longCoord >= 0)) {
        x_ERROR("MetricValidator: Wrong longCoord.");
        check = false;
    }
    if (!(metrics.latCoord >= 0)) {
        x_ERROR("MetricValidator: Wrong latCoord.");
        check = false;
    }
    if (!(metrics.batteryStatusInPercent >= 0)) {
        x_ERROR("MetricValidator: Wrong batteryStatusInPercent.");
        check = false;
    }
    return check;
};
...
bool MetricValidator::isValid(Monitoring::AbstractSystemResourcesReaderPtr reader, Monitoring::RegistrationMetrics metrics) {
    ...
    if (reader->getReaderType() == SystemResourcesReaderType::AbstractReader) {
        x_WARNING("MetricValidator: AbstractReader used for NodeRegistrationMetrics. Returning true");
        return true;
    }

    if (!(metrics.cpuQuotaUS >= -1)) {
        x_ERROR("MetricValidator: Wrong cpuQuotaUS.");
        check = false;
    }
    if (!(metrics.cpuPeriodUS >= -1)) {
        x_ERROR("MetricValidator: Wrong cpuPeriodUS.");
        check = false;
    }
    if (!(metrics.totalCPUJiffies > 0)) {
        x_ERROR("MetricValidator: Wrong totalCPUJiffies.");
        check = false;
    }
    if (!(metrics.cpuCoreNum > 0)) {
        x_ERROR("MetricValidator: Wrong cpuCoreNum.");
        check = false;
    }
    if (!(metrics.totalMemoryBytes >= 0)) {
        x_ERROR("MetricValidator: Wrong totalMemoryBytes.");
        check = false;
    }
    if (metrics.hasBattery) {
        x_ERROR("MetricValidator: Wrong hasBattery.");
        check = false;
    }
    if (metrics.isMoving) {
        x_ERROR("MetricValidator: Wrong isMoving.");
        check = false;
    }
    return check;
};
...
bool MetricValidator::isValid(Monitoring::AbstractSystemResourcesReaderPtr reader, Monitoring::CpuMetricsWrapper cpuMetrics) {
    ...
    if (reader->getReaderType() == SystemResourcesReaderType::AbstractReader) {
        x_WARNING("MetricValidator: AbstractReader used for CpuMetricsWrapper. Returning true");
        return true;
    }

    if (cpuMetrics.size() <= 0) {
        x_ERROR("MetricValidator: Wrong getNumCores.");
        check = false;
    }
    for (uint64_t i = 0; i < cpuMetrics.size(); i++) {
        if (!(cpuMetrics.getValue(i).user > 0)) {
            x_ERROR("MetricValidator: Wrong cpuMetrics.getValue(i).user.");
            check = false;
        }
    }
    if (!(cpuMetrics.getTotal().user > 0)) {
        x_ERROR("MetricValidator: Wrong cpuMetrics.getTotal().user.");
        check = false;
    }
    return check;
};
...
bool MetricValidator::isValid(Monitoring::AbstractSystemResourcesReaderPtr reader,
    ...
    if (reader->getReaderType() == SystemResourcesReaderType::AbstractReader) {
        x_WARNING("MetricValidator: AbstractReader used for NetworkMetricsWrapper. Returning true");
        return true;
    }

    if (networkMetrics.getInterfaceNames().empty()) {
        x_ERROR("MetricValidator: Wrong getInterfaceNames().");
        check = false;
    }
    return check;
};
...
bool MetricValidator::isValid(Monitoring::AbstractSystemResourcesReaderPtr reader, Monitoring::MemoryMetrics memoryMetrics) {
    ...
    if (reader->getReaderType() == SystemResourcesReaderType::AbstractReader) {
        x_WARNING("MetricValidator: AbstractReader used for MemoryMetrics. Returning true");
        return true;
    }

    if (!(memoryMetrics.FREE_RAM > 0)) {
        x_ERROR("MetricValidator: Wrong FREE_RAM.");
        check = false;
    }
    return check;
};
...
bool MetricValidator::isValid(Monitoring::AbstractSystemResourcesReaderPtr reader, Monitoring::DiskMetrics diskMetrics) {
    ...
    if (reader->getReaderType() == SystemResourcesReaderType::AbstractReader) {
        x_WARNING("MetricValidator: AbstractReader used for DiskMetrics. Returning true");
        return true;
    }

    if (!(diskMetrics.fBavail >= 0)) {
        x_ERROR("MetricValidator: Wrong fBavail.");
        check = false;
    }
    return check;
}
...
bool MetricValidator::isValid(Monitoring::AbstractSystemResourcesReaderPtr reader,
    ...
    if (!storedMetrics->contains(expectedType)) {
        x_ERROR("MetricValidator: Metrics for node {} are missing type {}", expectedNodeId, toString(expectedType));
        return false;
    }
    ...
    x_INFO("MetricValidator: Stored metrics for ID {}: {}",
             expectedNodeId,
             Monitoring::MetricUtils::toJson(storedMetrics).dump());
    if (retMetric->getMetricType() != expectedType) {
        x_ERROR("MetricValidator: MetricType is not as expected {} != {}",
    ...
    if (storedMetrics->size() != expectedSize) {
        x_ERROR("MetricValidator: Stored metrics do not match expected size {} != {}", storedMetrics->size(), expectedSize);
        check = false;
    }
    return check;
}
...
bool MetricValidator::isValidAll(Monitoring::AbstractSystemResourcesReaderPtr reader, nlohmann::json json) {
    ...
    if (reader->getReaderType() == SystemResourcesReaderType::AbstractReader) {
        x_WARNING("MetricValidator: AbstractReader used. Returning true");
        return true;
    }

    if (!json.contains(std::string(magic_enum::enum_name(Monitoring::MetricType::RegistrationMetric)))) {
        x_ERROR("MetricValidator: Missing field registration");
        check = false;
    } else {
...
bool MetricValidator::isValidRegistrationMetrics(Monitoring::AbstractSystemResourcesReaderPtr reader, nlohmann::json json) {
    ...
    if (reader->getReaderType() == SystemResourcesReaderType::AbstractReader) {
        x_WARNING("MetricValidator: AbstractReader used for DiskMetrics.");
        auto numFields = json.size();
        if (numFields != Monitoring::RegistrationMetrics::getSchema("")->getSize()) {
            x_ERROR("MetricValidator: Entries for registration metrics missing");
            return false;
        }
        return true;
    }
    ...
    if (!(json.contains("TotalMemory"))) {
        x_ERROR("MetricValidator: Wrong TotalMemory.");
        check = false;
    }
...
bool MetricValidator::checkNodeIds(nlohmann::json json, uint64_t nodeId) {
    ...
    for (auto& [key, val] : json.items()) {
        if (json[key].contains("NODE_ID") && json[key]["NODE_ID"] != nodeId) {
            x_ERROR("MetricValidator: Wrong node ID for {} where {} != {}", key, json[key]["NODE_ID"], nodeId);
            check = false;
        }
    }
    return check;
...
bool MetricValidator::MetricValidator::checkEntriesOfStream(std::set<std::string> setOfStr, nlohmann::json jsons) {
    ...
    for (auto elem : setOfStr) {
        ...
        for (int i = 0; i < static_cast<int>(jsons.size()); i++) {
            auto json = jsons[i];
            x_DEBUG("Json Values of {}. entry: {}", std::to_string(i), json.dump());
            for (auto& [key, val] : json.items()) {
...
bool MetricValidator::waitForMonitoringStreamsOrTimeout(const std::set<Monitoring::MetricType>& monitoringStreams,
    ...
    for (int i = 0; i < maxTimeout; i++) {
        ...
        x_INFO("MetricValidator: Testing metric call {}", json.dump());
        bool missing = false;
        for (uint64_t nodeId = 1; nodeId <= json.size(); nodeId++) {
            for (auto stream : monitoringStreams) {
                ...
                if (!json[std::to_string(nodeId)].contains(strStream)) {
                    x_ERROR("MetricValidator: Missing metric {} for node {} ", strStream, nodeId);
                    missing = true;
                    break;
                }
            }
        ...
        if (missing) {
            ...
            x_INFO("MetricValidator: All metrics available");
            return true;
        }
    }
    x_ERROR("MetricValidator: Timeout passed");
    return false;
}
