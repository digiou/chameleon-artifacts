...
bool TestSink::writeData(Runtime::TupleBuffer& inputBuffer, Runtime::WorkerContext&) {
    ...
    if (resultBuffers.size() < numExpectedResultBuffers) {
        x_DEBUG("Already saw {} buffers and expects a total of {}", resultBuffers.size(), numExpectedResultBuffers);
    } else if (resultBuffers.size() == numExpectedResultBuffers) {
        completed.set_value(numExpectedResultBuffers);
    } else if (resultBuffers.size() > numExpectedResultBuffers) {
        x_ERROR("result buffer size {} and expected buffer={} do not match", resultBuffers.size(), numExpectedResultBuffers);
        EXPECT_TRUE(false);
    }
    return true;
}
...
void TestSink::cleanupBuffers() {
    x_DEBUG("TestSink: cleanupBuffers()");
    std::unique_lock lock(m);
    resultBuffers.clear();
}
