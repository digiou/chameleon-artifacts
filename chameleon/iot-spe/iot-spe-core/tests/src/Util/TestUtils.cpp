...
[[nodiscard]] Util::Subprocess startCoordinator(std::initializer_list<std::string> list) {
    ...
    return {std::string(PATH_TO_BINARY_DIR) + "/x-core/xCoordinator", list};
}
...
[[nodiscard]] Util::Subprocess startWorker(std::initializer_list<std::string> flags) {
    x_INFO("Start worker");
    return {std::string(PATH_TO_BINARY_DIR) + "/x-core/xWorker", flags};
}
...
[[nodiscard]] std::shared_ptr<Util::Subprocess> startWorkerPtr(std::initializer_list<std::string> flags) {
    x_INFO("Start worker");
    return std::make_shared<Util::Subprocess>(std::string(PATH_TO_BINARY_DIR) + "/x-core/xWorker", flags);
}
...
[[nodiscard]] bool checkCompleteOrTimeout(const Runtime::NodeEnginePtr& ptr, QueryId queryId, uint64_t expectedResult) {
    if (ptr->getQueryStatistics(queryId).empty()) {
        x_ERROR("checkCompleteOrTimeout query does not exists");
        return false;
    }
    auto timeoutInSec = std::chrono::seconds(defaultTimeout);
    auto start_timestamp = std::chrono::system_clock::now();
    while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
        x_TRACE("checkCompleteOrTimeout: check result NodeEnginePtr");
        ...
        if (ptr->getQueryStatistics(queryId)[0]->getProcessedBuffers() == expectedResult
            ...
            x_TRACE("checkCompleteOrTimeout: NodeEnginePtr results are correct");
            return true;
        }
        x_TRACE("checkCompleteOrTimeout: NodeEnginePtr sleep because val={} < {}",
                  ptr->getQueryStatistics(queryId)[0]->getProcessedTuple(),
                  expectedResult);
        std::this_thread::sleep_for(sleepDuration);
    }
    x_TRACE("checkCompleteOrTimeout: NodeEnginePtr expected results are not reached after timeout");
    return false;
}
...
waitForQueryToStart(QueryId queryId, const QueryCatalogServicePtr& queryCatalogService, std::chrono::seconds timeoutInSec) {
    x_TRACE("TestUtils: wait till the query {} gets into Running status.", queryId);
    auto start_timestamp = std::chrono::system_clock::now();

    x_TRACE("TestUtils: Keep checking the status of query {} until a fixed time out", queryId);
    while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
        auto queryCatalogEntry = queryCatalogService->getEntryForQuery(queryId);
        if (!queryCatalogEntry) {
            x_ERROR("TestUtils: unable to find the entry for query {} in the query catalog.", queryId);
            return false;
        }
        x_TRACE("TestUtils: Query {} is now in status {}", queryId, queryCatalogEntry->getQueryStatusAsString());
        ...
        switch (queryState) {
            ...
            case QueryState::FAILED: {
                x_ERROR("Query failed to start. Expected: Running or Optimizing but found {}",
                          magic_enum::enum_name(queryState));
                return false;
            }
            default: {
                x_WARNING("Expected: Running or Scheduling but found {}", magic_enum::enum_name(queryState));
                break;
            }
        }
    ...
    x_TRACE("checkCompleteOrTimeout: waitForStart expected results are not reached after timeout");
    return false;
}
...
checkFailedOrTimeout(QueryId queryId, const QueryCatalogServicePtr& queryCatalogService, std::chrono::seconds timeout) {
    ...
    auto start_timestamp = std::chrono::system_clock::now();
    while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
        x_TRACE("checkFailedOrTimeout: check query status");
        if (queryCatalogService->getEntryForQuery(queryId)->getQueryState() == QueryState::FAILED) {
            x_DEBUG("checkFailedOrTimeout: status reached stopped");
            return true;
        }
        x_TRACE("checkFailedOrTimeout: status not reached as status is={}",
                  queryCatalogService->getEntryForQuery(queryId)->getQueryStatusAsString());
        std::this_thread::sleep_for(sleepDuration);
    }
    x_WARNING("checkStoppedOrTimeout: expected status not reached within set timeout");
    return false;
}
...
[[nodiscard]] bool checkOutputOrTimeout(string expectedContent, const string& outputFilePath, uint64_t customTimeout) {
    ...
    x_TRACE("using timeout={}", timeoutInSec.count());
    auto start_timestamp = std::chrono::system_clock::now();
    uint64_t found = 0;
    uint64_t count = 0;
    while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
        ...
        found = 0;
        count = 0;
        x_TRACE("checkOutputOrTimeout: check content for file {}", outputFilePath);
        std::ifstream ifs(outputFilePath);
        if (ifs.good() && ifs.is_open()) {
            std::vector<std::string> expectedlix = x::Util::splitWithStringDelimiter<std::string>(expectedContent, "\n");
            std::string content((std::istreambuf_iterator<char>(ifs)), (std::istreambuf_iterator<char>()));
            count = std::count(content.begin(), content.end(), '\n');
            if (expectedlix.size() != count) {
                x_TRACE("checkoutputortimeout: number of expected lix {} not reached yet with {} lix content={} file={}",
                          expectedlix.size(),
                          count,
                          content,
                          outputFilePath);
            ...
            if (content.size() != expectedContent.size()) {
                x_TRACE("checkoutputortimeout: number of chars {} not reached yet with chars content={} lix content={}",
            ...
            for (auto& expectedline : expectedlix) {
                if (content.find(expectedline) != std::string::npos) {
                    found++;
                }
            }
            if (found == count) {
                x_TRACE("all lix found final content={}", content);
                return true;
            }
            x_TRACE("only {} lix found final content={}", found, content);
        }
    }
    x_ERROR("checkOutputOrTimeout: expected ({}) result not reached ({}) within set timeout content", count, found);
    return false;
}
...
/**
   ...
   * @return true if successful
   */
[[nodiscard]] bool checkIfOutputFileIsNotEmtpy(uint64_t minNumberOfLix, const string& outputFilePath, uint64_t customTimeout) {
    std::chrono::seconds timeoutInSec;
    if (customTimeout == 0) {
        timeoutInSec = std::chrono::seconds(defaultTimeout);
    } else {
    ...
    while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
        std::this_thread::sleep_for(sleepDuration);
        count = 0;
        x_TRACE("checkIfOutputFileIsNotEmtpy: check content for file {}", outputFilePath);
        std::ifstream ifs(outputFilePath);
        if (ifs.good() && ifs.is_open()) {
            std::string content((std::istreambuf_iterator<char>(ifs)), (std::istreambuf_iterator<char>()));
            count = std::count(content.begin(), content.end(), '\n');
            if (count < minNumberOfLix) {
                x_TRACE("checkIfOutputFileIsNotEmtpy: number of min lix {} not reached yet with {} lix content={}",
                          minNumberOfLix,
                          count,
                          content);
                continue;
            }
            x_TRACE("at least {} are found in content={}", minNumberOfLix, content);
            return true;
        }
    }
    x_ERROR("checkIfOutputFileIsNotEmtpy: expected ({}) result not reached ({}) within set timeout content",
              count,
              minNumberOfLix);
    return false;
}
...
bool checkIfBuffersContainTheSameTuples(Runtime::TupleBuffer buffer1, Runtime::TupleBuffer buffer2, uint64_t schemaSizeInByte) {
    x_DEBUG("Checking if the buffers contain the same tuples...");
    if (buffer1.getNumberOfTuples() != buffer2.getNumberOfTuples()) {
        x_DEBUG("Buffers do not contain exactly the same tuples, as they do not have the same number of tuples");
        return false;
    }
    ...
    for (auto idxBuffer1 = 0UL; idxBuffer1 < buffer1.getNumberOfTuples(); ++idxBuffer1) {
        ...
        if (!idxFoundInBuffer2) {
            x_DEBUG("Buffers do not contain the same tuples, as tuple could not be found in both buffers for idx: {}",
                      idxBuffer1);
            return false;
        }
    }
...
[[nodiscard]] bool checkFileCreationOrTimeout(const string& outputFilePath) {
    ...
    while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
        std::this_thread::sleep_for(sleepDuration);
        x_TRACE("checkFileCreationOrTimeout: for file {}", outputFilePath);
        std::ifstream ifs(outputFilePath);
        if (ifs.good() && ifs.is_open()) {
            return true;
        }
    }
    x_TRACE("checkFileCreationOrTimeout: expected result not reached within set timeout");
    return false;
}
...
[[nodiscard]] bool checkRESTServerStartedOrTimeout(uint64_t restPort, uint64_t customTimeout) {
    ...
    while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
        std::this_thread::sleep_for(sleepDuration);
        x_INFO("check if x REST interface is up");
        auto future =
            cpr::GetAsync(cpr::Url{BASE_URL + std::to_string(restPort) + "/v1/x/connectivity/check"}, cpr::Timeout{3000});
        future.wait();
...
[[nodiscard]] bool checkCompleteOrTimeout(QueryId queryId, uint64_t expectedNumberBuffers, const std::string& restPort) {
    ...
    x_DEBUG("checkCompleteOrTimeout: Check if the query goes into the Running status within the timeout");
    while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
        std::string url = "http://localhost:" + restPort + "/v1/x/queryCatalog/status";
        ...
        x_DEBUG("checkCompleteOrTimeout: sleep because current status ={}", currentStatus);
        std::this_thread::sleep_for(std::chrono::milliseconds(sleepDuration));
    }
    x_DEBUG("checkCompleteOrTimeout: end with status ={}", currentStatus);

    while (std::chrono::system_clock::now() < start_timestamp + timeoutInSec) {
        x_DEBUG("checkCompleteOrTimeout: check result NodeEnginePtr");

        std::string url = "http://localhost:" + restPort + "/v1/x/queryCatalog/getNumberOfProducedBuffers";
        nlohmann::json jsonReturn;
        auto future = cpr::GetAsync(cpr::Url{url}, cpr::Parameters{{"queryId", std::to_string(queryId)}});
        future.wait();
        auto response = future.get();
        ...
        if (response.status_code == cpr::status::HTTP_OK && result.contains("producedBuffers")) {
            ...
            if (currentResult >= expectedNumberBuffers) {
                x_DEBUG("checkCompleteOrTimeout: results are correct");
                return true;
            }
        }
        x_DEBUG("checkCompleteOrTimeout: sleep because val={} < {}", currentResult, expectedNumberBuffers);
        std::this_thread::sleep_for(std::chrono::milliseconds(sleepDuration));
    }
    x_DEBUG("checkCompleteOrTimeout: QueryId expected results are not reached after timeout currentResult={}"
...
[[nodiscard]] bool checkRunningOrTimeout(QueryId queryId, const std::string& restPort) {
    ...
    x_DEBUG("checkCompleteOrTimeout: QueryId expected results are not reached after timeout");
    return false;
}
...
[[nodiscard]] bool stopQueryViaRest(QueryId queryId, const std::string& restPort) {
    ...
    std::string url = BASE_URL + restPort + "/v1/x/query/stop-query";
    nlohmann::json jsonReturn;
    auto future = cpr::DeleteAsync(cpr::Url{url}, cpr::Parameters{{"queryId", std::to_string(queryId)}});
    future.wait();
    auto response = future.get();
    nlohmann::json result = nlohmann::json::parse(response.text);
    x_DEBUG("stopQueryViaRest: status ={}", result.dump());
...
[[nodiscard]] nlohmann::json startQueryViaRest(const string& queryString, const std::string& restPort) {
    ...
    std::string url = BASE_URL + restPort + "/v1/x/query/execute-query";
    nlohmann::json jsonReturn;
    auto future = cpr::PostAsync(cpr::Url{url}, cpr::Header{{"Content-Type", "application/json"}}, cpr::Body{queryString});
    future.wait();
    auto response = future.get();
    nlohmann::json result = nlohmann::json::parse(response.text);
    x_DEBUG("startQueryViaRest: status ={}", result.dump());
...
[[nodiscard]] nlohmann::json makeMonitoringRestCall(const string& restCall, const std::string& restPort) {
    std::string url = BASE_URL + restPort + "/v1/x/monitoring/" + restCall;
    auto future = cpr::GetAsync(cpr::Url{url});
    future.wait();
    auto response = future.get();
    nlohmann::json result = nlohmann::json::parse(response.text);
    x_DEBUG("getAllMonitoringMetricsViaRest: status ={}", result.dump());
...
[[nodiscard]] bool addLogicalSource(const string& schemaString, const std::string& restPort) {
    ...
    std::string url = BASE_URL + restPort + "/v1/x/sourceCatalog/addLogicalSource";
    ...
    x_DEBUG("addLogicalSource: status ={}", jsonResponse.dump());
    return jsonResponse["success"].get<bool>();
}

bool waitForWorkers(uint64_t restPort, uint16_t maxTimeout, uint16_t expectedWorkers) {
    auto baseUri = "http://localhost:" + std::to_string(restPort) + "/v1/x/topology";
    x_INFO("TestUtil: Executing GET request on URI {}", baseUri);
    nlohmann::json json_return;
    size_t nodeNo = 0;
    ...
    for (int i = 0; i < maxTimeout; i++) {
        try {
            ...
            if (nodeNo == expectedWorkers + 1U) {
                x_INFO("TestUtils: Expected worker number reached correctly {}", expectedWorkers);
                x_DEBUG("TestUtils: Received topology JSON:\n{}", jsonResponse.dump());
                return true;
            }
            std::this_thread::sleep_for(std::chrono::milliseconds(sleepDuration));
        } catch (const std::exception& e) {
            x_ERROR("TestUtils: WaitForWorkers error occured {}", e.what());
            std::this_thread::sleep_for(std::chrono::milliseconds(sleepDuration));
        }
    }

    x_ERROR("E2ECoordinatorMultiWorkerTest: Expected worker number not reached correctly {} but expected {}",
              nodeNo,
              expectedWorkers);
    return false;
}
...
Runtime::TupleBuffer mergeBuffers(std::vector<Runtime::TupleBuffer>& buffersToBeMerged,
    ...
    for (auto& buffer : buffersToBeMerged) {
        ...
        if (cnt > maxPossibleTuples) {
            x_WARNING("Too many tuples to fit in a single buffer.");
            return retBuffer;
        }
 ...
 */
std::vector<x::Spatial::DataTypes::Experimental::Waypoint> getWaypointsFromCsv(const std::string& csvPath,
                                                                                 Timestamp startTime) {
    std::vector<x::Spatial::DataTypes::Experimental::Waypoint> waypoints;
    std::string csvLine;
    ...
    x_DEBUG("Creating list of waypoints with startTime {}", startTime)
    ...
    while (std::getline(inputStream, csvLine)) {
        ...
        x_TRACE("Read from csv: {}, {}, {}", latitudeString, longitudeString, time);
        ...
        waypoints.push_back(x::Spatial::DataTypes::Experimental::Waypoint(
            x::Spatial::DataTypes::Experimental::GeoLocation(std::stod(latitudeString), std::stod(longitudeString)),
            time));
    }
    return waypoints;
}
...
std::vector<Runtime::TupleBuffer> TestUtils::fillBufferFromCsv(const std::string& csvFileName,
    ...
    x_DEBUG("read file={}", fullPath);
    x_ASSERT2_FMT(std::filesystem::exists(std::filesystem::path(fullPath)), "File " << fullPath << " does not exist!!!");
