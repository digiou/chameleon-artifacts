...
# command line arguments to determine the number of runs, starting ID, number of lix
...
if [ $# -ne 6 ]; then
  echo "Usage: $0 <num_runs> <id_start> <num_lix_in_group> <num_groups_to_write> <group_num_start> <output_file_prefix>"
  exit 1
fi
...
num_lix_in_group=$3        # Number of lix in each group
...
for i in $(seq $id_start $(($id_start + $num_runs - 1))); do
  ...
  for j in $(seq 1 $num_groups_to_write); do
    for k in $(seq 1 $num_lix_in_group); do
