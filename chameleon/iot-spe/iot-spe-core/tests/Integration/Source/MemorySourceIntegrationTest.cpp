...
class MemorySourceIntegrationTest : public Testing::BaseIntegrationTest {
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("MemorySourceIntegrationTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup MemorySourceIntegrationTest test class.");
    }
};
...
TEST_F(MemorySourceIntegrationTest, testMemorySource) {
    ...
    x_INFO("MemorySourceIntegrationTest: Start coordinator");
    xCoordinatorPtr crd = std::make_shared<xCoordinator>(coordinatorConfig);
    uint64_t port = crd->startCoordinator(/**blocking**/ false);
    EXPECT_NE(port, 0UL);
    x_INFO("MemorySourceIntegrationTest: Coordinator started successfully");
    ...
    x_INFO("MemorySourceIntegrationTest: Start worker 1");
    ...
    x_INFO("MemorySourceIntegrationTest: Worker1 started successfully");
    ...
    x_INFO("MemorySourceIntegrationTest: Remove query");
    ...
    //    x_INFO("MemorySourceIntegrationTest: content={}", content);
...
TEST_F(MemorySourceIntegrationTest, DISABLED_testMemorySourceHalfFullBuffer) {
    ...
    while (std::getline(infile, line)) {
        if (lineCnt > 0) {
            std::string expectedString = std::to_string(lineCnt - 1) + "," + std::to_string(lineCnt - 1);
            x_DEBUG(" line={} expected={}", line, expectedString);
            ASSERT_EQ(line, expectedString);
        }
        lineCnt++;
    }
