...
class ArrowSourceIntegrationTest : public Testing::BaseIntegrationTest {
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("ArrowSourceIntegrationTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup ArrowSourceIntegrationTest test class.");
    }
};

/// This test reads from a arrow source which contains all data types compatible with x and writes
/// them to a csv sink
TEST_F(ArrowSourceIntegrationTest, testArrowSourceWithMultipleDatatypes) {
    ...
    x_INFO("ArrowIntegrationTest: Start coordinator");
    ...
    EXPECT_NE(port, 0UL);
    x_DEBUG("ArrowIntegrationTest: Coordinator started successfully");
    x_DEBUG("ArrowIntegrationTest: Start worker 1");
    ...
    x_INFO("ArrowIntegrationTest: Worker1 started successfully");
    ...
    x_INFO("ArrowSourceIntegrationTest: Remove query");
    ...
    x_INFO("ArrowSourceIntegrationTest: content={}", content);
    x_INFO("ArrowSourceIntegrationTest: expContent={}", expectedContent);
