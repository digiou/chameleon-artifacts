...
class ComplexSequenceTest : public Testing::BaseIntegrationTest {
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("ComplexSequenceTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup ComplexSequenceTest test class.");
    }
};
...
TEST_F(ComplexSequenceTest, DISABLED_complexTestSingleNodeSingleWindowSingleJoin) {
    ...
    auto testHarxs = TestHarxs(queryWithJoinOperator, *restPort, *rpcCoordinatorPort, getTestResourceFolder())
...
TEST_F(ComplexSequenceTest, complexTestDistributedNodeSingleWindowSingleJoin) {
    ...
    ASSERT_EQ(testHarxs.getTopology()->getRoot()->getChildren().size(), 2ULL);
    ASSERT_EQ(testHarxs.getTopology()->getRoot()->getChildren()[0]->getChildren().size(), 1ULL);
    ASSERT_EQ(testHarxs.getTopology()->getRoot()->getChildren()[1]->getChildren().size(), 1ULL);
...
TEST_F(ComplexSequenceTest, DISABLED_ComplexTestSingleNodeMultipleWindowsMultipleJoins) {
    ...
    auto testHarxs = TestHarxs(queryWithJoinAndWindowOperator, *restPort, *rpcCoordinatorPort, getTestResourceFolder())
                           .addLogicalSource("window1", window1Schema)
                           .addLogicalSource("window2", window2Schema)
    ...
    std::vector<Output> actualOutput = testHarxs.getOutput<Output>(expectedOutput.size(), "TopDown", "NONE", "IN_MEMORY");
...
TEST_F(ComplexSequenceTest, DISABLED_complexTestDistributedNodeMultipleWindowsMultipleJoinsWithTopDown) {
    ...
    ASSERT_EQ(testHarxs.getWorkerCount(), 6U);

    testHarxs.getTopology()->print();

    // Check if the topology matches the expected hierarchy
    ASSERT_EQ(testHarxs.getTopology()->getRoot()->getChildren().size(), 3U);
    ASSERT_EQ(testHarxs.getTopology()->getRoot()->getChildren()[0]->getChildren().size(), 1U);
    ASSERT_EQ(testHarxs.getTopology()->getRoot()->getChildren()[1]->getChildren().size(), 1U);
    ASSERT_EQ(testHarxs.getTopology()->getRoot()->getChildren()[2]->getChildren().size(), 1U);
...
TEST_F(ComplexSequenceTest, complexTestDistributedNodeMultipleWindowsMultipleJoinsWithBottomUp) {
    ...
    ASSERT_EQ(testHarxs.getWorkerCount(), 5ULL);

    testHarxs.getTopology()->print();

    // Check if the topology matches the expected hierarchy
    ASSERT_EQ(testHarxs.getTopology()->getRoot()->getChildren().size(), 1U);
    ASSERT_EQ(testHarxs.getTopology()->getRoot()->getChildren()[0]->getChildren().size(), 2U);
    ASSERT_EQ(testHarxs.getTopology()->getRoot()->getChildren()[0]->getChildren()[0]->getChildren().size(), 2U);
    ASSERT_EQ(testHarxs.getTopology()->getRoot()->getChildren()[0]->getChildren()[1]->getChildren().size(), 0U);
