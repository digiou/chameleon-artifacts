...
class OrOperatorTest : public Testing::BaseIntegrationTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("OrOperatorTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup OrOperatorTest test class.");
    }
...
TEST_F(OrOperatorTest, testPatternOneOr) {
    ...
    xCoordinatorPtr crd = std::make_shared<xCoordinator>(coordinatorConfiguration);
    uint64_t port = crd->startCoordinator(/**blocking**/ false);
    EXPECT_NE(port, 0UL);
    x_INFO("OrOperatorTest: Coordinator started successfully");
    ...
    x_INFO("OrOperatorTest: Start worker 1 with physical source");
    ...
    x_INFO("OrOperatorTest: Worker1 started successfully");

    x_INFO("OrOperatorTest: Start worker 2 with physical source");
    ...
    xWorkerPtr wrk2 = std::make_shared<xWorker>(std::move(worker2Configuration));
    bool retStart2 = wrk2->start(/**blocking**/ false, /**withConnect**/ true);
    EXPECT_TRUE(retStart2);
    x_INFO("OrOperatorTest: Worker2 started successfully");

    std::string outputFilePath = getTestResourceFolder() / "testPatternOr1.out";
    remove(outputFilePath.c_str());

    x_INFO("OrOperatorTest: Submit orWith pattern");
    ...
    x_INFO("OrOperatorTest: Remove query");
    ...
    x_DEBUG("contents={}", content);
    size_t n = std::count(content.begin(), content.end(), '\n');
    x_DEBUG("TUPLE NUMBER={}", n);
...
TEST_F(OrOperatorTest, testPatternOrMap) {
    ...
    x_INFO("OrOperatorTest: Submit andWith pattern");
...
TEST_F(OrOperatorTest, DISABLED_testPatternMultiOr) {
    ...
    x_INFO("OrOperatorTest: Start worker 3 with physical source");
    ...
    xWorkerPtr wrk3 = std::make_shared<xWorker>(std::move(worker3Configuration));
    bool retStart3 = wrk3->start(/**blocking**/ false, /**withConnect**/ true);
    EXPECT_TRUE(retStart3);
    x_INFO("OrOperatorTest: Worker3 started successfully");
...
TEST_F(OrOperatorTest, DISABLED_testOrPatternFilter) {
    ...
    crd->getSourceCatalogService()->registerLogicalSource("QnV2", qnv);
    x_INFO("SimplePatternTest: Coordinator started successfully");
    ...
    x_INFO("SimplePatternTest: Submit orWith pattern");
    ...
    x_DEBUG("OrOperatorTest: Result produced content {}", resultContent);
