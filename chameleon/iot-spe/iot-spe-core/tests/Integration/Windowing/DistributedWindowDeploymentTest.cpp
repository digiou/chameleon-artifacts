...
/**
 * @brief NOTE ALL TESTS IN THIS FILE ARE DISABLED and must migrated to the testHarxs
 */
class DistributedWindowDeploymentTest : public Testing::BaseIntegrationTest {
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("DistributedWindowDeploymentTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup DistributedWindowDeploymentTest test class.");
    }
};
...
TEST_F(DistributedWindowDeploymentTest,
    ...
    TestHarxs testHarxs = TestHarxs(query, *restPort, *rpcCoordinatorPort, getTestResourceFolder()).enableNautilus();
    ...
    testHarxs.addLogicalSource("window", testSchema)
    ...
    ASSERT_EQ(testHarxs.getWorkerCount(), 5UL);
...
TEST_F(DistributedWindowDeploymentTest, DISABLED_testDistributedTumblingWindowQueryEventTimeWithMergeAndComputeOnSameNodes) {
    ...
    x_INFO("DistributedWindowDeploymentTest: Start coordinator");
    ...
    EXPECT_NE(port, 0UL);
    x_DEBUG("DistributedWindowDeploymentTest: Coordinator started successfully");

    x_DEBUG("DistributedWindowDeploymentTest: Start worker 1");
    ...
    x_INFO("DistributedWindowDeploymentTest: Worker 1 started successfully");

    x_INFO("DistributedWindowDeploymentTest: Start worker 2");
    ...
    x_INFO("DistributedWindowDeploymentTest: Worker 2 started successfully");

    x_INFO("DistributedWindowDeploymentTest: Start worker 3");
    ...
    x_INFO("DistributedWindowDeploymentTest: Worker 3 started successfully");

    x_INFO("DistributedWindowDeploymentTest: Start worker 4");
    ...
    xWorkerPtr wrk4 = std::make_shared<xWorker>(std::move(workerConfig4));
    bool retStart4 = wrk4->start(/**blocking**/ false, /**withConnect**/ true);
    wrk4->replaceParent(1, 2);
    EXPECT_TRUE(retStart4);
    x_INFO("DistributedWindowDeploymentTest: Worker 2 started successfully");

    x_INFO("DistributedWindowDeploymentTest: Start worker 5");
    ...
    xWorkerPtr wrk5 = std::make_shared<xWorker>(std::move(workerConfig5));
    bool retStart5 = wrk5->start(/**blocking**/ false, /**withConnect**/ true);
    wrk5->replaceParent(1, 2);
    EXPECT_TRUE(retStart5);
    x_INFO("DistributedWindowDeploymentTest: Worker 6 started successfully");
    ...
    x_INFO("DistributedWindowDeploymentTest: Submit query");
    auto query = Query::from("window")
                     .window(TumblingWindow::of(EventTime(Attribute("ts")), Seconds(1)))
                     .byKey(Attribute("id"))
                     .apply(Sum(Attribute("value")))
    ...
    x_INFO("DistributedWindowDeploymentTest: Remove query");
    EXPECT_TRUE(TestUtils::checkStoppedOrTimeout(queryId, queryCatalogService));

    x_INFO("DistributedWindowDeploymentTest: Stop worker 1");
    bool retStopWrk1 = wrk1->stop(true);
    EXPECT_TRUE(retStopWrk1);

    x_INFO("DistributedWindowDeploymentTest: Stop worker 2");
    bool retStopWrk2 = wrk2->stop(true);
    EXPECT_TRUE(retStopWrk2);

    x_INFO("DistributedWindowDeploymentTest: Stop worker 3");
    bool retStopWrk3 = wrk3->stop(true);
    EXPECT_TRUE(retStopWrk3);

    x_INFO("DistributedWindowDeploymentTest: Stop worker 4");
    bool retStopWrk4 = wrk4->stop(true);
    EXPECT_TRUE(retStopWrk4);

    x_INFO("DistributedWindowDeploymentTest: Stop worker 5");
    bool retStopWrk5 = wrk5->stop(true);
    EXPECT_TRUE(retStopWrk5);

    x_INFO("DistributedWindowDeploymentTest: Stop Coordinator");
    bool retStopCord = crd->stopCoordinator(true);
    EXPECT_TRUE(retStopCord);
    remove(outputFilePath.c_str());
    x_INFO("DistributedWindowDeploymentTest: Test finished");
}
...
TEST_F(DistributedWindowDeploymentTest, DISABLED_testMultipleAggregationFunctionsOnMultipleWorkers) {
    ...
    // given: Set up the test harxs with two workers
    ...
    testHarxs.pushElement<InputData>({1000U, 1U, 1U}, 2);
    testHarxs.pushElement<InputData>({1100U, 2U, 2U}, 2);
    testHarxs.pushElement<InputData>({1200U, 1U, 3U}, 2);
    testHarxs.pushElement<InputData>({1300U, 2U, 4U}, 2);
    // given: Set up the input data on second worker.
    // Groups 1, 3 with 2 values each, value sums are 12, 14..
    testHarxs.pushElement<InputData>({1000U, 1U, 5U}, 3);
    testHarxs.pushElement<InputData>({1100U, 3U, 6U}, 3);
    testHarxs.pushElement<InputData>({1200U, 1U, 7U}, 3);
    testHarxs.pushElement<InputData>({1300U, 3U, 8U}, 3);
    // given: The internal coordinator also uses THREAD_LOCAL aggregation and distributed window is disabled.
    testHarxs.validate().setupTopology([](CoordinatorConfigurationPtr coordinatorConfiguration) {
    ...
    auto actual = testHarxs.getOutput<Output>(expected.size(), "BottomUp", "NONE", "IN_MEMORY");
    for (auto i : {0, 1, 2}) {
        x_DEBUG("{} = ({},{},{},{},{})", i, actual[i].start, actual[i].end, actual[i].key, actual[i].sum, actual[i].count);
    }
...
TEST_F(DistributedWindowDeploymentTest, DISABLED_testDistributedNonKeyTumblingWindowIngestionTime) {
    ...
    EXPECT_TRUE(retStart1);
    x_INFO("DistributedWindowDeploymentTest: Worker1 started successfully");
    ...
    EXPECT_TRUE(retStart2);
    x_INFO("DistributedWindowDeploymentTest: Worker2 started successfully");
...
TEST_F(DistributedWindowDeploymentTest, DISABLED_testLongWindow) {
    ...
    auto func = [&recordCounter](Runtime::TupleBuffer& buffer, uint64_t) {
        ...
        x_WARNING("Lambda last entry is={}", records[100 - 1].toString());
