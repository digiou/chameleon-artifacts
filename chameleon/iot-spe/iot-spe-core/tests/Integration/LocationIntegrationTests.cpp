...
class LocationIntegrationTests : public Testing::BaseIntegrationTest {
  public:
    static void SetUpTestCase() {
        x::Logger::setupLogging("LocationIntegrationTests.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup LocationIntegrationTests test class.");
        ...
        std::vector<x::Spatial::DataTypes::Experimental::Waypoint> waypointsToInterpolate;
    ...
    static std::optional<x::Spatial::DataTypes::Experimental::GeoLocation>
     ...
     */
    static bool isClose(x::Spatial::DataTypes::Experimental::GeoLocation location1,
                        x::Spatial::DataTypes::Experimental::GeoLocation location2,
                        double error) {
    ...
    static std::vector<x::Spatial::DataTypes::Experimental::Waypoint>
    interpolatePath(std::vector<x::Spatial::DataTypes::Experimental::Waypoint> waypoints, Timestamp amount) {
        std::vector<x::Spatial::DataTypes::Experimental::Waypoint> interpolatedPath;
        ...
        for (auto waypointsItr = waypoints.cbegin(); waypointsItr != waypoints.cend(); ++waypointsItr) {
            ...
            S2Point prevPoint = x::Spatial::Util::S2Utilities::geoLocationToS2Point(prevLocation);
            S2Point nextPoint = x::Spatial::Util::S2Utilities::geoLocationToS2Point(nextLocation);
            ...
            for (Timestamp i = 0; i < timerange; i += step) {
                ...
                interpolatedPath.emplace_back(x::Spatial::DataTypes::Experimental::GeoLocation(interpolated.lat().degrees(),
    ...
    static void TearDownTestCase() {
        x_INFO("Tear down LocationIntegrationTests class.");
        std::string singleLocationPath = std::string(TEST_DATA_DIRECTORY) + "singleLocation.csv";
        remove(singleLocationPath.c_str());
...
TEST_F(LocationIntegrationTests, testFieldNodes) {
    ...
    x_INFO("start worker 2");
    WorkerConfigurationPtr wrkConf2 = WorkerConfiguration::create();
    wrkConf2->coordinatorPort = (port);
    wrkConf2->locationCoordinates.setValue(x::Spatial::DataTypes::Experimental::GeoLocation::fromString(location2));
    wrkConf2->nodeSpatialType.setValue(x::Spatial::Experimental::SpatialType::FIXED_LOCATION);
    ...
    x_INFO("start worker 3");
    WorkerConfigurationPtr wrkConf3 = WorkerConfiguration::create();
    wrkConf3->coordinatorPort = (port);
    wrkConf3->locationCoordinates.setValue(x::Spatial::DataTypes::Experimental::GeoLocation::fromString(location3));
    wrkConf3->nodeSpatialType.setValue(x::Spatial::Experimental::SpatialType::FIXED_LOCATION);
    ...
    x_INFO("start worker 4");
    WorkerConfigurationPtr wrkConf4 = WorkerConfiguration::create();
    wrkConf4->coordinatorPort = (port);
    wrkConf4->locationCoordinates.setValue(x::Spatial::DataTypes::Experimental::GeoLocation::fromString(location4));
    wrkConf4->nodeSpatialType.setValue(x::Spatial::Experimental::SpatialType::FIXED_LOCATION);
    xWorkerPtr wrk4 = std::make_shared<xWorker>(std::move(wrkConf4));
    bool retStart4 = wrk4->start(/**blocking**/ false, /**withConnect**/ false);
    ASSERT_TRUE(retStart4);

    x_INFO("worker1 started successfully");
    bool retConWrk1 = wrk1->connect();
    ASSERT_TRUE(retConWrk1);
    x_INFO("worker 1 started connected ");
    ...
    ASSERT_EQ(topologyManagerService->getGeoLocationForNode(node2->getId()),
              x::Spatial::DataTypes::Experimental::GeoLocation(52.53736960143897, 13.299134894776092));
    topologyManagerService->updateGeoLocation(
        node2->getId(),
        x::Spatial::DataTypes::Experimental::GeoLocation(52.51094383152051, 13.463078966025266));
    ASSERT_EQ(topologyManagerService->getGeoLocationForNode(node2->getId()),
              x::Spatial::DataTypes::Experimental::GeoLocation(52.51094383152051, 13.463078966025266));
    ...
    topologyManagerService->updateGeoLocation(
        node3->getId(),
        x::Spatial::DataTypes::Experimental::GeoLocation(53.559524264262194, 10.039384739854102));
...
TEST_F(LocationIntegrationTests, testMobileNodes) {
    ...
    wrkConf1->locationCoordinates.setValue(x::Spatial::DataTypes::Experimental::GeoLocation::fromString(location2));
    wrkConf1->nodeSpatialType.setValue(x::Spatial::Experimental::SpatialType::MOBILE_NODE);
    ...
    ASSERT_EQ(wrk1->getLocationProvider()->getSpatialType(), x::Spatial::Experimental::SpatialType::MOBILE_NODE);
    ASSERT_EQ(wrk2->getLocationProvider()->getSpatialType(), x::Spatial::Experimental::SpatialType::FIXED_LOCATION);

    ASSERT_EQ(wrk2->getLocationProvider()->getCurrentWaypoint().getLocation(),
              x::Spatial::DataTypes::Experimental::GeoLocation::fromString(location2));

    TopologyNodePtr node1 = topology->findNodeWithId(wrk1->getWorkerId());
    TopologyNodePtr node2 = topology->findNodeWithId(wrk2->getWorkerId());

    ASSERT_EQ(node1->getSpatialNodeType(), x::Spatial::Experimental::SpatialType::MOBILE_NODE);
    ASSERT_EQ(node2->getSpatialNodeType(), x::Spatial::Experimental::SpatialType::FIXED_LOCATION);
    ...
    ASSERT_EQ(node2Location, x::Spatial::DataTypes::Experimental::GeoLocation::fromString(location2));
...
TEST_F(LocationIntegrationTests, testLocationFromCmd) {
    ...
    ASSERT_EQ(workerConfigPtr->locationCoordinates.getValue(), x::Spatial::DataTypes::Experimental::GeoLocation(23.88, -3.4));
}
...
TEST_F(LocationIntegrationTests, testMovingDevice) {
    ...
    ASSERT_TRUE(retStart1);
    auto sourceCsv =
        std::static_pointer_cast<x::Spatial::Mobility::Experimental::LocationProviderCSV,
                                 x::Spatial::Mobility::Experimental::LocationProvider>(wrk1->getLocationProvider());
    ...
    std::vector<x::Spatial::DataTypes::Experimental::Waypoint> actualWayPoints;
    while (currentTimeStamp <= endTime) {
        auto currentGeoLocation = topologyManagerService->getGeoLocationForNode(wrk1id);
    ...
    for (auto actualIt = actualWayPoints.cbegin(); actualIt != actualWayPoints.cend(); ++actualIt) {
        ...
        x_DEBUG("comparing actual waypoint {} to expected waypoint {}",
...
TEST_F(LocationIntegrationTests, testGetLocationViaRPC) {
    ...
    ASSERT_EQ(loc2.getLocation(), x::Spatial::DataTypes::Experimental::GeoLocation::fromString(location2));
...
TEST_F(LocationIntegrationTests, testReconnectingParentOutOfCoverage) {
    ...
    std::vector<x::Spatial::DataTypes::Experimental::GeoLocation> locVec;
    ...
    properties[x::Worker::Configuration::SPATIAL_SUPPORT] = x::Spatial::Experimental::SpatialType::FIXED_LOCATION;
    ...
    for (auto elem : locVec) {
        ...
        topologyManagerService->addGeoLocation(currNode->getId(), x::Spatial::DataTypes::Experimental::GeoLocation(elem));
        nodeIndex.Add(x::Spatial::Util::S2Utilities::geoLocationToS2Point(elem), currNode->getId());
        idCount++;
    }

    std::vector<x::Spatial::DataTypes::Experimental::GeoLocation> offPathVec(
    ...
    auto outOfCoverageLocation = x::Spatial::DataTypes::Experimental::GeoLocation(52.7091055763087, 13.128662109375002);
    ...
    topologyManagerService->addGeoLocation(currNode->getId(),
                                           x::Spatial::DataTypes::Experimental::GeoLocation(outOfCoverageLocation));
    nodeIndex.Add(x::Spatial::Util::S2Utilities::geoLocationToS2Point(outOfCoverageLocation), currNode->getId());
...
TEST_F(LocationIntegrationTests, testSequenceWithBuffering) {
    ...
    while (recv_tuples < 5000 && std::chrono::system_clock::now() < startTimestamp + defaultTimeoutInSec) {
        ...
        x_DEBUG("recv before buffering: {}", recv_tuples)
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
    ...
    for (int i = 0; i < 5; ++i) {
        std::ifstream inFile(testFile);
        recv_tuples = std::count(std::istreambuf_iterator<char>(inFile), std::istreambuf_iterator<char>(), '\n');
        x_DEBUG("recv while buffering: {}", recv_tuples)
    ...
    while (recv_tuples < 10000 && std::chrono::system_clock::now() < startTimestamp + defaultTimeoutInSec) {
        std::ifstream inFile(testFile);
        recv_tuples = std::count(std::istreambuf_iterator<char>(inFile), std::istreambuf_iterator<char>(), '\n');
        x_DEBUG("recv after buffering: {}", recv_tuples)
        std::this_thread::sleep_for(std::chrono::milliseconds(100));
    }
...
TEST_F(LocationIntegrationTests, testSequenceWithReconnecting) {
    ...
    std::vector<xWorkerPtr> fieldNodes;
    for (auto elem : locVec) {
        WorkerConfigurationPtr wrkConf = WorkerConfiguration::create();
        wrkConf->coordinatorPort.setValue(*rpcCoordinatorPort);
        wrkConf->nodeSpatialType.setValue(x::Spatial::Experimental::SpatialType::FIXED_LOCATION);
