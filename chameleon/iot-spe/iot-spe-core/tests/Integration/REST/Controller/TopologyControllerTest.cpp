...
TEST_F(TopologyControllerTest, testGetTopology) {
    ...
    cpr::Response r = cpr::Get(cpr::Url{BASE_URL + std::to_string(*restPort) + "/v1/x/topology"});
    ...
    x_DEBUG("shut down coordinator with rest port {}", coordinatorConfig->restPort.getValue());
    EXPECT_TRUE(stopCrd);
}
...
TEST_F(TopologyControllerTest, testAddParentMissingParentId) {
    ...
    request["childId"] = 1;
    auto response = cpr::Post(cpr::Url{BASE_URL + std::to_string(*restPort) + "/v1/x/topology/addParent"},
...
TEST_F(TopologyControllerTest, testRemoveParent) {
    ...
    request["parentId"] = 1;
    request["childId"] = 2;
    auto asyncResp = cpr::DeleteAsync(cpr::Url{BASE_URL + std::to_string(*restPort) + "/v1/x/topology/removeParent"},
                                      cpr::Header{{"Content-Type", "application/json"}},
                                      cpr::Body{request.dump()},
                                      cpr::ConnectTimeout{3000},
                                      cpr::Timeout{30000});
