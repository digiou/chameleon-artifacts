...
class UDFCatalogControllerTest : public Testing::BaseIntegrationTest {
  public:
    ...
    void startCoordinator() {
        x_INFO("UdfCatalogController: Start coordinator");
        ...
        x_INFO("UDFCatalogControllerTest: Coordinator started successfully");
    }
...
TEST_F(UDFCatalogControllerTest, getUdfDescriptorReturnsUdf) {
    ...
    auto future = cpr::GetAsync(cpr::Url{BASE_URL + std::to_string(*restPort) + "/v1/x/udfCatalog/getUdfDescriptor"},
...
TEST_F(UDFCatalogControllerTest, testErrorIfUnknownEndpointIsUsed) {
    ...
    auto future = cpr::PostAsync(cpr::Url{BASE_URL + std::to_string(*restPort) + "/v1/x/udfCatalog/SUPER_SECRET_URL"},
...
TEST_F(UDFCatalogControllerTest, testIfRegisterEndpointHandlesExceptionsWithoutReturningAStackTrace) {
    ...
    auto future = cpr::PostAsync(cpr::Url{BASE_URL + std::to_string(*restPort) + "/v1/x/udfCatalog/registerJavaUdf"},
                                 cpr::Header{{"Content-Type", "text/plain"}},
...
TEST_F(UDFCatalogControllerTest, testRemoveUdfEndpoint) {
    ...
    auto future = cpr::DeleteAsync(cpr::Url{BASE_URL + std::to_string(*restPort) + "/v1/x/udfCatalog/removeUdf"},
                                   cpr::Parameters{{"udfName", udfName}});
    future.wait();
    auto response = future.get();
...
TEST_F(UDFCatalogControllerTest, testIfListUdfsEndpointReturnsListAsExpected) {
    ...
    auto future = cpr::GetAsync(cpr::Url{BASE_URL + std::to_string(*restPort) + "/v1/x/udfCatalog/listUdfs"});
    future.wait();
    auto response = future.get();
    // then the response is OK
    ASSERT_EQ(response.status_code, Status::CODE_200.code);
