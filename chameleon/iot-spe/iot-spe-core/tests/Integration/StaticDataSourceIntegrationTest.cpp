...
class StaticDataSourceIntegrationTest : public Testing::BaseIntegrationTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("StaticDataSourceIntegrationTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup StaticDataSourceIntegrationTest test class.");
    }
    ...
    void SetUp() {
        ...
        ASSERT_EQ(schema_customer->getSchemaSizeInBytes(), 236ULL);
        {
            ...
            x_ASSERT(file.is_open(), "Invalid path.");
            int num_lix = std::count(std::istreambuf_iterator<char>(file), std::istreambuf_iterator<char>(), '\n');
            x_ASSERT(num_lix == 200, "The table file table_path_customer_l0200 does not contain exactly 200 lix.");
        }
        ...
        ASSERT_EQ(schema_nation->getSchemaSizeInBytes(), 195ULL);
        {
            ...
            x_ASSERT(file.is_open(), "Invalid path.");
            int num_lix = std::count(std::istreambuf_iterator<char>(file), std::istreambuf_iterator<char>(), '\n');
            x_ASSERT(num_lix == 25, "The table file table_path_nation_s0001 does not contain exactly 25 lix.");
        }
        ...
        ASSERT_EQ(schema_integers_0->getSchemaSizeInBytes(), 16ULL);
        {
            ...
            x_ASSERT(file.is_open(), "Invalid path.");
            int num_lix = std::count(std::istreambuf_iterator<char>(file), std::istreambuf_iterator<char>(), '\n');
            x_ASSERT(num_lix == 10, "The table file table_path_integers_0 does not contain exactly 10 lix.");
        }
        ...
        ASSERT_EQ(schema_integers_1->getSchemaSizeInBytes(), 24ULL);
        {
            ...
            x_ASSERT(file.is_open(), "Invalid path.");
            int num_lix = std::count(std::istreambuf_iterator<char>(file), std::istreambuf_iterator<char>(), '\n');
            x_ASSERT(num_lix == 10, "The table file table_path_integers_1 does not contain exactly 10 lix.");
        }
    }
    ...
    uint64_t printTotalQueryRuntime(std::vector<Runtime::QueryStatisticsPtr> stats) {
        ...
        x_INFO("Total query runtime since query start: {}ms.", diffToStart);
        x_INFO("Total query runtime since first Task: {}ms.", diffToFirstTask);
...
TEST_F(StaticDataSourceIntegrationTest, testCustomerTableDistributed) {
    ...
    x_INFO("StaticDataSourceIntegrationTest: Start coordinator");
    xCoordinatorPtr crd = std::make_shared<xCoordinator>(crdConf);
    uint64_t port = crd->startCoordinator(/**blocking**/ false);
    ASSERT_NE(port, 0UL);
    x_INFO("StaticDataSourceIntegrationTest: Coordinator started successfully");
    ...
    x_INFO("StaticDataSourceIntegrationTest: Start worker 1");
    ...
    x_INFO("StaticDataSourceIntegrationTest: Worker1 started successfully");
    ...
    x_INFO("StaticDataSourceIntegrationTest: Remove query");
...
TEST_F(StaticDataSourceIntegrationTest, DISABLED_testCustomerTableProjection) {
    ...
    x_ASSERT(numResultTuples == numExpectedTuples,
...
TEST_F(StaticDataSourceIntegrationTest, testBatchJoinNationCustomer200lix) {
    CoordinatorConfigurationPtr crdConf = CoordinatorConfiguration::createDefault();
    ...
    const std::string expectedContent =
        ...
        "the unusual, bold a\n9,INDOxIA,2, slyly express asymptotes. regular deposits haggle slyly. carefully ironic hockey "
        ...
        "pinto beans impress realms. final dependencies \n9,INDOxIA,2, slyly express asymptotes. regular deposits haggle "
        ...
        "platelets r\n9,INDOxIA,2, slyly express asymptotes. regular deposits haggle slyly. carefully ironic hockey players "
        "sleep blithely. "
        ...
        "r,116,Customer#000000116,yCuVxIgsZ3,qyK2rloThy3u,16,26-632-309-5792,8403.990000,BUILDING,as. quickly final sauterx "
        ...
        "regular\n9,INDOxIA,2, slyly express asymptotes. regular deposits haggle slyly. carefully ironic hockey players sleep "
        ...
        "regular packages on the quiet dependencies\n9,INDOxIA,2, slyly express asymptotes. regular deposits haggle slyly. "
...
TEST_F(StaticDataSourceIntegrationTest, DISABLED_testBatchJoinIntegersOnlyRemoteProbeSource) {
    ...
    xWorkerPtr wrkRemote = std::make_shared<xWorker>(std::move(wrkConfRemote));
    bool retStartRemote = wrkRemote->start(/**blocking**/ false, /**withConnect**/ true);
    EXPECT_TRUE(retStartRemote);
    x_INFO("StaticDataSourceIntegrationTest: Remote worker started successfully");
...
TEST_F(StaticDataSourceIntegrationTest, testBatchJoinCustomerWithIntTable) {
    ...
    x_DEBUG("StaticDataSourceIntegrationTest::testBatchJoinCustomerWithIntTable: num work {} num buff {}",
              crdConf->worker.numWorkerThreads.getValue(),
              crdConf->worker.numberOfBuffersInGlobalBufferManager.getValue());
    ...
    if (!benchmark) {
        ...
        x_DEBUG("numResultTuples: {}, numExpectedTuples: {}", numResultTuples, numExpectedTuples);
...
TEST_F(StaticDataSourceIntegrationTest, testBatchJoinLargeIntTables) {
    ...
    for (int sizeBuildTable : sizeBuildTableVec) {
        ...
        for (int sizeProbeTable : sizeProbeTableVec) {
            ...
            // extract total query runtime from statistics
            auto stats = crd->getQueryStatistics(globalQueryPlan->getSharedQueryId(queryId));
            x_WARNING("sizeProbeTable: {} sizeBuildTable: {}", sizeProbeTable, sizeBuildTable);
            printTotalQueryRuntime(stats);
