...
class NemoIntegrationTest : public Testing::BaseIntegrationTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("NemoIntegrationTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup NemoIntegrationTest test class.");
    }
    ...
    TestHarxs createTestHarxs(uint64_t layers,
            ...
            for (auto parent : parents) {
                ...
                for (uint64_t j = 0; j < nodeCnt; j++) {
                    ...
                    if (i == layers) {
                        ...
                        testHarxs.attachWorkerWithCSVSourceToWorkerWithId("car", csvSource, parent);
                        x_DEBUG("NemoIntegrationTest: Adding CSV source for node:{}", nodeId);
                        continue;
                    }
                    testHarxs.attachWorkerToWorkerWithId(parent);
                    x_DEBUG("NemoIntegrationTest: Adding worker to worker with ID:{}", parent);
                }
            }
            parents = newParents;
        }
        testHarxs.validate().setupTopology(crdFunctor);
        return testHarxs;
    }
...
TEST_F(NemoIntegrationTest, testThreeLevelsTopologyTopDown) {
    ...
    auto testHarxs = createTestHarxs(3, 2, 4, childThreshold, combinerThreshold);
    std::vector<Output> actualOutput = testHarxs.getOutput<Output>(expectedTuples, "BottomUp", "NONE", "IN_MEMORY");
    EXPECT_EQ(actualOutput.size(), expectedTuples);

    TopologyPtr topology = testHarxs.getTopology();
    QueryPlanPtr queryPlan = testHarxs.getQueryPlan();
    x_DEBUG("NemoIntegrationTest: Executed with topology \n{}", topology->toString());
    x_INFO("NemoIntegrationTest: Executed with plan \n{}", queryPlan->toString());
