...
class MultipleJoinsTest : public Testing::BaseIntegrationTest,
  ...
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("MultipleJoinsTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup MultipleJoinsTest test class.");
    }

    /* Will be called before a test is executed. */
    void SetUp() override {
        x_INFO("QueryExecutionTest: Setup MultipleJoinsTest test class.");
        BaseIntegrationTest::SetUp();

        joinStrategy = x::Runtime::Execution::MultipleJoinsTest::GetParam();
    ...
    std::vector<ResultRecord> runJoinQuery(const Query& query,
        ...
        for (auto i = 0_u64; i < joinParams.inputSchemas.size(); ++i) {
            ...
            testHarxs.addLogicalSource(logicalSourceName, joinParams.inputSchemas[i])
                .attachWorkerWithCSVSourceToCoordinator(logicalSourceName, sourceConfig);
        }

        auto actualResult = testHarxs.validate().setupTopology().getOutput<ResultRecord>(expectedRecords.size());
...
TEST_P(MultipleJoinsTest, testJoin3WithDifferentSourceTumblingWindowOnCoodinatorxted) {
    struct ResultRecord {
        uint64_t window1window2window3window4start;
        uint64_t window1window2window3window4end;
        uint64_t window1window2window3window4key;
...
// TODO this test can be enabled once #3353 is merged
TEST_P(MultipleJoinsTest, DISABLED_testJoin3WithDifferentSourceSlidingWindowOnCoodinatorxted) {
    struct ResultRecord {
        uint64_t window1window2window3window4start;
        uint64_t window1window2window3window4end;
        uint64_t window1window2window3window4key;
...
INSTANTIATE_TEST_CASE_P(
    testJoinQueries,
    MultipleJoinsTest,
    ::testing::Values(QueryCompilation::StreamJoinStrategy::xTED_LOOP_JOIN),
