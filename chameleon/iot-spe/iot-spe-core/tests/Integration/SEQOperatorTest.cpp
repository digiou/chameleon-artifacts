...
class SeqOperatorTest : public Testing::BaseIntegrationTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("SeqOperatorTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup SeqOperatorTest test class.");
    }
 ...
 */
TEST_F(SeqOperatorTest, testPatternOximpleSeq) {
    CoordinatorConfigurationPtr coordinatorConfig = CoordinatorConfiguration::createDefault();
    coordinatorConfig->rpcPort = *rpcCoordinatorPort;
    coordinatorConfig->restPort = *restPort;
    x_INFO("MultipleJoinsTest: Start coordinator");
    ...
    x_DEBUG("SeqOperatorTest: Coordinator started successfully");

    x_DEBUG("SeqOperatorTest: Start worker 1");
    ...
    x_INFO("MultipleJoinsTest: Worker1 started successfully");

    x_DEBUG("MultipleJoinsTest: Start worker 2");
    ...
    x_INFO("MultipleJoinsTest: Worker2 started successfully");

    std::string outputFilePath = getTestResourceFolder() / "testSeqPatternWithTestStream1.out";
    remove(outputFilePath.c_str());

    x_INFO("SeqOperatorTest: Submit seqWith pattern");
    ...
    x_INFO("SeqOperatorTest: Remove query");
 ...
 */
TEST_F(SeqOperatorTest, testPatternOxeq) {
    CoordinatorConfigurationPtr coordinatorConfig = CoordinatorConfiguration::createDefault();
    coordinatorConfig->rpcPort = *rpcCoordinatorPort;
    coordinatorConfig->restPort = *restPort;
    x_INFO("MultipleJoinsTest: Start coordinator");
    ...
    bool retStart1 = wrk1->start(/**blocking**/ false, /**withConnect**/ true);
    EXPECT_TRUE(retStart1);
    x_INFO("SeqOperatorTest: Worker1 started successfully");

    x_DEBUG("SeqOperatorTest: Start worker 2");
    WorkerConfigurationPtr workerConfig2 = WorkerConfiguration::create();
    workerConfig2->coordinatorPort = port;
    auto csvSourceType2 = CSVSourceType::create();
    ...
    bool retStart2 = wrk2->start(/**blocking**/ false, /**withConnect**/ true);
    EXPECT_TRUE(retStart2);
    x_INFO("SeqOperatorTest: Worker2 started successfully");
...
TEST_F(SeqOperatorTest, DISABLED_testPatternSeqWithSlidingWindow) {
    ...
    coordinatorConfig->rpcPort = *rpcCoordinatorPort;
    coordinatorConfig->restPort = *restPort;
    x_INFO("SeqOperatorTest: Start coordinator");
    xCoordinatorPtr crd = std::make_shared<xCoordinator>(coordinatorConfig);
    uint64_t port = crd->startCoordinator(/**blocking**/ false);//id=1
    EXPECT_NE(port, 0UL);
    //register logical source qnv
    ...
    x_INFO("AndOperatorTest: Remove query");
    EXPECT_TRUE(TestUtils::checkStoppedOrTimeout(queryId, queryCatalogService));
...
TEST_F(SeqOperatorTest, DISABLED_testPatternSeqWithEarlyTermination) {
    ...
    x_DEBUG("MultipleJoinsTest: Coordinator started successfully");

    x_DEBUG("MultipleJoinsTest: Start worker 1");
    WorkerConfigurationPtr workerConfig1 = WorkerConfiguration::create();
    workerConfig1->coordinatorPort = port;
    auto csvSourceType1 = CSVSourceType::create();
    csvSourceType1->setFilePath(std::string(TEST_DATA_DIRECTORY) + "QnV_short_R2000070.csv");
...
TEST_F(SeqOperatorTest, DISABLED_testMultiSeqPattern) {
    ...
    x_DEBUG("SeqOperatorTest: Start worker 3");
