...
class MultiThreadedTest : public Testing::BaseIntegrationTest,
  ...
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("MultiThreadedTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup MultiThreadedTest test class.");
    }
    ...
    void TearDown() override {
        x_INFO("QueryExecutionTest: Tear down MultiThreadedTest test case.");

        // Stopping the execution engine
        EXPECT_TRUE(executionEngine->stop());
        x::Testing::BaseIntegrationTest::TearDown();
    }
...
// TODO currently we do not support sliding windows #3651
