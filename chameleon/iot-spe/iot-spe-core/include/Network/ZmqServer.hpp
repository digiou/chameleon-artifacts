...
#ifndef x_CORE_INCLUDE_NETWORK_ZMQSERVER_HPP_
#define x_CORE_INCLUDE_NETWORK_ZMQSERVER_HPP_
...
namespace fmt {
template<>
struct formatter<x::Network::ZmqServer> : formatter<std::string> {
    auto format(const x::Network::ZmqServer& zmq, format_context& ctx) -> decltype(ctx.out()) {
...
#endif// x_CORE_INCLUDE_NETWORK_ZMQSERVER_HPP_
