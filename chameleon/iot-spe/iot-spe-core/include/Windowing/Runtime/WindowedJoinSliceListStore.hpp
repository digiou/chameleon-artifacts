...
#ifndef x_CORE_INCLUDE_WINDOWING_RUNTIME_WINDOWEDJOINSLICELISTSTORE_HPP_
#define x_CORE_INCLUDE_WINDOWING_RUNTIME_WINDOWEDJOINSLICELISTSTORE_HPP_
  ...
  public:
    ...
    uint64_t getSliceIndexByTs(int64_t timestamp) {
        ...
        if (timestamp < 0 and !isDefault) {
            x_ERROR("getSliceIndexByTs for could not find a slice, because the timestamp is "
                      "neither -1 nor positive: {}",
                      timestamp);
            x_THROW_RUNTIME_ERROR("getSliceIndexByTs for could not find a slice, this should not happen.");
        }
        ...
        for (uint64_t i = 0ULL; i < sliceMetaData.size(); ++i) {
            ...
            x_TRACE("slice begin={} slice end ={}", slice.getStartTs(), slice.getEndTs());
            if (isDefault || (slice.getStartTs() <= ts && slice.getEndTs() > ts)) {
                x_TRACE("return slice id={}", i);
                return i;
            }
        }
        x_ERROR("getSliceIndexByTs for could not find a slice, this should not happen ts {} {}", timestamp, ts);
        return UINT64_MAX;
    }
    ...
    inline void removeSlicesUntil(uint64_t watermark) {
        ...
        for (; itSlice != sliceMetaData.end(); ++itSlice) {
            ...
            x_TRACE("WindowedJoinSliceListStore removeSlicesUntil: watermark={} from slice endts={}, sliceMetaData size={} "
                      "content size={}",
                      watermark,
                      itSlice->getEndTs(),
                      sliceMetaData.size(),
        ...
        x_TRACE("WindowedJoinSliceListStore: removeSlicesUntil size after cleanup slice={} content={}",
                  sliceMetaData.size(),
                  content.size());
    }
    ...
    inline void appendSlice(SliceMetaData slice) {
        x_TRACE("appendSlice with start {}", slice.getStartTs());
        std::lock_guard lock(internalMutex);
    ...
    inline void append(uint64_t index, ValueType&& value) {
        std::lock_guard lock(internalMutex);
        if (index != UINT64_MAX) {
            x_VERIFY(content.size() > index, "invalid index");
    ...
    */
    inline void append(uint64_t index, ValueType& value) {
        std::lock_guard lock(internalMutex);
        if (index != UINT64_MAX) {
            x_VERIFY(content.size() > index, "invalid index for content size" << content.size() << " idx=" << index);
...
#endif// x_CORE_INCLUDE_WINDOWING_RUNTIME_WINDOWEDJOINSLICELISTSTORE_HPP_
