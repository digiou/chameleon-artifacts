...
#ifndef x_CORE_INCLUDE_WINDOWING_EXPERIMENTAL_KEYEDTIMEWINDOW_KEYEDSLICE_HPP_
#define x_CORE_INCLUDE_WINDOWING_EXPERIMENTAL_KEYEDTIMEWINDOW_KEYEDSLICE_HPP_
...
class KeyedSlice {
  public:
     ...
     */
    KeyedSlice(std::shared_ptr<x::Experimental::HashMapFactory> hashMapFactory, uint64_t start, uint64_t end);

    /**
     * @brief Constructor to create a uninitialized slice.
     * @param hashMapFactory
     */
    KeyedSlice(std::shared_ptr<x::Experimental::HashMapFactory> hashMapFactory);
     ...
     * @return uint64_t
     */
    inline x::Experimental::Hashmap& getState() { return state; }
  ...
  private:
    uint64_t start;
    uint64_t end;
    x::Experimental::Hashmap state;
};

}// namespace x::Windowing::Experimental

#endif// x_CORE_INCLUDE_WINDOWING_EXPERIMENTAL_KEYEDTIMEWINDOW_KEYEDSLICE_HPP_
