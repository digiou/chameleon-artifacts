...
#ifndef x_CORE_INCLUDE_WINDOWING_WINDOWACTIONS_EXECUTABLESLICEAGGREGATIONTRIGGERACTION_HPP_
#define x_CORE_INCLUDE_WINDOWING_WINDOWACTIONS_EXECUTABLESLICEAGGREGATIONTRIGGERACTION_HPP_
...
class ExecutableSliceAggregationTriggerAction
  ...
  public:
    ...
    bool doAction(Runtime::StateVariable<KeyType, WindowSliceStore<PartialAggregateType>*>* windowStateVariable,
        ...
        x_TRACE("ExecutableSliceAggregationTriggerAction {}: doAction for currentWatermark={} lastWatermark={}",
                  id,
                  currentWatermark,
                  lastWatermark);

        if (this->weakExecutionContext.expired()) {
            x_FATAL_ERROR("ExecutableSliceAggregationTriggerAction {}: the weakExecutionContext was already expired!", id);
            return false;
        }
        ...
        for (auto& it : windowStateVariable->rangeAll()) {
            x_TRACE("ExecutableSliceAggregationTriggerAction {}: {} check key={} nextEdge={}",
                      id,
                      toString(),
                      it.first,
                      it.second->nextEdge);
  ...
  */
    void aggregateWindows(KeyType key,
        ...
        // TODO we should add a allowed latexs to support out of order events
        ...
        x_TRACE("ExecutableSliceAggregationTriggerAction {}: trigger {} slices key={} current watermark is={} "
        ...
        for (uint64_t sliceId = 0; sliceId < slices.size(); sliceId++) {
            ...
            x_TRACE("ExecutableSliceAggregationTriggerAction {}: check slice start={} end={} key={} currentWatermark={}",
                      id,
            ...
            if (slices[sliceId].getEndTs() <= currentWatermark) {
                x_TRACE("ExecutableSliceAggregationTriggerAction {}: write result slices[sliceId].getStartTs()={} "
                ...
                if ((currentNumberOfTuples + 1) * this->windowSchema->getSchemaSizeInBytes() > tupleBuffer.getBufferSize()
                    ...
                    if (Logger::getInstance()->getCurrentLogLevel() == LogLevel::LOG_TRACE) {
                        ...
                        //write full buffer
                        x_TRACE("ExecutableSliceAggregationTriggerAction {}: Dispatch intermediate output buffer with {} "
                ...
                x_TRACE("ExecutableSliceAggregationTriggerAction {}: SL: Dont write result because "
...
#endif// x_CORE_INCLUDE_WINDOWING_WINDOWACTIONS_EXECUTABLESLICEAGGREGATIONTRIGGERACTION_HPP_
