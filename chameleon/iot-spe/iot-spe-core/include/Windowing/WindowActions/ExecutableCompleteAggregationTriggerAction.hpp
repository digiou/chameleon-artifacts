...
#ifndef x_CORE_INCLUDE_WINDOWING_WINDOWACTIONS_EXECUTABLECOMPLETEAGGREGATIONTRIGGERACTION_HPP_
#define x_CORE_INCLUDE_WINDOWING_WINDOWACTIONS_EXECUTABLECOMPLETEAGGREGATIONTRIGGERACTION_HPP_
...
class ExecutableCompleteAggregationTriggerAction
  ...
  public:
    ...
    explicit ExecutableCompleteAggregationTriggerAction(
        ...
        x_DEBUG("ExecutableCompleteAggregationTriggerAction intialized with schema:{} id={}", outputSchema->toString(), id);
    ...
    bool doAction(Runtime::StateVariable<KeyType, WindowSliceStore<PartialAggregateType>*>* windowStateVariable,
        ...
        x_TRACE("ExecutableCompleteAggregationTriggerAction (id={} {}): doAction for currentWatermark={} lastWatermark={}",
                  id,
        ...
        if (this->weakExecutionContext.expired()) {
            x_FATAL_ERROR("ExecutableCompleteAggregationTriggerAction: the weakExecutionContext was already expired!");
            return false;
        }
        ...
        for (auto& it : windowStateVariable->rangeAll()) {
            ...
            x_TRACE("ExecutableCompleteAggregationTriggerAction ({}): {} check key={} nextEdge={} id={}",
                      this->windowDefinition->getDistributionType()->toString(),
                      toString(),
                      it.first,
                      it.second->nextEdge,
  ...
  */
    void aggregateWindows(KeyType key,
        ...
        x_TRACE("AggregateWindows for ExecutableCompleteAggregationTriggerAction id={}", id);
        ...
        if (this->weakExecutionContext.expired()) {
            x_FATAL_ERROR("ExecutableCompleteAggregationTriggerAction id={}: the weakExecutionContext was already expired!",
                            id);
        }
        auto executionContext = this->weakExecutionContext.lock();
        ...
        for (uint64_t sliceId = 0; sliceId < slices.size(); sliceId++) {
            x_TRACE("ExecutableCompleteAggregationTriggerAction{}: ({}): trigger sliceid={} start={} end={}",
                      id,
                      this->windowDefinition->getDistributionType()->toString(),
                      sliceId,
        ...
        if (currentWatermark > lastWatermark) {
            x_TRACE("ExecutableCompleteAggregationTriggerAction {}: aggregateWindows trigger because currentWatermark={} > "
                      "lastWatermark={}",
                      id,
                      currentWatermark,
                      lastWatermark);
            ...
            x_TRACE("ExecutableCompleteAggregationTriggerAction {} ({}): trigger Complete or combining window for slices={}",
                      id,
                      this->windowDefinition->getDistributionType()->toString(),
                      slices.size(),
                      windows.size());
        } else {
            x_TRACE("ExecutableCompleteAggregationTriggerAction {}: aggregateWindows No trigger because NOT "
        ...
        for (uint64_t sliceId = 0; sliceId < slices.size(); sliceId++) {
            for (uint64_t windowId = 0; windowId < windows.size(); windowId++) {
                ...
                x_TRACE("ExecutableCompleteAggregationTriggerAction {}: ({}): key={} window.getStartTs()={} "
                ...
                if (window.getStartTs() <= slices[sliceId].getStartTs() && window.getEndTs() >= slices[sliceId].getEndTs()
                    ...
                    x_TRACE("ExecutableCompleteAggregationTriggerAction {}: ({}): create partial agg windowId={} sliceId={} "
                              "key={} partAgg={} recCnt={}",
                              id,
                              this->windowDefinition->getDistributionType()->toString(),
                              windowId,
                    ...
                    x_TRACE("ExecutableCompleteAggregationTriggerAction {}: CC: condition not true", id);
                }
            }
        }
        ...
        if (!windows.empty()) {
            ...
            for (auto i = static_cast<typename decltype(partialFinalAggregates)::size_type>(0); i < partialFinalAggregates.size();
                ...
                x_TRACE("ExecutableCompleteAggregationTriggerAction {}: ({}) write i={} key={} value={} window.start()={} "
            ...
            x_TRACE("ExecutableCompleteAggregationTriggerAction {}: ({}): remove slices until={}",
                      id,
                      this->windowDefinition->getDistributionType()->toString(),
                      currentWatermark);
            //remove the old slices from current watermark - allowed latexs as there could be no tuple before that
            ...
            x_TRACE("ExecutableCompleteAggregationTriggerAction {}: ({}): aggregate: no window qualifies",
                      id,
                      this->windowDefinition->getDistributionType()->toString());
        }
    }
...
#endif// x_CORE_INCLUDE_WINDOWING_WINDOWACTIONS_EXECUTABLECOMPLETEAGGREGATIONTRIGGERACTION_HPP_
