...
#ifndef x_CORE_INCLUDE_WINDOWING_WINDOWHANDLER_ABSTRACTWINDOWHANDLER_HPP_
#define x_CORE_INCLUDE_WINDOWING_WINDOWHANDLER_ABSTRACTWINDOWHANDLER_HPP_
...
class AbstractWindowHandler : public detail::virtual_enable_shared_from_this<AbstractWindowHandler, false>,
  ...
  public:
    ...
    ~AbstractWindowHandler() x_NOEXCEPT(false) override = default;
    ...
    */
    ...
    void updateMaxTs(uint64_t ts, OriginId originId, uint64_t sequenceNumber, Runtime::WorkerContextRef workerContext) {
        ...
        x_DEBUG("updateMaxTs={} orId={}", ts, originId);
        if (windowDefinition->getTriggerPolicy()->getPolicyType() == Windowing::TriggerType::triggerOnWatermarkChange) {
            ...
            if (oldWatermark < newWatermark) {
                x_DEBUG("AbstractWindowHandler trigger for before={}, afterMin={}", oldWatermark, newWatermark);
...
#endif// x_CORE_INCLUDE_WINDOWING_WINDOWHANDLER_ABSTRACTWINDOWHANDLER_HPP_
