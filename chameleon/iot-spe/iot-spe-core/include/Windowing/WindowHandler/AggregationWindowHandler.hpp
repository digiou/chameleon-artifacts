...
#ifndef x_CORE_INCLUDE_WINDOWING_WINDOWHANDLER_AGGREGATIONWINDOWHANDLER_HPP_
#define x_CORE_INCLUDE_WINDOWING_WINDOWHANDLER_AGGREGATIONWINDOWHANDLER_HPP_
...
class AggregationWindowHandler : public AbstractWindowHandler {
  public:
    explicit AggregationWindowHandler(
        ...
        x_ASSERT(this->windowDefinition, "invalid definition");
        x_ASSERT(this->windowDefinition->getNumberOfInputEdges() >= 0,
    ...
    ~AggregationWindowHandler() override {
        x_DEBUG("~AggregationWindowHandler({}, {}) finished destructor", handlerType, id);
        stop();
    }
   ...
   */
     ...
     */
    bool stop() override {
        x_DEBUG("AggregationWindowHandler({}, {}): stop called", handlerType, id);
        auto expected = true;
        bool result = false;
        ...
        x_DEBUG("AggregationWindowHandler({}, {}):  stop result = {}", handlerType, id, result);
        return result;
    }
    ...
    void trigger(Runtime::WorkerContextRef workerContext, bool forceFlush = false) override {
        std::unique_lock lock(windowMutex);
        x_TRACE("AggregationWindowHandler({},{}):  run window trigger {} distribution type={} forceFlush={}",
                  handlerType,
                  id,
        ...
        if (!forceFlush) {
            ...
            if (windowDefinition->getWindowType()->isTimeBasedWindowType()) {
                ...
                if (timeBasedWindowType->getTimeBasedSubWindowType() == TimeBasedWindowType::TUMBLINGWINDOW) {
                    ...
                    x_THROW_RUNTIME_ERROR("AggregationWindowHandler: Undefined Window Type");
                }
            }

            auto allowedLatexs = windowManager->getAllowedLatexs();
            x_TRACE("For flushing maxWatermark = {} window size={} trigger ts ={} allowedLatexs={}",
                      watermark,
                      windowSize,
                      watermark + windowSize,
                      allowedLatexs);
            watermark = watermark + windowSize + allowedLatexs;
        }
        ...
        if (lastWatermark == 0) {
            ...
            if (runningWatermark != std::numeric_limits<uint64_t>::max()) {
                ...
                x_TRACE("AggregationWindowHandler({}, {}): set lastWatermark to min value of stores={}",
                          handlerType,
                          id,
                          lastWatermark);

            } else {
                x_TRACE("AggregationWindowHandler({}, {}): as there is no buffer yet in any store, we cannot trigger",
                          handlerType,
                          id);
                return;
            }
        ...
        x_TRACE("AggregationWindowHandler({}, {}): run doing with watermark={}", handlerType, id, lastWatermark);

        executableWindowAction->doAction(getTypedWindowState(), watermark, lastWatermark, workerContext);
        x_TRACE("AggregationWindowHandler({}, {}):  set lastWatermark to={}",
                  handlerType,
                  id,
    ...
    */
    bool setup(Runtime::Execution::PipelineExecutionContextPtr pipelineExecutionContext) override {
        ...
        this->windowManager =
            std::make_shared<WindowManager>(windowDefinition->getWindowType(), windowDefinition->getAllowedLatexs(), id);
...
#endif// x_CORE_INCLUDE_WINDOWING_WINDOWHANDLER_AGGREGATIONWINDOWHANDLER_HPP_
