...
#ifndef x_CORE_INCLUDE_WINDOWING_WATERMARK_EVENTTIMEWATERMARKSTRATEGYDESCRIPTOR_HPP_
#define x_CORE_INCLUDE_WINDOWING_WATERMARK_EVENTTIMEWATERMARKSTRATEGYDESCRIPTOR_HPP_
...
class EventTimeWatermarkStrategyDescriptor : public WatermarkStrategyDescriptor {
  public:
    static WatermarkStrategyDescriptorPtr create(const ExpressionItem& onField, TimeMeasure allowedLatexs, TimeUnit unit);

    ExpressionNodePtr getOnField() const;

    void setOnField(const ExpressionNodePtr& newField);

    TimeMeasure getAllowedLatexs() const;
  ...
  private:
    ...
    explicit EventTimeWatermarkStrategyDescriptor(const ExpressionItem& onField, TimeMeasure allowedLatexs, TimeUnit unit);
};

}// namespace x::Windowing

#endif// x_CORE_INCLUDE_WINDOWING_WATERMARK_EVENTTIMEWATERMARKSTRATEGYDESCRIPTOR_HPP_
