...
#ifndef x_CORE_INCLUDE_WINDOWING_WATERMARK_EVENTTIMEWATERMARKSTRATEGY_HPP_
#define x_CORE_INCLUDE_WINDOWING_WATERMARK_EVENTTIMEWATERMARKSTRATEGY_HPP_
...
class EventTimeWatermarkStrategy : public WatermarkStrategy {
  public:
    EventTimeWatermarkStrategy(FieldAccessExpressionNodePtr onField, uint64_t allowedLatexs, uint64_t multiplier);
    ~EventTimeWatermarkStrategy() noexcept override = default;

    FieldAccessExpressionNodePtr getField();

    uint64_t getAllowedLatexs() const;
    ...
    create(const FieldAccessExpressionNodePtr& onField, uint64_t allowedLatexs, uint64_t multiplier);
...
#endif// x_CORE_INCLUDE_WINDOWING_WATERMARK_EVENTTIMEWATERMARKSTRATEGY_HPP_
