...
#ifndef x_CORE_INCLUDE_STATE_STATEMANAGER_HPP_
#define x_CORE_INCLUDE_STATE_STATEMANAGER_HPP_
...
class StateManager {
  ...
  public:
    ...
    StateVariable<Key, Value>* registerState(const StateId& variableStateId) {
        ...
        x_ASSERT(stateVariables.find(variableStateId) == stateVariables.end(), "State variable with this id already exists");
        stateVariables[variableStateId] = stateVariable;
        return stateVariable;
    }
    ...
    void unRegisterState(StateVariable<Key, Value>* stateVariable) {
        std::unique_lock<std::mutex> lock(mutex);
        x_ASSERT(stateVariable, "State variable is null");
    ...
    void unRegisterState(const StateVariable<Key, Value>* variable) {
        x_ASSERT(variable, "State variable is null");
        std::unique_lock<std::mutex> lock(mutex);
...
#endif// x_CORE_INCLUDE_STATE_STATEMANAGER_HPP_
