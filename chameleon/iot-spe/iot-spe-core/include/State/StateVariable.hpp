...
#ifndef x_CORE_INCLUDE_STATE_STATEVARIABLE_HPP_
#define x_CORE_INCLUDE_STATE_STATEVARIABLE_HPP_
...
class StateVariable : public detail::Destroyable {
     ...
     */
        ...
        */
        Value valueOrDefaultWithCallback() {
            x_VERIFY(defaultCallback, "invalid default callback");
            if (!backend.contains(key)) {
  ...
  public:
    ...
    explicit StateVariable(StateId stateId, std::function<Value(const Key&)> defaultCallback)
        ...
        x_ASSERT(this->defaultCallback, "invalid default callback");
    }
...
#endif// x_CORE_INCLUDE_STATE_STATEVARIABLE_HPP_
