...
#ifndef x_CORE_INCLUDE_RUNTIME_ASYNCTASKEXECUTOR_HPP_
#define x_CORE_INCLUDE_RUNTIME_ASYNCTASKEXECUTOR_HPP_
...
class AsyncTaskExecutor {
    ...
    class AsyncTaskWrapper {
      public:
        ...
        explicit AsyncTaskWrapper(std::function<void(void)>&& func, std::function<void(void)>&& releaseCallback)
            ...
            x_VERIFY(!!this->func, "Invalid callback");
            x_VERIFY(!!this->gc, "Invalid gc callback");
        }
  ...
  public:
    ...
    [[nodiscard]] AsyncTaskFuture<std::invoke_result_t<std::decay_t<Function>, std::decay_t<Args>...>> runAsync(Function&& f,
        ...
        x_ASSERT(taskPtr, "Cannot allocate async task");
        auto* task =
        ...
        x_ASSERT(!!asyncCallback, "Cannot allocate async task");
        x_ASSERT(!!releaseCallback, "Cannot allocate async task");
        {
...
#endif// x_CORE_INCLUDE_RUNTIME_ASYNCTASKEXECUTOR_HPP_
