...
*/
#ifndef x_CORE_INCLUDE_REST_CONTROLLER_UDFCATALOGCONTROLLER_HPP_
#define x_CORE_INCLUDE_REST_CONTROLLER_UDFCATALOGCONTROLLER_HPP_
  ...
  public:
    ...
    ENDPOINT("GET", "/getUdfDescriptor", getUdfDescriptor, QUERY(String, udf, "udfName")) {
        try {
            ...
            if (udfDescriptor == nullptr) {
                ...
                x_DEBUG("REST client tried retrieving UDF descriptor for non-existing Java UDF: {}", udfName);
                response.set_found(false);
                return createResponse(Status::CODE_404, response.SerializeAsString());
            } else {
                // Return the UDF descriptor to the client.
                x_DEBUG("Returning UDF descriptor to REST client for Java UDF: {}", udfName);
    ...
    ENDPOINT("POST", "/registerJavaUdf", registerJavaUdf, BODY_STRING(String, request)) {
        try {
            ...
            x_DEBUG("Parsing Java UDF descriptor from REST request");
            ...
            x_DEBUG("Registering Java UDF '{}'.'", javaUdfRequest.udf_name());
            ...
            x_WARNING("Exception occurred during UDF registration: {}", e.what());
    ...
    ENDPOINT("DELETE", "/removeUdf", removeUdf, QUERY(String, udf, "udfName")) {
        try {
            std::string udfName = udf.getValue("");
            x_DEBUG("Removing Java UDF '{}'", udfName);
...
#endif// x_CORE_INCLUDE_REST_CONTROLLER_UDFCATALOGCONTROLLER_HPP_
