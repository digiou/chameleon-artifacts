...
#ifndef x_CORE_INCLUDE_REST_CONTROLLER_SOURCECATALOGCONTROLLER_HPP_
#define x_CORE_INCLUDE_REST_CONTROLLER_SOURCECATALOGCONTROLLER_HPP_
  ...
  public:
    ...
    ENDPOINT("GET", "/allLogicalSource", getAllLogicalSource) {
        try {
            ...
            if (allLogicalSourceAsString.empty()) {
                x_DEBUG("No Logical Source Found");
    ...
    ENDPOINT("GET", "/allPhysicalSource", getPhysicalSource, QUERY(String, logicalSourceName, "logicalSourceName")) {
        try {
            ...
            x_ERROR("SourceCatalogController: get allPhysicalSource: Exception occurred while building the query plan for user "
                      "request.");
    ...
    ENDPOINT("POST", "/addLogicalSource", addLogicalSource, BODY_STRING(String, request)) {
        ...
        x_DEBUG("SourceCatalogController: addLogicalSource: REST received request to add new Logical Source.");
        try {
            ...
            x_DEBUG("SourceCatalogController: addLogicalSource: Try to add new Logical Source {} and {}",
                      logicalSourceName,
                      schema);
            bool added = sourceCatalogService->registerLogicalSource(logicalSourceName, schema);
            x_DEBUG("SourceCatalogController: addLogicalSource: Successfully added new logical Source ? {}", added);
            ...
            x_ERROR("SourceCatalogController: addLogicalSource: Exception occurred while trying to add new "
                      "logical source {}",
                      exc.what());
            return errorHandler->handleError(Status::CODE_500, exc.what());
        } catch (...) {
    ...
    ENDPOINT("POST", "/addLogicalSource-ex", addLogicalSourceEx, BODY_STRING(String, request)) {
        ...
        try {
            ...
            if (!protobufMessage->ParseFromArray(req.data(), req.size())) {
                x_DEBUG("SourceCatalogController: handlePost -addLogicalSource: invalid Protobuf message");
                nlohmann::json errorResponse{};
                errorResponse["detail"] = "Invalid Protobuf message";
                return errorHandler->handleError(Status::CODE_400, errorResponse.dump());
            }

            x_DEBUG("SourceCatalogController: handlePost -addLogicalSource: Start trying to add new logical source");
            ...
            x_DEBUG("SourceCatalogController: handlePost -addLogicalSource: Successfully added new logical Source ? {}", added);
            ...
            x_ERROR("SourceCatalogController: addLogicalSource-ex: Exception occurred while trying to add new "
                      "logical source {}",
                      exc.what());
            return errorHandler->handleError(Status::CODE_500, exc.what());
        } catch (...) {
    ...
    ENDPOINT("POST", "/updateLogicalSource", updateLogicalSource, BODY_STRING(String, request)) {
        ...
        x_DEBUG("SourceCatalogController: updateLogicalSource: REST received request to update the given Logical Source.");
        try {
            std::string req = request.getValue("{}");
            //check if json is valid
            if (!nlohmann::json::accept(req)) {
            ...
            std::string schema = reqJson["schema"];
            x_DEBUG("SourceCatalogController: updateLogicalSource: Try to update  Logical Source {} with schema {}",
                      sourceName,
                      schema);
            bool updated = sourceCatalogService->updateLogicalSource(sourceName, schema);
            x_DEBUG("SourceCatalogController: addLogicalSource: Successfully added new logical Source ? {}", updated);
            // Prepare the response
            if (updated) {
                nlohmann::json success;
                ...
                x_DEBUG("SourceCatalogController: updateLogicalSource: unable to find given source");
                return errorHandler->handleError(Status::CODE_400, "Unable to update logical source.");
            }
        } catch (const std::exception& exc) {
            x_ERROR("SourceCatalogController: updateLogicalSource: Exception occurred while updating "
                      "Logical Source {}",
                      exc.what());
            return errorHandler->handleError(Status::CODE_500, exc.what());
        } catch (...) {
    ...
    ENDPOINT("POST", "/updateLogicalSource-ex", updateLogicalSourceEx, BODY_STRING(String, request)) {
        ...
        try {
            ...
            if (!protobufMessage->ParseFromArray(req.data(), req.size())) {
                x_DEBUG("SourceCatalogController: handlePost -updateLogicalSource-ex: invalid Protobuf message");
                nlohmann::json errorResponse{};
                errorResponse["detail"] = "Invalid Protobuf message";
                return errorHandler->handleError(Status::CODE_400, errorResponse.dump());
            }

            x_DEBUG("SourceCatalogController: handlePost -updateLogicalSource: Start trying to update logical source");
            // decode protobuf message into c++ obj repr
            auto deserializedSchema = SchemaSerializationUtil::deserializeSchema(protobufMessage->schema());
            std::string sourceName = protobufMessage->sourcename();
    ...
    ENDPOINT("DELETE", "/deleteLogicalSource", deleteLogicalSource, QUERY(String, logicalSourceName, "logicalSourceName")) {
        x_DEBUG("SourceCatalogController: deleteLogicalSource: REST received request to delete the given Logical Source.");
        try {
            bool deleted = sourceCatalogService->unregisterLogicalSource(logicalSourceName);
            x_DEBUG("SourceCatalogController: deleteLogicalSource: Successfully deleted the given logical Source: {}", deleted);
            // Prepare the response
            if (deleted) {
                nlohmann::json success;
                ...
                x_DEBUG("SourceCatalogController: deleteLogicalSource: unable to find given source");
            ...
            x_ERROR("SourceCatalogController: deleteLogicalSource: Exception occurred while building the query plan for user "
                      "request.");
            return errorHandler->handleError(Status::CODE_500, exc.what());
        } catch (...) {
            return errorHandler->handleError(Status::CODE_500, "SourceCatalogController:unknown exception.");
...
#endif// x_CORE_INCLUDE_REST_CONTROLLER_SOURCECATALOGCONTROLLER_HPP_
