...
#ifndef x_CORE_INCLUDE_SINKS_MEDIUMS_ZMQSINK_HPP_
#define x_CORE_INCLUDE_SINKS_MEDIUMS_ZMQSINK_HPP_
...
namespace fmt {
template<>
struct formatter<x::ZmqSink> : formatter<std::string> {
    auto format(const x::ZmqSink& zmq_sink, format_context& ctx) -> decltype(ctx.out()) {
        return fmt::format_to(ctx.out(), "{} {}", zmq_sink.getHost(), zmq_sink.getPort());
    }
};
}//namespace fmt

#endif// x_CORE_INCLUDE_SINKS_MEDIUMS_ZMQSINK_HPP_
