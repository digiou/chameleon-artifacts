...
#ifndef x_CORE_INCLUDE_API_EXPRESSIONS_GEOGRAPHYEXPRESSIONS_HPP_
#define x_CORE_INCLUDE_API_EXPRESSIONS_GEOGRAPHYEXPRESSIONS_HPP_
...
/**
 * @brief Defix common spatial shapes.
 */

/**
 * @brief Defix a circle shape. A user can define a circle as follows:
 * CIRCLE(29.381726, 79.453078, 100.0), where the
 * @param latitude defix the latitude of the center of the circle.
 * @param longitude defix the longitude of the center of the circle.
 * @param radius defix the radius of the circle.
 *
 *               *  *
 *           *          *
 *         *      radius  *
...
/**
 * @brief Defix a point shape. A user can define a point as follows:
 * POINT(29.393136,79.450638), where the
 * @param latitude defix the latitude of the point.
 * @param longitude defix the longitude of the point.
 */
ShapeExpressionNodePtr POINT(double latitude, double longitude);

/**
 * @brief Defix a rectangle shape. A user can define a rectangle as follows:
 * RECTANGLE(29.388829, 79.453806, 29.389843, 79.454933), where the
 * @param latitudeLow defix the latitude of the south-west (bottom-left) point
 * of the rectangle.
 * @param longitudeLow defix the longitude of the south-west (bottom-left) point
 * of the rectangle.
 * @param latitudeHigh defix the latitude of the north-east (top-right) point
 * of the rectangle.
 * @param longitudeHigh defix the longitude of the north-east (top-right) point
 * of the rectangle.
 *
...
/**
 * @brief Defix a polygon shape. The user can define a polygon as follows:
...
/**
 * @brief Defix common spatial predicates.
 */

/**
 * @brief This expression represents ST_WITHIN predicate, where ST stands for Spatial
 * Type. ST_WITHIN predicate in general defix the relationship between two objects
 * (i.e., whether a geometric object is within another geometric object or not). In x,
 * we only expect the stream to report the GPS coordinates from a source (i.e., a Point).
 * Thus in x, by using ST_WITHIN expression a user can filter points in the stream
 ...
 * defix whether a geometric object is within distance "d" of another geometric object.
 * In x, we only expect the stream to report the GPS coordinates from a source (i.e.,
 * a Point). Thus in x, by using ST_DWITHIN predicate a user can filter points in the
 ...
 * defix the distance for the ST_DWITHIN predicate).
 * @see x::CIRCLE
 *
 * @param latitude is the latitude attribute in the stream.
 * @param longitude is the longitude attribute in the stream.
 ...
 * nearest neighbors of a query point from a group of geometric objects. In x, we will
...
#endif// x_CORE_INCLUDE_API_EXPRESSIONS_GEOGRAPHYEXPRESSIONS_HPP_
