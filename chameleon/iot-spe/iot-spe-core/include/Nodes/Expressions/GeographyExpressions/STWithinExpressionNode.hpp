...
#ifndef x_CORE_INCLUDE_NODES_EXPRESSIONS_GEOGRAPHYEXPRESSIONS_STWITHINEXPRESSIONNODE_HPP_
#define x_CORE_INCLUDE_NODES_EXPRESSIONS_GEOGRAPHYEXPRESSIONS_STWITHINEXPRESSIONNODE_HPP_
 ...
 * ST_WITHIN predicate in general defix the relationship between two objects (i.e.,
 * whether a geometric object is within another geometric object or not). In x, we
 * only expect the stream to report the GPS coordinates from a source (i.e., a Point).
 * Thus in x, by using ST_WITHIN expression a user can determine whether a point is
...
#endif// x_CORE_INCLUDE_NODES_EXPRESSIONS_GEOGRAPHYEXPRESSIONS_STWITHINEXPRESSIONNODE_HPP_
