...
#ifndef x_CORE_INCLUDE_UTIL_EXPERIMENTAL_CRC32HASH_HPP_
#define x_CORE_INCLUDE_UTIL_EXPERIMENTAL_CRC32HASH_HPP_
...
class CRC32Hash : public Hash<CRC32Hash> {
  public:
    inline uint64_t hashKey(uint64_t, hash_t) const { x_NOT_IMPLEMENTED(); }
    inline uint64_t hashKey(uint64_t k) const { return hashKey(k, 0); }

    inline uint64_t hashKey(const void*, int, uint64_t) const { x_NOT_IMPLEMENTED(); }
};
...
#endif// x_CORE_INCLUDE_UTIL_EXPERIMENTAL_CRC32HASH_HPP_
