...
#include <assert.h>
#ifndef x_CORE_INCLUDE_UTIL_EXPERIMENTAL_HASHMAP_HPP_
#define x_CORE_INCLUDE_UTIL_EXPERIMENTAL_HASHMAP_HPP_
...
inline auto Hashmap::calculateHash(KeyType& key) const {
    return hasher(key, x::Experimental::Hash<x::Experimental::CRC32Hash>::SEED);
}

template<class KeyType>
inline uint8_t* Hashmap::getEntry(KeyType& key) {
    const auto h = hasher(key, x::Experimental::Hash<x::Experimental::CRC32Hash>::SEED);
...
#endif// x_CORE_INCLUDE_UTIL_EXPERIMENTAL_HASHMAP_HPP_
