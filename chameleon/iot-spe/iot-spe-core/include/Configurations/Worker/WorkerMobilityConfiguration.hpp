...
*/
#ifndef x_CORE_INCLUDE_CONFIGURATIONS_WORKER_WORKERMOBILITYCONFIGURATION_HPP_
#define x_CORE_INCLUDE_CONFIGURATIONS_WORKER_WORKERMOBILITYCONFIGURATION_HPP_
...
class WorkerMobilityConfiguration : public BaseConfiguration {
  public:
    ...
    /**
     * @brief defix how many locations should be saved in the buffer which is used to calculate the predicted path
     */
    ...
    /**
     * @brief defix after how many path prediction update steps a new location should be saved to the buffer
     */
    UIntOption locationBufferSaveRate = {
        LOCATION_BUFFER_SAVE_RATE_CONFIG,
        4,
        "Determix after how many location updates a new location will be inserted in the location buffer"};

    /**
     * @brief defix the minimum distance in meters between the current predicted path and the device position that will lead to a recalculation of the prediction
     */
    ...
    /**
     * @brief defix the radius of the circle used to determine the area within which all field node data will be downloaded during an update of the local index
     */
    ...
    /**
     * @brief defix the distance from the edge of the covered by the current node index which when reached will trigger an update.
    ...
    BoolOption pushDeviceLocationUpdates = {PUSH_DEVICE_LOCATION_UPDATES_CONFIG,
                                            true,
                                            "determix if position updates should be sent to the coordinator"};
     ...
     * a simulation
     */
    EnumOption<x::Spatial::Mobility::Experimental::LocationProviderType> locationProviderType = {
        LOCATION_PROVIDER_TYPE_CONFIG,
        x::Spatial::Mobility::Experimental::LocationProviderType::BASE,
...
#endif// x_CORE_INCLUDE_CONFIGURATIONS_WORKER_WORKERMOBILITYCONFIGURATION_HPP_
