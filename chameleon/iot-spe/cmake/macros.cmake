...
macro(get_header_x HEADER_FILES)
...
macro(register_public_header TARGET HEADER_FILE_DIR)
    add_custom_command(TARGET ${TARGET} POST_BUILD
            ...
            ${HEADER_FILE_DIR} ${CMAKE_BINARY_DIR}/include/IoTSPE)
endmacro()
...
macro(register_public_header_dir TARGET HEADER_FILE_DIR TARGET_DIR)
    add_custom_command(TARGET ${TARGET} POST_BUILD
            COMMENT "COPY ${CMAKE_CURRENT_SOURCE_DIR}"
            COMMAND ${CMAKE_COMMAND} -E copy_directory
            ${HEADER_FILE_DIR} ${CMAKE_BINARY_DIR}/include/IoTSPE/${TARGET_DIR})
endmacro()
...
macro(get_header_x_client HEADER_FILES)
    file(GLOB_RECURSE ${HEADER_FILES} "include/*.h" "include/*.hpp")
endmacro()
...
macro(project_enable_clang_format)
    string(CONCAT FORMAT_DIRS
            "${CMAKE_CURRENT_SOURCE_DIR}/x-benchmark/src,"
            "${CMAKE_CURRENT_SOURCE_DIR}/x-benchmark/include,"
            "${CMAKE_CURRENT_SOURCE_DIR}/x-core/src,"
            "${CMAKE_CURRENT_SOURCE_DIR}/x-core/tests,"
            "${CMAKE_CURRENT_SOURCE_DIR}/x-core/include,"
            "${CMAKE_CURRENT_SOURCE_DIR}/x-common/src,"
            "${CMAKE_CURRENT_SOURCE_DIR}/x-common/tests,"
            "${CMAKE_CURRENT_SOURCE_DIR}/x-common/include,"
            "${CMAKE_CURRENT_SOURCE_DIR}/x-compiler/src,"
            "${CMAKE_CURRENT_SOURCE_DIR}/x-compiler/tests,"
            "${CMAKE_CURRENT_SOURCE_DIR}/x-compiler/include,"
            "${CMAKE_CURRENT_SOURCE_DIR}/x-client/src,"
            "${CMAKE_CURRENT_SOURCE_DIR}/x-client/tests,"
            "${CMAKE_CURRENT_SOURCE_DIR}/x-client/include,"
            "${CMAKE_CURRENT_SOURCE_DIR}/x-data-types/src,"
            "${CMAKE_CURRENT_SOURCE_DIR}/x-data-types/tests,"
            "${CMAKE_CURRENT_SOURCE_DIR}/x-data-types/include,"
            "${CMAKE_CURRENT_SOURCE_DIR}/x-runtime/src,"
            "${CMAKE_CURRENT_SOURCE_DIR}/x-runtime/tests,"
            "${CMAKE_CURRENT_SOURCE_DIR}/x-runtime/include")
...
macro(project_enable_release)
    ...
    elseif (${${PROJECT_NAME}_BRANCH_NAME} STREQUAL "master")
        add_custom_target(release COMMAND echo "Releasing Patch Version of IoTSPE")
        add_custom_command(TARGET release
                COMMAND echo "GIT-CI: Updating x version"
        ...
        add_custom_target(minor_release COMMAND echo "Releasing Minor Version of IoTSPE")
        ...
        add_custom_target(major_release COMMAND echo "Releasing Major Version of IoTSPE")
...
macro(project_enable_version)
    if (NOT IS_GIT_DIRECTORY)
        ...
        add_custom_command(TARGET version COMMAND cp -f ${CMAKE_CURRENT_SOURCE_DIR}/cmake/semver/version.hpp.in ${CMAKE_CURRENT_SOURCE_DIR}/x-core/include/Version/version.hpp
                COMMAND sed -i 's/@x_VERSION_MAJOR@/${${PROJECT_NAME}_VERSION_MAJOR}/g' ${CMAKE_CURRENT_SOURCE_DIR}/x-core/include/Version/version.hpp
                COMMAND sed -i 's/@x_VERSION_MINOR@/${${PROJECT_NAME}_VERSION_MINOR}/g' ${CMAKE_CURRENT_SOURCE_DIR}/x-core/include/Version/version.hpp
                COMMAND sed -i 's/@x_VERSION_PATCH@/${${PROJECT_NAME}_VERSION_PATCH}/g' ${CMAKE_CURRENT_SOURCE_DIR}/x-core/include/Version/version.hpp
                COMMAND sed -i 's/@x_VERSION_POST_FIX@/${${PROJECT_NAME}_x_VERSION_POST_FIX}/g' ${CMAKE_CURRENT_SOURCE_DIR}/x-core/include/Version/version.hpp)
    endif ()
endmacro(project_enable_version)

macro(get_x_log_level_value x_LOGGING_VALUE)
    message(STATUS "Provided log level is: ${x_LOG_LEVEL}")
    if (${x_LOG_LEVEL} STREQUAL "TRACE")
        message("-- Log level is set to TRACE!")
        set(x_SPECIFIC_FLAGS "${x_SPECIFIC_FLAGS} -Dx_LOGLEVEL_TRACE=1")
    elseif (${x_LOG_LEVEL} STREQUAL "DEBUG")
        message("-- Log level is set to DEBUG!")
        set(x_SPECIFIC_FLAGS "${x_SPECIFIC_FLAGS} -Dx_LOGLEVEL_DEBUG=1")
    elseif (${x_LOG_LEVEL} STREQUAL "INFO")
        message("-- Log level is set to INFO!")
        set(x_SPECIFIC_FLAGS "${x_SPECIFIC_FLAGS} -Dx_LOGLEVEL_INFO=1")
    elseif (${x_LOG_LEVEL} STREQUAL "WARN")
        message("-- Log level is set to WARN!")
        set(x_SPECIFIC_FLAGS "${x_SPECIFIC_FLAGS} -Dx_LOGLEVEL_WARNING=1")
    elseif (${x_LOG_LEVEL} STREQUAL "ERROR")
        message("-- Log level is set to ERROR!")
        set(x_SPECIFIC_FLAGS "${x_SPECIFIC_FLAGS} -Dx_LOGLEVEL_ERROR=1")
    elseif (${x_LOG_LEVEL} STREQUAL "FATAL_ERROR")
        message("-- Log level is set to FATAL_ERROR!")
        set(x_SPECIFIC_FLAGS "${x_SPECIFIC_FLAGS} -Dx_LOGLEVEL_FATAL_ERROR=1")
    elseif (${x_LOG_LEVEL} STREQUAL "LEVEL_NONE")
        message("-- Log level is set to LEVEL_NONE!")
        set(x_SPECIFIC_FLAGS "${x_SPECIFIC_FLAGS} -Dx_LOGLEVEL_NONE=1")
    else ()
        message(WARNING "-- Could not set x_LOG_LEVEL as ${x_LOG_LEVEL} did not equal any logging level!!!  Defaulting to DEBUG!")
        set(x_SPECIFIC_FLAGS "${x_SPECIFIC_FLAGS} -Dx_LOGLEVEL_DEBUG=1")
    endif ()
endmacro(get_x_log_level_value x_LOGGING_VALUE)
...
function(set_linux_musl_release_information)
    ...
    # Look for lix like "ID="..." and VERSION_ID="..."
