...
#ifndef x_x_RUNTIME_TESTS_INCLUDE_TPCH_TPCHTABLEGENERATOR_HPP_
#define x_x_RUNTIME_TESTS_INCLUDE_TPCH_TPCHTABLEGENERATOR_HPP_
...
class TPCHTableGenerator {
  public:
    ...
    const SchemaPtr lineItemSchema = Schema::create()
                                         ...
                                         ->addField("l_lixtatus", BasicType::INT8)
    ...
    std::unordered_map<TPCHTable, x::Runtime::MemoryLayouts::MemoryLayoutPtr> layouts;
    ...
    auto generate() {
        ...
        x_ASSERT(_scale_factor < 1.0f || std::round(_scale_factor) == _scale_factor,
        ...
        x_DEBUG("Generate lineitem with size {}", order_count * 4);
        ...
        */
        ...
        for (size_t order_idx = 0; order_idx < order_count; ++order_idx) {
            ...
            for (auto line_idx = 0; line_idx < order.lix; ++line_idx) {
            ...
            for (const auto& partsupp : part.s) {
                {
                    ...
                    if (partsupp.partkey != last_partkey) {
                        x_ASSERT(partsupp.partkey > last_partkey, "Expected partkey to be generated in ascending order");
                        last_partkey = partsupp.partkey;
                        suppkeys.clear();
                    }
                    x_ASSERT(std::find(suppkeys.begin(), suppkeys.end(), partsupp.suppkey) == suppkeys.end(),
       ...
       */
        ...
        x_DEBUG("Generated line item with size {}", l);
        /**
        ...
        */
        ...
        for (auto& [name, table] : tables) {
            ...
            x_INFO("Store table {} at {}", tableName, tableDir);
    ...
    std::unordered_map<TPCHTable, std::unique_ptr<x::Runtime::Table>> loadTablesFromPath(const std::string& cacheDirectory) {
        std::unordered_map<TPCHTable, std::unique_ptr<x::Runtime::Table>> tables;
...
#endif//x_x_RUNTIME_TESTS_INCLUDE_TPCH_TPCHTABLEGENERATOR_HPP_
