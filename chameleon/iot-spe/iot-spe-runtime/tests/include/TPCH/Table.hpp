...
#ifndef x_x_RUNTIME_TESTS_INCLUDE_TPCH_TABLE_HPP_
#define x_x_RUNTIME_TESTS_INCLUDE_TPCH_TABLE_HPP_
  ...
  public:
    ...
    static void store(std::unique_ptr<Table>& table, std::string tableDir) {
        for (size_t i = 0; i < table->chunks.size(); i++) {
            ...
            auto chunkPath = fmt::format("{}/chunk_{}.x", tableDir, i);
    ...
    load(const std::string& tableDir, const Runtime::BufferManagerPtr& bm, const MemoryLayouts::MemoryLayoutPtr& layout) {
        ...
        for (const auto& chunkFile : std::filesystem::directory_iterator(tableDir)) {
            ...
            if (!buffer.has_value()) {
                x_THROW_RUNTIME_ERROR("BufferManager is out of buffers");
            }
            ...
            x_ASSERT(chunkFileSize <= chunk.getBufferSize() + sizeof(uint64_t),
...
#endif//x_x_RUNTIME_TESTS_INCLUDE_TPCH_TABLE_HPP_
