...
namespace x::Runtime::Execution::Util {
...
void writeNautilusRecord(uint64_t recordIndex,
    ...
    if (schema->getLayoutType() == Schema::MemoryLayoutType::ROW_LAYOUT) {
        ...
        x_THROW_RUNTIME_ERROR("Schema Layout not supported!");
    }
}
...
std::vector<Runtime::TupleBuffer> mergeBuffersSameWindow(std::vector<Runtime::TupleBuffer>& buffers,
    ...
    if (schema->getLayoutType() == Schema::MemoryLayoutType::COLUMNAR_LAYOUT) {
        x_FATAL_ERROR("Column layout is not support for this function currently!");
    }

    x_INFO("Merging buffers together!");
...
[[maybe_unused]] std::vector<Runtime::TupleBuffer> createBuffersFromCSVFile(const std::string& csvFile,
    ...
    x_ASSERT2_FMT(std::filesystem::exists(std::filesystem::path(csvFile)), "CSVFile " << csvFile << " does not exist!!!");
    ...
    do {
        ...
        auto values = x::Util::splitWithStringDelimiter<std::string>(line, delimiter);
        ...
        if (tupleCount >= maxTuplesPerBuffer) {
            ...
            x_DEBUG("watermarkTS {} sequenceNumber {} originId {}", watermarkTS, sequenceNumber, originId);
        ...
        if (physicalType->isBasicType()) {
            ...
            switch (basicPhysicalType->nativeType) {
                ...
                case x::BasicPhysicalType::NativeType::TEXT: {
                    x_TRACE("Parser::writeFieldValueToTupleBuffer(): trying to write the variable length input string: {} to "
                              "tuple buffer",
                              inputString);
...
bool checkIfBuffersAreEqual(Runtime::TupleBuffer buffer1, Runtime::TupleBuffer buffer2, uint64_t schemaSizeInByte) {
    x_DEBUG("Checking if the buffers are equal, so if they contain the same tuples...");
    if (buffer1.getNumberOfTuples() != buffer2.getNumberOfTuples()) {
        x_DEBUG("Buffers do not contain the same tuples, as they do not have the same number of tuples");
        return false;
    }
