...
class ProxyFunctionInliningCompilationTest : public Testing::BaseUnitTest, public AbstractCompilationBackendTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("ProxyFunctionInliningCompilationTest.log", x::LogLevel::LOG_DEBUG);
        x_DEBUG("Setup ProxyFunctionInliningCompilationTest test class.");
    }
...
Value<UInt64> getNumberOfTuples(Value<MemRef> tupleBufferRef) {
    return FunctionCall<>("x__Runtime__TupleBuffer__getNumberOfTuples",
                          x::Runtime::ProxyFunctions::x__Runtime__TupleBuffer__getNumberOfTuples,
                          tupleBufferRef);
}
...
void executeFunctionWithOptions(const CompilationOptions& options, const DumpHelper& dumpHelper) {
    ...
    x_DEBUG("Function result: {}", function((uint8_t*) std::addressof(tupleBuffer)));
}
...
TEST_P(ProxyFunctionInliningCompilationTest, getNumberOfTuplesInliningTest) {
    ...
    x_ASSERT2_FMT(generatedProxyIR.peek() != std::ifstream::traits_type::eof(), "No proxy file was generated.");
    ...
    while (std::getline(generatedProxyIR, line)) {
        if (!foundExecute) {
            ...
            x_ASSERT2_FMT(line.find("@tail call") == std::string::npos, "execute contained a tail call even though \
...
TEST_P(ProxyFunctionInliningCompilationTest, getNumberOfTuplesInliningWithoutOptimizationTest) {
    ...
    x_ASSERT2_FMT(callFound, "The execute function should contain a call to getNumberOfTuples when compiling with \
