...
class IfCompilationTest : public Testing::BaseUnitTest, public AbstractCompilationBackendTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("IfCompilationTest.log", x::LogLevel::LOG_DEBUG);
        x_DEBUG("Setup IfCompilationTest test class.");
    }

    /* Will be called after all tests in this class are finished. */
    static void TearDownTestCase() { x_INFO("Tear down IfCompilationTest test class."); }
};
...
TEST_P(IfCompilationTest, xtedIFThenElseConditionTest) {
    auto executionTrace = Nautilus::Tracing::traceFunctionWithReturn([]() {
        return xtedIfThenElseCondition();
    });
    auto engine = prepare(executionTrace);
    auto function = engine->getInvocableMember<int32_t>("execute");
...
TEST_P(IfCompilationTest, xtedIFThenNoElse) {
    auto executionTrace = Nautilus::Tracing::traceFunctionWithReturn([]() {
        return xtedIfNoElseCondition();
    });
    auto engine = prepare(executionTrace);
    auto function = engine->getInvocableMember<int32_t>("execute");
...
Value<> deeplyxtedIfElseCondition() {
    Value value = Value(1);
    Value iw = Value(5);
    if (iw < 8) {
        if (iw > 6) {
...
TEST_P(IfCompilationTest, deeplyxtedIfElseCondition) {
    auto executionTrace = Nautilus::Tracing::traceFunctionWithReturn([]() {
        return deeplyxtedIfElseCondition();
    });
    auto engine = prepare(executionTrace);
    auto function = engine->getInvocableMember<int32_t>("execute");
    ASSERT_EQ(function(), 12);
...
Value<> deeplyxtedIfElseIfCondition() {
    Value value = Value(1);
    Value iw = Value(5);
    if (iw < 8) {
        iw = iw + 10;
...
TEST_P(IfCompilationTest, deeplyxtedIfElseIfCondition) {
    auto executionTrace = Nautilus::Tracing::traceFunctionWithReturn([]() {
        return deeplyxtedIfElseIfCondition();
    });
    auto engine = prepare(executionTrace);
    auto function = engine->getInvocableMember<int32_t>("execute");
...
TEST_P(IfCompilationTest, xtedBooleanFunction) {
