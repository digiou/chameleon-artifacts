...
class StructuredControlFlowPhaseTest : public testing::Test, public AbstractCompilationBackendTest {
  public:
     ...
     */
    bool checkIRForCorrectxs(const std::vector<IR::BasicBlockPtr>& dpsSortedBlocks,
        ...
        for (auto currentBlock : dpsSortedBlocks) {
            if (currentBlock->getTerminatorOp()->getOperationType() == IR::Operations::Operation::OperationType::IfOp) {
                ...
                if (correctBlocks.contains(currentBlock->getIdentifier())) {
                    ...
                    if (!correctMergeBlockId.empty()) {
                        if (!ifOp->getMergeBlock()) {
                            x_ERROR("CurrentBlock: {} did not contain a merge block even though the solution suggest it has a "
                                      "merge-block with id: {}",
                        ...
                        if (!noMergeBlockCorrectlySet) {
                            x_ERROR("The current merge block: {} contains a merge-block with id: {}, even though it should "
                ...
                // Check loop operation for correctxs.
                if (correctBlocks.contains(currentBlock->getIdentifier())) {
                    if (correctBlocks.at(currentBlock->getIdentifier())->countedLoopInfo) {
                        ...
                        if (!loopInfoIsCorrect) {
                            x_ERROR("Loop operation in block: {} should -not- contain counted loop info.",
                                      currentBlock->getIdentifier());
                        } else {
                            ...
                            if (!loopInfoIsCorrect) {
                                x_ERROR("Loop info set incorrectly. Check values: LowerBound: {} vs {}, UpperBound: {} vs {}, "
                ...
                if (correctBlocks.contains(currentBlock->getIdentifier())) {
                    mergeBlocksAreCorrect = false;
                    x_ERROR("CurrentBlock with id: {} was part of solution set(correctBlocks), but it does not contain an "
                              "if-operation.",
                              currentBlock->getIdentifier());
                }
            }
        ...
        if (!numCheckedBlocksMatchesNumCorrectBlocks) {
            x_ERROR("The number of checked IR blocks {} does not match the number of given 'correctBlocks': {}",
                      numCheckedBlocks,
                      correctBlocks.size());
        }
...
//==----------------------------------------------------------==//
Value<> threeIfOperationsOnextedThreeMergeBlocks_1() {
    Value agg = Value(0);
    if (agg < 40) {
        agg = agg + 10;
    } else {
...
TEST_P(StructuredControlFlowPhaseTest, 1_threeIfOperationsOnextedThreeMergeBlocks) {
    ...
    auto dpsSortedGraphNodes = createTraceAndApplyPhases(&threeIfOperationsOnextedThreeMergeBlocks_1);
    ASSERT_EQ(checkIRForCorrectxs(dpsSortedGraphNodes, correctBlocks), true);
}
...
TEST_P(StructuredControlFlowPhaseTest, 2_doubleVerticalDiamondInTrueBranch) {
    ...
    ASSERT_EQ(checkIRForCorrectxs(dpsSortedBlocks, correctBlocks), true);
}
...
Value<> IfOperationFollowedByLoopWithDeeplyxtedIfOperationsWithSeveralxtedLoops_11() {
    Value agg = Value(0);
    Value limit = Value(1000000);
    if (agg < 150) {
        agg = agg + 1;
...
// Breaks in release mode.
TEST_P(StructuredControlFlowPhaseTest, 11_IfOperationFollowedByLoopWithDeeplyxtedIfOperationsWithSeveralxtedLoops) {
    std::unordered_map<std::string, CorrectBlockValuesPtr> correctBlocks;
    createCorrectBlock(correctBlocks, "0", 0, "3");
    ...
    auto dpsSortedBlocks =
        createTraceAndApplyPhases(&IfOperationFollowedByLoopWithDeeplyxtedIfOperationsWithSeveralxtedLoops_11);
    ASSERT_EQ(checkIRForCorrectxs(dpsSortedBlocks, correctBlocks), true);
}
...
Value<> xtedLoopWithFalseBranchPointingToParentLoopHeader_17() {
    Value agg = Value(0);
    Value limit = Value(10);
    while (agg < limit) {
        if (agg < limit) {
...
TEST_P(StructuredControlFlowPhaseTest, 17_xtedLoopWithFalseBranchPointingToParentLoopHeader) {
    std::unordered_map<std::string, CorrectBlockValuesPtr> correctBlocks;
    auto dpsSortedBlocks = createTraceAndApplyPhases(&xtedLoopWithFalseBranchPointingToParentLoopHeader_17);
    createCorrectBlock(correctBlocks, "2", 2, "");
    createCorrectBlock(correctBlocks, "3", 0, "2");
    createCorrectBlock(correctBlocks, "6", 1, "", createCorrectCountedLoopInfo(0, 10, 1, "7"));
    ASSERT_EQ(checkIRForCorrectxs(dpsSortedBlocks, correctBlocks), true);
...
Value<> twoCountedLoopsOneLargeOxmall_23() {
    Value agg = Value(0);
    Value upperBound1 = Value(1000);
...
TEST_P(StructuredControlFlowPhaseTest, twoCountedLoopsOneLargeOxmall) {
    std::unordered_map<std::string, CorrectBlockValuesPtr> correctBlocks;
    ...
    createCorrectBlock(correctBlocks, "34", 1, "", createCorrectCountedLoopInfo(0, 1000, 2, "35"));
    auto dpsSortedBlocks = createTraceAndApplyPhases(&twoCountedLoopsOneLargeOxmall_23);
    ASSERT_EQ(checkIRForCorrectxs(dpsSortedBlocks, correctBlocks), true);
}
