...
class RemoveBranchOnlyBlocksPhaseTest : public Testing::BaseUnitTest, public AbstractCompilationBackendTest {
  public:
    ...
    static void SetUpTestCase() {
        ...
        x_INFO("Setup TraceTest test class.");
    }

    // Takes a Nautilus function, creates the trace, converts it Nautilus IR, and applies all available phases.
    std::shared_ptr<x::Nautilus::IR::IRGraph> createTraceAndApplyPhases(std::function<Value<>()> nautilusFunction) {
     ...
     * @param correctBlockId: The ID of the block that we want to check state correctxs for.
     ...
     */
    bool checkIRForCorrectxs(IR::BasicBlockPtr currentBlock,
...
TEST_P(RemoveBranchOnlyBlocksPhaseTest, 0_SimpleIfOperationWithoutFalseBranch) {
    ...
    EXPECT_EQ(checkIRForCorrectxs(ir->getRootOperation()->getFunctionBasicBlock(), correctBlocks), true);
}
