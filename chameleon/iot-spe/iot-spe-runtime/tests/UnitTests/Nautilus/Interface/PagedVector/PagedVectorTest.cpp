...
#include <memory>
namespace x::Nautilus::Interface {
...
class PagedVectorTest : public Testing::BaseUnitTest {
  public:
    ...
    std::unique_ptr<std::pmr::memory_resource> allocator = std::make_unique<Runtime::xDefaultMemoryAllocator>();

    /* Will be called before any test in this class are executed. */
    static void SetUpTestCase() {
        x::Logger::setupLogging("PagedVectorTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup PagedVectorTest test class.");
    }
    void SetUp() override { Testing::BaseUnitTest::SetUp(); }

    /* Will be called after all tests in this class are finished. */
    static void TearDownTestCase() { x_INFO("Tear down PagedVectorTest test class."); }
    ...
    void insertAndAppendAllPages(const uint64_t entrySize,
        ...
        // Inserting data into each PagedVector and checking for correctxs
        std::vector<std::unique_ptr<PagedVector>> allPagedVectors;
        for (auto& allItems : allItemsAndVectors) {
            allPagedVectors.emplace_back(
                std::make_unique<PagedVector>(std::make_unique<Runtime::xDefaultMemoryAllocator>(), entrySize, pageSize));
