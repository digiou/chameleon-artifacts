...
class H3HashTest : public Testing::BaseUnitTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("H3HashTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("H3HashTest test class SetUpTestCase.");
    }
    ...
    static void TearDownTestCase() { x_INFO("H3HashTest test class TearDownTestCase."); }
};
...
uint64_t getBitMask(uint64_t numberOfKeyBits) {
    switch (numberOfKeyBits) {
        ...
        default: x_THROW_RUNTIME_ERROR("getBitMask got a numberOfKeyBits that was not expected!");
    }
}
