...
class SymbolicTracingTest : public Testing::BaseUnitTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("SymbolicExecutionTest.log", x::LogLevel::LOG_DEBUG);
        x_DEBUG("Setup SymbolicExecutionTest test class.");
    }

    /* Will be called after all tests in this class are finished. */
    static void TearDownTestCase() { x_DEBUG("Tear down SymbolicExecutionTest test class."); }
};
...
TEST_F(SymbolicTracingTest, assignmentOperatorTest) {
    ...
    x_INFO("{}", executionTraceAsString.str());
...
TEST_F(SymbolicTracingTest, xtedIfElseConditionTest) {
    auto executionTrace = Nautilus::Tracing::traceFunction([]() {
        xtedIfThenElseCondition();
    });
    std::stringstream executionTraceString;
    executionTraceString << *executionTrace;
    x_INFO("{}", executionTraceString.str());
...
TEST_F(SymbolicTracingTest, xtedLoopTest) {
    auto execution = Nautilus::Tracing::traceFunction([]() {
        xtedLoop();
    });
    std::stringstream executionString;
...
TEST_F(SymbolicTracingTest, xtedLoopIfTest) {
    auto execution = Nautilus::Tracing::traceFunction([]() {
        xtedLoopIf();
    });
    std::stringstream executionString;
...
TEST_F(SymbolicTracingTest, xtedFunctionCallTest) {
    auto execution = Nautilus::Tracing::traceFunction([]() {
        f1();
    });
    std::stringstream executionString;
