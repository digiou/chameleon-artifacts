...
class AggregationFunctionDataTypeTest : public Testing::BaseUnitTest, public ::testing::WithParamInterface<std::string> {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("AddExpressionTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup AddExpressionTest test class.");
    }
    ...
    static PhysicalTypePtr getDataType(std::string dataTypeString, DefaultPhysicalTypeFactory factory) {
        if (std::equal(dataTypeString.begin(), dataTypeString.end(), "i8")) {
            ...
            x_ERROR("Unknown data type: {}", dataTypeString);
            x_THROW_RUNTIME_ERROR("Unknown data type: " << dataTypeString);
        }
    }
    ...
    static std::shared_ptr<Aggregation::AggregationValue> getAggregationValue(std::string aggFnType, std::string dataTypeString) {
        if (std::equal(aggFnType.begin(), aggFnType.end(), "sum")) {
            ...
            x_ERROR("Unknown aggregation function type type: {}", aggFnType);
            x_THROW_RUNTIME_ERROR("Unknown aggregation function type type: " << aggFnType);
        }
    }
 ...
 */
TEST_P(AggregationFunctionDataTypeTest, scanEmitPipelixum) {
    physicalDataTypeFactory = DefaultPhysicalTypeFactory();

    auto testParam = x::Runtime::Execution::Expressions::AggregationFunctionDataTypeTest_scanEmitPipelixum_Test::GetParam();
