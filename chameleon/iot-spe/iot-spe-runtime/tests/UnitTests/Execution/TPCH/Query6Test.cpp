...
class TPCH_Q6 : public Testing::BaseUnitTest, public AbstractPipelineExecutionTest {
  ...
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("TPCH_Q6.log", x::LogLevel::LOG_DEBUG);

        x_INFO("Setup TPCH_Q6 test class.");
    }

    /* Will be called before a test is executed. */
    void SetUp() override {
        Testing::BaseUnitTest::SetUp();
        x_INFO("Setup TPCH_Q6 test case.");
...
TEST_P(TPCH_Q6, aggregationPipeline) {
    ...
    if (targetScaleFactor == TPCH_Scale_Factor::F1) {
        x_INFO("{:f}", resultDynamicBuffer[0][0].read<float>());
