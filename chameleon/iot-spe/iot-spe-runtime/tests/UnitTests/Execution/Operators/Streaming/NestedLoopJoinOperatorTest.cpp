...
class xtedLoopJoinOperatorTest : public Testing::BaseUnitTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("xtedLoopJoinOperatorTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup xtedLoopJoinOperatorTest test class.");
    }

    /* Will be called before a test is executed. */
    void SetUp() override {
        BaseUnitTest::SetUp();
        x_INFO("Setup xtedLoopJoinOperatorTest test case.");
    ...
    void checkRecordsInBuild(uint64_t windowIdentifier,
        ...
        for (auto pos = windowStartPos; pos < windowEndPos; ++pos, ++posInWindow) {
            ...
            x_TRACE("readRecord {} record{}", readRecord.toString(), record.toString());
    ...
    void checkWindowsInBuild(uint64_t maxTimestamp, std::vector<Record>& allLeftRecords, std::vector<Record>& allRightRecords) {
        ...
        for (auto windowIdentifier = windowSize; windowIdentifier < maxWindowIdentifier; windowIdentifier += windowSize) {
            ...
            x_DEBUG("Check window={}", windowIdentifier);
...
TEST_F(xtedLoopJoinOperatorTest, joinBuildSimpleTestOneRecord) {
...
TEST_F(xtedLoopJoinOperatorTest, joinBuildSimpleTestMultipleRecords) {
...
TEST_F(xtedLoopJoinOperatorTest, joinBuildSimpleTestMultipleWindows) {
...
TEST_F(xtedLoopJoinOperatorTest, joinProbeSimpleTestOneWindow) {
...
TEST_F(xtedLoopJoinOperatorTest, joinProbeSimpleTestMultipleWindows) {
    auto numberOfRecordsLeft = 2000;
    auto numberOfRecordsRight = 2000;
