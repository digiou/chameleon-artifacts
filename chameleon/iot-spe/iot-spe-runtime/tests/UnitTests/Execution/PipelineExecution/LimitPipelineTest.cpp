...
class LimitPipelineTest : public Testing::BaseUnitTest, public AbstractPipelineExecutionTest {
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("LimitPipelineTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup LimitPipelineTest test class.");
    }

    /* Will be called before a test is executed. */
    void SetUp() override {
        Testing::BaseUnitTest::SetUp();
        x_INFO("Setup LimitPipelineTest test case.");
    ...
    static void TearDownTestCase() { x_INFO("Tear down LimitPipelineTest test class."); }
};
...
TEST_P(LimitPipelineTest, LimitPipelineTest) {
    ...
    for (uint64_t i = 0; i < TUPLES; ++i) {
        dynamicBuffer[i]["f1"].write((uint64_t) i);
    ...
    for (uint64_t i = 0; i < LIMIT; ++i) {
        ASSERT_EQ(resultDynamicBuffer[i]["f1"].read<uint64_t>(), (uint64_t) i);
    }
}
