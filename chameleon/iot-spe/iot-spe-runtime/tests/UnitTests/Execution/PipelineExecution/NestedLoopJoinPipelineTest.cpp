...
class xtedLoopJoinMockedPipelineExecutionContext : public Runtime::Execution::PipelineExecutionContext {
  public:
    xtedLoopJoinMockedPipelineExecutionContext(BufferManagerPtr bufferManager,
...
class xtedLoopJoinPipelineTest : public Testing::BaseUnitTest, public AbstractPipelineExecutionTest {
  ...
  public:
    ...
    static void SetUpTestCase() {
        x::Logger::setupLogging("xtedLoopJoinPipelineTest.log", x::LogLevel::LOG_DEBUG);
        x_INFO("Setup xtedLoopJoinPipelineTest test class.");
    }

    /* Will be called before a test is executed. */
    void SetUp() override {
        BaseUnitTest::SetUp();
        x_INFO("Setup xtedLoopJoinPipelineTest test case.");
    ...
    bool checkIfNLJWorks(const std::string& fileNameBuffersLeft,
        ...
        pipelixink->setRootOperator(nljProbe);

        auto curPipelineId = 0;
        auto noWorkerThreads = 1;
        auto pipelineExecCtxLeft =
            xtedLoopJoinMockedPipelineExecutionContext(bufferManager, noWorkerThreads, nljOperatorHandler, curPipelineId++);
        ...
        nljWorks = nljWorks && (executablePipelixink->setup(pipelineExecCtxSink) == 0);
...
TEST_P(xtedLoopJoinPipelineTest, nljSimplePipeline) {
...
TEST_P(xtedLoopJoinPipelineTest, nljSimplePipelineDifferentInput) {
    const auto leftSchema = Schema::create(Schema::MemoryLayoutType::ROW_LAYOUT)
                                ->addField("left$id", BasicType::UINT64)
                                ->addField("left$value", BasicType::UINT64)
                                ->addField("left$timestamp", BasicType::UINT64);
...
TEST_P(xtedLoopJoinPipelineTest, nljSimplePipelineDifferentNumberOfAttributes) {
    const auto leftSchema = Schema::create(Schema::MemoryLayoutType::ROW_LAYOUT)
                                ->addField("left$id", BasicType::UINT64)
                                ->addField("left$value", BasicType::UINT64)
                                ->addField("left$timestamp", BasicType::UINT64);
...
INSTANTIATE_TEST_CASE_P(xtedLoopJoinPipelineTest,
                        xtedLoopJoinPipelineTest,
                        ::testing::Values("PipelineInterpreter", "PipelineCompiler", "CPPPipelineCompiler"),
                        [](const testing::TestParamInfo<xtedLoopJoinPipelineTest::ParamType>& info) {
                            return info.param;
                        });
