...
TanExpression::TanExpression(const x::Runtime::Execution::Expressions::ExpressionPtr& expression) : expression(expression) {}

double calculateTan(double x) { return std::tan(x); }

Value<> TanExpression::execute(x::Nautilus::Record& record) const {
