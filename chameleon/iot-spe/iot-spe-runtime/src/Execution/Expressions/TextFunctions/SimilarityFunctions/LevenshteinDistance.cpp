...
LevenshteinDistance::LevenshteinDistance(const x::Runtime::Execution::Expressions::ExpressionPtr& leftSubExpression,
...
uint64_t textLevenshtein(const TextValue* leftText, const TextValue* rightText) {
    ...
    x_DEBUG("The left string length is {} and the right string length is {}",
...
Value<> LevenshteinDistance::execute(x::Nautilus::Record& record) const {
    ...
    if (leftValue->isType<Text>() && rightValue->isType<Text>()) {
        ...
        x_THROW_RUNTIME_ERROR("This expression is only defined on input arguments that are Text.");
    }
}
