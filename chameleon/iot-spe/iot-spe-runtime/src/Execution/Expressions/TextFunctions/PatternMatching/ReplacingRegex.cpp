...
ReplacingRegex::ReplacingRegex(const x::Runtime::Execution::Expressions::ExpressionPtr& textValue,
                               const x::Runtime::Execution::Expressions::ExpressionPtr& regexpPattern,
                               const x::Runtime::Execution::Expressions::ExpressionPtr& textReplacement)
...
TextValue* regexReplace(TextValue* text, TextValue* reg, TextValue* replacement) {
    ...
    x_DEBUG("Received the following source string {}", strText);
    std::regex tempRegex(std::string(reg->c_str(), reg->length()));
    x_DEBUG("Received the following reg string {}", std::string(reg->c_str(), reg->length()));
    std::string strReplacement = std::string(replacement->c_str(), replacement->length());
    x_DEBUG("Received the following replacement string {}", strReplacement);
    std::string strReplaced = std::regex_replace(strText, tempRegex, strReplacement);
    x_DEBUG("Created the string {}", strReplaced)
    return TextValue::create(strReplaced);
}

Value<> ReplacingRegex::execute(x::Nautilus::Record& record) const {
    ...
    if (text->isType<Text>() && pattern->isType<Text>() && replacement->isType<Text>()) {
        ...
        x_THROW_RUNTIME_ERROR("This expression is only defined on input arguments of type Text.");
    }
}
