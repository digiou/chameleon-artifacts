...
ExtractingRegex::ExtractingRegex(const x::Runtime::Execution::Expressions::ExpressionPtr& textValue,
                                 const x::Runtime::Execution::Expressions::ExpressionPtr& regexpPattern,
                                 const x::Runtime::Execution::Expressions::ExpressionPtr& idx)
...
* @param idx result index as Integer defix at what position in the textValue matching starts (only the first match is returned)
* @return TextValue*
*/
...
Value<> ExtractingRegex::execute(x::Nautilus::Record& record) const {
    ...
    if (text->isType<Text>() && pattern->isType<Text>()) {
        ...
        if (pos->isType<Int8>()) {
            ...
            x_THROW_RUNTIME_ERROR("Idx only defined on a numeric input argument that is of type Integer.");
        }
    } else {
        x_THROW_RUNTIME_ERROR("This expression is only defined for inputs of type Text and an index as Integer.");
    }
}
