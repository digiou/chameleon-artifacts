...
MatchingRegex::MatchingRegex(const x::Runtime::Execution::Expressions::ExpressionPtr& textValue,
                             const x::Runtime::Execution::Expressions::ExpressionPtr& regexpPattern,
                             const x::Runtime::Execution::Expressions::ExpressionPtr& caseSensitive)
...
Value<> MatchingRegex::execute(x::Nautilus::Record& record) const {
    ...
    if (text->isType<Text>() && pattern->isType<Text>() && caseSensitiveFlag->isType<Boolean>()) {
        ...
        x_DEBUG("{}", text->getType()->toString());
        std::stringstream patternType;
        patternType << pattern->getType();
        x_DEBUG("{}", patternType.str());
        std::stringstream type;
        type << caseSensitiveFlag->getType();
        x_DEBUG("{}", type.str());
        x_THROW_RUNTIME_ERROR("This expression is only defined on input arguments that are Text and a Boolean for case "
