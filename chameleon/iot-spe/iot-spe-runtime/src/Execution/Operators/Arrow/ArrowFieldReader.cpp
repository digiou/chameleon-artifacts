...
template<typename ArrowType>
Value<> ArrowFieldReader<ArrowType>::getColumn(const Value<x::Nautilus::MemRef>& recordBatch) {
...
template<typename ArrowType>
Value<> ArrowFieldReader<ArrowType>::getValue(const Value<x::Nautilus::MemRef>& column, const Value<UInt64>& index) {
    if constexpr (HasRawValues<ArrowType>) {
