...
ExecutionContext::ExecutionContext(const Value<x::Nautilus::MemRef>& workerContext,
                                   const Value<x::Nautilus::MemRef>& pipelineContext)
...
void* allocateBufferProxy(void* workerContextPtr) {
    if (workerContextPtr == nullptr) {
        x_THROW_RUNTIME_ERROR("worker context should not be null");
    }
...
void ExecutionContext::emitBuffer(const x::Runtime::Execution::RecordBuffer& buffer) {
...
Operators::OperatorState* ExecutionContext::getLocalState(const Operators::Operator* op) {
    ...
    if (stateEntry == localStateMap.end()) {
        x_THROW_RUNTIME_ERROR("No local state registered for operator: " << op);
    }
...
void ExecutionContext::setLocalOperatorState(const Operators::Operator* op, std::unique_ptr<Operators::OperatorState> state) {
    if (localStateMap.contains(op)) {
        x_THROW_RUNTIME_ERROR("Operators state already registered for operator: " << op);
    }
...
void* getGlobalOperatorHandlerProxy(void* pc, uint64_t index) {
    ...
    if (index >= size) {
        x_THROW_RUNTIME_ERROR("operator handler at index " + std::to_string(index) + " is not registered");
    }
