...
void PythonUDFOperatorHandler::initPython() {
    ...
    if (pythonCode == NULL) {
        if (PyErr_Occurred()) {
            PyErr_Print();
            PyErr_Clear();
            x_THROW_RUNTIME_ERROR("Could not compile String.");
        }
    }
    ...
    if (this->pythonModule == NULL) {
        ...
        x_THROW_RUNTIME_ERROR("Cannot add function " << this->functionName << " to module " << this->moduleName);
    }
}
...
void PythonUDFOperatorHandler::finalize() {
    ...
    if (Py_IsInitialized()) {
        if (Py_FinalizeEx() == -1) {
            PyErr_Print();
            PyErr_Clear();
            x_THROW_RUNTIME_ERROR("Something went wrong with finalizing Python");
        }
    }
}
