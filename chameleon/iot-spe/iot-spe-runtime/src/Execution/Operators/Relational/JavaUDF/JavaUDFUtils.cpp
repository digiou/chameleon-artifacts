...
void* allocateObject(void* clazzPtr) {
    x_ASSERT2_FMT(clazzPtr != nullptr, "clazzPtr should not be null");
...
T getField(void* state, void* classPtr, void* objectPtr, int fieldIndex, std::string signature) {
    x_ASSERT2_FMT(state != nullptr, "op handler context should not be null");
    x_ASSERT2_FMT(classPtr != nullptr, "classPtr should not be null");
    x_ASSERT2_FMT(objectPtr != nullptr, "objectPtr should not be null");
    auto handler = static_cast<JavaUDFOperatorHandler*>(state);
