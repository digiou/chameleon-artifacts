...
StreamWindowPtr StreamJoinOperatorHandler::createNewWindow(uint64_t timestamp) {
    ...
    if (joinStrategy == QueryCompilation::StreamJoinStrategy::xTED_LOOP_JOIN) {
        auto* nljOpHandler = dynamic_cast<NLJOperatorHandler*>(this);
        x_DEBUG("Create NLJ Window for window start={} windowend={} for ts={}", windowStart, windowEnd, timestamp);
        ...
        newWindow = std::make_shared<StreamHashJoinWindow>(numberOfWorkerThreads,
                                                           ...
                                                           joinStrategy);
        x_DEBUG("Create Hash Window for window start={} windowend={} for ts={}", windowStart, windowEnd, timestamp);
    }
...
void StreamJoinOperatorHandler::deleteWindow(uint64_t windowIdentifier) {
    x_DEBUG("StreamJoinOperatorHandler trying to delete window with id={}", windowIdentifier);
    {
...
void StreamJoinOperatorHandler::setup(uint64_t newNumberOfWorkerThreads) {
    if (alreadySetup) {
        x_DEBUG("StreamJoinOperatorHandler::setup was called already!");
        return;
    }
    alreadySetup = true;

    x_DEBUG("HashJoinOperatorHandler::setup was called!");
    ...
    std::vector<uint64_t> windowIdentifiers;
    {
        ...
        for (auto& window : *windowsLocked) {
            ...
            if (window->shouldTriggerDuringTerminate() && window->getNumberOfTuplesLeft() > 0
                ...
                x_DEBUG("Added window with id {} to the triggerable windows with {} tuples left and {} tuples right...",
