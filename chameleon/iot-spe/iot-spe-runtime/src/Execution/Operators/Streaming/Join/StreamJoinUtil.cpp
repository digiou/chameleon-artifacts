...
SchemaPtr createJoinSchema(const SchemaPtr& leftSchema, const SchemaPtr& rightSchema, const std::string& keyFieldName) {
    x_ASSERT(leftSchema->getLayoutType() == rightSchema->getLayoutType(),
               "Left and right schema do not have the same layout type");
    x_ASSERT(leftSchema->contains(keyFieldName) || rightSchema->contains(keyFieldName) || leftSchema->hasFieldName(keyFieldName)
    ...
    x_DEBUG("Created joinSchema {} from leftSchema {} and rightSchema {}.",
              retSchema->toString(),
              leftSchema->toString(),
