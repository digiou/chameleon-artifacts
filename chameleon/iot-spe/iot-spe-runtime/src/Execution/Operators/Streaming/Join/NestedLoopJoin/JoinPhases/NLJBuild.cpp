...
void setNumberOfWorkerThreadsProxy(void* ptrOpHandler, void* ptrPipelineContext) {
    ...
    x_ASSERT2_FMT(ptrPipelineContext != nullptr, "pipeline context should not be null!");
...
void triggerAllWindowsProxy(void* ptrOpHandler, void* ptrPipelineCtx) {
    ...
    auto* opHandler = static_cast<NLJOperatorHandler*>(ptrOpHandler);
    auto* pipelineCtx = static_cast<PipelineExecutionContext*>(ptrPipelineCtx);
    x_DEBUG("Triggering all windows for pipelineId {}!", pipelineCtx->getPipelineID());
...
void NLJBuild::updateLocalJoinState(LocalxtedLoopJoinState* localJoinState,
    ...
    x_DEBUG("Updating LocalJoinState!");
...
void NLJBuild::execute(ExecutionContext& ctx, Record& record) const {
    ...
    auto localJoinState = dynamic_cast<LocalxtedLoopJoinState*>(ctx.getLocalState(this));
...
void NLJBuild::open(ExecutionContext& ctx, RecordBuffer&) const {
    ...
    auto localJoinState = std::make_unique<LocalxtedLoopJoinState>(opHandlerMemRef, windowReference, pagedVectorRef);
