...
void* getNLJWindowRef(void* ptrOpHandler, uint64_t windowIdentifier) {
    ...
    x_INFO("windowIdentifier: {}", windowIdentifier);
...
void deleteWindowProxyForxtedLoopJoin(void* ptrOpHandler, uint64_t windowIdentifier) {
    x_ASSERT2_FMT(ptrOpHandler != nullptr, "op handler context should not be null");
...
void NLJProbe::open(ExecutionContext& ctx, RecordBuffer& recordBuffer) const {
    ...
    Nautilus::FunctionCall("deleteWindowProxyForxtedLoopJoin",
                           deleteWindowProxyForxtedLoopJoin,
