...
NLJOperatorHandler::NLJOperatorHandler(const std::vector<OriginId>& inputOrigins,
    ...
    : StreamJoinOperatorHandler(inputOrigins,
                                outputOriginId,
                                windowSize,
                                QueryCompilation::StreamJoinStrategy::xTED_LOOP_JOIN,
                                sizeOfTupleInByteLeft,
                                sizeOfTupleInByteRight),
      leftPageSize(sizePageLeft), rightPageSize(sizePageRight) {}

void NLJOperatorHandler::start(PipelineExecutionContextPtr, StateManagerPtr, uint32_t) { x_DEBUG("start NLJOperatorHandler"); }
...
void NLJOperatorHandler::stop(QueryTerminationType, PipelineExecutionContextPtr) { x_DEBUG("stop NLJOperatorHandler"); }
...
void NLJOperatorHandler::triggerWindows(std::vector<uint64_t>& windowIdentifiersToBeTriggered,
    ...
    for (auto& windowIdentifier : windowIdentifiersToBeTriggered) {
        ...
        if (!nljWindow.has_value()) {
            x_THROW_RUNTIME_ERROR("Could not find window for " << std::to_string(windowIdentifier)
        ...
        x_TRACE("Emitted windowIdentifier for window {} with watermarkTs {} sequenceNumber {} originId {}",
                  windowVal->toString(),
                  buffer.getWatermark(),
...
void* getNLJPagedVectorProxy(void* ptrNljWindow, uint64_t workerId, uint64_t joinBuildSideInt) {
    x_ASSERT2_FMT(ptrNljWindow != nullptr, "nlj window pointer should not be null!");
