...
void AppendToSliceStoreHandler<Slice>::triggerSlidingWindows(Runtime::WorkerContext& wctx,
    ...
    x_ASSERT(sliceStore != 0, "slice store is not initialized");
    ...
    x_TRACE("Trigger sliding windows between {}-{}", lastTriggerWatermark, currentWatermark);
    ...
    for (const auto& [windowStart, windowEnd] : windows) {
        ...
        x_TRACE("Deploy window ({}-{}) merge task for {} slices  ", windowStart, windowEnd, slicesForWindow.size());
...
template<class Slice>
void AppendToSliceStoreHandler<Slice>::stop(x::Runtime::QueryTerminationType queryTerminationType,
                                            x::Runtime::Execution::PipelineExecutionContextPtr ctx) {
    x_DEBUG("stop AppendToSliceStoreHandler: {}", queryTerminationType);
