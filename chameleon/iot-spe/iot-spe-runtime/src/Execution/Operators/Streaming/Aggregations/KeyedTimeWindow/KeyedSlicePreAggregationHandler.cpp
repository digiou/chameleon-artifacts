...
void KeyedSlicePreAggregationHandler::setup(Runtime::Execution::PipelineExecutionContext& ctx,
                                            uint64_t keySize,
                                            uint64_t valueSize) {
    x_ASSERT(threadLocalSliceStores.empty(), "The thread local slice store must be empty");
...
KeyedSlicePreAggregationHandler::~KeyedSlicePreAggregationHandler() { x_DEBUG("~GlobalSlicePreAggregationHandler"); }
