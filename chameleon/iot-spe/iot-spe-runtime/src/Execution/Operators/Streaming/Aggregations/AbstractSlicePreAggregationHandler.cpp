...
void AbstractSlicePreAggregationHandler<SliceType, SliceStore>::dispatchSliceMergingTasks(
    ...
    for (const auto& [metaData, slices] : collectedSlices) {
        ...
        task->slices = slices;
        x_DEBUG("{} Deploy merge task for slice {}-{} ", windowSize, task->startSlice, task->endSlice);
...
void AbstractSlicePreAggregationHandler<SliceType, SliceStore>::trigger(WorkerContext& wctx,
    ...
    x_TRACE("{} Trigger {}-{}-{}", windowSize, originId, sequenceNumber, watermarkTs);
    ...
    x_TRACE("{} Trigger slices between {}-{}", windowSize, lastTriggerWatermark, currentWatermark);
    ...
    for (auto& threadLocalSliceStore : threadLocalSliceStores) {
        for (const auto& slice : threadLocalSliceStore->getSlices()) {
            x_TRACE("Slices in slice store {}-{}", slice->getStart(), slice->getEnd());
        }
        auto slices = threadLocalSliceStore->extractSlicesUntilTs(lastTriggerWatermark);
        for (const auto& slice : slices) {
            x_TRACE("Assign thread local slices {}-{}", slice->getStart(), slice->getEnd());
...
void AbstractSlicePreAggregationHandler<SliceType, SliceStore>::start(PipelineExecutionContextPtr, StateManagerPtr, uint32_t) {
    x_DEBUG("start AbstractSlicePreAggregationHandler");
}
...
void AbstractSlicePreAggregationHandler<SliceType, SliceStore>::stop(QueryTerminationType queryTerminationType,
    ...
    x_DEBUG("shutdown AbstractSlicePreAggregationHandler: {}", queryTerminationType);
