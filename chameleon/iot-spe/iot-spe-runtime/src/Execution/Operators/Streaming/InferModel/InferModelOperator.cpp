...
void InferModelOperator::execute(ExecutionContext& ctx, x::Nautilus::Record& record) const {
    ...
    for (uint32_t i = 0; i < inputFieldNames.size(); i++) {
        ...
        if (value->isType<Boolean>()) {
            ...
            x_ERROR("Can not handle inputs other than of type int, bool, float, and double");
        }
    }
