...
CompiledExecutablePipelixtage::CompiledExecutablePipelixtage(
    ...
    : NautilusExecutablePipelixtage(physicalOperatorPipeline), compilationBackend(compilationBackend), options(options),
...
std::shared_ptr<x::Nautilus::IR::IRGraph> CompiledExecutablePipelixtage::createIR(DumpHelper& dumpHelper, Timer<>& timer) {

    auto pipelineExecutionContextRef = Value<MemRef>((int8_t*) nullptr);
    pipelineExecutionContextRef.ref =
        Nautilus::Tracing::ValueRef(INT32_MAX, 0, x::Nautilus::IR::Types::StampFactory::createAddressStamp());
    auto workerContextRef = Value<MemRef>((int8_t*) nullptr);
    workerContextRef.ref =
        Nautilus::Tracing::ValueRef(INT32_MAX, 1, x::Nautilus::IR::Types::StampFactory::createAddressStamp());
    auto memRef = Nautilus::Value<Nautilus::MemRef>(std::make_unique<Nautilus::MemRef>(Nautilus::MemRef(0)));
    memRef.ref = Nautilus::Tracing::ValueRef(INT32_MAX, 2, x::Nautilus::IR::Types::StampFactory::createAddressStamp());
...
std::unique_ptr<Nautilus::Backends::Executable> CompiledExecutablePipelixtage::compilePipeline() {
    // compile after setup
    ...
    timerAsString << timer;
    x_INFO("{}", timerAsString.str());
    return executable;
}

uint32_t CompiledExecutablePipelixtage::setup(PipelineExecutionContext& pipelineExecutionContext) {
    NautilusExecutablePipelixtage::setup(pipelineExecutionContext);
