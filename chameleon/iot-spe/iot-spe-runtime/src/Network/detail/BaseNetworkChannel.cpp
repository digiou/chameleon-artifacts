...
void BaseNetworkChannel::onError(Messages::ErrorMessage& errorMsg) { x_ERROR("{}", errorMsg.getErrorTypeAsString()); }
...
void BaseNetworkChannel::close(bool isEventOnly, Runtime::QueryTerminationType terminationType) {
    ...
    zmqSocket.close();
    x_DEBUG("Socket(\"{}\") closed for {} {}", socketAddr, channelId, (isEventOnly ? " Event" : " Data"));
    isClosed = true;
}
}// namespace x::Network::detail