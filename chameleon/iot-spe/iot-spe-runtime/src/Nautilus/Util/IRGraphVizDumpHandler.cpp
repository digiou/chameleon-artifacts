...
xIRDumpHandler::xIRDumpHandler(std::ostream& out) : out(out) {}

std::shared_ptr<xIRDumpHandler> xIRDumpHandler::create(std::ostream& out) { return std::make_shared<xIRDumpHandler>(out); }

IR::BasicBlockPtr xIRDumpHandler::getNextLowerOrEqualLevelBasicBlock(BasicBlockPtr thenBlock, int ifParentBlockLevel) {
...
void xIRDumpHandler::dumpHelper(OperationPtr const& terminatorOp, int32_t scopeLevel) {
...
void xIRDumpHandler::dumpHelper(BasicBlockPtr const& basicBlock) {
...
void xIRDumpHandler::dump(const std::shared_ptr<Operations::FunctionOperation> funcOp) {
    out << funcOp->toString() << " {";
