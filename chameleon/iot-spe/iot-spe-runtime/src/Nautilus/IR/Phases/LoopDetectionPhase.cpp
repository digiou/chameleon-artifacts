...
void LoopDetectionPhase::LoopDetectionPhaseContext::process() {
    std::shared_ptr<x::Nautilus::IR::Operations::FunctionOperation> rootOperation = ir->getRootOperation();
...
void LoopDetectionPhase::LoopDetectionPhaseContext::checkBranchForLoopHeadBlocks(
    ...
    // Follow the true-branch of the current, and all xted if-operations until either
        ...
        if (currentBlock->getTerminatorOp()->getOperationType() == Operations::Operation::OperationType::IfOp) {
            ...
            if (currentBlock->getNumLoopBackEdges() < 2 && currentBlock->getPredecessors().size() == 2) {
                ...
                if (compareOp) {
                    ...
                    if (compareOp->getComparator() != Operations::CompareOperation::EQ
                        ...
                        if (compareOpConstants.first && compareOpConstants.second) {
                            if (compareOp->getLeftInput() == loopInductionVarArg) {
                                ...
                                x_DEBUG("Could not detect counted loop. The loop induction variable is not part of the "
                                          "loop-header comparison operation.");
                                return;
                            }
                        } else {
                            x_DEBUG("Could not detect counted loop. Either the loop induction variable or the loop stop "
                        ...
                        if (!stepSize) {
                            x_DEBUG("Could not detect counted loop. The loop induction and/or the stepSize is/are manipulated "
                        ...
                        if (inductionVar->getValue() < upperBound->getValue() && stepSize->getValue() > 0) {
                            ...
                            x_DEBUG("Could not detect a counted loop. Reason 1: UpperBound == LowerBound, Reason 2: "
                        ...
                        x_DEBUG("Could not detect counted loop. Possible reasons: \n"
                    ...
                    x_DEBUG("Loop header without comparison operation not supported. This currently includes '!='");
                }
            }
        }
    }
...
void LoopDetectionPhase::LoopDetectionPhaseContext::findLoopHeadBlocks(IR::BasicBlockPtr currentBlock) {
    ...
    do {
        ...
        if (!noMoreIfBlocks) {
            ...
            // The false branch might contain xted loop-operations.
