...
PagedVector::PagedVector(std::unique_ptr<std::pmr::memory_resource> allocator, uint64_t entrySize, uint64_t pageSize)
    ...
    appendPage();
    x_ASSERT2_FMT(entrySize > 0, "Entrysize for a pagedVector has to be larger than 0!");
    x_ASSERT2_FMT(capacityPerPage > 0, "There has to fit at least one tuple on a page!");
}
...
void PagedVector::moveFromTo(uint64_t oldPos, uint64_t newPos) const {
    ...
    x_DEBUG("Moved from {} to {}", oldPos, newPos);
}
...
void PagedVector::appendAllPages(PagedVector& other) {
    x_ASSERT2_FMT(pageSize == other.pageSize, "Can not combine PagedVector of different pageSizes for now!");
    x_ASSERT2_FMT(entrySize == other.entrySize, "Can not combine PagedVector of different entrySize for now!");
