...
size_t getCapacity(uint64_t numberOfKeys) {
    ...
    x_ASSERT(exp < sizeof(size_t) * 8, "invalid exp");
...
ChainedHashMap::ChainedHashMap(uint64_t keySize,
    ...
    x_ASSERT(nrEntries != 0, "invalid num entries");
    x_ASSERT(keySize != 0, "invalid key size");
    x_ASSERT(pageSize >= entrySize, "invalid page size");
