...
Value<> evalBinary(
    ...
    x_THROW_RUNTIME_ERROR("No plugin registered that can handle this operation between " << left->toString() << " and "
                                                                                           << right->toString());
}
...
Value<> NegateOp(const Value<>& input) {
    ...
    x_THROW_RUNTIME_ERROR("No plugin registered that can handle this operation");
}
