...
uint64_t stringtomillisecondsproxy(TextValue* t) {
    ...
    if (timeString.find('T') != std::string::npos) {
        ...
        x_DEBUG(" the year {} the month {} the day {} the hour {} the minute {} and the second {}",
                  year,
                  month,
                  day,
                  hour,
        ...
        x_DEBUG(" the year {} the month {} and the day {}", year, month, day);
    }
...
TimeStamp::TimeStamp(Value<> x) : Any(&type), milliseconds(x) {
    if (x->isType<UInt64>()) {
        ...
        x_THROW_RUNTIME_ERROR("Can not make a TimeStamp object out of" << x);
    }
}
...
TextValue* intervalproxy(int64_t milliseconds) {
    ...
    return x::Nautilus::TextValue::create(inti);
}
...
TextValue* weekdayNameproxy(int64_t milliseconds) {
    ...
    const std::string weekdayNames[] = {"Sunday", "Monday", "Tuesday", "Wedxday", "Thursday", "Friday", "Saturday"};
    // Return the name of the day of the week
    auto weekdayName = weekdayNames[dayOfWeek];
    //Value<TextValue> valueweekdayname = (TextValue)weekdayName);
    x_TRACE("Find the weekday name {}", weekdayName);
    x::Nautilus::TextValue* t = x::Nautilus::TextValue::create(weekdayName);
    return t;
}
...
TextValue* getMonthNameProxy(int64_t milliseconds) {
    ...
    switch (month) {
        ...
        default: x_THROW_RUNTIME_ERROR("Invalid Month");
    }
}
