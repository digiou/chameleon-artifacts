...
Text::Text(TypedRef<x::Nautilus::TextValue> rawReference) : Any(&type), rawReference(rawReference){};

bool textEquals(const TextValue* leftText, const TextValue* rightText) {
    x_DEBUG("Check if {} and {} are equal", leftText->c_str(), rightText->c_str())
...
TextValue* textSubstring(const TextValue* text, uint32_t index, uint32_t len) {
    if (text->length() < index + len) {
        x_THROW_RUNTIME_ERROR("Text was not long enough");
    }
...
bool textPrefix(const TextValue* leftText, const TextValue* rightText) {
    if (rightText->length() > leftText->length()) {
        x_THROW_RUNTIME_ERROR("prefixText is longer than sourceText");
    }
    if (leftText->length() < rightText->length()) {
        return false;
    }
...
TextValue* textRepeat(const TextValue* text, uint32_t n) {
    if (n <= 0) {
        x_THROW_RUNTIME_ERROR("repetitions require a positive int, but received n<=0");
    }
...
uint32_t textPosition(const TextValue* text, const TextValue* target) {
    ...
    if (text->length() < target->length()) {
        x_THROW_RUNTIME_ERROR("TargetText length is longer than sourceText length");
    }
    for (uint32_t i = 0; i < text->length(); i++) {
...
bool textSimilarTo(const TextValue* text, TextValue* pattern) {
    ...
    x_DEBUG("Received the following source string {}", target);
    std::string strPattern = std::string(pattern->str(), pattern->length());
    x_DEBUG("Received the following source string {}", strPattern);
...
bool textLike(const TextValue* text, TextValue* inputPattern, Boolean caseSensitive) {
    ...
    x_DEBUG("Checking in textLike if {} and {} are a match.", text->c_str(), pattern);
    ...
    std::string target = std::string(text->c_str(), text->length());
    x_DEBUG("Received the following source string {}", target);
    x_DEBUG("Received the following source string {}", pattern);
