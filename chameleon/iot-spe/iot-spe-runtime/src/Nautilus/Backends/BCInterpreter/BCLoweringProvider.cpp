...
short BCLoweringProvider::RegisterProvider::allocRegister() {
    x_ASSERT(currentRegister <= REGISTERS, "allocated to many registers.");
...
std::tuple<Code, RegisterFile> BCLoweringProvider::LoweringContext::process() {
    ...
    x_INFO("Allocated Registers: {}", this->registerProvider.allocRegister());
...
void BCLoweringProvider::LoweringContext::process(const std::shared_ptr<IR::Operations::Operation>& opt,
    ...
    switch (opt->getOperationType()) {
        ...
        default: {
            x_THROW_RUNTIME_ERROR("Operation " << opt->toString() << " not handled");
            return;
        }
    }
}
...
void BCLoweringProvider::LoweringContext::processDynamicCall(const std::shared_ptr<IR::Operations::ProxyCallOperation>& opt,
    ...
    x_DEBUG("CREATE {}: {}", opt->toString(), opt->getStamp()->toString())
    ...
    for (auto& arg : arguments) {
        ...
        switch (argType) {
            ...
            default: x_THROW_RUNTIME_ERROR("Type not implemented");
        }
...
void BCLoweringProvider::LoweringContext::process(const std::shared_ptr<IR::Operations::CastOperation>& castOp,
    ...
    if (srcType == Type::i8) {
        switch (targetType) {
            ...
            default: x_THROW_RUNTIME_ERROR("Cast from i8 not supported");
        }
        ...
        switch (targetType) {
            ...
            default: x_THROW_RUNTIME_ERROR("Cast from i16 not supported");
        }
        ...
        switch (targetType) {
            ...
            default: x_THROW_RUNTIME_ERROR("Cast from i32 not supported");
        }
        ...
        switch (targetType) {
            ...
            default: x_THROW_RUNTIME_ERROR("Cast from i64 not supported");
        }
        ...
        switch (targetType) {
            ...
            default: x_THROW_RUNTIME_ERROR("Cast from ui8 not supported");
        }
        ...
        switch (targetType) {
            ...
            default: x_THROW_RUNTIME_ERROR("Cast from ui16 not supported");
        }
        ...
        x_THROW_RUNTIME_ERROR("Cast from not supported");
    }
    OpCode oc = {bc, input, -1, resultReg};
    program.blocks[block].code.emplace_back(oc);
}
