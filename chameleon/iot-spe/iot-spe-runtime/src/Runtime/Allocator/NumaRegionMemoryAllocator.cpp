...
void* NumaRegionMemoryAllocator::do_allocate(size_t sizeInBytes, size_t) {
    ...
    x_ASSERT2_FMT(mem != MAP_FAILED, "Cannot allocate memory " << sizeInBytes << " with errno " << strerror(errno));
