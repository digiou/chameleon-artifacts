...
#define bit_SSE4_2 (1 << 20)
    ...
    if (cpuid[2]
        ...
        apex::memcpy = apex::memmove =
            &apex::
                tiberium;//	`tiberium` CAN run with the SSE2 instructions alone, however there was something about very old SSE2 (only) computers, some penalty when using `tiberiums` algorithm, so I've restricted `tiberium` to Core/Core 2 based machix on 32-bit by doing this!
