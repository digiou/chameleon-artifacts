...
LocalBufferPool::LocalBufferPool(const BufferManagerPtr& bufferManager,
    ...
    x_ASSERT2_FMT(this->bufferManager, "Invalid buffer manager");
    while (!buffers.empty()) {
...
void LocalBufferPool::destroy() {
    ...
    x_ASSERT2_FMT(numberOfReservedBuffers == exclusiveBufferCount,
                    "one or more buffers were not returned to the pool: " << exclusiveBufferCount << " but expected "
                                                                          << numberOfReservedBuffers);

    x_DEBUG("buffers before={} size of local buffers={}", bufferManager->getAvailableBuffers(), exclusiveBuffers.size());
#ifndef x_USE_LATCH_FREE_BUFFER_MANAGER
