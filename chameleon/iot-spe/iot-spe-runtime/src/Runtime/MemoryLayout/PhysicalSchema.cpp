...
PhysicalFieldPtr PhysicalSchema::createPhysicalField(uint64_t fieldIndex, uint64_t bufferOffset) {
    x_ASSERT(validFieldIndex(fieldIndex), "field index invalid");
...
uint64_t PhysicalSchema::getFieldOffset(uint64_t fieldIndex) {
    x_ASSERT(validFieldIndex(fieldIndex), "field value invalid");
    uint64_t offset = 0;
...
bool PhysicalSchema::validFieldIndex(uint64_t fieldIndex) {
    ...
    if (fieldIndex > fields.size()) {
        x_FATAL_ERROR("PhysicalSchema: field index {} is out of bound. Schema only contains {} fields.",
                        fieldIndex,
                        fields.size());
