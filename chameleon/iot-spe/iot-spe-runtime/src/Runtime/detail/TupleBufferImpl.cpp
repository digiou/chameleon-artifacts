...
MemorySegment::MemorySegment(uint8_t* ptr,
    ...
    this->ptr = ptr;
    if (!this->ptr) {
        x_THROW_RUNTIME_ERROR("[MemorySegment] invalid pointer");
    }
    if (!this->size) {
        x_THROW_RUNTIME_ERROR("[MemorySegment] invalid size");
    }
}
...
MemorySegment::MemorySegment(uint8_t* ptr,
                             ...
                             bool)
    : ptr(ptr), size(size) {
    x_ASSERT2_FMT(this->ptr, "invalid ptr");
    x_ASSERT2_FMT(this->size, "invalid size");
...
MemorySegment::~MemorySegment() {
    if (ptr) {
        ...
        ///      the release function in general. Do you agree?).
        {
            ...
            x_ASSERT(refCnt == 0, "[MemorySegment] invalid reference counter" << refCnt << " on mem segment dtor");
        }
...
void BufferControlBlock::resetBufferRecycler(BufferRecycler* recycler) {
    x_ASSERT2_FMT(recycler, "invalid recycler");
    auto* oldRecycler = owningBufferRecycler.exchange(recycler);
    x_ASSERT2_FMT(recycler != oldRecycler, "invalid recycler");
}
...
bool BufferControlBlock::prepare() {
    ...
    x_ERROR("Invalid reference counter: {}", expected);
    return false;
}
...
void BufferControlBlock::dumpOwningThreadInfo() {
    std::unique_lock lock(owningThreadsMutex);
    x_FATAL_ERROR("Buffer {} has {} live references", getOwner(), referenceCounter.load());
    for (auto& item : owningThreads) {
        for (auto& v : item.second) {
            x_FATAL_ERROR("Thread {} has buffer {} requested on callstack: {}", v.threadName, getOwner(), v.callstack);
        }
    }
}
#endif
...
bool BufferControlBlock::release() {
    if (uint32_t const prevRefCnt = referenceCounter.fetch_sub(1); prevRefCnt == 1) {
        ...
        x_ASSERT(prevRefCnt != 0, "BufferControlBlock: releasing an already released buffer");
    }
#ifdef x_DEBUG_TUPLE_BUFFER_LEAKS
    {
        std::unique_lock lock(owningThreadsMutex);
...
void zmqBufferRecyclingCallback(void*, void* hint) {
    x_VERIFY(hint != nullptr, "Hint cannot be null");
...
bool BufferControlBlock::loadChildBuffer(uint16_t index, BufferControlBlock*& control, uint8_t*& ptr, uint32_t& size) {
    x_ASSERT2_FMT(index < children.size(), "Invalid index");
