...
#ifdef __linux__
#ifdef x_ENABLE_CXA_THROW_HOOK
void xCxaThrowHook(void* ex, std::type_info* info, void (**deleter)(void*)) noexcept {
    ...
    if (status == 0) {
        ...
        if (strcmp(zmqErrorType, exceptionName) != 0) {
            auto stacktrace = x::collectAndPrintStacktrace();
            x_ERROR("Exception caught: {} with stacktrace: {}" << exceptionName, stacktrace);
        }
    }
...
struct ErrorHandlerLoader {
  public:
    explicit ErrorHandlerLoader() {
        std::set_terminate(xTerminateHandler);
        std::set_new_handler(xTerminateHandler);
#ifdef __linux__
#ifdef x_ENABLE_CXA_THROW_HOOK
        folly::exception_tracer::registerCxaThrowCallback(xCxaThrowHook);
#endif
#endif
#ifdef __linux__
        std::set_unexpected(xUnexpectedException);
#elif defined(__APPLE__)
        // unexpected was removed in C++17 but only apple clang libc did actually remove it..
#else
#error "Unknown platform"
#endif
        std::signal(SIGABRT, xErrorHandler);
        std::signal(SIGSEGV, xErrorHandler);
        std::signal(SIGBUS, xErrorHandler);
        std::signal(SIGKILL, xKillHandler);
    }
};
