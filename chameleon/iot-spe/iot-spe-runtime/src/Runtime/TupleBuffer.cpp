...
uint32_t TupleBuffer::storeChildBuffer(TupleBuffer& buffer) noexcept {
    ...
    x_ASSERT2_FMT(controlBlock != control, "Cannot attach buffer to self");
...
TupleBuffer TupleBuffer::loadChildBuffer(uint32_t bufferIndex) noexcept {
    ...
    x_ASSERT(controlBlock->loadChildBuffer(bufferIndex, childBuffer.controlBlock, childBuffer.ptr, childBuffer.size),
               "Cannot load tuple buffer");
    return childBuffer;
}

bool recycleTupleBuffer(void* bufferPointer) {
    x_ASSERT2_FMT(bufferPointer, "invalid bufferPointer");
    auto buffer = reinterpret_cast<uint8_t*>(bufferPointer);
