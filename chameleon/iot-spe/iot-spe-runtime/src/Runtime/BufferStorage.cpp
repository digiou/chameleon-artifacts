...
void BufferStorage::insertBuffer(x::Runtime::TupleBuffer buffer) { this->storage.push(buffer); }

void BufferStorage::trimBuffer(uint64_t timestamp) {
    while (!this->storage.empty() && this->storage.top().getWatermark() < timestamp) {
        x_TRACE("BufferStorage: Delete tuple with watermark {}", this->storage.top().getWatermark());
        this->storage.pop();
    }
}
