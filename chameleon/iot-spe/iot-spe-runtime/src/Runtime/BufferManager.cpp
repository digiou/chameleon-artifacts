...
void BufferManager::destroy() {
    ...
    if (isDestroyed.compare_exchange_strong(expected, true)) {
        ...
        auto success = true;
        x_DEBUG("Shutting down Buffer Manager");
        ...
        if (allBuffers.size() != numOfAvailableBuffers) {
            x_ERROR("[BufferManager] total buffers {} :: available buffers {}", allBuffers.size(), numOfAvailableBuffers);
            success = false;
        }
        ...
        if (!success) {
            x_THROW_RUNTIME_ERROR("[BufferManager] Requested buffer manager shutdown but a buffer is still used allBuffers="
        ...
        x_DEBUG("Shutting down Buffer Manager completed");
...
void BufferManager::initialize(uint32_t withAlignment) {
    ...
    x_DEBUG("x memory allocation requires {} out of {} available bytes", requiredMemorySpace, memorySizeInBytes);

    //    x_ASSERT2_FMT(bufferSize && !(bufferSize & (bufferSize - 1)), "size must be power of two " << bufferSize);
    x_ASSERT2_FMT(requiredMemorySpace < memorySizeInBytes,
                    "x tries to allocate more memory than physically available requested="
                        << requiredMemorySpace << " available=" << memorySizeInBytes);
    if (withAlignment > 0) {
        if ((withAlignment & (withAlignment - 1))) {// not a pow of two
            x_THROW_RUNTIME_ERROR("x tries to align memory but alignment is not a pow of two");
        }
    } else if (withAlignment > page_size) {
        x_THROW_RUNTIME_ERROR("x tries to align memory but alignment is invalid");
    }

    x_ASSERT2_FMT(alignof(detail::BufferControlBlock) <= withAlignment,
    ...
    x_TRACE("Allocated {} bytes with alignment {} buffer size {} num buffer {} controlBlockSize {} {}",
              allocatedAreaSize,
              withAlignment,
              alignedBufferSize,
              numOfBuffers,
    ...
    if (basePointer == nullptr) {
        x_THROW_RUNTIME_ERROR("memory allocation failed");
    }
    ...
    x_DEBUG("BufferManager configuration bufferSize={} numOfBuffers={}", this->bufferSize, this->numOfBuffers);
}
...
TupleBuffer BufferManager::getBufferBlocking() {
    ...
    while (availableBuffers.empty()) {
        x_TRACE("All global Buffers are exhausted");
...
std::optional<TupleBuffer> BufferManager::getUnpooledBuffer(size_t bufferSize) {
    ...
    if (ptr == nullptr) {
        x_THROW_RUNTIME_ERROR("BufferManager: unpooled memory allocation failed");
    }
    x_TRACE("Ptr: {} alignedBufferSize: {} alignedBufferSizePlusControlBlock: {} controlBlockSize: {}",
...
void BufferManager::recyclePooledBuffer(detail::MemorySegment* segment) {
    ...
    if (!segment->isAvailable()) {
        x_THROW_RUNTIME_ERROR("Recycling buffer callback invoked on used memory segment");
    }
...
LocalBufferPoolPtr BufferManager::createLocalBufferPool(size_t numberOfReservedBuffers) {
    ...
    x_DEBUG("availableBuffers.size()={} requested buffers={}", availableBuffers.size(), numberOfReservedBuffers);
    x_ASSERT2_FMT((size_t) availableBuffers.size() >= numberOfReservedBuffers, "not enough buffers");//TODO improve error
...
FixedSizeBufferPoolPtr BufferManager::createFixedSizeBufferPool(size_t numberOfReservedBuffers) {
    std::unique_lock lock(availableBuffersMutex);
    std::deque<detail::MemorySegment*> buffers;
    x_ASSERT2_FMT((size_t) availableBuffers.size() >= numberOfReservedBuffers,
...
TupleBuffer allocateVariableLengthField(std::shared_ptr<AbstractBufferProvider> provider, uint32_t size) {
    ...
    x_ASSERT2_FMT(!!optBuffer, "Cannot allocate buffer of size " << size);
    return *optBuffer;
}
