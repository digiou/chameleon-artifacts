...
std::vector<cl_platform_id> retrieveOpenCLPlatforms() {
    ...
    if (numPlatforms == 0) {
        x_DEBUG("Did not find any OpenCL platforms.");
    } else {
...
std::vector<cl_device_id> retrieveOpenCLDevices(const cl_platform_id platformId) {
    ...
    if (numDevices == 0) {
        x_DEBUG("Did not find any OpenCL device.");
    } else {
...
// Wrapper around clGetDeviceInfo which adds error handling and optionally converts to a non-OpenCL type used in x.
template<typename CLType, typename xType>
xType retrieveOpenCLDeviceInfo(const cl_device_id deviceId, const cl_device_info param, const std::string& paramName) {
    CLType result;
...
OpenCLManager::OpenCLManager() {
    try {
        x_DEBUG("Determining installed OpenCL devices ...");
        ...
        for (const auto& platformId : platforms) {
            ...
            x_DEBUG("Found OpenCL platform: {} {}", platformVendor, platformName);
            ...
            for (const auto& device : devices) {
                ...
                x_INFO("Found OpenCL device: [platformVendor={}, platformName={}, deviceName={}, doubleFPSupport={}, "
...
OpenCLManager::OpenCLManager() { x_DEBUG("OpenCL support was disabled at build time"); }
#endif// ENABLE_OPENCL
