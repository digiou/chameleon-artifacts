...
#ifndef x_RUNTIME_INCLUDE_RUNTIME_ALLOCATOR_FIXEDPAGESALLOCATOR_HPP_
#define x_RUNTIME_INCLUDE_RUNTIME_ALLOCATOR_FIXEDPAGESALLOCATOR_HPP_
...
T* allocAligned(size_t size, size_t alignment = 16) {
    void* tmp = nullptr;
    x_ASSERT2_FMT(0 == posix_memalign(&tmp, alignment, sizeof(T) * size),
...
T* allocHugePages(size_t size) {
    void* tmp = nullptr;
    x_ASSERT2_FMT(0 == posix_memalign(&tmp, huge_page_size, sizeof(T) * size),
                    "Cannot allocate " << sizeof(T) * size << " bytes: " << strerror(errno));
#ifdef __linux__
    madvise(tmp, size * sizeof(T), MADV_HUGEPAGE);
#endif

    x_ASSERT2_FMT(tmp != nullptr, "Cannot remap as huge pages");
...
class FixedPagesAllocator {
  public:
    explicit FixedPagesAllocator(size_t totalSize) {
        head = x::Runtime::detail::allocHugePages<uint8_t>(totalSize);
    ...
    uint8_t* getNewPage(size_t pageSize) {
        ...
        allocCnt++;
        x_ASSERT2_FMT(ptr < overrunAddress,
...
#endif// x_RUNTIME_INCLUDE_RUNTIME_ALLOCATOR_FIXEDPAGESALLOCATOR_HPP_
