...
#ifndef x_RUNTIME_INCLUDE_RUNTIME_EXECUTION_EXECUTABLEPIPELIxTAGE_HPP_
#define x_RUNTIME_INCLUDE_RUNTIME_EXECUTION_EXECUTABLEPIPELIxTAGE_HPP_
...
class ExecutablePipelixtage {
  public:
    virtual ~ExecutablePipelixtage() = default;

    explicit ExecutablePipelixtage(PipelixtageArity arity = PipelixtageArity::Unary) : arity(arity) {
        // nop
    }
     ...
     */
    [[nodiscard]] PipelixtageArity getArity() const { return arity; }
...
namespace fmt {
template<>
struct formatter<x::Runtime::Execution::ExecutablePipelixtage> : formatter<std::string> {
    auto format(const x::Runtime::Execution::ExecutablePipelixtage& ex_pipeline_stage, format_context& ctx)
        -> decltype(ctx.out()) {
...
#endif// x_RUNTIME_INCLUDE_RUNTIME_EXECUTION_EXECUTABLEPIPELIxTAGE_HPP_
