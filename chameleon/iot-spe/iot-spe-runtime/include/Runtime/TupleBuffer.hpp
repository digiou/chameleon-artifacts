...
#ifndef x_RUNTIME_INCLUDE_RUNTIME_TUPLEBUFFER_HPP_
#define x_RUNTIME_INCLUDE_RUNTIME_TUPLEBUFFER_HPP_
 ...
 * result in a Runtime error that the BufferManager will raise by the termination of the x program.
 *
 * A TupleBuffer may store one or more child/xted TupleBuffer. As soon as a TupleBuffer is attached to a parent,
...
class TupleBuffer {
    ...
    template<typename T>
    friend class x::Network::detail::NetworkDataSender;
    template<typename T>
    friend class x::Network::detail::NetworkEventSender;
  ...
  public:
    ...
    using xtedTupleBufferKey = uint32_t;
    ...
    ///@brief attach a child tuple buffer to the parent. the child tuple buffer is then identified via xtedTupleBufferKey
    [[nodiscard]] xtedTupleBufferKey storeChildBuffer(TupleBuffer& buffer) noexcept;

    ///@brief retrieve a child tuple buffer via its xtedTupleBufferKey
    [[nodiscard]] TupleBuffer loadChildBuffer(xtedTupleBufferKey bufferIndex) noexcept;
...
/**
 * @brief This method determix the control block based on the ptr to the data region and decrements the reference counter.
...
#endif// x_RUNTIME_INCLUDE_RUNTIME_TUPLEBUFFER_HPP_
