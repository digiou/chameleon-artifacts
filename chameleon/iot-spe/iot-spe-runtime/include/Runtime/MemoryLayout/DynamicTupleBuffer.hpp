...
#ifndef x_RUNTIME_INCLUDE_RUNTIME_MEMORYLAYOUT_DYNAMICTUPLEBUFFER_HPP_
#define x_RUNTIME_INCLUDE_RUNTIME_MEMORYLAYOUT_DYNAMICTUPLEBUFFER_HPP_
...
class DynamicField {
  public:
     ...
     * @tparam Type of the field requires to be a xType and a pointer type.
     * @throws BufferAccessException if the passed Type is not the same as the physicalType of the field.
     * @return Pointer type
     */
    template<class Type>
        requires IsxType<Type> && std::is_pointer<Type>::value
     ...
     * @tparam Type of the field requires to be a xType.
     * @throws BufferAccessException if the passed Type is not the same as the physicalType of the field.
     * @return Value of the field.
     */
    template<class Type>
        requires(IsxType<Type> && not std::is_pointer<Type>::value)
    inline Type& read() const {
        if (!PhysicalTypes::isSamePhysicalType<Type>(physicalType)) {
            throw BufferAccessException("Wrong field type passed. Field is of type " + physicalType->toString()
                                        + " but accessed as " + typeid(Type).name());
     ...
     * @tparam Type of the field. Type has to be a xType and to be compatible with the physical type of this field.
     * @param value of the field.
     * @throws BufferAccessException if the passed Type is not the same as the physicalType of the field.
     */
    template<class Type>
        requires(IsxType<Type>)
  ...
  public:
    ...
    */
    ...
    std::tuple<Types...> readRecordFromBuffer(uint64_t recordIndex) {
        x_ASSERT((sizeof...(Types)) == memoryLayout->getFieldSizes().size(),
...
#endif// x_RUNTIME_INCLUDE_RUNTIME_MEMORYLAYOUT_DYNAMICTUPLEBUFFER_HPP_
