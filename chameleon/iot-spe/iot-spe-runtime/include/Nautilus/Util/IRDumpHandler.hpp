...
#ifndef x_RUNTIME_INCLUDE_NAUTILUS_UTIL_IRDUMPHANDLER_HPP_
#define x_RUNTIME_INCLUDE_NAUTILUS_UTIL_IRDUMPHANDLER_HPP_
...
/**
 * @brief Converts query plans and pipeline plans to the .xviz format and dumps them to a file.m
 */
class xIRDumpHandler {

  public:
    virtual ~xIRDumpHandler() = default;
    static std::shared_ptr<xIRDumpHandler> create(std::ostream& out);
    explicit xIRDumpHandler(std::ostream& out);

    /**
    * @brief Dump the xIR of the funcOp into the 'out' stringstream.
    * @param funcOp: FunctionOperation that exists on the top level of a xIR module.
    */
  ...
  private:
    ...
    /**
     * @brief Traverses the xIR to find a BB that is on the same or higher 'blockScopeLevel' compared to the initial 'basicBlock'.
     *        Note: There is always a 'next block', since we always have a return block at the very end of a function.
     * @param basicBlock: Initially the block that we want to find the next BB for. Replaced while recursively traversing xIR.
...
#endif// x_RUNTIME_INCLUDE_NAUTILUS_UTIL_IRDUMPHANDLER_HPP_
