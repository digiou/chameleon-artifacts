...
#ifndef x_RUNTIME_INCLUDE_NAUTILUS_BACKENDS_MLIR_MLIRUTILITY_HPP_
#define x_RUNTIME_INCLUDE_NAUTILUS_BACKENDS_MLIR_MLIRUTILITY_HPP_
...
class MLIRUtility {
  public:
    ...
    /**
     * @brief Takes xIR, lowers it to MLIR, optimizes it, and returns an MLIR ExecutionEngine.
     * @param ir: xIR that is lowered to MLIR, and JIT compiled.
     * @return std::unique_ptr<mlir::ExecutionEngine> Provides a function pointer to executable function.
     */
    static std::unique_ptr<mlir::ExecutionEngine> compilexIRToMachineCode(std::shared_ptr<x::Nautilus::IR::IRGraph> ir);
};

}// namespace x::Nautilus::Backends::MLIR
#endif// x_RUNTIME_INCLUDE_NAUTILUS_BACKENDS_MLIR_MLIRUTILITY_HPP_
