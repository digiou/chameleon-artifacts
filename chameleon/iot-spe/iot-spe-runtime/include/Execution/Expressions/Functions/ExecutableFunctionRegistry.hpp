...
#ifndef x_RUNTIME_INCLUDE_EXECUTION_EXPRESSIONS_FUNCTIONS_EXECUTABLEFUNCTIONREGISTRY_HPP_
#define x_RUNTIME_INCLUDE_EXECUTION_EXPRESSIONS_FUNCTIONS_EXECUTABLEFUNCTIONREGISTRY_HPP_
...
class UnaryFunctionProvider : public FunctionExpressionProvider {
  public:
    std::unique_ptr<Expression> create(std::vector<ExpressionPtr>& args) override {
        x_ASSERT(args.size() == 1, "A unary function should receive one argument");
...
class BinaryFunctionProvider : public FunctionExpressionProvider {
  public:
    std::unique_ptr<Expression> create(std::vector<ExpressionPtr>& args) override {
        x_ASSERT(args.size() == 2, "A binary function should receive two arguments");
...
#endif// x_RUNTIME_INCLUDE_EXECUTION_EXPRESSIONS_FUNCTIONS_EXECUTABLEFUNCTIONREGISTRY_HPP_
