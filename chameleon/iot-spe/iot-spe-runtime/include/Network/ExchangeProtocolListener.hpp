...
#ifndef x_RUNTIME_INCLUDE_NETWORK_EXCHANGEPROTOCOLLISTENER_HPP_
#define x_RUNTIME_INCLUDE_NETWORK_EXCHANGEPROTOCOLLISTENER_HPP_
...
class ExchangeProtocolListener {
  public:
     ...
     * @param xPartition partition that receives the event
     * @param Runtime::BaseEvent& ref to the actual event
     */
    virtual void onEvent(xPartition, Runtime::BaseEvent&) = 0;

    /**
     * @brief This is called on every data buffer that the network stack receives
     * for a specific x partition.
     * @param xPartition partition that receives the data buffer
     * @param Runtime::TupleBuffer& ref to the actual data buffer
     */
    virtual void onDataBuffer(xPartition, Runtime::TupleBuffer&) = 0;
    /**
     * @brief this is called once a x partition receives an end of stream message.
...
#endif// x_RUNTIME_INCLUDE_NETWORK_EXCHANGEPROTOCOLLISTENER_HPP_
