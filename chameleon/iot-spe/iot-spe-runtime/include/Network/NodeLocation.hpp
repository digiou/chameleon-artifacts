...
#ifndef x_RUNTIME_INCLUDE_NETWORK_NODELOCATION_HPP_
#define x_RUNTIME_INCLUDE_NETWORK_NODELOCATION_HPP_
...
/**
 * @brief This is a network location of a x node. It contains a node id, an ip address, and port for data transfer.
 */
class NodeLocation {
  public:
    ...
    explicit NodeLocation(NodeId nodeId, std::string hostname, uint32_t port)
        ...
        x_ASSERT2_FMT(this->hostname.size() > 0, "Empty hostname passed on " << nodeId);
    }
...
#endif// x_RUNTIME_INCLUDE_NETWORK_NODELOCATION_HPP_
