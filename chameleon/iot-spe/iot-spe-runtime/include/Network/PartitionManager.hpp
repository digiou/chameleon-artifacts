...
#ifndef x_RUNTIME_INCLUDE_NETWORK_PARTITIONMANAGER_HPP_
#define x_RUNTIME_INCLUDE_NETWORK_PARTITIONMANAGER_HPP_
...
class PartitionManager {
  ...
  public:
     ...
     */
    bool registerSubpartitionConsumer(xPartition partition, NodeLocation nodeLocation, DataEmitterPtr emitter);

    /**
     * @brief Increment the subpartition counter
     * @param partition the partition
     */
    void pinSubpartitionConsumer(xPartition partition);
     ...
     */
    bool unregisterSubpartitionConsumer(xPartition partition);
     ...
     */
    std::optional<uint64_t> getSubpartitionConsumerCounter(xPartition partition);
     ...
     */
    PartitionRegistrationStatus getConsumerRegistrationStatus(xPartition partition) const;
     ...
     * @return true if this is  the first time the partition was registered, false otherwise
     */
    bool registerSubpartitionProducer(xPartition partition, NodeLocation nodeLocation);

    /**
     * @brief Increment the subpartition counter
     * @param partition the partition
     */
    void pinSubpartitionProducer(xPartition partition);
     ...
     * @return true if the partition counter got to zero, false otherwise
     */
    bool unregisterSubpartitionProducer(xPartition partition);
     ...
     * @throw  std::out_of_range  If no such data is present.
     */
    std::optional<uint64_t> getSubpartitionProducerCounter(xPartition partition);
     ...
     * @return a PartitionRegistrationStatus
     */
    PartitionRegistrationStatus getProducerRegistrationStatus(xPartition partition) const;
     ...
     */
    DataEmitterPtr getDataEmitter(xPartition partition);
     ...
     */
    bool addSubpartitionEventListener(xPartition partition,
     ...
     */
    Runtime::RuntimeEventListenerPtr getEventListener(xPartition partition) const;
  ...
  private:
    std::unordered_map<xPartition, PartitionProducerEntry> producerPartitions;
    std::unordered_map<xPartition, PartitionConsumerEntry> consumerPartitions;
...
#endif// x_RUNTIME_INCLUDE_NETWORK_PARTITIONMANAGER_HPP_
