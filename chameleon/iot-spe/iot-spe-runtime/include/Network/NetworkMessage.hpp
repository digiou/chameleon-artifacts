...
#ifndef x_RUNTIME_INCLUDE_NETWORK_NETWORKMESSAGE_HPP_
#define x_RUNTIME_INCLUDE_NETWORK_NETWORKMESSAGE_HPP_
...
/**
 * @brief This magic number is written as first 64bits of every x network message.
 * We use this as a checksum to validate that we are not transferring garbage data.
 */
using x_magic_number_t = uint64_t;
static constexpr x_magic_number_t x_NETWORK_MAGIC_NUMBER = 0xBADC0FFEE;
...
/// this enum defix the errors that can occur in the network stack logic
...
/*
    This is how a x Network Message looks like on the wire

    +------------------+-----------------+-----------------------+
    |  Zmq Routing Id  |  MessageHeader  |   OPTIONAL subclass   |
    |   xChannelId   |    13 bytes     |   of ExchangeMessage  |
...
class MessageHeader {
  public:
    explicit MessageHeader(MessageType msgType, uint32_t msgLength)
        : magicNumber(x_NETWORK_MAGIC_NUMBER), msgType(msgType), msgLength(msgLength) {}

    [[nodiscard]] x_magic_number_t getMagicNumber() const { return magicNumber; }
  ...
  private:
    ...
    const x_magic_number_t magicNumber;
...
/**
 * @brief This is the base class for all messages that can be sent in x
 */
...
#endif// x_RUNTIME_INCLUDE_NETWORK_NETWORKMESSAGE_HPP_
