...
AttributeFieldPtr Schema::get(const std::string& fieldName) {
    ...
    x_FATAL_ERROR("Schema: No field in the schema with the identifier {}", fieldName);
...
AttributeFieldPtr Schema::get(uint32_t index) {
    ...
    x_FATAL_ERROR("Schema: No field in the schema with the id {}", index);
...
std::string Schema::getQualifierNameForSystemGeneratedFields() {
    ...
    x_ERROR("Schema::getQualifierNameForSystemGeneratedFields: a schema is not allowed to be empty when a qualifier is "
              "requested");
    return "";
}
...
bool Schema::contains(const std::string& fieldName) {
    for (const auto& field : this->fields) {
        x_TRACE("contain compare field={} with other={}", field->getName(), fieldName);
        if (field->getName() == fieldName) {
            return true;
        }
    }
