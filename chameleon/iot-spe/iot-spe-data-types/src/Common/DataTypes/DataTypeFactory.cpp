...
DataTypePtr DataTypeFactory::copyTypeAndIncreaseLowerBound(DataTypePtr stamp, double minLowerBound) {
    if (stamp->isFloat()) {
        ...
        if (auto const intStamp = DataType::as<Integer>(stamp); intStamp->lowerBound < minLowerBound) {
            x_WARNING("DataTypeFactory: A Float is passed as the minimum lower bound of an Integer data type. Will be executed "
        ...
        x_ERROR("DataTypeFactory: Can not increase a lower bound on a non-numeric data type.");
    }
...
DataTypePtr DataTypeFactory::copyTypeAndIncreaseLowerBound(DataTypePtr stamp, int64_t minLowerBound) {
    if (stamp->isInteger()) {
        ...
        if (auto const floatStamp = DataType::as<Float>(stamp); floatStamp->lowerBound < minLowerBound) {
            x_INFO("DataTypeFactory: An Integer is passed as the minimum lower bound of a Float data type. Progresses with "
    ...
    x_INFO("DataTypeFactory: Increase of lower bound does not apply. Returning original stamp.");
    return stamp;// increase does not apply -> return shared pointer given as argument
}
...
DataTypePtr DataTypeFactory::copyTypeAndDecreaseUpperBound(DataTypePtr stamp, double maxUpperBound) {
    if (stamp->isFloat()) {
        ...
        if (auto const intStamp = DataType::as<Integer>(stamp); maxUpperBound < intStamp->upperBound) {
            x_WARNING("DataTypeFactory: A Float is passed as the maximum upper bound of an Integer data type. Progresses with "
    ...
    x_INFO("DataTypeFactory: Decrease of upper bound does not apply. Returning original stamp.");
...
DataTypePtr DataTypeFactory::copyTypeAndDecreaseUpperBound(DataTypePtr stamp, int64_t maxUpperBound) {
    if (stamp->isInteger()) {
        ...
        if (auto const floatStamp = DataType::as<Float>(stamp); maxUpperBound < floatStamp->upperBound) {
            x_INFO("DataTypeFactory: An Integer is passed as the maximum upper bound of an Float data type. Progresses with "
                     "standard casting to Double.");
...
DataTypePtr DataTypeFactory::copyTypeAndTightenBounds(DataTypePtr stamp, int64_t minLowerBound, int64_t maxUpperBound) {
    if (stamp->isInteger()) {
        ...
        x_INFO("DataTypeFactory: Integers are passed as new bounds of an Float data type. Progresses with standard casting to "
                 "Double.");
        ...
        // non-numeric data types do not have a lower bound
        x_ERROR("DataTypeFactory: Can not modify bounds on a non-numeric data type.");
    }
    x_INFO("DataTypeFactory: Lower and upper bound do not need to be changed. Returning original stamp.");
...
DataTypePtr DataTypeFactory::copyTypeAndTightenBounds(DataTypePtr stamp, double minLowerBound, double maxUpperBound) {
    if (stamp->isFloat()) {
        ...
        x_INFO("DataTypeFactory: Floats are passed as new bounds of an Integer data type. Progresses with the floor of the "
...
DataTypePtr DataTypeFactory::createFloatFromInteger(DataTypePtr stamp) {
    if (stamp->isInteger()) {
        ...
        x_INFO("DataTypeFactory: A Float is passed to be converted to a Float. Return stamp passed as argument.");
    } else {
