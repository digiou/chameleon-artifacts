...
#ifndef x_DATA_TYPES_INCLUDE_COMMON_EXECUTABLETYPE_ARRAY_HPP_
#define x_DATA_TYPES_INCLUDE_COMMON_EXECUTABLETYPE_ARRAY_HPP_

#include <Common/ExecutableType/xType.hpp>
...
class ArrayBase : public std::array<T, s>, public xType {
  public:
  ...
  private:
    ...
    initializeFrom(E const& payload, std::size_t pSize, std::integer_sequence<std::size_t, is...>) noexcept(false) {
        ...
        if constexpr (std::is_same_v<T, char>) {
            if (pSize > size) {
                throw std::runtime_error("The initializer passed to x::ArrayType is too long for the contained type.");
            }
            std::memcpy(&(this[0]), &(payload[0]), pSize);
        } else {
            if (pSize != size) {
                throw std::runtime_error("The initializer's size does not match the size of the x::ArrayType that "
...
class Array<char, size> final : public ArrayBase<char, size> {
  ...
  public:
     ...
     *        `size` <= str.size()).
     *
     * @param str string from which the x::Array is constructed.
     *
...
/// Compiler hint which computes the size of a x::Array and its type from the arguments.
template<typename T,
         typename... Ts,
...
/**
 * Specialize the std::hash for x Array data type
 * @tparam T data type stored in the array
 * @tparam N size of the array
 */
template<class T, size_t N>
struct std::hash<x::ExecutableTypes::Array<T, N>> {
    auto operator()(const x::ExecutableTypes::Array<T, N>& key) const {
        std::hash<T> hasher;
        size_t result = 0;
...
namespace fmt {
template<std::size_t size>
struct formatter<x::ExecutableTypes::Array<char, size>> : formatter<std::string> {
    auto format(const x::ExecutableTypes::Array<char, size> executable_type_array, format_context& ctx) -> decltype(ctx.out()) {
        return format_to(ctx.out(), "{}", executable_type_array);
    }
};
}//namespace fmt

#endif// x_DATA_TYPES_INCLUDE_COMMON_EXECUTABLETYPE_ARRAY_HPP_
